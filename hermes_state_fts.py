...
from hermes_state_common import (
    FTS_CJK_STALE_KEY, FTS_STALE_KEY, _FTS_CJK_TRIGGERS, _FTS_TRIGGERS,
    StateDbProbeTimeout, _arm_probe_deadline, _fts_integrity_check_sql, _message_trigram_disabled,
    _is_interrupted_error,
)

# Same derivation schema.py uses; kept local rather than imported from there, which would be a
# cycle. The trigram triggers are the subset of _FTS_TRIGGERS naming the trigram index.
...
class SessionFtsSetupMixin:
    ...
    def _ensure_fts_schema(self, cursor: sqlite3.Cursor, table_name: str, ddl: str) -> bool:
        if table_name == "messages_fts_trigram" and _message_trigram_disabled():
            # Opted out via HERMES_DISABLE_MESSAGE_TRIGRAM. This is the single choke point
            # every trigram-creation path routes through — main setup AND the v10 / #16751
            # backfill migrations — so dropping here and returning False stops all of them,
            # and the migrations skip their backfills because they gate on this return value.
