...
def _live_system_guard(request, monkeypatch):
    ...
    *PID scans* themselves are handled separately by ``_pid_scan_guard``
    ...
    if hasattr(_os, "killpg"):
        ...
        def _guarded_killpg(pgid, sig, *args, **kwargs):
            ...
            raise RuntimeError(
                ...
                f"os.killpg({pgid}, {sig}) — PGID is outside the test "  # windows-footgun: ok -- prose in a diagnostic message, not a call
    ...
    _WRAPPER_COMMANDS = (
        "cmd", "powershell", "pwsh", "sh", "bash", "zsh", "dash", "env", "nohup", "setsid",
