...
async def test_explicit_media_tag_still_delivers_post_stream(tmp_path, monkeypatch):
    ...
    assert images_kwargs["images"] == [(media_file.as_uri(), "")]



@pytest.mark.asyncio
async def test_kanban_artifact_uses_native_file_uri(tmp_path, monkeypatch):
    ...
    await GatewayKanbanWatchersMixin._deliver_kanban_artifacts(
        SimpleNamespace(), adapter=adapter, chat_id="C123CHAN", metadata={},
        event_payload={"artifacts": [str(media_file)]}, task=None,
    )
    adapter.send_multiple_images.assert_awaited_once_with(
        chat_id="C123CHAN", images=[(media_file.as_uri(), "")], metadata={},
    )
