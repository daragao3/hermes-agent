...
def patch_startup_side_effects(monkeypatch, tmp_path):
    ...
    # The fork warms the turn machinery on a worker thread at startup: it imports run_agent's whole
    # graph, builds a system prompt and runs env_probe. None of these tests assert on it, and on a
    # loaded box it alone blows the 300s per-test budget.
    monkeypatch.setattr("gateway.run._warm_turn_machinery_sync", lambda: 0)
...
async def test_startup_aborts_when_restart_begins_during_platform_connect(tmp_path, monkeypatch):
    ...
    runner.config.platforms = {
        # DISCORD, not telegram: this fork connects telegram in the BACKGROUND
        # (_BACKGROUND_CONNECT_PLATFORMS), and a backgrounded connect is deliberately outside the
        # inline task map whose teardown this test is about.
        Platform.DISCORD: PlatformConfig(enabled=True, token="***"),
        Platform.SLACK: PlatformConfig(enabled=True, token="***"),
    }
    first_disconnected = asyncio.Event()
    discord = StartupRaceAdapter(
        Platform.DISCORD,
    ...
    async def disconnect_and_release():
        discord.disconnected = True
        first_disconnected.set()

    discord.disconnect = disconnect_and_release
    runner._create_adapter = MagicMock(side_effect=[discord, slack])
...
async def test_startup_aborts_after_registered_adapter_restart(tmp_path, monkeypatch):
    ...
    # 0.21.1 connects the pending platforms concurrently, so the peer's connect is already in
    # flight when the restart request lands and may complete. What the abort owes us is that it is
    # TORN DOWN and never published as connected (asserted below), not that it never connected.
    assert discord.disconnected is True
    assert runner._running is False
    assert runner.adapters == {}
    assert runner._update_runtime_status.call_args_list[-1].args[0] == "stopped"
