...
def test_platform_discovery_is_not_empty():
    assert _PLATFORM_NAMES, f"No platforms discovered under {PLATFORMS_DIR}"
...
class _MockPluginContext:
    ...
    Implements platform and ancillary registration so we can exercise the plugin's
    ...
    def register_cli_command(self, name, help, setup_fn, handler_fn=None, description=""):
        assert name and callable(setup_fn)
        assert handler_fn is None or callable(handler_fn)

    def register_hook(self, hook_name, callback):
        assert hook_name and callable(callback)
