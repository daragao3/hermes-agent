...
def test_default_footer_renders_byte_identically(
    monkeypatch, tmp_path, model, tokens, window, cwd, expected
):
    ...
    monkeypatch.setenv("USERPROFILE", str(tmp_path / "home"))
    if cwd:
        native_cwd = str(tmp_path / "data")
        expected = expected.replace(cwd, native_cwd)
        cwd = native_cwd
    out = format_runtime_footer(
        model=model,
...
def test_default_build_footer_line_ignores_turn_seconds(monkeypatch, tmp_path):
    """build_footer_line with default fields is unaffected by turn_seconds."""
    monkeypatch.delenv("TERMINAL_CWD", raising=False)
    monkeypatch.setenv("HOME", str(tmp_path / "home"))
    monkeypatch.setenv("USERPROFILE", str(tmp_path / "home"))
    cwd = str(tmp_path / "data")
    common = dict(
