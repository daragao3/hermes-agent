...
async def _settle_test_connects(runner):
    """Test real adapter phases; settle background outcomes before assertions.

    Full service boot is covered by the Telegram/WhatsApp background journeys.
    This contract does not start unrelated warmup, cron or watcher services.
    """
    pending = [(platform, cfg, runner._create_adapter(platform, cfg))
               for platform, cfg in runner.config.platforms.items() if cfg.enabled]
    try:
        raw = await runner._start_connect_pending(pending)
        assert raw is not None
        await runner._start_aggregate_connect_results(raw, [], [])
        runner._running = True
        tasks = list(getattr(runner, "_startup_background_connects", ()))
        if tasks:
...
async def test_startup_connects_platforms_concurrently(monkeypatch, tmp_path):
    ...
    await _settle_test_connects(runner)
...
class TestTelegramColdStartCap:
    ...
    async def test_initial_connect_times_out_at_cap_and_queues_retry(
        ...
        await asyncio.wait_for(_settle_test_connects(runner), timeout=30)
