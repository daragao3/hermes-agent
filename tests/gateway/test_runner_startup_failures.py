...
async def test_start_gateway_replace_aborts_when_force_killed_pid_still_alive(
    ...
    # The graceful ask is not a bare SIGTERM in this fork: on Windows it is the planned-stop
    # marker the incumbent's own watcher polls (a SIGTERM there is TerminateProcess -- no drain,
    # in-flight cron work lost), on POSIX the signal plus that marker. Record the ask and report
    # that the target did NOT exit, which is the scenario this test is about.
    asks: list[int] = []

    async def _record_ask(existing_pid, *, timeout):
        asks.append(existing_pid)
        return False

    monkeypatch.setattr("gateway.run._request_incumbent_shutdown", _record_ask)
    ...
    assert ok is False
    assert asks == [42], "the incumbent must be asked to drain before any force kill"
    assert calls == [(42, True)], "and the escalation is the only terminate_pid"
...
async def test_start_gateway_replace_writes_takeover_marker_before_sigterm(
    ...
    # The stop request itself: this fork asks the incumbent to drain (planned-stop marker on
    # Windows, signal + marker on POSIX) and escalates to terminate_pid only on timeout, so the
    # ask -- not a SIGTERM -- is the event that must follow the marker write.
    async def record_request_shutdown(existing_pid, *, timeout):
        events.append(f"request_shutdown(pid={existing_pid})")
        return True

    monkeypatch.setattr("gateway.run._request_incumbent_shutdown", record_request_shutdown)
    monkeypatch.setattr("gateway.run.os.getpid", lambda: 100)
    ...
    assert ok is True
    # Ordering: marker written BEFORE the incumbent is asked to stop.
    assert events[0] == "write_marker(target_pid=42)"
    assert any(
        e.startswith("request_shutdown(pid=42") or e.startswith("terminate_pid(pid=42")
        for e in events[1:]
    ), events
...
class _ForeignTokenLockAdapter(BasePlatformAdapter):
    ...
    DISCORD, not telegram: this fork backgrounds telegram's boot connect
    (``_BACKGROUND_CONNECT_PLATFORMS``), so a telegram failure never reaches the startup
    router's zero-connected classification -- it is queued for retry on its own task. Discord
    connects inline, which is where the rule under test actually runs.
    """
...
async def test_live_foreign_token_lock_at_startup_exits_ex_config(monkeypatch, tmp_path):
    ...
    *mid-run* reconnect can recover once the holder exits.  The startup router
    ...
    monkeypatch.setattr(
        ...
        lambda scope, identity, metadata=None, **_kw: (
            False,
    ...
    assert state["platforms"]["discord"]["error_code"] == "discord-bot-token_lock"
...
async def test_token_lock_plus_retryable_peer_stays_alive(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "gateway.status.acquire_scoped_lock",
        lambda scope, identity, metadata=None, **_kw: (False, {"pid": 424242, "start_time": 1}),
    )
    # DISCORD holds the contested lock and SLACK is the transient peer: telegram is backgrounded
    # in this fork, so it could not play either role at startup (see _ForeignTokenLockAdapter).
    config = GatewayConfig(
        platforms={
            Platform.DISCORD: PlatformConfig(enabled=True, token="***"),
    ...
    class _SlackBlip(_RetryableFailureAdapter):
        def __init__(self):
            BasePlatformAdapter.__init__(
                self, PlatformConfig(enabled=True, token="***"), Platform.SLACK
            )
    ...
    monkeypatch.setattr(
        ...
        "_create_adapter",
        lambda platform, cfg: (
            _ForeignTokenLockAdapter() if platform is Platform.DISCORD else _SlackBlip()
        ),
    )
    ...
    try:
        ...
        assert set(runner._failed_platforms) == {Platform.SLACK}
        state = read_runtime_status()
        assert state["gateway_state"] == "running"
        assert state["platforms"]["discord"]["state"] == "fatal"
        assert state["platforms"]["slack"]["state"] == "retrying"
    finally:
        await runner.stop()
...
async def test_start_gateway_replace_force_uses_terminate_pid(monkeypatch, tmp_path):
    ...
    # **_kw: the fork's force path also passes expected_start_time (PID-identity guard).
    def _mock_terminate_pid(pid, force=False, reason=None, **_kw):
        calls.append((pid, force))
        if force:
            _pid_state["alive"] = False
    monkeypatch.setattr("gateway.status.terminate_pid", _mock_terminate_pid)
    # Ownership guard (#89315): --replace refuses a PID it cannot prove belongs to this profile's
    # gateway, so the record must be a valid BOUND one for pid 42 in THIS home and the live
    # start-time probe must agree with it.
    monkeypatch.setattr(
        "gateway.status._read_pid_record",
        lambda path=None: {
            "pid": 42,
    ...
    async def _refuses_to_drain(pid, *, timeout):
        # Must be a coroutine function: the caller awaits it, and a plain lambda makes the await
        # raise TypeError, which the replace path swallows into a failed start.
        return False

    monkeypatch.setattr("gateway.run._request_incumbent_shutdown", _refuses_to_drain)
    monkeypatch.setattr(
