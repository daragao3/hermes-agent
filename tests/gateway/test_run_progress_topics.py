...
class ProgressCaptureAdapter(BasePlatformAdapter):
    #: Optional ``threading.Event`` a test can bind so a fake agent can wait for the FIRST
    #: progress bubble to actually exist instead of sleeping a fixed interval. ``None`` (the
    #: default) makes ``_note_send`` a no-op, so no other test is affected. Declared on the
    #: BASE because ``_note_send`` is defined here.
...
class ManyProgressLinesAgent:
    ...
    def run_conversation(self, message, conversation_history=None, task_id=None, **kwargs):
        ...
        # Wait for the progress task to actually create the first editable bubble, then
        # enqueue the rest quickly.  The cancellation drain must roll them into fresh
        # editable bubbles instead of trying to edit the first one past limit.
        #
        # This was a 0.35s sleep, i.e. a bet that the event loop would get to the progress
        # task inside the window.  Under the 12-worker harness the file runs ~1.9x slower and
        # the bet is lost: this test flaked in the 2026-08-19 gate runs (failed attempt 1,
        # passed on retry).
