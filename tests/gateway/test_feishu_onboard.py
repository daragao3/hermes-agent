...
class TestProbeBot:
    ...
    # _load_lark_oapi, not check_feishu_requirements: 0.21.1's probe_bot asks the loader
    # directly, so the old stub was accepted and intercepted nothing -- on a box that HAS
    # lark_oapi installed the SDK branch ran and this fallback was never exercised.
    @patch("plugins.platforms.feishu.adapter._load_lark_oapi", return_value=False)
    @patch("plugins.platforms.feishu.adapter.urlopen")
    ...
    # See above. This one "passed" with the stale stub only because the SDK branch also
    # returns None -- a false green, so it is retargeted too.
    @patch("plugins.platforms.feishu.adapter._load_lark_oapi", return_value=False)
    @patch("plugins.platforms.feishu.adapter.urlopen")
