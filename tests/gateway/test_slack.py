...
class TestMessageSplitting:
    ...
    async def test_send_passes_unfurl_options_to_every_chunk(self, adapter):
        ...
        # `posted`, not `call`: unittest.mock.call is imported at module level and used
        # elsewhere in this file, so a loop variable of the same name shadows it (F402).
        for posted in adapter._app.client.chat_postMessage.call_args_list:
            assert posted.kwargs["unfurl_links"] is False
            assert posted.kwargs["unfurl_media"] is False
