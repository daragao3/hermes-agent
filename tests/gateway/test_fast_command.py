...
def test_discord_title_provenance_reaches_shared_turn_owner_and_rename():
    ...
    agent = SimpleNamespace(
        session_id="session-1", platform="discord", _session_db=MagicMock(),
        _session_db_created=True, _session_init_model_config={"model": "test-model"},
    )
    turn = object.__new__(TurnRunner)
    turn._runner = runner
    turn._attach_session_title_callback(agent, SimpleNamespace(source=source))
    with patch("agent.title_generator.maybe_auto_title") as title:
        _maybe_title_session_at_turn_start(agent, [{"role": "user", "content": "raw prompt"}])
    title.assert_called_once()
    assert title.call_args.kwargs["source"] == "discord"
    assert title.call_args.kwargs["model_config"] == {"model": "test-model"}
    callback = title.call_args.kwargs["title_callback"]
    with patch.object(runner, "_schedule_discord_semantic_thread_rename") as rename:
        callback("Interim title", "fallback")
        rename.assert_not_called()
        callback("Semantic Session Title", "llm")
    rename.assert_called_once_with(source, "session-1", "Semantic Session Title")
