...
class TestStreamTtsTempfileFallback:
    ...
    def test_tempfile_handle_closed_before_playback(self, monkeypatch):
        import wave
        import tools.tts_tool as tts_origin
        ...
        monkeypatch.setattr(tts_origin, "_import_sounddevice", _no_sounddevice)

        events = []  # ordered log of ("close", name) / ("play", path)

        # Spy on NamedTemporaryFile to record when the handle is closed.
        real_ntf = tts_speaker.tempfile.NamedTemporaryFile
        ...
        monkeypatch.setattr(tts_speaker.tempfile, "NamedTemporaryFile", _spy_ntf)
