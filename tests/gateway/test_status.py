...
def _no_taskkill(monkeypatch):
    """The Windows force path never shells out (2026-09-17: ``taskkill /T`` adopted orphans
    of a recycled pid); any ``subprocess.run`` from ``terminate_pid`` is the old bug."""
...
class TestTerminatePid:
    def test_force_walks_the_tree_pinned_to_the_creation_time_on_windows(self, monkeypatch):
        """Root identity is the centisecond guard; the walk is pinned to the float creation
        time read BEFORE that guard, so a pid recycled after it kills nothing."""
        from hermes_cli import _subprocess_compat as compat

        monkeypatch.setattr(status, "_IS_WINDOWS", True)
        _no_taskkill(monkeypatch)
        walks = []
        monkeypatch.setattr(compat, "windows_process_created", lambda pid: 4.56)
    ...
    def test_windows_force_refuses_reused_pid(self, monkeypatch):
        ...
        _no_taskkill(monkeypatch)
        monkeypatch.setattr(compat, "windows_kill_process_tree",
                            lambda pid, **kw: (_ for _ in ()).throw(AssertionError("walked a refused pid")))

        with pytest.raises(OSError, match="process identity changed"):
            status.terminate_pid(123, force=True, expected_start_time=456)

    def test_force_kill_on_dead_target_is_not_fatal(self, monkeypatch):
        """A walk that reports nothing terminated while the target is actually gone must
        NOT raise: process liveness is the source of truth, not the kill's own report.
        Regression class of the 2026-08-11 outage, where the old kill's exception
        propagated through ``_force_terminate_known_gateway_pids`` -> ``stop()`` ->
        ``restart()`` and left :8642 with no listener for ~5 minutes."""
        from hermes_cli import _subprocess_compat as compat
        ...
        monkeypatch.setattr(compat, "windows_kill_process_tree", lambda pid, **kw: [])
        # The kill landed (or the root exited on its own): the target is gone by the time we look.
        monkeypatch.setattr(status, "_pid_exists", lambda pid: False)

        status.terminate_pid(47264, force=True, expected_start_time=123)  # must not raise

    def test_force_kill_on_live_target_raises_oserror(self, monkeypatch):
        """A target that survives the walk is a real failure, surfaced as ``OSError`` — the
        type every caller in the kill path already handles."""
        from hermes_cli import _subprocess_compat as compat
    ...
    def test_force_kill_polls_until_the_target_disappears(self, monkeypatch):
        ...
        immediately after the kill would call a dying process alive and
        ...
        _no_taskkill(monkeypatch)
        monkeypatch.setattr(compat, "windows_process_created", lambda pid: 1.23)
        monkeypatch.setattr(compat, "windows_kill_process_tree", lambda pid, **kw: [pid])
