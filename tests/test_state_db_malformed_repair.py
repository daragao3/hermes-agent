...
def test_probe_deadline_interrupts_a_long_running_statement(tmp_path):
    ...
    from hermes_state_local_fts import _arm_probe_deadline
...
def test_bounded_probe_raises_timeout_rather_than_reporting_corruption(tmp_path):
    ...
    from hermes_state_local_fts import StateDbProbeTimeout
    from hermes_state_repair import _db_opens_cleanly
...
def test_unbounded_probe_is_the_default_and_never_times_out(tmp_path, monkeypatch):
    ...
    # Exhaustive rank=1 FTS diagnostics now have a separate entry point.
    # A default repair probe must never call that expensive diagnostic.
    import hermes_state_local_fts

    def unexpected_full_fts_check(*args, **kwargs):
        raise AssertionError("default repair probe ran exhaustive FTS diagnostics")

    monkeypatch.setattr(
        hermes_state_local_fts, "_fts_integrity_reason", unexpected_full_fts_check
    )
