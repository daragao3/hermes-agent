...
from utils import (
    ...
    atomic_roundtrip_yaml_update as atomic_roundtrip_yaml_update,
    atomic_yaml_write,
)
...
def test_atomic_yaml_write_restores_owner_on_real_symlink_target(
    ...
    monkeypatch.setattr(
        "utils.os.chown",  # windows-footgun: ok -- guarded by the os.name check at the top of each test
...
def test_windows_readonly_target_still_raises(tmp_path: Path) -> None:
    ...
    target.write_text("old", encoding="utf-8")
    tmp = _write_tmp(tmp_path, "new")
    subprocess.run(["attrib", "+R", str(target)], capture_output=True, check=True, timeout=10)
    try:
