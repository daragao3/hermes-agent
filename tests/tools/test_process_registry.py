...
class TestSpawnRewriteCompoundBackground:
    ...
    def test_pty_path_uses_rewritten_command(self, registry):
        ...
        with patch("tools.process_registry._find_shell", return_value="/bin/bash"), \
             patch.dict("sys.modules", {"winpty" if sys.platform == "win32" else "ptyprocess": mock_pty_module}), \
             patch("threading.Thread", return_value=fake_thread), \
             patch.object(registry, "_write_checkpoint"):
            session = registry.spawn_local(
...
from tools.process_registry_notifications import format_process_notification as format_process_notification
