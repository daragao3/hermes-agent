...
class TestInflightProcessRegistry:
    ...
    def test_execute_registers_for_the_wait_window_only(self, monkeypatch):
        ...
        owner_tid = threading.current_thread().ident

        def _wait(p, timeout=120, *, bounded_capture=False, watch_interrupt_tid=None):
            assert watch_interrupt_tid == owner_tid
            seen["during"] = owner_tid in base.inflight_process_threads()
