...
class TestInflightProcessRegistry:
    ...
    def test_execute_registers_for_the_wait_window_only(self, monkeypatch):
        ...
        proc = MagicMock()
        seen = {}
        # The wait runs on a bounded worker thread; the registry is keyed on the TOOL thread
        # (this one), which is what the scheduler's run_tool_thread_ids collects.
        tool_tid = threading.current_thread().ident

        def _wait(p, timeout=120, *, bounded_capture=False, **_kw):
            seen["during"] = tool_tid in base.inflight_process_threads()
