...
class TestStdioPidTracking:
    ...
    def test_kill_orphaned_handles_dead_pids(self):
        ...
        with patch("tools.mcp_tool_lifecycle.time.sleep"):
        ...
        async def _run():
            ...
            with patch("tools.mcp_tool._MCP_AVAILABLE", True), \
                ...
                with patch("tools.mcp_tool.stdio_client", return_value=cm), \
                     patch("tools.mcp_tool_lifecycle.time.sleep"):
                    try:
    ...
    def test_kill_orphaned_can_filter_by_server_name(self):
        ...
        with patch("tools.mcp_tool_lifecycle.os.kill") as mock_kill, \
             patch("gateway.status._pid_exists", return_value=False), \
             patch("tools.mcp_tool_lifecycle.time.sleep") as mock_sleep:
...
class TestStdioPgroupReaping:
    ...
    def test_killpg_used_when_pgid_tracked(self, monkeypatch):
        ...
        with patch("tools.mcp_tool_lifecycle.os.killpg") as mock_killpg, \
    ...
    def test_killpg_skipped_when_pgid_matches_gateway_own_pgroup(self, monkeypatch):
        ...
        with patch("tools.mcp_tool_lifecycle.os.getpgrp", return_value=gateway_pgid), \
             patch("tools.mcp_tool_lifecycle.os.killpg") as mock_killpg, \
             patch("tools.mcp_tool_lifecycle.os.kill") as mock_kill, \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("time.sleep"):
            _kill_orphaned_mcp_children()
