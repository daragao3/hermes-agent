...
def test_delegate_task_background_batch_runs_as_one_unit(monkeypatch):
    ...
    monkeypatch.setattr(dt, "_resolve_delegation_credentials", lambda *a, **k: creds)
    # Goals must be >= 10 characters -- 0.21.1 rejects a short goal before dispatch, so the
    # old "a"/"b"/"c" returned {"error": ...} and never reached the background path.
    goals = ["first batch member", "second batch member", "third batch member"]
    out = dt.delegate_task(
    ...
    assert summaries == sorted(f"done: {g}" for g in goals)
    ...
    assert all(f"done: {g}" in text for g in goals)
