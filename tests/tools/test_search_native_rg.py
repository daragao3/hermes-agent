"""Local content/file search runs rg as a direct argv subprocess.
...
def test_native_runner_honours_deadline_and_interrupt_while_rg_is_silent(tree, ops_factory, monkeypatch):
    ...
    result = ops._run_rg_native([shlex.quote(sys.executable), "-c", shlex.quote("import time; time.sleep(30)")], 5, timeout=1)
    ...
    try:
        started = time.monotonic()
        result = ops._run_rg_native([shlex.quote(sys.executable), "-c", shlex.quote("import time; time.sleep(30)")], 5, timeout=30)
    finally:
