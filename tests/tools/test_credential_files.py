...
class TestLateNativePathCarry:
    def test_backslash_declarations_refused_by_registration_and_config(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        token = tmp_path / "service" / "token.json"
        token.parent.mkdir()
        token.write_text("{}", encoding="utf-8")
        monkeypatch.setattr("hermes_cli.config.read_raw_config", lambda: {
            "terminal": {"credential_files": [r"service\token.json"]}
        })
        assert register_credential_file(r"service\token.json") is False
        assert get_credential_file_mounts() == []
        assert register_credential_file("service/token.json") is True
        assert get_credential_file_mounts() == [{
            "host_path": str(token.resolve()),
            "container_path": "/root/.hermes/service/token.json",
        }]

    @pytest.mark.parametrize("kind", ["skills", "cache/documents"])
    def test_nested_native_files_have_posix_container_paths(self, tmp_path, monkeypatch, kind):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        ...
        item = tmp_path / kind / "nested" / "file.txt"
        item.parent.mkdir(parents=True)
        item.write_text("fixture", encoding="utf-8")
        entries = iter_skills_files() if kind == "skills" else iter_cache_files()
        matches = [entry for entry in entries if Path(entry["host_path"]) == item]
        assert matches == [{
            "host_path": str(item),
            "container_path": f"/root/.hermes/{kind}/nested/file.txt",
        }]
...
class TestSkillsDirectoryMount:
    ...
    def test_sanitized_copy_skips_bookkeeping_dirs(self, tmp_path):
        ...
        (skills_dir / "cat" / "myskill" / "references" / "api.md").write_text("ref", encoding="utf-8")
...
class TestIterSkillsFiles:
    ...
    def test_skips_excluded_bookkeeping_dirs(self, tmp_path):
        ...
        for excluded in (".hub", ".archive", ".curator_backups", "node_modules"):
            junk = skills_dir / excluded / "vendored"
            junk.mkdir(parents=True)
            (junk / "SKILL.md").write_text("# stale copy", encoding="utf-8")
        # Also nested inside an otherwise-valid skill package.
        cache = skills_dir / "cat" / "myskill" / "__pycache__"
        cache.mkdir()
        (cache / "helper.cpython-311.pyc").write_text("bytecode", encoding="utf-8")
