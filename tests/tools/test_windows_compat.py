    ...
    def test_preexec_fn_is_guarded(self, relpath):
        ...
        for val in values:
            ...
            assert "attr='setsid'" not in val or "IfExp" in val or "None" in val, (
                f"{relpath} has unconditional preexec_fn=os.setsid"  # windows-footgun: ok -- prose in an assertion message, not a call
            )
...
class TestKillpgGuarded:
    ...
    def test_no_unguarded_killpg(self, relpath):
        ...
        for i, line in enumerate(lines):
            stripped = line.strip()
            if "os.killpg" in stripped or "os.getpgid" in stripped:  # windows-footgun: ok -- source-scan needle for the guard this meta-test enforces, not a call
                ...
                assert "_IS_WINDOWS" in context or "else:" in context, (
                    f"{relpath}:{i + 1} has unguarded os.killpg/os.getpgid call"  # windows-footgun: ok -- prose in an assertion message, not a call
                )
