...
def _missing_and_installable(monkeypatch):
    ...
    monkeypatch.setattr(lazy_deps, "_run_installer",
                        lambda cmd, **kw: lazy_deps.subprocess.CompletedProcess(cmd, 1, "", "mock installer failure"))
...
def _no_installer(monkeypatch):
    ...
    monkeypatch.setattr(lazy_deps, "_run_installer", _boom)
