...
def _cache(
    tmp_path, *, package, deps=None, bin_field, make_bin=True, entry="abc123",
    windows=None,
):
    """Build a fake npx cache entry with the host's executable launcher."""
    ...
    if is_windows:
        target = target.with_name(target.name + ".cmd")
    if make_bin:
        target.write_text(
            "@node dist/index.js %*\n" if is_windows else "#!/usr/bin/env node\n",
            encoding="utf-8",
        )
        target.chmod(0o755)
    return target
...
def test_windows_selects_launchers_never_the_sh_script():
    ...
    assert [path.replace("\\", "/") for path in win] == [
        "/c/bin/mcp-linear.cmd",
        "/c/bin/mcp-linear.exe",
    ]
    assert not any(c.endswith("mcp-linear") for c in win), "sh script must not be a candidate"

    assert [
        path.replace("\\", "/")
        for path in _npx_bin_candidates("/bin", "mcp-linear", windows=False)
    ] == ["/bin/mcp-linear"]


def test_posix_resolution_uses_the_helper(tmp_path, monkeypatch):
    ...
    target = _cache(
        tmp_path,
        package="mcp-linear",
        bin_field={"mcp-linear": "i.js"},
        windows=False,
    )
    original = mcp_tool_config._npx_bin_candidates
    monkeypatch.setattr(
        mcp_tool_config,
        "_npx_bin_candidates",
        lambda bin_dir, name: original(bin_dir, name, windows=False),
    )
