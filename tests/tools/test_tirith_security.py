...
def _reset_resolved_path(request, monkeypatch):
    """Reset process state and isolate platform-neutral tests from this host.

    Windows has no Tirith build and is covered explicitly by
    ``TestUnsupportedPlatform``. Every other class tests scanner/installer
    behavior after platform admission, so force that prerequisite true.
    """
    if request.node.cls is not TestUnsupportedPlatform:
...
class TestHermesHomeIsolation:
    ...
    def test_failure_marker_respects_hermes_home(self):
        ...
        assert result == os.path.join(
            os.path.normpath("/custom/hermes"), ".tirith-install-failed"
        )

    def test_get_hermes_home_fallback(self):
        """Without HERMES_HOME set, the canonical resolver uses OS home."""
        ...
        minimal_home = {"HOME": home}
        if os.name == "nt":
            minimal_home["USERPROFILE"] = home
            minimal_home["LOCALAPPDATA"] = os.path.join(home, "AppData", "Local")
        with patch.dict(os.environ, minimal_home, clear=True):
