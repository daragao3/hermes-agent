...
class TestRegistration:
    ...
    def test_cua_driver_cmd_env_override_is_resolved_dynamically(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            cua_backend_driver.shutil,
            "which",
            lambda command: str(driver) if command == str(driver) else None,
        )
    ...
    def test_check_fn_true_on_windows_when_binary_present(self):
        ...
        with patch("tools.computer_use.tool.sys.platform", "win32"), \
             patch("tools.computer_use.cua_backend_driver.cua_driver_binary_available", return_value=True):
    ...
    def test_check_fn_false_on_windows_without_binary(self):
        from tools.computer_use import tool as cu_tool
        with patch("tools.computer_use.tool.sys.platform", "win32"), \
             patch("tools.computer_use.cua_backend_driver.cua_driver_binary_available", return_value=False):
...
class TestAnthropicAdapterMultimodal:
    ...
    def test_content_parts_helper_filters_to_text_and_image(self):
        from agent.anthropic_message_convert import _content_parts_to_anthropic_blocks
...
class TestImageMimeTypePropagation:
    ...
    def test_extract_tool_result_handles_missing_mime_field(self):
        ...
        image_part = SimpleNamespace(type="image", data="/9j/4AAQ...")
...
class TestStructuredElementsConsumption:
    ...
    def test_vision_capture_falls_back_to_get_window_state_when_screenshot_dropped(self):
        ...
        def fake_call_tool(name, args, **_kwargs):
            if name == "list_windows":
    ...
    def test_capture_app_desktop_targets_shell_window(self):
...
class TestStartupTimeoutPhaseDetail:
    ...
    def test_timeout_error_defaults_to_unknown_phase(self):
        ...
        import tools.computer_use.cua_backend_session as session_mod
        # Same Event-factory stub as above: the pre-made event is replaced
        # inside _start_lifecycle_locked(), so patch the owner module's factory.
        fake_ready = MagicMock()
        fake_ready.wait.return_value = False
        with _patch.object(session_mod, "threading",
