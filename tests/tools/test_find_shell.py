...
class TestFindShellPrefersUserShell:
    ...
    @pytest.mark.skipif(os.name == "nt", reason="requires POSIX SHELL selection and executable mode bits")
...
class TestGitBashExternalProgramProbe:
    ...
    def test_probe_runs_external_msys_programs(self, monkeypatch):
        ...
        every host, so this stays on the Linux runner with ``run_text_capture``
        ...
        def reject_pipe_capture(*args, **kwargs):
            raise AssertionError("Git Bash health probe bypassed bounded capture")

        monkeypatch.setattr(gitbash_probe.subprocess, "run", reject_pipe_capture)
