...
class TestPlatformDefaultHermesHome:
    ...
    whether that directory happened to carry a root marker.  These tests pin
    ``sys.platform``, ``LOCALAPPDATA`` and ``Path.home`` instead, so the
    Windows preference rule is stated explicitly and runs identically on
    POSIX CI and on a Windows box.
    """

    @staticmethod
    def _constants(monkeypatch, platform, tmp_path, local_appdata=None):
        import importlib
        import hermes_constants
        importlib.reload(hermes_constants)
    ...
    def test_posix_default_is_dot_hermes_under_home(self, monkeypatch, tmp_path):
        constants = self._constants(monkeypatch, "linux", tmp_path)
    ...
    def test_windows_prefers_localappdata_when_neither_is_initialized(
        ...
        constants = self._constants(monkeypatch, "win32", tmp_path, appdata)
    ...
    def test_windows_falls_back_to_home_appdata_when_env_unset(
        ...
        constants = self._constants(monkeypatch, "win32", tmp_path, local_appdata=None)
        assert (
