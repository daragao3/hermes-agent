...
def _symlinks_creatable() -> bool:
    """Probe by ATTEMPT, not by an is-admin check.

    Creating a symlink on Windows needs SeCreateSymbolicLinkPrivilege, which an elevated
    shell has AND which Developer Mode grants to a normal user -- so an admin check skips
    hosts that could actually run these. Without a guard the failure lands in SETUP as
    ``OSError: [WinError 1314]``, which reads like an assertion failure about path
    canonicalization rather than a missing host privilege.
    """
    import tempfile
...
_SYMLINKS = pytest.mark.skipif(
    not _symlinks_creatable(), reason="host cannot create symlinks (Windows: needs Developer Mode or elevation)"
)
