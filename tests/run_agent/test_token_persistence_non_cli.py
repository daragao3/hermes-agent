...
def test_run_conversation_persists_tokens_for_cron_sessions():
    ...
    assert result["final_response"] == "done"
    # queue_token_counts, not update_token_counts: 0.21.1 enqueues per-call deltas for the
    # SessionDB background writer instead of writing them inline on the turn thread (a cold
    # state.db UPDATE there stalled the tool loop). Same as the telegram sibling above.
    session_db.queue_token_counts.assert_called_once()
    assert session_db.queue_token_counts.call_args.args[0] == "cron-session"
    # The point of naming a CRON session: its first token write is a first-touch write, and
    # without these two the row is stamped source="unknown" with no model config (#23270).
    assert session_db.queue_token_counts.call_args.kwargs["source"] == "cron"
    assert (
        session_db.queue_token_counts.call_args.kwargs["model_config"]
        is agent._session_init_model_config
    )
