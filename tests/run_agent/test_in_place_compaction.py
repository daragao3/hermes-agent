...
def _no_host_toolchain_probe(monkeypatch):
    """Compaction tests do not inspect the host Python installation."""
    monkeypatch.setattr("tools.env_probe.get_environment_probe_line", lambda: "")
...
class TestInPlaceCompaction:
    def test_in_place_keeps_same_session_id(self, session_db):
    ...
    def test_in_place_alternation_preserved(self, session_db):
        """The compacted list must not introduce consecutive same-role messages."""
        from agent.conversation_compression import compress_context

        sid = "20260619_120500_cccccc"
        _seed(session_db, sid, "alt")
        agent = _make_agent(session_db, sid, in_place=True)
        messages = [{"role": "user", "content": f"m{i}"} for i in range(8)]
    ...
    def test_rotation_still_preflushes(self, session_db):
        """Rotation MUST pre-flush so current-turn messages survive in the
        preserved old (parent) session before it is ended (#47202)."""
        from agent.conversation_compression import compress_context

        _seed(session_db, "rot_flush", "f")
        agent = _make_agent(session_db, "rot_flush", in_place=False)
        calls = {"n": 0}
...
class TestRotationFallbackWhenFlagOff:
    def test_rotation_when_flag_off(self, session_db):
        ...
        unchanged — forks a continuation session with the same title."""
        from agent.conversation_compression import compress_context
...
class TestInPlaceSignalForGateway:
    ...
    def test_signal_set_on_in_place_unset_on_rotation(self, session_db):
        from agent.conversation_compression import compress_context

        # in-place → flag True
        _seed(session_db, "s_ip", "ip")
        a_ip = _make_agent(session_db, "s_ip", in_place=True)
        compress_context(
        ...
        # rotation → flag False
        _seed(session_db, "s_rot", "rot")
        a_rot = _make_agent(session_db, "s_rot", in_place=False)
        compress_context(
...
class TestInPlaceAntiGrowthGuard:
    ...
    def test_in_place_refuses_growing_compression(self, session_db):
        from agent.conversation_compression import compress_context
    ...
    def test_in_place_salvages_near_break_even_growth(self, session_db):
    ...
    def test_in_place_still_commits_shrinking_compression(self, session_db):
...
class TestCompactedTurnsStaySearchable:
    ...
    def test_compacted_turns_found_by_default_search(self, session_db):
        db = session_db
        sid = "20260619_search"
        db.create_session(sid, "cli", model="test/model")
        for r, c in [
    ...
    def test_rewound_turns_stay_hidden(self, session_db):
