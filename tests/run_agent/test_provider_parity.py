...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_explicit_openrouter_remains_available(self, monkeypatch):
        # Local policy omits OpenRouter from discovery, but configured use remains supported.
        monkeypatch.setattr("hermes_cli.config.load_config_readonly",
                            lambda: {"auxiliary": {"compression": {"provider": "openrouter"}}})
    ...
    def test_explicit_nous_remains_available(self, monkeypatch):
        # Nous is also explicit-only under the preserved local discovery policy.
        monkeypatch.setattr("hermes_cli.config.load_config_readonly",
                            lambda: {"auxiliary": {"compression": {"provider": "nous"}}})
        monkeypatch.setattr("agent.auxiliary_client._resolve_nous_runtime_api",
                            lambda **kwargs: ("test-nous-key", "https://inference-api.nousresearch.com/v1"))
        monkeypatch.setattr("agent.nous_rate_guard.nous_rate_limit_remaining", lambda: None)
