...
class TestRunningJobGuard:
    ...
    def test_create_execution_failure_does_not_wedge_running_set(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sched, "get_due_and_skipped_jobs", lambda: ([failing_job, healthy_job], []))
...
class TestSequentialPool:
    """Sequential (profile) jobs use the persistent cron-seq pool.

    Verifies the follow-up fix: env-mutating jobs no longer run inline
    in the ticker thread, so a long profile job cannot starve the
    schedule the same way the parallel path used to. (A workdir alone is
    per execution since 0.21.1 and rides the parallel pool.)
    """

    def test_sequential_job_does_not_block_ticker(self, tmp_path, monkeypatch):
        """sync=False returns immediately even when a profile job is slow."""
        import cron.scheduler as sched
        ...
        job = {
            ...
            "next_run_at": "2020-01-01T00:00:00",
            "deliver": "local",
            "profile": "financier",  # makes it sequential
        }
    ...
    def test_sequential_running_guard_prevents_double_dispatch(self, tmp_path, monkeypatch):
        """A profile job already in _running_job_ids is skipped on next tick."""
        import cron.scheduler as sched
