...
def test_tick_process_job_sequence(monkeypatch):
    ...
    monkeypatch.setattr(s, "get_due_and_skipped_jobs", lambda: ([{"id": "j1", "name": "t"}], []))
...
def test_run_one_job_exception_delivers_failure_alert(monkeypatch):
    ...
    assert ok is False
    # The fork wraps the compact reason in the CRON_FAILED notification header
    # (_summarize_cron_failure_for_delivery._delivered); the bare upstream line
    # is only its fallback when formatting is unavailable.
    assert len(delivered) == 1
    assert delivered[0][0] == "j3"
    assert "morning" in delivered[0][1]
    assert delivered[0][1].endswith("Error: Gemini HTTP 503 (UNAVAILABLE)")
    assert marked == [
...
def test_escaped_failure_delivery_stays_quiet_below_the_threshold(monkeypatch):
    ...
    assert ok is False
    # Fork: the compact reason is wrapped in the CRON_FAILED notification header.
    assert len(delivered) == 1
    assert "scout" in delivered[0]
    assert delivered[0].endswith("Error: provider failed")
...
def test_reason_error_without_exit_code_marks_workload_failed(monkeypatch):
    ...
    monkeypatch.setattr(
        s,
        "run_job",
        lambda _job, *, defer_agent_teardown=None, **_kw: (True, "raw output", response, None),
    )
...
def test_structured_nonzero_exit_overrides_green_model_turn_everywhere(monkeypatch):
    ...
    monkeypatch.setattr(s, "mark_execution_running", lambda *_a, **_k: {})
    monkeypatch.setattr(
        s,
        "finish_execution",
    ...
    # Fork: finish_execution also records the delivery outcome for the ledger.
    assert len(executions) == 1
    assert executions[0]["execution_id"] == "exec-workload-failed"
    assert executions[0]["success"] is False
    assert executions[0]["error"] == "Workload reported exit_code=1 in AGENT_ITERATION_JSON"
...
def test_tick_structured_nonzero_exit_marks_workload_failed(monkeypatch):
    ...
    monkeypatch.setattr(s, "advance_next_runs", lambda _job_id: True)
    monkeypatch.setattr(s, "claim_job_for_fire", lambda _job_id, **_kwargs: True)
