...
def isolate_event_emitter(monkeypatch):
    monkeypatch.setattr(s, "_get_event_emitter", lambda: None)
...
def test_tick_structured_nonzero_exit_marks_workload_failed(monkeypatch):
    ...
    monkeypatch.setattr(s, "claim_job_for_fire", lambda _job_id, **_kw: True)
...
def test_reason_error_without_exit_code_marks_workload_failed(monkeypatch):
    ...
    monkeypatch.setattr(
        s,
        "mark_job_run",
        lambda jid, ok, err=None, delivery_error=None, **_kw: marks.append((jid, ok)),
    )
...
def test_run_one_job_exception_delivers_failure_alert(monkeypatch):
    ...
    assert "CRON_FAILED" in delivered[0][1]
    assert "morning" in delivered[0][1]
    assert "Gemini HTTP 503 (UNAVAILABLE)" in delivered[0][1]
    assert marked == [
...
def test_escaped_failure_delivery_stays_quiet_below_the_threshold(monkeypatch):
    ...
    assert ok is False
    assert len(delivered) == 1
    assert "CRON_FAILED" in delivered[0]
