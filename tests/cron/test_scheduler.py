...
class TestOneShotDispatchClaim:
    ...
    def test_claim_runs_before_run_job(self):
        order = []
        with patch("cron.scheduler.get_due_and_skipped_jobs", return_value=([self._oneshot()], [])), \
...
class TestRunJobWakeGate:
    ...
    def test_script_runs_only_once_on_wake(self):
        ...
        with patch.object(scheduler, "_run_job_script_with_claim_heartbeat", side_effect=_script_stub), \
...
class TestPerJobSoftDeadline:
    ...
    def test_process_job_branch_amends_instead_of_suppressing(
        ...
        with patch("cron.scheduler.get_due_and_skipped_jobs", return_value=([job], [])),              patch("cron.scheduler.advance_next_runs"),              patch("cron.scheduler.claim_job_for_fire", return_value=True),              patch("cron.scheduler.create_execution", return_value={"id": "exec-real"}),              patch("cron.scheduler.mark_execution_running"),              patch("cron.scheduler.finish_execution"),              patch("cron.scheduler.run_job", side_effect=slow_success),              patch("cron.scheduler.save_job_output", return_value="/tmp/late.md"),              patch("cron.scheduler.mark_job_run", return_value="deadline-stamp"),              patch("cron.scheduler.amend_late_outcome_after_abandon", side_effect=amend),              patch("cron.scheduler.amend_execution_after_abandon"):
...
class TestSuspendAwareTimeoutWiring:
    ...
    SECOND SEAM, since 0.21.1: the inactivity check no longer lives in the poll
    loop. It runs in a daemon thread (``cron-inactivity-<job>``) around
    ``_inactivity_watchdog_loop``, which blocks on a real ``Event.wait`` and
    therefore cannot be reached by a thread-scoped clock -- widening that fake
    by thread name is not enough, because the watchdog's own wait is real too.
    ``_run`` replaces that loop function with a pump the calling thread steps
    by hand (see ``fake_watchdog_loop``). The code under test is untouched: the
    ``_idle_seconds`` closure still subtracts the credit and the real poll loop
    still accumulates it. The inactivity tests therefore assert ``run["idles"]``
    -- the readings the watchdog actually took -- as their positive control,
    the way the wall-clock tests assert the stall warning.
    """
    ...
    def _run(self, tmp_path, monkeypatch, caplog, *, gaps,
        ...
        # --- the inactivity watchdog pump ----------------------------------
        # 0.21.1 moved the inactivity check OUT of the poll loop into its own
        # daemon thread (``_inactivity_watchdog_loop``, driven by a real
        # ``Event.wait``). The fakes above are thread-scoped on purpose, so
        # that thread reads the REAL clock and a scripted suspend can never
        # reach it -- widening the clock fake by thread name does not help
        # either, because the watchdog also blocks on its own real
        # ``_watch_stop.wait``.
        #
        # So the loop FUNCTION is replaced by a pump the calling thread steps
        # by hand. Everything the wiring under test owns stays production
        # code: the ``_idle_seconds`` closure (which subtracts the suspend
        # credit) and the poll loop that accumulates that credit. Only the
        # waiting is faked, which is exactly what the thread-scoped clock
        # already does for the poll loop.
        wants_watchdog = float(inactivity_timeout) > 0
        evaluate = threading.Event()
        evaluated = threading.Event()
        watch = {
            "thread": None, "fired": False, "idles": [], "limit": None,
            "poll_s": None, "future_done": None, "ready": threading.Event(),
        }

        def fake_watchdog_loop(*, get_idle_seconds, limit_s, poll_s, stop,
                               future_done):
            watch["limit"] = limit_s
            watch["poll_s"] = poll_s
            watch["future_done"] = future_done
            watch["thread"] = threading.current_thread()
            watch["ready"].set()
            while not stop.is_set():
                if not evaluate.wait(0.25):
                    continue
                evaluate.clear()
                try:
                    if stop.is_set():
                        return False
                    # ``future_done`` is deliberately NOT consulted: the mock
                    # agent's future is finished before the loop even starts,
                    # and ``fake_wait`` below is already the authority on when
                    # the run completes. That it was wired at all is asserted
                    # after the run instead.
                    idle = float(get_idle_seconds() or 0.0)
                    watch["idles"].append(idle)
                    if idle >= limit_s:
                        watch["fired"] = True
                        return True
                finally:
        ...
        def pump_watchdog():
            """Evaluate the watchdog ONCE, against the state the poll loop has
            just finished accumulating, and block until it has answered.

            The handshake is what makes the ordering deterministic: the credit
            for a gap is always added before the idle reading it discounts is
            taken, and a firing verdict is visible to the loop's
            ``if _inactivity_timeout`` check on the very next line.
            """
            if not wants_watchdog or watch["fired"]:
                return
            assert watch["ready"].wait(5), "the inactivity watchdog never started"
            evaluated.clear()
            evaluate.set()
            assert evaluated.wait(5), "the inactivity watchdog never evaluated"
            if watch["fired"]:
                # ``_watch_inactivity`` sets ``_inactivity_timeout`` AFTER the
                # loop function returns; joining is what orders that write
                # before the poll loop reads it.
                watch["thread"].join(5)
        ...
        def fake_wait(fs, timeout=None, **kwargs):
            ...
            pump_watchdog()
            if watch["fired"]:
                # Report NOT done so the loop reaches ``if _inactivity_timeout``
                # rather than completing on the (long-finished) future.
                return set(), set(fs)
        ...
        monkeypatch.setattr(
            "cron.scheduler._inactivity_watchdog_loop", fake_watchdog_loop)
        ...
        if wants_watchdog:
            # The watchdog must have been wired with the configured limit and
            # the future's own ``done``; a pump that was handed neither would
            # make every inactivity assertion below vacuous.
            assert watch["limit"] == pytest.approx(float(inactivity_timeout))
            from cron.scheduler import _CRON_RUN_POLL_INTERVAL_S
            assert watch["poll_s"] == pytest.approx(_CRON_RUN_POLL_INTERVAL_S)
            assert callable(watch["future_done"])
        return {
            "success": success,
            "error": error,
            "polls": polls["n"],
    ...
    def test_suspend_is_discounted_from_the_inactivity_check(
        ...
        # The watchdog really ran and really read the discounted idle.
        assert run["idles"], "the inactivity watchdog never evaluated"
        assert self._stalled(run), run["logs"]
        assert run["success"] is True, run["error"]
        run["agent"].interrupt.assert_not_called()
