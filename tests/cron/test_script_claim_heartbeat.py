...
def test_cancel_event_terminates_script_process_tree(tmp_path, monkeypatch):
    ...
    kill_diagnostics = []
    original_run = sched_script.subprocess.run

    def record_tree_kill(argv, *args, **kwargs):
        try:
            result = original_run(argv, *args, **kwargs)
        except Exception as exc:
            if isinstance(argv, list) and argv and str(argv[0]).lower() == "taskkill":
                kill_diagnostics.append(repr(exc))
            raise
        if isinstance(argv, list) and argv and str(argv[0]).lower() == "taskkill":
            kill_diagnostics.append((result.returncode, result.stdout, result.stderr))
        return result

    monkeypatch.setattr(sched_script.subprocess, "run", record_tree_kill)
    ...
    child_code = (
        ...
        f"time.sleep(30); Path({str(child_done)!r}).write_text('done')"
    )
    script.write_text(
    ...
    try:
        ...
        try:
            ...
            pytest.fail(f"script descendant survived cancellation: {kill_diagnostics!r}")
        assert not child_done.exists(), "child completed instead of being cancelled"
    finally:
        # This handle belongs to the exact child whose startup PID we captured.
        if child.is_running():
            child.kill()
            child.wait(timeout=3)
...
def test_repeated_heartbeat_errors_cancel_after_bounded_grace(monkeypatch):
    ...
    def heartbeat(*_args, **_kwargs):
        nonlocal calls, confirmed_at, failed_at
        calls += 1
        if calls == 1:
    ...
    assert scheduler.run_one_job(job) is True
    assert calls >= 2
    assert failed_at - confirmed_at >= 0.03
