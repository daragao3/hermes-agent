...
def _no_live_host_probes(monkeypatch):
    ...
    # 0.21.1: the probe is `_fetch_codex_oauth_context_lengths_with_source`, returning
    # (catalogue, came_from_http); `({}, False)` is exactly its own "not a fresh
    # provider confirmation" shape, so nothing downstream persists anything.
    monkeypatch.setattr(
