...
from cron.jobs import (
    ...
    load_jobs,
    mark_job_run,
    rearm_oneshot as rearm_oneshot,
    resume_job,
    save_jobs,
    trigger_job,
    update_job,
...
def test_paused_job_requires_explicit_resume_before_trigger(tmp_cron_dir):
    job = create_job("paused", "every 1h")
    from cron.jobs import JobPaused, pause_job

    pause_job(job["id"])
    before = copy.deepcopy(load_jobs())
    with pytest.raises(JobPaused):
        trigger_job(job["id"], caller="test:manual-run")
    assert load_jobs() == before
    resume_job(job["id"], caller="test:explicit-resume")
    triggered = trigger_job(job["id"], caller="test:manual-run")
    assert triggered["state"] == "scheduled"
    assert triggered["enabled"] is True
