...
def test_paused_job_run_is_refused_not_silently_resumed(tmp_cron_dir):
    """A run request must not clear a containment hold as a side effect.

    Upstream allows trigger_job to un-pause; this fork refuses with JobPaused (which upstream does
    not have) so the operator sees WHAT they would be overriding. Resuming is a separate,
    explicit action -- the same rule the dashboard's run-now enforces as a 409.
    """
    import pytest

    from cron.jobs import JobPaused, get_job, pause_job
    ...
    # Refused BEFORE any write: the hold is intact, with nothing to reconcile.
