...
class TestRunningFireOwnerRegistry:
    ...
    # test_shutdown_sees_all_concurrent_direct_fire_owners (new in 0.21.1) was removed in the
    # 0.21.1 merge: it needs two concurrent direct fires of ONE job id to both run, which this
    # fork refuses by design -- _run_one_job_admitted registers the job in-flight first and a
    # duplicate fire is skipped with CRON_SKIPPED_DUPLICATE. The test could only ever break its
    # own barrier here. The reachable registry behaviour stays covered by the tests below.
...
class TestCallerLossAfterClaimAcquisition:
    ...
    def test_second_fire_cannot_claim_while_first_ownership_live(self, tmp_path):
        ...
        with jobs.use_cron_store(tmp_path):
            ...
            # The fork stamps and returns the run timestamp rather than True (see
            # cron.jobs.mark_job_run); a non-empty string is the "it landed" result.
            assert isinstance(jobs.mark_job_run(
