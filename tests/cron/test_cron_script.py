...
class TestRunJobScript:
    ...
    def test_windows_pythonw_script_uses_sibling_python_for_captured_output(self, cron_env, tmp_path, monkeypatch):
        ...
        # 0.21.1 runs scripts through Popen + a cancel/deadline poll loop in cron.scheduler_script
        # (with a BOUNDED pipe drain), not through run_text_capture -- patch where the spawn is.
        from cron import scheduler_script as script_mod

        monkeypatch.setattr(script_mod.sys, "platform", "win32")
        monkeypatch.setattr(script_mod.sys, "executable", str(pythonw))
        ...
        # The sibling python is what makes captured output decodable at all; the Popen path asks
        # for the lossy utf-8 decode explicitly, so locale-mismatched bytes cannot raise in the
        # reader threads.
