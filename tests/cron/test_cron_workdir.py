...
class TestTickWorkdirPartition:
    ...
    def test_workdir_jobs_overlap_on_parallel_pool(self, tmp_path, monkeypatch):
        ...
        workdir_a.mkdir()
        workdir_b.mkdir()
        # timeout_seconds=0 disables the fork's per-job soft-deadline wrapper (which
        # would otherwise run the body on a "cron-job-<id>" thread), so the thread
        # observed here is the POOL lane itself.
        jobs = [
            {"id": "a", "name": "A", "workdir": str(workdir_a), "timeout_seconds": 0},
            {"id": "b", "name": "B", "workdir": str(workdir_b), "timeout_seconds": 0},
        ]
    ...
    def test_profile_jobs_run_sequentially(self, tmp_path, monkeypatch):
        """Profile jobs still take the fork's single-thread lane (see
        _job_mutates_process_globals); a workdir alone no longer does (0.21.1
        passes it as the per-execution cwd), so the writer here is ``profile``."""
        import cron.scheduler as sched

        # Two profile jobs (both sequential) + one parallel job.
        workdir_a = {"id": "a", "name": "A", "profile": "financier", "workdir": str(tmp_path)}
        workdir_b = {"id": "b", "name": "B", "profile": "financier", "workdir": str(tmp_path)}
