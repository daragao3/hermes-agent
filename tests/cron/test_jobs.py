...
class TestPauseResumeJob:
    ...
    def test_resume_rejects_past_oneshot(self, tmp_cron_dir, monkeypatch):
        ...
        with pytest.raises(ValueError, match="in the past"):
            resume_job("test-resume-past", caller="hermes_cli:cron_resume")
...
class TestGetDueJobs:
    ...
    def test_stale_past_due_fire_once_is_fast_forwarded(self, tmp_cron_dir):
        """Catch-up fires once while immediately closing the refire window.

        The due scan fast-forwards before returning so a crash before scheduler
        dispatch, and the external fire-due path that never calls
        advance_next_run(), cannot re-read the same missed occurrence.
        """
        job = create_job(prompt="Stale", schedule="every 1h")
        # Force next_run_at to 35 minutes ago (beyond the 30-min grace for hourly)
        jobs = load_jobs()
        ...
        assert len(due) == 1
        assert due[0]["id"] == job["id"]
        # Persisted immediately so a pre-run crash cannot rediscover the miss.
        updated = get_job(job["id"])
        from cron.jobs import _ensure_aware, _hermes_now
        next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert next_dt > _hermes_now()
