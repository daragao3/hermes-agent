...
def test_shell_hooks_hide_hook_command_windows(monkeypatch):
    ...
    assert captured[0][1]["timeout"] == shell_hooks.ShellHookSpec(event="post_tool_call", command="hook-bin --flag").timeout
...
def test_tui_shell_exec_rpc_hides_console_window(monkeypatch):
    ...
    def fake_popen(cmd, **kwargs):
        captured.append((cmd, kwargs))
        return SimpleNamespace(args=cmd, returncode=0, wait=lambda timeout=None: 0)
    ...
    assert spawns[0][1]["stdout"] != subprocess.PIPE
    assert spawns[0][1]["stderr"] != subprocess.PIPE
...
def test_lazy_deps_uv_install_hides_console_window(monkeypatch):
    ...
    _patch_hide_flags(monkeypatch)
    monkeypatch.setattr(lazy_deps.subprocess, "Popen", fake_popen)
    monkeypatch.setattr(lazy_deps, "_uv_binary", lambda: "/usr/bin/uv")
    monkeypatch.setattr(lazy_deps, "_warm_installed_bytecode", lambda *args: None)
...
def test_checkpoint_manager_git_hides_windows(monkeypatch):
    ...
    This runs on native Windows without replacing the platform. The check is a
