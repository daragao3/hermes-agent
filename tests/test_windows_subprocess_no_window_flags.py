...
def test_lazy_deps_uv_install_hides_console_window(monkeypatch):
    ...
    # The installer spawn goes through run_text_capture in this fork (uv forks a build backend
    # that inherits a capture PIPE and holds its write end open, so subprocess.run re-drains
    # forever after killing the direct child). That helper applies CREATE_NO_WINDOW itself, so
    # lazy_deps no longer imports windows_hide_flags -- there is no creationflags kwarg here to
    # assert on, and the no-console-flash guarantee lives one layer down.
...
def test_gateway_force_kill_hides_taskkill_window(monkeypatch):
    ...
    # A force kill now requires a pinned process identity: PIDs are reusable, and terminate_pid
    # refuses outright rather than risk taking out whatever inherited the number. Stub the lookup
    # so the identity check is deterministic, and pass the guard the way callers do.
    monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 1234.0)

    status.terminate_pid(123, force=True, expected_start_time=1234.0)
    ...
    assert kill_calls == [
        (
            ["taskkill", "/PID", "123", "/T", "/F"],
            {
                "capture_output": True,
                "text": True,
                # taskkill's output is decoded utf-8/replace: a non-UTF-8 Windows console
                # codepage otherwise raises in the reader thread mid-kill.
                "encoding": "utf-8",
                "errors": "replace",
...
def test_gateway_pid_scan_hides_wmic_and_powershell_windows(monkeypatch):
    ...
    # The scan spawns through bounded_probe_run (it bounds a slow WMI scan and hides the console
    # window itself). Patching gateway.subprocess.run alone is INERT here -- the scan then runs
    # for real against this box and returns the live gateway pid instead of the stub.
    monkeypatch.setattr(_subprocess_compat, "bounded_probe_run", fake_run)
    ...
    # Both spawns go through bounded_probe_run, which applies the hide flag itself -- so the
    # contract is now "the scan never spawns these two commands any other way", not a
    # creationflags kwarg on the caller's side.
    assert [cmd[0] for cmd, _kwargs in captured if cmd and cmd[0] in {"wmic", "powershell", "pwsh"}] == [
        "wmic", "powershell",
    ]
