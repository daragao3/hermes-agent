...
class TestCodexOAuthContextLength:
    ...
    def test_fallback_table_resolution_also_bumped(self, slug):
        """Only stale 272K fallback values are bumped; the measured local
        Sol fallback remains larger. Variant suffix stripping is unchanged."""
        from agent.model_metadata import get_model_context_length
        ...
        assert ctx == (1_050_000 if slug == "gpt-5.6-sol-900k" else 900_000)

    @pytest.mark.parametrize("slug", ["gpt-5.6-sol", "gpt-daybreak-blue-latest"])
    def test_fallback_table_base_slug_retains_route_fallback(self, slug):
        """Offline base slugs retain their route-specific fallback values."""
        from agent.model_metadata import get_model_context_length
        ...
        assert ctx == (1_050_000 if slug == "gpt-5.6-sol" else 272_000)
    ...
    _900K_TABLE = [
        ...
        ("gpt-5.6-sol-pro-900k",          False, 1_050_000, "gpt-5.6-sol-pro-900k"),
...
# Exercise the real metadata function through the documented guard opt-in.
# Every test below mocks requests.get; this does not authorize network traffic.
@pytest.mark.real_local_server_probe
class TestFetchEndpointModelMetadata:
    @pytest.fixture(autouse=True)
    def isolated_endpoint_disk_cache(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
        monkeypatch.setattr(
            "agent.model_metadata._get_endpoint_metadata_cache_path",
            lambda: tmp_path / "endpoint-models.json",
        )
