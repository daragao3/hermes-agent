...
class TestCaching:
    @staticmethod
    def _changing_command(*, json_payload: bool = False) -> str:
        code = "import json,time; value='tok-'+str(time.time_ns()); "
        code += (
            "print(json.dumps({'access_token': value, 'expires_in': 3600}))"
            if json_payload
            else "print(value)"
        )
        # shell=True uses cmd.exe on Windows and /bin/sh on POSIX; use each
        # shell's own quoting for the same Python command.
        if os.name == "nt":
    ...
    def test_token_is_cached_between_calls(self):
        ...
        source = CommandTokenSource(self._changing_command(), "dbx")
        assert source() == source()

    def test_expired_token_is_reminted(self):
        source = CommandTokenSource(self._changing_command(json_payload=True), "dbx")
        first = source()
...
class TestAbsoluteExpiry:
    ...
    def test_the_token_actually_gets_re_minted(self, tmp_path):
        ...
        cmd = (
            ...
            if os.name == "nt"
            else f"{shlex.quote(sys.executable)} -c {shlex.quote(code)}"
        )
