...
class TestEdgeCases:
    ...
    @pytest.mark.parametrize("usage", [{}, {"input_tokens": 10}, {"cache_read_tokens": 10}, {"api_call_count": 1}])
    def test_unknown_bucket_requires_billable_usage(self, db, usage):
        """Unpriced billable usage warns; a session that consumed nothing does not."""
        db.create_session(session_id="s1", source="cli", model="test")
        if usage:
            db.update_token_counts("s1", model="test", billing_provider="custom", cost_status="unknown", **usage)
        db._conn.commit()

        engine = InsightsEngine(db)
        report = engine.generate(days=30)
        text = engine.format_terminal(report)
        assert report["overview"]["unknown_cost_sessions"] == int(bool(usage))
        assert ("💰 Cost" in text) == bool(usage)
        assert ("Unknown" in text) == bool(usage)
