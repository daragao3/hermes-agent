...
class TestEdgeCases:
    ...
    def test_zero_usage_session_does_not_create_unknown_cost_warning(self, db):
        """A session with no API call or tokens incurred nothing to price.

        Counting it as unknown spend creates a permanent false warning: there
        is no route or usage record to resolve later. Sessions with billable
        usage and unknown pricing remain covered by the cost-bucket tests above.
        """
        db.create_session(session_id="s1", source="cli", model="test")
        db._conn.commit()
        ...
        report = engine.generate(days=30)
        text = engine.format_terminal(report)
        assert report["overview"]["unknown_cost_sessions"] == 0
