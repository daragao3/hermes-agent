...
def test_run_agent_imports_and_calls_loop_fault_at_abort():
    # 0.21.1 split non-retryable recovery out of conversation_loop.
    src = Path(__file__).resolve().parents[2] / "agent" / "turn_recovery.py"
    text = src.read_text(encoding="utf-8", errors="replace")
    # The emit must be wired in the non-retryable abort branch.
    assert "emit_agent_loop_fault" in text, "SR-471 emit not wired into turn_recovery"
