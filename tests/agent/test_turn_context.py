...
def test_prologue_title_preserves_session_provenance():
    from agent import turn_context
    agent = _TitlingAgent('cli')
    agent._session_source_for_persistence = lambda: 'native-cli'
    agent._session_init_model_config = {'provider': 'configured'}
    with patch('agent.title_generator.maybe_auto_title') as titler:
        turn_context._maybe_title_session_at_turn_start(agent, [{'role': 'user', 'content': 'Fix login'}])
    assert titler.call_args.kwargs['source'] == 'native-cli'
    assert titler.call_args.kwargs['model_config'] == agent._session_init_model_config
