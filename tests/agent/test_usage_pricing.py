...
def test_current_gemini_generations_are_priced_not_unknown():
    expected = {
        ...
        # 1M prompt tokens sits ABOVE the 200k context tier, so the whole
        # request bills at the tier rate (4.00 + 18.00); 2.00 + 12.00 is the
        # sub-200k figure. (0.21.1 models the tier; the flat 14.00 predates it.)
        "gemini-3.1-pro-preview": 22.00,
        "gemini-2.5-pro": 17.50,  # 2.50 + 15.00 above the 200k tier, same reason
