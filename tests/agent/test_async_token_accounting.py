...
class TestCoalesceFieldContract:
    ...
    def test_first_touch_attribution_partitions_coalescing(self, db):
        """Different source/model_config values must not collapse into one delta."""
        first = {
            "input_tokens": 1,
            "source": "cli",
            "model_config": {"provider": "nous", "fallbacks": ["a", "b"]},
        }
        second = {
            "input_tokens": 2,
            "source": "telegram",
            "model_config": {"provider": "openrouter", "fallbacks": ["c"]},
        }

        result = db._coalesce_token_deltas([("s", first), ("s", second)])

        assert result == [("s", first), ("s", second)]

    def test_equal_nested_model_config_still_coalesces(self, db):
        config_a = {"provider": "nous", "nested": {"models": ["a", "b"]}}
        config_b = {"nested": {"models": ["a", "b"]}, "provider": "nous"}

        result = db._coalesce_token_deltas(
            [
                ("s", {"input_tokens": 1, "source": "cli", "model_config": config_a}),
                ("s", {"input_tokens": 2, "source": "cli", "model_config": config_b}),
            ]
        )

        assert len(result) == 1
        assert result[0][1]["input_tokens"] == 3
        assert result[0][1]["model_config"] == config_a
