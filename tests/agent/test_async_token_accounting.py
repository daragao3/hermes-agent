...
class TestAttributionCarry:
    def test_queued_placeholder_keeps_source_and_model_config(self, db):
        db.queue_token_counts("attribution", input_tokens=7, model="model-a",
                              source="subagent", model_config={"_delegate_from": "parent"})
        assert db.flush_token_counts(timeout=10)
        row = db.get_session("attribution")
        assert row["source"] == "subagent"
        assert json.loads(row["model_config"]) == {"_delegate_from": "parent"}
        assert row["input_tokens"] == 7

    def test_unknown_placeholder_yields_once_to_real_source(self, db):
        db.update_token_counts("placeholder", input_tokens=1)
        db.update_token_counts("placeholder", input_tokens=2, source="cron",
                               model_config={"job": "one"})
        db.update_token_counts("placeholder", input_tokens=3, source="unknown")
        db.update_token_counts("placeholder", input_tokens=4, source="cli")
        row = db.get_session("placeholder")
        assert row["source"] == "cron"
        assert json.loads(row["model_config"]) == {"job": "one"}
        assert row["input_tokens"] == 10

    @pytest.mark.parametrize("later", [
        {"source": "cron", "model_config": {"job": "one"}},
        {"source": "subagent", "model_config": {"job": "two"}},
    ])
    def test_metadata_changes_prevent_coalescing(self, db, later):
        batch = [
            ("same", {"model": "a", "input_tokens": 1, "source": "subagent", "model_config": {"job": "one"}}),
            ("same", {"model": "a", "input_tokens": 2, **later}),
        ]
        assert db._coalesce_token_deltas(batch) == batch

    def test_equal_metadata_still_coalesces(self, db):
        batch = [("same", {"model": "a", "input_tokens": n, "source": "cron", "model_config": {"job": "one"}})
                 for n in (1, 2)]
        result = db._coalesce_token_deltas(batch)
        assert len(result) == 1
        assert result[0][1]["input_tokens"] == 3
        assert result[0][1]["model_config"] == {"job": "one"}

    def test_queued_metadata_is_a_call_time_snapshot(self, db, monkeypatch):
        started, release = threading.Event(), threading.Event()
        original = db.update_token_counts

        def delayed(*args, **kwargs):
            started.set()
            assert release.wait(10), "test failed to release the writer"
            return original(*args, **kwargs)

        monkeypatch.setattr(db, "update_token_counts", delayed)
        metadata = {"nested": {"model": "original"}}
        try:
            db.queue_token_counts("snapshot", input_tokens=1, source="cli", model_config=metadata)
            assert started.wait(10), "writer did not start"
            metadata["nested"]["model"] = "later"
        finally:
            release.set()
        assert db.flush_token_counts(timeout=10)
        assert json.loads(db.get_session("snapshot")["model_config"]) == {"nested": {"model": "original"}}
