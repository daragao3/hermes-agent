...
class TestAuxiliaryFallbackLayering:
    ...
    def test_auto_provider_uses_task_then_main_chain_before_builtin_chain(self, monkeypatch):
        ...
        mock_task_chain.assert_called_once()
        task_args, task_kwargs = mock_task_chain.call_args
        assert task_args == ("title_generation", "auto")
        assert task_kwargs["reason"] == "payment error"
        assert task_kwargs["failed_model"] is None
        mock_main_chain.assert_called_once_with(
            "title_generation", "auto", **task_kwargs)
