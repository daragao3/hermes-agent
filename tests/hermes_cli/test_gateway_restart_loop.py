...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    @pytest.mark.skipif(os.name != "nt", reason="native drive paths are Windows-only")
    def test_blocks_lifecycle_command_hidden_in_native_windows_path(self, monkeypatch, tmp_path):
        """Unquoted C:\ paths survive POSIX shlex when Git Bash invokes the script."""
        import tools.terminal_tool as tt

        script = tmp_path / "delayed-ops-native.sh"
        script.write_text("#!/bin/bash\nhermes gateway restart\n", encoding="utf-8")
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
