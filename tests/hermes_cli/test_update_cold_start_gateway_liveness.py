...
def _run_cold_start(monkeypatch, capsys, *, surviving_pids):
    monkeypatch.setattr(update_cmd_windows, "_COLD_START_SPAWNED_AT", None)
    monkeypatch.setattr(update_cmd_windows, "_COLD_START_PID", None)
    monkeypatch.setattr(update_cmd, "_desktop_owns_gateway_lifecycle", lambda: False)
    monkeypatch.setattr(gateway_windows, "_write_start_attestation", lambda *a: None)
    ...
    monkeypatch.setattr(gateway_windows, "_spawn_detached", lambda **kw: 4242)
