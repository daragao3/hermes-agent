...
def test_cold_start_does_not_respawn_inside_the_discovery_window(
    ...
    # update_cmd_windows, not main: 0.21.1 moved the cold-start helper (and with it this
    # module-level guard) out of hermes_cli/main.py.
    from hermes_cli import update_cmd_windows as _ucw
    monkeypatch.setattr(_ucw, "_COLD_START_SPAWNED_AT", None)
    # Upstream's post-spawn readiness poll would otherwise poll the real host for 6s+2s and
    # then RAISE, which is not what this test is about -- it is about the window in which the
    # spawned gateway is NOT yet discoverable.
    monkeypatch.setattr(gateway_windows, "_wait_for_gateway_ready", lambda **_k: [4242])
...
def test_resume_cold_starts_gateway_when_token_requests_it(
    ...
    assert spawned == ["update:windows-cold-start"]
    # The success line is 0.21.1's, and gated on the readiness poll rather than on the PID
    # Popen returned -- a created process is not a surviving one (#84185). Kept over the
    # fork's unconditional "Starting Windows gateway after update (PID 4242)".
