...
from hermes_cli import dashboard_procs as dashboard_procs
...
@pytest.mark.skipif(sys.platform != "win32", reason="Native Windows cold-start contract")
def test_cold_start_does_not_respawn_inside_the_discovery_window(
    ...
    """
    from hermes_cli import update_cmd_windows as windows_update
    import hermes_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(windows_update, "_COLD_START_SPAWNED_AT", None)
    monkeypatch.setattr(windows_update, "_COLD_START_PID", None)
    monkeypatch.setattr(update_cmd, "_desktop_owns_gateway_lifecycle", lambda: False)
    monkeypatch.setattr(gateway_windows, "_wait_for_gateway_ready", lambda: [4242])
    monkeypatch.setattr(gateway_windows, "_write_start_attestation", lambda *_a: None)
    ...
    windows_update._cold_start_windows_gateway_after_update()
    windows_update._cold_start_windows_gateway_after_update()
    windows_update._cold_start_windows_gateway_after_update()
