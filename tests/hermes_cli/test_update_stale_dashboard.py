...
from hermes_cli.main_dashboard import _restart_managed_dashboard_service as _restart_managed_dashboard_service
...
class TestKillStaleDashboardWindows:
    ...
    def test_already_gone_process_counts_as_stopped(self, monkeypatch, capsys):
        ...
        """
        monkeypatch.setattr("gateway.status.pid_exists", lambda _pid: False)
        monkeypatch.setattr("gateway.status.get_process_start_time", lambda _pid: 123.0)
        monkeypatch.setattr("hermes_cli._subprocess_compat.pid_is_hermes", lambda *_a, **_kw: True)
