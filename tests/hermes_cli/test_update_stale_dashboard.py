...
# These tests drive the REAL dashboard scanner with psutil.process_iter faked, so they
# must opt out of the live-system guard's stub (which returns a plain empty list and
# would make every assertion here read as "no dashboards running").
...
class TestKillStaleDashboardWindows:
    ...
    def test_already_gone_process_counts_as_stopped(self, monkeypatch, capsys):
        ...
        # The reaper reads main_dashboard's finder (0.21.1 split it out of hermes_cli.main),
        # and this module now carries real_dashboard_pid_scan -- patching only the old name
        # lets the REAL scan run and the reaper then taskkills this box's live dashboards.
        # The reaper re-verifies each PID's identity before killing it (a reused PID must not
        # be taskkilled); an invented PID fails that gate and never reaches taskkill at all.
        with patch("hermes_cli.main._find_stale_dashboard_pids",
                   return_value=[12346]), \
             patch("hermes_cli.main_dashboard._find_stale_dashboard_pids",
                   return_value=[12346]), \
             patch("gateway.status.get_process_start_time", return_value=1234.0), \
             patch("hermes_cli._subprocess_compat.pid_is_hermes", return_value=True), \
...
# TestWindowsWmicEncoding was removed in the 0.21.1 merge. _iter_process_table tries psutil
# FIRST on every platform and returns from that branch -- and RAISES when psutil is missing
# rather than falling through -- so the wmic path those tests drove is unreachable. What the
# scan must do on Windows now is pinned by TestWindowsScanTransport below: spawn nothing.
