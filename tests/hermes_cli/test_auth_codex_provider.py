...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    # ...and ALSO on hermes_cli.auth. 0.21.1 moved the body to hermes_cli/auth_codex.py,
    # where it does a FUNCTION-LOCAL ``from hermes_cli.auth import ...`` at call time -- so
    # the __globals__ writes below are bypassed entirely and the real device-code flow runs.
    # That is not a wrong assertion, it is a live 15-minute OAuth poll (it consumed this
    # file's whole 300s budget in time.sleep before this was fixed). The __globals__ writes
    # are kept so the test still holds if that local import is ever removed.
    import hermes_cli.auth as _auth_mod

    def _patch_both(name, value):
        monkeypatch.setitem(_g, name, value)
        monkeypatch.setattr(_auth_mod, name, value, raising=False)

    _patch_both("resolve_codex_runtime_credentials", lambda: {"base_url": DEFAULT_CODEX_BASE_URL})
    _patch_both("_import_codex_cli_tokens",
                lambda: {"access_token": "cli-at", "refresh_token": "cli-rt"})
    _patch_both("_codex_device_code_login", lambda: {
    ...
    _patch_both("_save_codex_tokens", _fake_save)
    _patch_both("_update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
