...
from hermes_cli import main_install_repair as main_install_repair
from hermes_cli import main_web_build as main_web_build
...
def _pack_into_staging(root: Path, content: str = "", returncode: int = 0):
    ...
    def _run(cmd, **kwargs):
        if len(cmd) >= 3 and cmd[1:3] == ["run", "pack"]:
            ...
            exe.parent.mkdir(parents=True, exist_ok=True)
            if sys.platform == "win32":
                # Keep the candidate's real structural PE integrity gate on.
                # The process launch remains mocked; this is not an Electron binary.
                from tests.hermes_cli.test_desktop_exe_integrity import make_pe

                make_pe(exe)
                with exe.open("ab") as output:
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    _pack_ok = subprocess.CompletedProcess(["npm", "run", "pack"], 0)
    _launch_ok = subprocess.CompletedProcess([str(packaged_exe)], 0)
