...
class TestInstallCuaDriverUpgrade:
    ...
    def test_upgrade_without_binary_runs_installer(self):
        ...
        with patch.object(tools_config.shutil, "which",
                          side_effect=lambda n: "/test/bin/" + n if n in {"curl", "powershell"} else None), \
             patch.object(tools_config, "_run_cua_driver_installer",
                          return_value=True) as runner:
            assert tools_config.install_cua_driver(upgrade=True) is True
            runner.assert_called_once()
    ...
    def test_quiet_refresh_closes_stdin_and_honors_custom_timeout(self):
        ...
        fake_proc.wait.assert_called_once_with(timeout=120)
...
class TestUpdateCheckTimeoutDefaults:
    ...
    @pytest.mark.skipif(sys.platform == "win32", reason="POSIX default requires an actual POSIX host")
...
class TestInstallerCapturePipeHazard:
    ...
    def test_capturing_path_uses_file_backed_capture_not_pipes(self):
        ...
        with patch("hermes_cli._subprocess_compat.run_text_capture", fake_capture), \
    ...
    def test_verbose_windows_timeout_kills_the_whole_tree(self):
        ...
        with patch("subprocess.Popen", return_value=fake_proc), \
             patch.object(tools_config, "_kill_installer_tree",
                   side_effect=lambda p, **kw: tree_killed.append(p.pid)), \
             patch.object(tools_config, "_cua_driver_cmd", return_value="cua-driver"), \
             patch.object(tools_config, "_clear_stale_cua_install_lock"), \
             patch.object(tools_config, "_print_warning"), \
             patch.object(tools_config, "_print_info"):
