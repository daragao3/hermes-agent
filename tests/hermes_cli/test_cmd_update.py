...
class TestCmdUpdateBranchFallback:
    ...
    # RETIRED: test_update_refreshes_repo_and_tui_node_dependencies.
    # Not a fork test -- present at the merge base 3ef6bbd201 (upstream v0.19.0) and REMOVED
    # upstream, replaced by TestNodeDependencyWorkspaces (in this file, green), which asserts
    # the OPPOSITE install shape: #43564 collapsed the two-pass
    # root(--workspaces=false) + workspace install into ONE workspace-scoped install naming
    # ui-tui and web, and upstream's test pins ``"--workspaces=false" not in joined``
    # explicitly. It is also gated on _npm_lockfile_changed, which this test never seeds, so
    # npm was never called at all. Two tests asserting opposite argv for one call; this is
    # the stale half.
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_resolve_rejects_windows_npm_and_rescans_path(self, monkeypatch):
        ...
        # main_install_repair, not main: 0.21.1 moved _resolve_node_runtime_npm there
        # (main keeps a frozen re-export). ``shutil`` is the same module object either way
        # so that patch still bites, but ``_is_windows`` is per-module -- patched on main it
        # stayed True on this host, the function returned the Windows npm unfiltered, and
        # the /mnt-skipping rescan this test is about never ran.
        from hermes_cli import main_install_repair as mir

        monkeypatch.setattr(mir, "_is_windows", lambda: False)
        ...
        monkeypatch.setattr(mir.shutil, "which", fake_which)
        assert mir._resolve_node_runtime_npm() == "/root/.local/bin/npm"

    def test_resolve_returns_none_when_only_windows_npm(self, monkeypatch):
        import hermes_constants

        from hermes_cli import main_install_repair as mir  # see the test above
        ...
        monkeypatch.setattr(mir.shutil, "which", fake_which)
        assert mir._resolve_node_runtime_npm() is None
...
class TestUpdateNodeDependencies:
    ...
    def test_returns_silently_when_npm_not_found(self, _which, mock_run, tmp_path, monkeypatch):
        """No npm anywhere → return without calling subprocess.

        The resolver, not shutil.which, is what decides this: it also knows the bundled Node
        runtime and WSL's view of a Windows npm, so a which-only patch leaves it finding the real
        npm on this box and running a live install.
        """
        from hermes_cli import main as hm
...
def _stub_update_side_effects(monkeypatch, no_update_sleep):
    ...
    * ``profiles.backfill_profile_envs`` — copies the default install's
    ...
    import hermes_cli.update_cmd as _update
    ...
    monkeypatch.setattr(
        _update, "_validate_critical_files_syntax", lambda *a, **k: (True, None, None)
    )
