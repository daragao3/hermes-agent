...
def _stub_update_side_effects(monkeypatch, no_update_sleep):
    ...
    * ``profiles.backfill_profile_envs`` — copies the default install's
    ...
    import hermes_cli.update_cmd_fleet as _fleet
    ...
    monkeypatch.setattr(
        update_cmd, "_validate_critical_files_syntax", lambda *a, **k: (True, None, None)
    )
    monkeypatch.setattr(_m, "_detect_venv_python_processes", lambda *a, **k: [])
    # The survivor sweep moved to the fleet module. Reuse the fixture's
    # forwarding clock without changing time.sleep for the whole process.
    monkeypatch.setattr(_fleet, "_time", _m._time)
