...
import hermes_cli.web_server_cron as ws
...
def test_single_profile_retains_ownership_gate(monkeypatch, _providers, tmp_path):
    _sp, builtin = _providers
    import hermes_cli.profiles as profiles_mod
    ...
    assert builtin.start_kwargs["profile_homes"] == [("default", tmp_path / "root")]
...
def test_enumeration_failure_fails_open(monkeypatch, _providers):
    ...
    assert builtin.start_kwargs["profile_homes"] == [(None, get_hermes_home())]
    assert callable(builtin.start_kwargs["profile_gate"])
