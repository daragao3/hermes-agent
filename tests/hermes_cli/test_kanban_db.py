...
import hermes_state as hermes_state
...
def _init_git_repo(repo: Path) -> None:
    ...
    subprocess.run(["git", "init", "-b", "main", str(repo)], check=True, capture_output=True, text=True, encoding="utf-8")
    subprocess.run(["git", "-C", str(repo), "config", "user.email", "kanban@example.com"], check=True, capture_output=True, text=True, encoding="utf-8")
    subprocess.run(["git", "-C", str(repo), "config", "user.name", "Kanban Test"], check=True, capture_output=True, text=True, encoding="utf-8")
    (repo / "README.md").write_text("hello\n", encoding="utf-8")
    subprocess.run(["git", "-C", str(repo), "add", "README.md"], check=True, capture_output=True, text=True, encoding="utf-8")
    subprocess.run(["git", "-C", str(repo), "commit", "-m", "init"], check=True, capture_output=True, text=True, encoding="utf-8")
...
@pytest.mark.skipif(not hasattr(os, "WIFEXITED"), reason="Requires native POSIX wait-status macros")
...
def test_check_file_length_invariant_never_opens_a_non_str_path():
    """Mocked PRAGMA results must never become raw file descriptors."""
    conn = unittest.mock.MagicMock()
    with unittest.mock.patch("builtins.open") as opener:
        kbc._check_file_length_invariant(conn)
    opener.assert_not_called()
    os.fstat(1)


def test_safe_file_length_probe_never_opens_mocked_connection_path():
    """The replacement PRAGMA/stat probe must retain raw-open avoidance."""
    from hermes_cli.sqlite_safe_read import file_length_matches_header
    conn = unittest.mock.MagicMock()
    with unittest.mock.patch("builtins.open") as opener:
        assert file_length_matches_header(conn) is None
    opener.assert_not_called()
    os.fstat(1)


def test_check_file_length_invariant_still_runs_for_a_real_str_path(tmp_path):
    """Rollback-mode databases still receive the real PRAGMA/stat check."""
    from hermes_cli import sqlite_safe_read
    conn = kbc.connect(db_path=tmp_path / "real.db")
    try:
        conn.execute("PRAGMA journal_mode=DELETE")
        with unittest.mock.patch.object(sqlite_safe_read.os.path, "getsize", wraps=os.path.getsize) as sampler:
            kbc._check_file_length_invariant(conn)
...
def test_write_txn_tolerates_transient_wal_checkpoint_window(tmp_path):
    """WAL commits must not compare a lagging main file with logical pages.

    Upstream replaces the old raw-header reread with WAL exclusion, preserving
    committed data without opening a live database and cancelling POSIX locks.
    """
    from hermes_cli import sqlite_safe_read
    conn = kbc.connect(db_path=tmp_path / "wal.db")
    try:
        if conn.execute("PRAGMA journal_mode").fetchone()[0].lower() != "wal":
            pytest.skip("Runtime WAL safety policy selected rollback mode; requires a WAL-safe SQLite build")
        with unittest.mock.patch.object(sqlite_safe_read, "file_length_matches_header", return_value=False) as probe:
            with kbc.write_txn(conn) as c:
                c.execute("INSERT INTO tasks (id, title, assignee, status, priority, created_at) VALUES ('t_wal01', 'wal window', 'tester', 'todo', 0, 1234567890)")
        probe.assert_not_called()
        assert conn.execute("SELECT title FROM tasks WHERE id='t_wal01'").fetchone()[0] == "wal window"
    finally:
        conn.close()
