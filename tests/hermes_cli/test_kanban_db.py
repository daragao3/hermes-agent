...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    # Production wait-status reaping is POSIX-only. Exercise the classifier's
    # platform-independent bookkeeping on Windows with the POSIX decoder seam.
    monkeypatch.setattr(_kbd.os, "WIFEXITED", lambda _status: True, raising=False)
    monkeypatch.setattr(_kbd.os, "WEXITSTATUS", lambda status: status >> 8, raising=False)
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
