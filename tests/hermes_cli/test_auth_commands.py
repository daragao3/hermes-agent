...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((hermes_home / "auth.json").read_text(encoding="utf-8"))
    assert auth_payload.get("active_provider") is None
    config_text = (hermes_home / "config.yaml").read_text(encoding="utf-8")
    assert "provider: auto" in config_text


def test_reset_config_provider_uses_atomic_config_transaction(tmp_path, monkeypatch):
...
def test_auth_remove_manual_entry_does_not_touch_env(tmp_path, monkeypatch):
    ...
    assert env_path.read_text(encoding="utf-8") == "SOME_KEY=some-value\n"
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((hermes_home / "auth.json").read_text(encoding="utf-8"))
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (hermes_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}), encoding="utf-8")
...
def test_interactive_strategy_preserves_other_provider_config(tmp_path, monkeypatch):
    from hermes_cli import auth_commands as commands
    ...
    monkeypatch.setattr(commands, "_pick_provider", lambda *a: "openai-codex")
    monkeypatch.setattr(commands, "_ask", lambda *a: "2")
    commands._interactive_strategy()
    config = load_config()
    assert config["custom_marker"] == "preserved"
    assert config["credential_pool_strategies"]["anthropic"] == "least_used"
    assert config["credential_pool_strategies"]["openai-codex"] == list(commands._STRATEGY_DESCRIPTIONS)[1]
