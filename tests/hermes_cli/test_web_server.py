...
class TestWebServerEndpoints:
    ...
    def test_post_memory_provider_setup_runs_declared_external_install(self, monkeypatch):
        ...
        #
        # Patch it on ``web_server_memory`` — 0.21.1 extracted the
        # memory-provider helpers out of ``web_server`` into that module, and
        # ``_run_setup_command`` resolves ``run_text_capture`` from its OWN
        # module globals, so that is where the name is bound.
        #
        ...
        monkeypatch.setattr(_web_server_memory, "run_text_capture", fake_run)
    ...
    def test_profiles_sessions_sidebar_flags_unmatched_recents_scope(self):
        """A recents_profile naming no known profile matches zero DBs, and that
        has to stay distinguishable from a genuinely empty profile — otherwise a
        desktop stuck on a deleted/ghost scope renders a permanently empty
        sidebar with nothing to detect and no way to fall back.

        The fork answered this with an explicit ``profile``/``profile_matched``
        pair on the recents slice. 0.21.1 rewrote this endpoint and answers the
        same question through ``profiles_truncated``, whose KEY SET is exactly
        the profiles the scope matched and opened: a ghost scope yields ``{}``,
        a matched-but-empty profile yields ``{"default": False}``. The fork's
        fields were dropped with their desktop consumers in the sidebar rewrite
        (batch88), so the invariant is pinned on the signal that survives rather
        than on the field that carried it."""
        from hermes_state import SessionDB
        ...
        # Matched NOTHING, said two independent ways: the explicit scope echo, and the
        # absence of any profile key in upstream's profiles_truncated map.
        assert recents["profile"] == "ghost-profile"
        assert recents["profile_matched"] is False
        assert recents["profiles_truncated"] == {}
        ...
        # Matched a real profile: the key is present even though the window is short.
        assert "default" in matched.json()["recents"]["profiles_truncated"]
        ...
        assert "default" in all_scope.json()["recents"]["profiles_truncated"]
    ...
    def test_profiles_sessions_catalog_routes_cap_pages_at_500(self):
        ...
        # An over-large ask is REFUSED at the validation layer, not clamped: this endpoint
        # fans out across every profile's state.db (#39200 / #74778,
        # tests/hermes_cli/test_dashboard_param_clamps.py). The fork used to clamp; upstream's
        # refusal is the stricter answer to the same problem and is what this tree now does.
        ...
        # What this test is named for: the window the caller can legally ask for is bounded
        # at 500 even with 501 sessions on disk.
        catalog = self.client.get(
            "/api/profiles/sessions"
            "?profile=all&limit=500&offset=0&min_messages=1"
            "&archived=exclude&order=recent&exclude_sources=cron,telegram"
        )
        ...
        # The cap this test is named for is the WINDOW: 501 showable sessions on disk, 500
        # rows returned, and ``total`` reporting the full count behind them.
    ...
    def test_get_session_messages_uses_read_only_db(self, monkeypatch):
        ...
        class _ReadOnlyMessagesDB:
            ...
            # **_kw: the transcript route gained a ``latest`` keyword in 0.21.1, and a
            # narrow double raises TypeError inside the request instead of failing an
            # assertion. This test is about read_only=True, not the call shape.
            def get_messages(self, session_id, *, limit=None, offset=0, **_kw):
...
class TestNewEndpoints:
    ...
    def test_cron_list(self):
        ...
        assert resp.status_code == 200
        body = resp.json()
        # A bare list. The fork's {"jobs", "errors"} envelope fed a per-profile
        # partial-failure banner in the desktop cron panel; that panel went in the
        # 0.21.1 desktop rewrite (batch88) and the envelope was retired with it
        # (batches 103-108) — upstream's router returns the plain job list and logs
        # the failing profile instead.
...
class TestProbeGatewayHealth:
    ...
    def test_normalizes_url_with_health_suffix(self, monkeypatch):
        ...
        monkeypatch.setattr(ws.urllib.request, "urlopen", mock_urlopen)
        # _probe_gateway_health moved to hermes_cli.web_server_gateway in 0.21.1.
        alive, body = _web_server_gateway._probe_gateway_health()
        assert alive is False
...
class TestStatusRemoteGateway:
    ...
    def test_status_headline_unretargeted_when_active_profile_default(
        ...
        # gateway.status, NOT hermes_cli.web_server: 0.21.1 reduced both names on
        # web_server to _PLUGIN_COMPAT_LAZY proxies, so patching them there is
        # ACCEPTED and intercepts nothing -- the status router resolves them with
        # late(..., "gateway.status"). Same seam the sibling remote-pid test uses.
        monkeypatch.setattr(_gw_status, "get_running_pid_cached", fake_pid)
        monkeypatch.setattr(_gw_status, "read_runtime_status", fake_rt)
