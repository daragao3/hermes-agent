...
def test_repeat_backups_dedupe_and_rotate(tmp_path: Path, monkeypatch):
    ...
    assert len(kept) == 2 and kept[0].read_text(encoding="utf-8") == "model: 2\n"
...
def test_legacy_siblings_move_but_user_named_copies_stay(tmp_path: Path):
    ...
    (tmp_path / "config.yaml.bak-my-note").write_text("mine", encoding="utf-8")
    ...
    assert (tmp_path / "config.yaml.bak-my-note").read_text(encoding="utf-8") == "mine"
