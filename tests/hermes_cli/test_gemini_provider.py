...
class TestGeminiAgentInit:
    ...
    def test_gemini_custom_base_url_keeps_openai_client(self, monkeypatch):
        ...
        # agent.process_bootstrap, not run_agent: 0.21.1 reduced run_agent.OpenAI to a lazy
        # re-export (run_agent's _PLUGIN_COMPAT_LAZY map), so patching it there is accepted
        # and intercepts nothing -- the agent builds its client from process_bootstrap.
