...
async def test_cron_mutation_without_profile_finds_named_profile_job(isolated_profiles):
    ...
    default_jobs = (await _rt_cron.list_cron_jobs(profile="default"))["jobs"]
    worker_jobs = (await _rt_cron.list_cron_jobs(profile="worker_alpha"))["jobs"]
...
@pytest.mark.asyncio
async def test_trigger_cron_job_preserves_pause_with_force_capable_provider(
    isolated_profiles,
    monkeypatch,
):
    from cron import jobs as cron_jobs
    ...
    before = _web_server_cron._call_cron_for_profile("worker_alpha", "get_job", job["id"])
    with pytest.raises(HTTPException) as exc:
        await _rt_cron.trigger_cron_job(job["id"], profile="worker_alpha")
    assert exc.value.status_code == 409
    assert "paused" in exc.value.detail["error"].lower()
    assert observed == {}, "A paused job must never reach even a force-capable provider"
    assert _web_server_cron._call_cron_for_profile("worker_alpha", "get_job", job["id"]) == before
...
async def test_cron_profile_scan_runs_off_event_loop(isolated_profiles, monkeypatch):
    ...
    jobs = (await _rt_cron.list_cron_jobs(profile="all"))["jobs"]
    paused = await _rt_cron.pause_cron_job(worker_job["id"])
...
async def test_dashboard_cron_noop_inference_fields_keep_existing_snapshots(
    ...
    from hermes_cli import runtime_provider, web_server as web_server
...
async def test_cron_delete_with_profile_deletes_only_target_profile(isolated_profiles):
    ...
    remaining_default = (await _rt_cron.list_cron_jobs(profile="default"))["jobs"]
    remaining_worker = (await _rt_cron.list_cron_jobs(profile="worker_alpha"))["jobs"]
...
async def test_cron_profile_validation_errors(isolated_profiles):
    ...
    with pytest.raises(HTTPException) as bad_name:
        (await _rt_cron.list_cron_jobs(profile="../bad"))["jobs"]
    assert bad_name.value.status_code == 400

    with pytest.raises(HTTPException) as missing:
        (await _rt_cron.list_cron_jobs(profile="missing_profile"))["jobs"]
...
async def test_list_cron_jobs_all_reports_an_unreadable_profile(
    ...
    healthy_job = _web_server_cron._call_cron_for_profile(
        "default",
        "create_job",
    ...
    real_call = _web_server_cron._call_cron_for_profile
    ...
    monkeypatch.setattr(_web_server_cron, "_call_cron_for_profile", failing_for_worker)

    listing = await _rt_cron.list_cron_jobs(profile="all")
...
def test_trigger_cron_job_on_a_paused_job_is_409_naming_the_reason(monkeypatch):
    ...
    The router rejects the resolved paused job before attempting a fire,
    returning 409 with the containment reason and timestamp.
    """
    ...
    monkeypatch.setattr(_web_server_cron, "_find_cron_job_profile", lambda _job_id: "main")
    monkeypatch.setattr(_web_server_cron, "_call_cron_for_profile", lambda *_a, **_kw: held)
    fire = Mock()
    monkeypatch.setattr(_rt_cron, "_fire_cron_job_for_profile", fire)

    with pytest.raises(HTTPException) as exc_info:
        _rt_cron._trigger_cron_job_sync("b74186b2eaa5")
