...
def test_daemon_keeps_going_after_tick_exception(kanban_home, monkeypatch):
    ...
    calls = [0]
    # 0.21.1 moved the loop to kanban_db_dispatch; patching the kanban_db compat proxy
    # would leave run_daemon calling the real dispatch_once next to it.
    orig_dispatch = kbd.dispatch_once
    ...
    def _runner():
        kbd.run_daemon(interval=0.05, stop_event=stop)
...
def test_gateway_dispatcher_retries_corrupt_board_after_quarantine(
    ...
    monkeypatch.setattr("gateway.kanban_watchers_dispatcher.time.monotonic",
                        _monotonic_for_gateway_dispatcher)

    calls = {"tick": 0, "to_thread": 0, "names": []}

    def _connect(*args, **kwargs):
        raise sqlite3.DatabaseError("file is not a database")

    async def _to_thread(fn, *args, **kwargs):
        # Stop on the CALL COUNT, not on recognising the tick by name: this fake never awaits, so
        # the loop never yields and the asyncio.wait_for timeout around it can never preempt a
        # run that fails to stop itself -- a renamed tick would hang the whole suite instead of
        # failing one assertion. Three to_thread calls per tick (zombie reaper, tick_once,
        # ready_nonempty), so 9 is the 3 ticks asserted below.
        calls["to_thread"] += 1
        # _to_thread_process_service hands asyncio.to_thread its context shim, so the real
        # callable is the first positional arg.
        target = args[0] if getattr(fn, "__name__", "") == "_run_in_fresh_context" and args else fn
        name = getattr(target, "__name__", "")
        calls["names"].append(name)
        result = fn(*args, **kwargs)
        if name == "tick_once":
            calls["tick"] += 1
        if calls["to_thread"] >= 9 or calls["tick"] >= 3:
            runner._running = False
        return result
    ...
    assert calls["tick"] == 3, calls["names"]
