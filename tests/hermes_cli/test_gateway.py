...
def test_gateway_start_diag_precedes_expensive_init(monkeypatch):
    ...
    exit_codes = []
    module._exit_after_graceful_shutdown = exit_codes.append
...
@pytest.mark.skipif(sys.platform != "win32", reason="Native Windows restart fallback")
...
class TestRespawnDoesNotInheritTheCallerCwd:
    ...
    @pytest.mark.skipif(sys.platform == "win32", reason="Native POSIX watcher contract")
    ...
    @pytest.mark.skipif(sys.platform != "darwin", reason="Native launchd fallback contract")
