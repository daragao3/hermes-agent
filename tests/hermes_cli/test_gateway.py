...
def test_find_gateway_pids_falls_back_to_pid_file_when_process_scan_fails(monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        if cmd[0] == "ps" and cmd[1] in ("-A", "-Aww"):  # BSD/macOS ps rejects `e`; upstream uses -Aww
...
def test_gateway_start_diag_precedes_expensive_init(monkeypatch):
    ...
    def _module_getattr(name):
        ...
        if name == "_exit_after_graceful_shutdown":
            # gateway.py imports it at call time; the diag ordering is what is under test.
...
def test_scan_gateway_pids_detects_windows_hermes_exe_case_variants(monkeypatch):
    ...
    # The Windows listing spawns through bounded_probe_run (pipe-wedge guard), not run().
    monkeypatch.setattr("hermes_cli._subprocess_compat.bounded_probe_run", fake_run)
