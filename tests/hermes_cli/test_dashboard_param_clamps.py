...
def client(tmp_path, monkeypatch):
    ...
    # Bind the token through the module rather than relying on the env var alone.
    # ``_SESSION_TOKEN`` is resolved AT IMPORT TIME, so setting the environment here only
    # works when this file happens to be the first test to import web_server; after any
    # earlier importer the module already holds a random token and every request 401s --
    # which, in a file about 422-vs-200 validation, reads like a production regression.
