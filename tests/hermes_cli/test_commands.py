...
# Only the three names that resolve on no other module. Everything else this block used to
# import is already imported above from the CANONICAL split modules; hermes_cli.commands
# still re-exports them, but through the compat shim that warns it is removed 2026-09-14
# and that check_compat_pointers.py fails CI over for internal code.
# _CMD_NAME_LIMIT was `_CMD_NAME_LIMIT = _CMD_NAME_LIMIT` in the fork — a pure alias, and on
# no module now. The test uses _CMD_NAME_LIMIT, imported above from commands_platforms.
# _clamp_telegram_names and _TG_NAME_LIMIT are imported from nowhere on purpose: neither is
# defined on either side of this merge. The tests that used them now call
# _clamp_command_names / _CMD_NAME_LIMIT, which are the real names.
