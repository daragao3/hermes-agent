...
# RETIRED: test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher.
# Not a fork test -- it is upstream's own, present at the merge base (v0.19.0), and
# upstream REPLACED it in this same file position with
# ``test_build_gateway_argv_keeps_venv_console_python_for_uv_venv`` (below). 0.21.1
# removed the pythonw / base-interpreter detour on purpose: the venv console python.exe
# is launched hidden (CREATE_NO_WINDOW) so descendants inherit ITS hidden console instead
# of each flashing their own (#54220/#56747). The merge kept both copies, leaving two
# tests asserting opposite argv for the same call; production implements upstream's and
# its replacement test is green, so this is the stale half.
...
def test_force_terminate_survives_a_taskkill_timeout(monkeypatch):
    ...
    # **_kw: force=True now also carries ``expected_start_time`` (the PID-identity guard
    # that stops a taskkill on a RECYCLED pid). This test is about TimeoutExpired not
    # aborting the sweep, not about the call shape.
    def fake_terminate_pid(target_pid, force=False, **_kw):
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda hermes_home: "")
    monkeypatch.setattr("hermes_cli.config.get_hermes_home", lambda: str(hermes_home))
    # The cwd decides which copy of the tree runs (`-m` puts it on sys.path[0], ahead of the
    # editable-install finder), so this fork anchors at the INSTALLED package root and keeps
    # HERMES_HOME only as the fallback. Pin the install record instead of reading the real host's.
    monkeypatch.setattr(gateway_windows, "find_installed_package_root", lambda: project)
    # Same rule for the child PYTHONPATH: the INSTALLED checkout, not the __file__-derived
    # PROJECT_ROOT of whatever copy this process imported.
