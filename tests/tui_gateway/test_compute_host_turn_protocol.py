...
def turn_env(monkeypatch, tmp_path):
    ...
    # A real initialized agent has already loaded its tool registry. The fake
    # below skips construction; warm that dependency before timing frame delivery.
    import importlib
    ...
    # Protocol fixtures have no goal; avoid opening/migrating a goal database
    # on the timed worker after message.complete. Goal behavior has its own suite.
    monkeypatch.setattr(server, "_active_goal_manager", lambda session: None)
