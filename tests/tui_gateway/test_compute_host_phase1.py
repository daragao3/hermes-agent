...
def test_supervisor_resolves_default_interpreter_and_current_home(tmp_path, monkeypatch):
    from tui_gateway import host_supervisor

    monkeypatch.setattr(host_supervisor, "real_executable", lambda: "resolved-python.exe")
    monkeypatch.setattr(host_supervisor, "get_hermes_home", lambda: tmp_path)
    supervisor = HostSupervisor(autostart=False, expected_build_sha="test")
    assert supervisor.argv == ["resolved-python.exe", "-m", "tui_gateway.compute_host"]
    assert supervisor.registry_path == tmp_path / "state" / "dashboard-compute-host.json"
    assert supervisor.expected_hermes_home == str(tmp_path)
    assert supervisor.pid == 0
    explicit = HostSupervisor(
        argv=["custom-python", "custom-module"], registry_path=tmp_path / "explicit.json",
        autostart=False, expected_build_sha="test")
    assert explicit.argv == ["custom-python", "custom-module"]
    assert explicit.registry_path == tmp_path / "explicit.json"
