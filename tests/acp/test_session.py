...
class TestSymlinkAliasNormalization:
    ...
    def test_missing_path_keeps_lexical_normalization(self, tmp_path):
        ...
        missing = tmp_path / "nonexistent" / "x" / ".." / "y"
        assert acp_session._normalize_cwd_for_compare(str(missing)) == acp_session._normalize_cwd_for_compare(
            str(tmp_path / "nonexistent" / "y")
        )
