...
import hermes_state
import hermes_state_wal as hermes_state_wal
...
class TestConnectionLifecycle:
    ...
    def test_read_only_open_retries_transient_wal_ioerr(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            _matches = read_only.search_messages("wal-race")
        finally:
            read_only.close()
...
class TestSchemaInit:
    ...
    def test_v9_migration_skips_v10_trigram_backfill_before_v11_rebuild(self, tmp_path, monkeypatch):
        """Direct v9→current migration skips superseded content-only backfills.

        v10 backfilled ``messages_fts_trigram`` with content-only rows. Current
        schema installs the current external-content indexes with tool metadata.
        A titled interactive session is not an orphaned delegate under v16.
        """
        ...
        conn.execute(
            ...
            ("s1", "cli", 1000.0, "Interactive migration fixture"),
        )
        conn.execute(
        ...
        # Tool output remains in the standard index; the current trigram view
        # deliberately indexes only eligible conversational rows.
        conn.execute(
            "INSERT INTO messages (session_id, role, content, timestamp) VALUES (?, ?, ?, ?)",
            ("s1", "user", "plain content", 1002.0),
        )
        conn.commit()
        conn.close()
        ...
        try:
            ...
            assert tool_hit == 0  # tool-role rows are intentionally excluded from trigram
        finally:
            migrated_db.close()
...
class TestOptimizeFts:
    ...
    def test_write_path_merges_fts_on_cadence(self, db, monkeypatch):
        ...
        db._FTS_MERGE_EVERY_N_WRITES = 5
        calls = {"n": 0}
        real_merge = db._merge_fts_incrementally

        def _counting_merge(**kwargs):
            calls["n"] += 1
...
class TestDefaultDbPathHermeticResolution:
    ...
    *before* the hermetic ``_hermetic_environment`` fixture redirects
    ...
    def test_default_sessiondb_path_resolves_to_per_test_hermes_home(
        ...
        default_db = hermes_state.SessionDB()
        default_db.close()
...
class TestExecuteReadRetry:
    ...
    def test_persistent_lock_raises_after_max_retries(self, db, monkeypatch):
        monkeypatch.setattr(db, "_WRITE_MAX_RETRIES", 3, raising=False)
        monkeypatch.setattr(hermes_state.time, "sleep", lambda *_a, **_k: None)
        calls = {"n": 0}
