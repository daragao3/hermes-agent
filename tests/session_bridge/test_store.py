...
from hermes_state import SCHEMA_VERSION, SessionDB, desktop_registry_value_hash
...
def _build_legacy_v33_registry(path, rows) -> None:
    """Leave *path* exactly as a pre-v34 database: value_json inline."""
    legacy = SessionDB(path)
    legacy._conn.executescript(
...
def test_desktop_registry_values_migration_is_all_or_nothing(
    ...
    # hermes_state_schema, NOT the hermes_state facade: the migration lives in the
    # schema module, which binds this helper from hermes_state_common, so that module
    # global is the name it actually calls. Patching the facade injects nothing and the
    # test fails as "DID NOT RAISE" -- an inert patch, not a production change.
    monkeypatch.setattr(hermes_state_schema, "desktop_registry_value_hash", _flaky)
    ...
    monkeypatch.setattr(hermes_state_schema, "desktop_registry_value_hash", real_hash)
