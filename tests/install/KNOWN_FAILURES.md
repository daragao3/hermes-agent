...
Evidence required: the CLI update phase failed, uv reports failure to remove that install's `Scripts/hermes.exe` with OS error 5, and the updater reports `Git update failed` on that `uv pip install -e .` with exit status 2. A generic access-denied error on another file does not match. How the ZIP fallback ends is not evidence. Usually its own install hits the same lock and prints a traceback through `hermes.exe/__main__.py` (run [35791535023](https://github.com/daragao3/hermes-agent/actions/runs/35791535023)); when GitHub rate-limits the download it stops at `ZIP update failed: HTTP Error 429` with no traceback (run [36075097022](https://github.com/daragao3/hermes-agent/actions/runs/36075097022)). Both are this failure.
