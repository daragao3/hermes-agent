...
"$PYTHON" -m compileall -q -j 1 -- $(git ls-files '*.py') >/dev/null 2>&1 || true
...
CLEAN_ENV+=("${WIN_ENV[@]}" "${TEST_ENV[@]}")
if [ -n "${HERMES_E2E_BROWSER:-}" ]; then
  CLEAN_ENV+=("HERMES_E2E_BROWSER=$HERMES_E2E_BROWSER")
fi
