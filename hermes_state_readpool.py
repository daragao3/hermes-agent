...
from typing import TYPE_CHECKING, Callable, ContextManager, Iterator, Optional, TypeVar
...
def execute_read_with_retry(
    read_context: Callable[[], ContextManager[sqlite3.Connection]],
    fn: Callable[[sqlite3.Connection], _ReadResult],
    *,
    max_attempts: int = 15,
    min_delay: float = 0.020,
    max_delay: float = 0.150,
) -> _ReadResult:
    """Retry a SELECT-only callback on transient locked/busy OperationalError.

    Each attempt borrows through the owner's read context, retaining pooled
    WAL reads and locked-writer fallback. Return the connection/release the
    lock before sleeping. Callbacks must materialize results before returning,
    and must not mutate state or open a transaction: the entire callback may
    run again. Other errors propagate without replay, just as in the local
    Session Bridge resume path.
    """
    if max_attempts < 1:
        raise ValueError('read retry requires at least one attempt')
    for attempt in range(max_attempts):
        try:
            ...
            if ('locked' in message or 'busy' in message) and attempt < max_attempts - 1:
                ...
                continue
            raise
    raise AssertionError('read retry exhausted without result or exception')
