...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        def _abort_worker_tasks() -> None:
            # Same shape as the timeout branch below: cancel inside the worker's own loop so
            # the coroutine observes it at its next await. Runs on the CANCELLING thread
            # (tools.inflight_call), which can get here before the worker built its loop.
        ...
        try:
            ...
            # Only OUR cancel becomes InterruptedError; anything else propagates unchanged.
...
def _run_on_persistent_loop(loop: asyncio.AbstractEventLoop, coro):
    ...
    The coroutine is wrapped in a Task UP FRONT — ``run_until_complete`` on a bare coroutine
    leaves the cancelling thread nothing to cancel — so a cancel from another thread (the
    cron scheduler's operator stop or timeout, via
    ``tools.inflight_call.cancel_inflight_calls``) can ``call_soon_threadsafe(task.cancel)``
    and wake this blocking call at the coroutine's next await.

    A cancel that was OURS surfaces as ``InterruptedError`` — a plain Exception, so
    ``ToolRegistry.dispatch`` turns it into a normal ``{"error": ...}`` result rather than
    unwinding the turn. Any other ``CancelledError`` propagates unchanged."""
