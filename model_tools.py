    ...
    if loop and loop.is_running():
        ...
        def _run_in_worker():
            ...
            try:
                ...
                # Publish readiness only once there is a task to cancel. A
                # stop before publication is retained by abort_requested.
                loop_ready.set()
        ...
        def _abort_worker_tasks() -> None:
            # Remember an early stop even if worker startup is delayed.
            # Readiness publishes both references; cancellation itself is
            # scheduled on the worker loop, never by scanning it cross-thread.
            abort_requested.set()
            if loop_ready.is_set() and worker_loop is not None and worker_task is not None:
                try:
