...
from hermes_state_bridge_schema import BRIDGE_DEFERRED_SQL
...
class SessionSchemaMixin:
    ...
    def has_session_schema(self) -> bool:
        """Distinguish session stores from delegation-only profile databases.

        File existence and size do not identify a SQLite schema, especially
        under WAL. Only an actual sessions table qualifies. Read failures
        propagate so profile callers can report corruption, contention or I/O
        errors instead of silently treating an unreadable store as absent.
        """
    ...
    def _migrate_bounded_tool_fts_triggers(self, cursor: sqlite3.Cursor, *, legacy: bool) -> None:
        ...
        concurrent writer lands in a trigger gap. A fresh store has no historical index to
        migrate; its FTS family is created later under rebuild admission."""
        if not self._sqlite_table_exists(cursor, "messages_fts"):
            return
        # The local v32/v33 index is single-column external-content, not inline.
        # Its view exposes full text; installing bounded inline triggers would
        # make future indexed text disagree with that view. Require the explicit
        # storage conversion before entering upstream's legacy trigger path.
        declaration = cursor.execute(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name='messages_fts'"
        ).fetchone()
        if declaration and "content='messages_fts_source'" in "".join((declaration[0] or "").split()):
            raise RuntimeError("Local external-content FTS requires explicit storage conversion before trigger migration")
    ...
    def _reconcile_columns(self, cursor: sqlite3.Cursor) -> None:
        """Reconcile both schema domains after their CREATE TABLE declarations have run."""
    ...
    @staticmethod
    def _ensure_native_event_key_index(cursor: sqlite3.Cursor) -> None:
        """Retain duplicate legacy evidence when the partial unique index cannot be created."""
        try:
            cursor.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS idx_messages_native_event_key "
                "ON messages(session_id, native_event_key) WHERE native_event_key IS NOT NULL"
            )
    ...
    def _init_schema(self, *, _offline_local33_conversion=False):
        ...
        cursor = self._conn.cursor()
        # Local v33 and upstream v30 are independent migration histories. Until
        # explicit conversion establishes the upstream history, never relabel
        # an existing newer scalar or run version-gated writes against it.
        if cursor.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='schema_version'").fetchone():
            existing = cursor.execute('SELECT version FROM schema_version LIMIT 1').fetchone()
            if _offline_local33_conversion and (existing is None or existing[0] not in (33, 34)):
                raise RuntimeError('Offline conversion requires verified local33/local34 source history')
            if existing is not None and existing[0] > SCHEMA_VERSION and not _offline_local33_conversion:
                raise RuntimeError(
                    f'SessionDB schema history {existing[0]} requires explicit migration-domain conversion '
                    f'before opening with upstream schema {SCHEMA_VERSION}'
                )
        cursor.executescript(SCHEMA_SQL)
        self._initialize_bridge_tables(cursor)
        # Preserve the audited explicit ALTER before generic column reconciliation.
        ...
        self._initialize_bridge_migrations(cursor)
        ...
        cursor.executescript(DEFERRED_INDEX_SQL + BRIDGE_DEFERRED_SQL)
        self._ensure_native_event_key_index(cursor)
        ...
        if row is None:
            ...
            # Explicit offline conversion keeps source33 until the migration chain
            # completes. Local v29 carried upstream usage v20/v22, but those older
            # paths swallowed errors: safely replay their idempotent repairs too.
            # Local v16/v18 provenance is retained; never read host sessions.json
            # as part of an offline conversion. Normal opens retain their history.
            migration_version = 19 if _offline_local33_conversion else row[0]
            self._run_data_migrations(cursor, migration_version, fts5_available)
    ...
    def _run_data_migrations(self, cursor: sqlite3.Cursor, current_version: int, fts5_available: bool) -> None:
        ...
        if current_version < 25 or cursor.execute(
            "SELECT 1 FROM sessions WHERE system_prompt IS NOT NULL LIMIT 1"
        ).fetchone() is not None:
            # v25: de-duplicate system prompt snapshots (old column stays a read fallback).
            # Local schema33 has an independent history and may still hold inline prompts.
            # A paused earlier pass also resumes from actual remaining data, not its version.
