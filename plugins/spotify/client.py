...
if TYPE_CHECKING:  # annotations only — `from __future__ import annotations` above
    # keeps every ``httpx.*`` annotation a string at runtime, so this never executes.
    import httpx
...
def __getattr__(name: str):
    ...
    if name == "httpx":
        ...
        # ``httpx`` used to be a module-scope import here, so ``<this module>.httpx``
        # was a real attribute; callers reaching the client through the module path and
        # the tests that patch ``"<this module>.httpx.<verb>"`` both depend on it. This
