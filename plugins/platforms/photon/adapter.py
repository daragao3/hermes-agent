...
def _reinstall_sidecar_deps() -> None:
    ...
    if not npm:
        ...
        return
    # run_text_capture, not subprocess.run(capture_output=True): on Windows ``npm`` is a shim
    # that spawns node as a GRANDCHILD, which inherits the capture pipes and holds their write
    # end open past the shim's death. subprocess.run kills only the shim and then blocks
    # re-draining a pipe that can never reach EOF, so the TimeoutExpired caught below would
    # never fire. The helper captures into temp files, tree-kills, and sets CREATE_NO_WINDOW
    # itself — which is what windows_hide_flags was here for.
    from hermes_cli._subprocess_compat import run_text_capture

    def _run(verb: str) -> subprocess.CompletedProcess:
        return run_text_capture(  # noqa: S603
            [npm, verb], cwd=str(_sidecar_dir()), timeout=_NPM_REINSTALL_TIMEOUT)
    try:
        result = _run("ci")
