...
def escape_chunk_indicators(chunks: List[str]) -> List[str]:
    ...
    ``truncate_message()`` runs *after* :meth:`format_message` has escaped the body, so the chunk
    indicator it appends carries raw parentheses. Telegram then rejects the whole chunk with
    "character '(' is reserved and must be escaped with the preceding '\\'", the send falls back to
    plain text (losing every formatting entity), and the retry counts against the flood limit.

    Call this on the chunk list whenever ``parse_mode`` is MarkdownV2 -- and only then, since a
    backslash is a literal character under HTML parse mode. Single-chunk sends get no indicator
    and are returned unchanged.

    Shared by the in-gateway adapter's ``send()`` and the standalone
    ``tools.send_message_senders`` path so the two cannot drift again (the standalone copy
    lacked this from 2026-09-02 back).
    """
    if len(chunks) <= 1:
        return chunks
    return [
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_polling_network_error(self, error: Exception) -> None:
        ...
        cap) up to MAX_NETWORK_RETRIES, then *retryable*-fatal.

        The retryable fatal is routed by ``GatewayRunner._handle_adapter_fatal_error`` into the
        ``_failed_platforms`` reconnect-watcher queue, which re-initializes THIS adapter on its own
        backoff cadence. The gateway process and every other platform (WhatsApp, api_server, cron,
        the event bus) stay up — a transient DNS/VPN flap must not bounce the whole multi-platform
        gateway (death forensics 2026-07-16)."""
        if self._teardown_started or self.has_fatal_error:
            return
    ...
    def _register_handlers(self, app) -> None:
        ...
        # Pipeline reply commands BEFORE the generic command handler: PTB dispatches in
        # registration order within a group, so the generic one would swallow them.
        # The closure default-arg `v=_verb` is required to capture the loop variable.
        for _verb in ("approve", "reject", "archive"):
            app.add_handler(CommandHandler(
                _verb, lambda u, c, v=_verb: self._handle_reply_command(u, c, v)))
    ...
    async def _initialize_app_with_retries(self, builder) -> None:
        ...
        for _attempt in range(_max_connect):
            ...
            try:
                ...
                # Attempt 1 succeeds on essentially every boot — log it as progress. Only a
                # retry means something actually went wrong and deserves WARNING.
                logger.log(
                    logging.INFO if _attempt == 0 else logging.WARNING,
                    "[%s] Connecting to Telegram (attempt %d/%d)…", self.name, _attempt + 1, _max_connect)
    ...
    async def send(
        ...
        try:
            ...
            # truncate_message appends a raw " (1/2)" suffix; escape the MarkdownV2-special
            # parentheses. Shared with the standalone send path in tools.send_message_senders.
    ...
    async def _handle_callback_query(self, update: "Update", context: "ContextTypes.DEFAULT_TYPE") -> None:
        ...
        for prefix, handler in (
            ...
            ("rl:", self._handle_rate_limit_reroute_callback),
    ...
    async def _handle_text_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not self._gate_or_observe(msg, update, MessageType.TEXT):
            return
        self._emit_user_inbound_message(msg)
    ...
    def _emit_user_inbound_message(self, msg) -> None:
        """Publish an inbound user message on the event bus (scribe telemetry, voice tuning).

        ONE copy, called from both the text and the command path. The fork carried two verbatim
        copies of this block; a second inbound path is all it takes for one of them to be missed.

        Called AFTER the authorization gate on purpose — a blocked sender must not reach
        subscribers — and never raises: telemetry must not be able to drop a real message.
        """
        try:
                ...
                bus.emit(
                    ...
                    payload={
                        ...
                        "thread_id": (str(msg.message_thread_id)
                                      if getattr(msg, "message_thread_id", None) else None),
                        "message_id": str(getattr(msg, "message_id", "") or ""),
                        "in_reply_to": (str(reply_to.message_id) if reply_to is not None else None),
                    },
    ...
    async def _handle_reply_command(
        ...
        """``/approve``, ``/reject``, ``/archive`` — route through ``reply_handlers.execute``.

        ``is_authorized_telegram`` is a SEPARATE allowlist from the adapter's own message gate: these
        verbs mutate pipeline state, so being allowed to talk to the bot is not enough.
        """
    ...
    async def _handle_rate_limit_reroute_callback(self, query, data: str, cb: Dict[str, Any]) -> None:
        """``rl:<action>:<token>`` — the model-rate-limit reroute buttons.

        ``action`` is divert / choose / dismiss (``events/override_buttons.py``). ``token`` is an
        OPAQUE handle, NEVER a model name: it is resolved to a (provider, model,
        replacement_provider, replacement_model) record through ``events.override_callback_state``,
        populated by ``events/subscribers/telegram_notifier.py`` when the buttons were sent. That
        lookup is what stops a tap naming an arbitrary model through ``callback_data``.
        """
        parts = data.split(":", 2)
        if len(parts) != 3:
            return
        ...
        if not await self._callback_authorized(
                query, cb, "⛔ You are not authorized to answer this prompt."):
            return

        # Refuse an unrecognized action BEFORE the pop() below. The action is trusted straight from
        # the callback payload and validated nowhere else — without this, a bad client or a probe
        # would consume the single-use token and silently disarm the legitimate button, leaving the
        # user's real next tap with "already resolved".
        if action not in ("divert", "choose", "dismiss"):
            await query.answer(text="⚠️ Unknown action.")
            return

        # "Choose model…" is not wired up yet, so it must be a TRUE no-op: handled before the pop()
        # and returning immediately. After the pop it would burn the token AND fall through to the
        # retiring edit_message_text below (reply_markup=None) — the most likely FIRST tap would
        # destroy the alert text and permanently disarm the only working control in one go.
        if action == "choose":
            await query.answer(text="🔧 Model picker isn't wired up yet — coming soon.")
            return

        # Single-use by construction: an unknown token and an already-consumed one both come back
        ...
        user_display = getattr(query.from_user, "first_name", "User")
        # Set only on a TRANSIENT set_override failure (disk full, a Windows AV/indexer sharing
        # violation, the store's backoff window). It gates BOTH re-arming the token and skipping the
        # button-retiring edit below. Before the I2 fix (events/model_override.py) set_override only
        # returned False for a PERMANENT rejection, where burning the button is correct because a
        # retry can never succeed; I2 widened False to cover a write that simply did not reach disk,
        # which a retry COULD fix. There is no `hermes overrides set` — only list/clear — so a burned
        # button means no retry until the next MODEL_RATE_LIMITED alert mints a fresh one.
        ...
        if action == "dismiss":
            ...
            if not target["replacement_provider"] or not target["replacement_model"]:
                # The token's own record has no usable fallback target — e.g. a detector emitted
                # MODEL_RATE_LIMITED with outcome="diverted" but omitted fallback_provider/
                # fallback_model. That is a VALIDATION problem with the data, not a transient write
                # failure: a retry reads the same blank fields every time, so the button must still
                # burn. set_override does not itself reject an empty target, so calling it here would
                # write a "/" override that enforcement (agent_runtime_helpers.py) then honours for
                # the full 6h TTL — blocking restoration of the primary model while diverting to
                # nothing.
                ok, reason = False, "no fallback target recorded for this alert"
            else:
                from events.model_override import SET_PERSIST_FAILURE_REASON, set_override

                # set_override does blocking disk I/O (_save_store -> atomic_replace) and this handler
                # runs on the asyncio loop serving ALL Telegram traffic, so a slow or contended write
                # would stall every other chat. Offload it, as this file already does for ffprobe.
            ...
            if ok:
                ...
                # Re-arm the token under the SAME string so the button on the still-visible message
                # keeps working — the tap only failed to persist, it was never invalid. Without this
                # the pop() above already consumed it and the next tap answers "already resolved".
                ...
                # A rejected write (self-target, divert-into-a-wall, blank target, internal error)
                # must surface its reason: a button that silently does nothing is the exact failure
        ...
        if retry_persist_failure:
            # Leave the message and its keyboard exactly as they were — editing it (even just the
            # text, with reply_markup=None as every other branch does) would retire the only working
            # control before the operator gets a chance to tap it again.
            return

        # Retire the buttons so a stale message cannot be tapped twice (the token is consumed either
        # way).
        try:
            await query.edit_message_text(
...
def interactive_setup() -> None:
    """Configure Telegram credentials and allowlist via the CLI setup wizard."""
