...
from reply_handlers import (
    parse as parse_reply_command, execute as execute_reply_command,
    is_authorized_telegram, ParseError,
)
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_polling_network_error(self, error: Exception) -> None:
        ...
        cap) up to MAX_NETWORK_RETRIES, then retryable-fatal so the reconnect watcher rebuilds this adapter."""
        if self._teardown_started or self.has_fatal_error:
            return
    ...
    def _register_handlers(self, app) -> None:
        ...
        for verb in ("approve", "reject", "archive"):
            app.add_handler(CommandHandler(
                verb, lambda update, context, v=verb: self._handle_reply_command(update, context, v)))
    ...
    async def _initialize_app_with_retries(self, builder) -> None:
        ...
        for _attempt in range(_max_connect):
            ...
            try:
                ...
                logger.log(logging.INFO if _attempt == 0 else logging.WARNING,
    ...
    def _emit_local_inbound_text(self, msg) -> None:
        try:
                ...
                _bus.emit(
                    ...
                    payload={
                        ...
                        "thread_id": (str(msg.message_thread_id)
                                      if msg.message_thread_id else None),
                        "message_id": str(msg.message_id),
                        "in_reply_to": (str(msg.reply_to_message.message_id)
    ...
    async def _handle_rate_limit_callback(self, query, data: str, cb: Dict[str, Any]) -> None:
        """Resolve opaque single-use local reroute buttons through the callback auth gate."""
    ...
    async def _handle_callback_query(self, update: "Update", context: "ContextTypes.DEFAULT_TYPE") -> None:
        ...
        for prefix, handler in (
            ...
            ("rl:", self._handle_rate_limit_callback),
    ...
    async def _handle_text_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not self._gate_or_observe(msg, update, MessageType.TEXT):
            return
        self._emit_local_inbound_text(msg)
