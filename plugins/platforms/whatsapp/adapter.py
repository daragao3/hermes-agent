...
from hermes_constants import (find_node_executable, get_hermes_dir,
                             node_executable_present, with_hermes_node_path)
...
def _listener_pids_on_port(port: int) -> list:
    """PIDs *listening* on ``port`` — never clients (a bare ``lsof -i :PORT`` once killed the
    user's browser).

    psutil FIRST, on every platform: it reads the kernel's TCP table in-process (16ms here
    against 851 connections) where every shell probe pays a process spawn this box cannot
    afford — ``netstat -ano -p TCP`` measured 8.2s, 9.6s and 21.3s on three consecutive IDLE
    runs. The Windows branch used to run netstat under ``timeout=5`` inside a bare
    ``except Exception: pass``, so it did not merely run slowly: it found nothing and killed
    nothing, every time, and the replacement bridge then failed to bind. The shell probes stay
    as the fallback for a host without psutil, or one that denies the connection table."""
    pids: list = []
    try:
        import psutil
...
def _kill_port_process(port: int) -> None:
    ...
    with suppress(Exception):
        # One resolver on every platform now: _listener_pids_on_port is psutil-first and
        # falls back to netstat on Windows itself.
...
def check_whatsapp_requirements() -> bool:
    ...
    if not _node:
        return False
    try:
        return subprocess.run([_node, "--version"],
                              timeout=_NODE_PROBE_TIMEOUT_S, **_RUN_TEXT).returncode == 0
    except subprocess.TimeoutExpired:
        # Node resolved to a real executable above — that IS the installation check. A probe
        # that overruns its budget says the host is loaded, not that the runtime is absent,
        # and returning False here makes _preflight stamp a NON-RETRYABLE
        # whatsapp_node_missing that keeps WhatsApp down until the next gateway boot
        # (observed 2026-08-11 23:23:47 with node v24.14.0 installed). If node really is
        # wedged, the bridge spawn fails with a retryable error instead.
        logger.warning(
            "node --version exceeded %ss; treating Node as present since %s resolved on disk",
            _NODE_PROBE_TIMEOUT_S, _node)
        return True
    except Exception:
        return False
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def _ensure_bridge_deps(self, bridge_dir: Path) -> bool:
        ...
        try:  # Default 300s accommodates slow systems like an Unraid NAS.
            # run_text_capture, not subprocess.run: _npm_bin is npm.cmd on Windows, so the
            # direct child is cmd.exe and the install runs in a node GRANDCHILD holding the
            # capture pipes. run() would kill only cmd.exe on timeout and then block
            # re-draining a pipe that never reaches EOF — WHATSAPP_NPM_INSTALL_TIMEOUT would
            # not bound this at all, and a wedged registry would hang gateway startup silently.
            from hermes_cli._subprocess_compat import run_text_capture

            install_result = run_text_capture(
                [_npm_bin, "install", "--silent"], cwd=str(bridge_dir),
    ...
    def _preflight(self) -> bool:
        ...
        checks = (
            (check_whatsapp_requirements, ("[%s] Node.js not found. WhatsApp requires Node.js.", self.name),
             "whatsapp_node_missing", "Node.js is not installed — install Node.js", True),
            (bridge_path.exists, ("[%s] Bridge script not found: %s", self.name, bridge_path),
             "whatsapp_bridge_missing", f"WhatsApp bridge script missing at {bridge_path}.", True),
        ...
        for ok, warn_args, code, message, environmental in checks:
            if not ok():
                logger.warning(*warn_args)
                # An ENVIRONMENTAL precondition is retryable up to a ceiling. Node missing and
                # a missing bridge tree read like static conditions, but on this host they have
                # only ever been transient — a loaded probe, or a tree mid-`npm install`. A
                # wrong non-retryable costs silent multi-hour downtime; a wrong retryable costs
                # one more 30s attempt. Not-paired is genuinely static and stays non-retryable.
                retryable = _count_env_fatal_and_get_retryable() if environmental else False
                if environmental and code == "whatsapp_node_missing":
                    message += ("; WhatsApp reconnects on its own once it is there."
                                if retryable else " and re-run `hermes gateway`.")
                self._set_fatal_error(code, message, retryable=retryable)
                return False
        # Both environment preconditions hold — whatever tripped the ceiling is gone, so the
        # next outage gets a full retry budget of its own.
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Observed, not guessed: a fixed 1s sleep is either wasted or not enough.
            if not await _wait_for_port_release(self._bridge_port):
                print(f"[{self.name}] Port {self._bridge_port} still bound after kill; "
            ...
            # Rotate HERE: the old node was just killed, so its handle is released, and the
            # fresh one is opened on the next line. This is the durable size cap.
    ...
    async def _build_message_event(self, data: Dict[str, Any]) -> Optional[MessageEvent]:
        ...
        try:
            if not self._should_process_message(data):
                return None
            self._emit_user_inbound_message(data)
            # Pipeline reply commands (/approve, /reject, /archive) short-circuit before an
            # LLM-routable event is built at all.
            ...
            # (24881232134201@lid) still reached this handler and triggered the pairing flow.
            # Re-checked here so a bridge-side regression cannot reach past the platform
            # boundary. The mixin's _should_process_message does not cover it: no alias
            # expansion, and the @lid form is exactly what got through.
            if self._is_sender_blocked_by_strict_allowlist(data, chat_id_value):
                return None
            msg_type = self._classify_bridge_message(data)
            # Defence in depth: if the bridge ever forgets isGroup, derive it from the @g.us
            # suffix so the pairing flow's group hard-stop still fires.
            _is_group = bool(data.get("isGroup")) or chat_id_value.endswith("@g.us")
            source = self.build_source(chat_id=chat_id_value, chat_name=data.get("chatName"),
                                       chat_type="group" if _is_group else "dm",
    ...
    # --- strict allowlist backstop and pipeline reply commands ---------------------------------
    # NOT provided by WhatsAppBehaviorMixin: its _should_process_message has no alias expansion,
    # and the `@lid` form is exactly what got past the bridge filter on 2026-04-19.
    ...
    def _emit_user_inbound_message(self, data: Dict[str, Any]) -> None:
        """Publish USER_INBOUND_MESSAGE so downstream subscribers see user feedback.

        Every inbound text, structured slash command or free-form alike. Best-effort: a bus
        failure must never cost the message itself."""
        try:
            ...
            try:
                bus.emit(
                    event_type=EventType.USER_INBOUND_MESSAGE, source="whatsapp",
                    payload={
                        "platform": "whatsapp", "text": str(data.get("body") or ""),
                        "user_id": str(data.get("senderId") or data.get("from") or ""),
                        "chat_id": str(data.get("chatId") or ""),
                        "thread_id": None,  # WhatsApp has no topics
                        "message_id": str(data.get("id") or ""),
                        "in_reply_to": (str(data.get("quotedId")) if data.get("quotedId") else None),
                    },
...
def register(ctx) -> None:
    ctx.register_platform(
        ...
        # check_fn SPAWNS `node --version`; the deps gate must not pay a process spawn on
        # every platform-enable sweep.
...
# --- bridge lifecycle: probes, retry ceiling, port release, log rotation ----------------------
