...
# Bound explicitly rather than through ``globals().update(...)``: a runtime globals update is
# invisible to static analysis, and every use of these names below then reads as an undefined
# name (F821) under this repo's own tree-wide ruff gate. ``_load_lark_oapi`` rebinds them from
# the table above, which stays the single source of truth for WHICH names exist.
