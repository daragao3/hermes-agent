...
# A Windows absolute path is a third alternative, not a variant of the POSIX one: without it
# a cleanup command naming C:/tmp/x.log or C:\tmp\x.log contributes no path at all.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        with contextlib.suppress(ValueError):  # tokenise — catches `touch /tmp/hermes-x/test_foo.py`
            # split_command_line, not shlex.split: POSIX mode treats a backslash as an escape,
            # so `touch C:\tmp\x.log` tokenises to `C:tmpx.log`.
            from hermes_cli._subprocess_compat import split_command_line
            paths.update(
                tok for tok in split_command_line(cmd)
