...
def node_command(args):
    from plugins.google_meet.node.cli import node_command as dispatch
    return dispatch(args)


def _register_node_cli(parser):
    try:
        from plugins.google_meet.node.cli import register_cli as register
        register(parser)
    except Exception as exc:
        # Exception names are cleared after except; bind the diagnostic durably.
        reason = str(exc)
        ...
        parser.set_defaults(func=unavailable)
...
def _cmd_install(*, realtime: bool, assume_yes: bool) -> int:
    ...
    def _install_pkgs(prompt: str, cmd: list[str], fail_msg: str) -> None:
        ...
        if not ok:
            ...
            return
        print(f"  $ {' '.join(cmd)}")
        # subprocess-stdin: — sudo/brew may prompt on the tty; user explicitly confirmed above
