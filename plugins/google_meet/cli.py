...
def _cmd_install(*, realtime: bool, assume_yes: bool) -> int:
    ...
    def _install_pkgs(prompt: str, cmd: list[str], fail_msg: str) -> None:
        ...
        if not ok:
            ...
            return
        print(f"  $ {' '.join(cmd)}")
        # Exempt (noqa: subprocess-stdin) — sudo/brew may prompt on the tty; user confirmed above.
        # Parenthesised so ruff does not read the line as a malformed lint directive;
        # check_subprocess_stdin.py scans the 4 lines above the call for the marker substring.
    ...
    try:
        ...
        # is an MSIX app-execution alias, and spawning it starts the child SUSPENDED pending
        # an activation handoff — a parent that exits first strands it forever.
        from hermes_constants import real_executable
        if subprocess.run([real_executable(), "-m", "playwright", "install", "chromium"], check=False,
