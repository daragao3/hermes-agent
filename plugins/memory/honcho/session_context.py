...
class SessionContextMixin:
    ...
    def create_conclusion(self, session_key: str, content: str, peer: str = "user") -> bool:
        ...
        def _create() -> bool:
            ...
            # get_or_create registers only the user + assistant peers (via add_peers); a
            # conclusion about any other peer — the capture path's dedicated ``hermes-events``
            # peer, for one — fails server-side ("Peer ... not found in workspace") unless
            # that peer already exists. add_peers is idempotent, so this is a safe no-op for
            # peers that are already registered, and non-fatal if it fails: the server may
            # already know the peer, so the write is still attempted.
            if target_peer_id not in (session.user_peer_id, session.assistant_peer_id):
                try:
                    self._sdk_session(session.honcho_session_id).add_peers(
                        [self._get_or_create_peer(target_peer_id)])
                except Exception as exc:
                    logger.warning(
                        "Could not register conclusion peer %r in session %r: %s",
                        target_peer_id, session_key, exc)
