...
def _run_brv(args: List[str], timeout: int = _QUERY_TIMEOUT, cwd: str = None) -> dict:
    ...
    try:
        # run_text_capture, not subprocess.run(capture_output=True): brv is a Node CLI, so
        # the real work happens in a grandchild that keeps the capture pipe open past the
        # direct child's death and defeats the timeout on Windows.
        from hermes_cli._subprocess_compat import run_text_capture
        result = run_text_capture(
            [brv_path] + args, timeout=timeout, cwd=effective_cwd, env=env,
