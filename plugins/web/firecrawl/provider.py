...
from agent.firecrawl_run_state import (
    CIRCUIT_OPEN_INFO, CREDITS_EXHAUSTED_INFO, FirecrawlCircuitOpenError,
    raise_if_firecrawl_circuit_open, record_firecrawl_credits_exhausted,
)
...
async def _scrape_one(url: str, formats: List[str], format: Optional[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            scrape_result = await asyncio.wait_for(asyncio.to_thread(_scrape_with_run_admission, _get_firecrawl_client(), url=url, formats=formats), timeout=60)
        ...
        if isinstance(scrape_err, FirecrawlCircuitOpenError):
            return {**_error_entry(url, str(scrape_err), raw=True), "error_info": dict(scrape_err.error_info)}
        if error_info := _credit_error_info(scrape_err):
            record_firecrawl_credits_exhausted()
            return {**_error_entry(url, "Firecrawl account credits are exhausted", raw=True), "error_info": error_info}
...
class FirecrawlWebSearchProvider(BaseWebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            return {**search_fail(str(exc)), "error_info": dict(exc.error_info)}
        try:
            ...
            if error_info := _credit_error_info(exc):
                record_firecrawl_credits_exhausted()
                return {**search_fail("Firecrawl account credits are exhausted"), "error_info": error_info}
    ...
    async def extract(self, urls: List[str], **kwargs: Any) -> List[Dict[str, Any]]:
        ...
        for url in urls:
            if _is_interrupted():
                result = {"url": url, "error": "Interrupted", "title": ""}
            elif credit_circuit_open:
                result = {**_error_entry(url, "Firecrawl account credit circuit is open", raw=True),
                          "error_info": _credit_circuit_info()}
            else:
                result = await _scrape_one(url, formats, format)
                credit_circuit_open = result.get("error_info", {}).get("code") in {
