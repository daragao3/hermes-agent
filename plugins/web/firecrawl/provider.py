...
_CREDITS_EXHAUSTED_MSG = "Firecrawl account credits are exhausted"


def _credits_exhausted(exc: Exception) -> bool:
    """True for the 402 that means the ACCOUNT is out of credits.

    Deliberately status-code only: the response body is never inspected, and never surfaced
    to the model, because a billing error page is not something a caller should be reading."""
...
async def _scrape_one(url: str, formats: List[str], format: Optional[str]) -> Dict[str, Any]:
    ...
    try:
        # Final admission check, immediately before the I/O: an earlier URL in this same run
        # may have already proved the account is out of credits.
        raise_if_firecrawl_circuit_open()
    ...
    except FirecrawlCircuitOpenError as circuit_err:
        # Never raises out of here: extract()'s contract is one entry per URL, so the breaker
        # reports itself as an entry. The distinction a caller needs is in error_info.
        logger.debug("Firecrawl scrape skipped for %s: account circuit open", url)
        return {**_error_entry(url, str(circuit_err), raw=True),
                "error_info": dict(circuit_err.error_info)}
    except Exception as scrape_err:  # noqa: BLE001
        if _credits_exhausted(scrape_err):
            logger.debug("Firecrawl scrape failed for %s: account credits exhausted", url)
            record_firecrawl_credits_exhausted()
            return {**_error_entry(url, _CREDITS_EXHAUSTED_MSG, raw=True),
                    "error_info": dict(CREDITS_EXHAUSTED_INFO)}
...
class FirecrawlWebSearchProvider(BaseWebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            return {**search_fail(str(circuit_err)),
                    "error_info": dict(circuit_err.error_info)}
        ...
        try:
            ...
            if _credits_exhausted(exc):
                # Open the breaker so the rest of this run stops paying for guaranteed 402s.
                logger.warning("Firecrawl search failed: account credits exhausted")
                record_firecrawl_credits_exhausted()
                return {**search_fail(_CREDITS_EXHAUSTED_MSG),
                        "error_info": dict(CREDITS_EXHAUSTED_INFO)}
