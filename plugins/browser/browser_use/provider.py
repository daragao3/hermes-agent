...
if TYPE_CHECKING:  # annotations only — `from __future__ import annotations` above
    # keeps every ``requests.*`` annotation a string at runtime, so this never executes.
    import requests
...
def __getattr__(name):  # PEP 562 — lazy so no import cycles
    if name == "requests":
        # NOT a compat pointer — do not drop this with the block around it.
        # ``requests`` used to be a module-scope import here, so ``<this module>.requests``
        # was a real attribute; callers reaching the client through the module path and
        # the tests that patch ``"<this module>.requests.<verb>"`` both depend on it. This
        # resolves to the same global module object, only later — plugin discovery
        # imports every plugin module, and paying for this one there is what the lazy
        # form exists to avoid (tests/hermes_cli/test_plugin_discovery_import_cost.py).
        import requests
