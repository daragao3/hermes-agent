...
# cleanup_vm / cleanup_browser are RE-EXPORTS, not dead imports: agent/client_lifecycle.py
# does `from run_agent import _quietly, cleanup_browser, cleanup_vm`, and several tests patch
# `run_agent.cleanup_vm` / `run_agent.cleanup_browser` by that path. Deleting them (ruff's
# suggested fix) breaks both, so the lint is silenced on the import instead.
from tools.terminal_tool_lifecycle import cleanup_vm, get_active_env  # noqa: F401
from tools.interrupt import set_interrupt as _set_interrupt
from tools.browser_tool_lifecycle import cleanup_browser  # noqa: F401
...
try:
    # Patches the openai SDK's parse_response against the R57 TypeError on a null
    # ``output``. Agent startup goes through this module, so the guard is armed here too.
    from agent.openai_codex_compat import apply_codex_output_none_guard

    apply_codex_output_none_guard()
except Exception:  # pragma: no cover — the guard must never break agent startup
    logging.getLogger(__name__).debug(
        "codex output=None guard not applied in run_agent", exc_info=True)
...
class AIAgent(
    ...
    def __init__(
        ...
        # Forwarded verbatim into init_agent via locals(); see _record_activity_response.
    ...
    def _session_source_for_persistence(self) -> str:
        """This agent's session source, as persisted on rows and usage attribution.

        The method form of :func:`_session_source_for_agent` so a caller holding an agent does
        not have to know where the platform lives. Six modules use it — acp_adapter/server.py,
        agent/codex_runtime.py, agent/conversation_loop.py, cli.py, gateway/run.py and
        tui_gateway/server.py."""
        return _session_source_for_agent(self.platform)

    def _record_activity_response(self, response: Any, usage: Optional[Dict[str, Any]] = None) -> None:
        """Attribute sanitized response usage without affecting inference.

        Inert unless an ``activity_recorder`` was supplied (the constructor parameter lands
        with agent/agent_init.py). A recorder failure is logged by TYPE only and swallowed:
        accounting must never take down a model call."""
...
def _called_with_defaults(bound: dict) -> bool:
    ...
    ``main(query=...)``. Without it, a programmatic caller running inside a host process
    whose argv carries unrelated flags (pytest, a wrapper script) would have those flags
    handed to fire and parsed as agent options."""
    import inspect
...
def main(
    ...
    """
    # Bare call + a non-empty argv means the console-script shim: hand off to fire ONCE.
...
def cli_main():
    ...
    Kept separate from ``main()`` so that stays a plain callable for programmatic use. Sets
    ``_CLI_DISPATCHED`` so the argv guard inside ``main()`` fires exactly once per process."""
