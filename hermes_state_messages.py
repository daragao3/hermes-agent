...
_INSERT_MESSAGE_SQL = """INSERT INTO messages (session_id, role, content, tool_call_id,
                   ...
                   display_metadata, display_identity, native_event_key)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
...
class SessionMessagesMixin:
        ...
        *keep_reasoning* False NULLs every reasoning column). ``platform_message_id`` falls back to
        ...
        return (session_id, role, encoded_content, msg.get("tool_call_id"),
            ...
            *(self._reasoning_json_text(_reasoning(k))
            ...
            # External-ingest identity. Feeds the partial unique index
            # idx_messages_native_event_key, which is what makes a re-ingest structurally
            # unable to append a second copy of an event already stored.
    ...
    def _insert_message_rows_with_ids(
        ...
        """:meth:`_insert_message_rows` returning the new rowids instead of a count.

        Public-by-use: session_bridge/store.py zips the returned ids against its pending
        messages with ``strict=True``, so this returns exactly one entry per input message, in
        order — including a None for any row SQLite gave no lastrowid.

        A WRAPPER, not a second implementation. The fork carried a full 95-line copy of the
        insert loop here; upstream had meanwhile refactored its own into helpers
        (``_INSERT_MESSAGE_SQL`` + ``_message_row_params``) and added display_kind /
        display_metadata / display_identity / _compressed_summary, while the fork's copy alone
        wrote native_event_key. Keeping both would have meant two insert paths disagreeing about
        which columns a message has, depending on the caller. native_event_key now lives in the
        shared params, so there is one insert path with the union of the columns.
        """
        _, tool_calls_total = self._insert_message_rows(conn, session_id, messages)
        return [msg.get("_row_id") for msg in messages], tool_calls_total
