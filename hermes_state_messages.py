...
_INSERT_MESSAGE_SQL = """INSERT INTO messages (session_id, role, content, tool_call_id,
                   ...
                   codex_message_items, platform_message_id, observed, _compressed_summary, active, api_content, display_kind, display_metadata, native_event_key)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
...
class SessionMessagesMixin:
    ...
    def _check_transcript_write_guards(self, conn, session_id: str, compression_lock_holder: Optional[str],
        ...
        from hermes_state_errors import (
            CompressionSessionClosedError, SessionCompressionInProgressError, SessionTurnLeaseLostError,
        )
        ...
        *keep_reasoning* False NULLs every reasoning column). ``platform_message_id`` falls back to
        ...
        return (session_id, role, self._encode_content(msg.get("content")), msg.get("tool_call_id"),
            ...
            *(self._reasoning_json_text(_reasoning(k))
            ...
            self._encode_display_metadata(msg.get("display_metadata")), msg.get("native_event_key"))
    ...
    def _insert_message_rows(self, conn, session_id: str, messages: List[Dict[str, Any]]) -> tuple[int, int]:
        """Compatibility wrapper returning counts while bridge callers can obtain exact IDs."""
        ids, tool_calls_total = self._insert_message_rows_with_ids(conn, session_id, messages)
        return len(ids), tool_calls_total

    def _insert_message_rows_with_ids(self, conn, session_id: str, messages: List[Dict[str, Any]]) -> tuple[list[int], int]:
        """Insert *messages* in the caller's txn -> ``(inserted_ids, tool_call_count)``.
