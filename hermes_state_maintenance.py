...
class SessionMaintenanceMixin:
    ...
    def prune_sessions(self, older_than_days: Optional[float] = 90, source: str = None,
        ...
        *sessions_dir*, transcript files are removed outside the DB transaction.
        ...
        ``max_batch`` bounds selected sessions per transaction (not message count or elapsed time).
        Each committed page advances past protected rows; rows newly eligible behind that
        cursor remain for a later sweep. Earlier pages stay committed if a later page fails."""
        ...
        def _do(conn):
            ...
            scanned = [row["id"] for row in conn.execute(sql, params).fetchall()]
            session_ids = set(scanned)
            ...
            self._delete_unreferenced_system_prompts(conn)
            return scanned, list(session_ids)
        count = 0
        while True:
            # Publish progress and clean files only after this callback commits;
            # retries/rollback must not leak speculative deletion IDs. Advance
            # past protected rows too, or a guarded first page starves the rest.
            scanned, removed_ids = self._execute_write(_do)
            count += len(removed_ids)
            for sid in removed_ids:
                self._remove_session_files(sessions_dir, sid)
            if max_batch is None or len(scanned) < max_batch:
                break
            after_id = scanned[-1]
        return count
    ...
    def maybe_auto_prune_and_vacuum(
        ...
        try:
            ...
            result["pruned"] = pruned = self.prune_sessions(
                ...
                exclude_active_write_guards=True, max_batch=max_batch)
            ...
            # Preserve local failure throttling without releasing the upstream
            # cross-process owner early. Contention must not trigger a sweep
            # on every startup. An already-throttled call does not extend it.
