...
class SessionMaintenanceMixin:
    ...
    def prune_sessions(self, older_than_days: Optional[float] = 90, source: str = None,
                       ...
                       max_batch: Optional[int] = None, **filters) -> int:
        ...
        *sessions_dir*, transcript files are removed outside the DB transaction.
        ...
        if max_batch is not None:
            max_batch = max(1, int(max_batch))  # <=0 would spin the chunk drain forever
        def _do(conn):
        ...
        if max_batch is None:
            ...
            # Each chunk is its own transaction, so the write lock is released between
            # them. Bounds lock-hold time and WAL growth on a large backlog (a retention
            # change, or a post-outage catch-up) so an online prune never starves the live
            # gateway. A short chunk means the backlog drained.
            count = 0
            while True:
    ...
    def maybe_auto_prune_and_vacuum(
        ...
        if maintenance_lock is None:
            result["skipped"] = True
            return result
        # Bound before the try so the finally can always stamp the marker.
        now = time.time()
        try:
            ...
            # (the throttle marker is stamped in the finally, on success AND failure)
            ...
            # Stamp the attempt on FAILURE too, not just success. The success-only write
            # above is skipped when the body raises, so a transient `database is locked`
            # left the throttle unset and re-triggered the whole sweep on every startup.
            # Skips are exempt: they never attempted anything.
