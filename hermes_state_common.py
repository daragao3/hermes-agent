...
def inheritable_local_child_cwd(parent, parent_session_id, child_source):
    """Select exact persisted metadata from the transaction's parent row."""
    if (not isinstance(parent_session_id, str) or not parent_session_id
            or not isinstance(child_source, str) or child_source not in _LOCAL_PERSISTED_CWD_SOURCES):
        return None
    if parent is None or parent["id"] != parent_session_id or parent["source"] != child_source:
        return None
    cwd = parent["cwd"]
