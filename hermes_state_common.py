...
#
# Includes messages_fts_cjk, which the fork's two-table version predates: the 0.21.1 merge
# brought upstream's CJK index (hermes_state_fts._FTS_TABLES lists all three, and
# SessionDB tracks _fts_cjk_loaded/_fts_cjk_available). Leaving it out would not error —
# _fts_indexes_present skips absent tables — it would silently EXCLUDE a present index from
# the integrity probe, so `hermes doctor` would report FTS healthy with a corrupt CJK index.
# Keep this in step with hermes_state_fts.SessionFtsSetupMixin._FTS_TABLES; the whole point
# of the constant is that one set is shared.
_FTS_INDEX_TABLES: Tuple[str, ...] = ("messages_fts", "messages_fts_trigram", "messages_fts_cjk")
...
# 34, not 31: the fork's ladder ran ahead of upstream's (its gates were renumbered to 29
# at the 0.19.0 merge, then 32 and 33 were added), so existing fork databases are stamped
# 33 — verified on the live state.db — while upstream's newest gate is 30. Sitting above
# BOTH lineages keeps 'which is newer' answerable. The 30..34 compensating block in
# _init_schema is what actually gets upstream's v23/v25/v30 work onto those databases;
# bumping this number alone would only paper over the gap.
SCHEMA_VERSION = 34
...
-- Named triggers preserve the lease invariant on legacy databases whose table
-- CHECK predates lease_kind, and give callers a stable actionable error instead
-- of SQLite's generic CHECK expression. These lived in old hermes_state.py and
-- must remain deferred until reconciliation has added lease_kind.
