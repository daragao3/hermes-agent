...
def _drain_post_turn_notifications(
    rid, sid: str, session: dict, *, db_path=None,
) -> None:
    """Drain completion events against the delegation DB captured by the turn.

    The caller runs after its profile-home override unwinds. A carried path
    keeps claim/complete operations on the same store; None preserves live
    resolution for direct callers.
    """
    try:
        ...
        drained = process_registry.drain_notifications(
            ...
            owns_event=lambda event: _session_owns_notification_event(
                sid, session, event
            ),
        ...
        _hook_failure("completion queue drain", drain_exc)
...
def _run_post_turn_followups(
    ...
    # Safety net for completion events that arrived mid-turn. Ownership is
    # positive-proof and compression-chain aware; unclaimable events return to
    # the poller. The path was captured before any per-turn profile override.
    _drain_post_turn_notifications(rid, sid, session, db_path=db_path)
...
def _run_prompt_submit(
    ...
    def run():
        # Capture before _prepare_turn_input installs the session profile's
        # HERMES_HOME override. Followups run after _finish_turn unwinds it, but
        # delivery claims must stay on this process-home store.
        try:
            from tools.async_delegation import _db_path as _delegation_db_path

            turn_db_path = _delegation_db_path()
        except Exception:
            turn_db_path = None
        ...
        _run_post_turn_followups(
            rid, sid, session, st.result, goal_followup, db_path=turn_db_path
        )
