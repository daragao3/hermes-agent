...
def _persisted_session_cwd(session: dict) -> str | None:
    """Persist the actual execution directory so another harness can resume safely."""
    return str(session.get("cwd") or "").strip() or None
...
def _ensure_session_db_row(session: dict) -> bool:
    ...
    Persist the session's actual execution directory, including a resolved desktop default,
    so another harness can resume in the same context. Missing cwd stays unset.
