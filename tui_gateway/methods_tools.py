...
def _captured_exec(rid, cmd, timeout: int, *, on_result, timeout_err: tuple, fail_code: int,
    ...
    try:
        capture = _tools_mod("hermes_cli._subprocess_compat").run_text_capture
        return on_result(capture(cmd, cwd=os.getcwd(), shell=shell, env=env, timeout=timeout))
...
def _(rid, params: dict) -> dict:
    ...
    return _captured_exec(
        rid, [real_executable(), "-m", "hermes_cli.main", *argv], min(int(params.get("timeout", 240)), 600),
...
def _dispatch_quick(rid, params, session, name, arg):
    ...
    if qc.get("type") == "exec":
        ...
        try:
            r = _tools_mod("hermes_cli._subprocess_compat").run_text_capture(
                qc.get("command", ""), shell=True, env=env, timeout=30)
        except subprocess.TimeoutExpired:
...
def _(rid, params: dict) -> dict:
    ...
    sections = [
        ...
        {"title": "Environment", "rows": [["Working Dir", os.getcwd()], ["Config File", str(_resolve_hermes_home() / "config.yaml")]]},
    ]
