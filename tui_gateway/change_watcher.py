...
def _watcher_home() -> Path:
    ...
    # _resolve_hermes_home(), not the raw _hermes_home: that global is None unless a test
    # pinned it, and Path(None) raises. Both names arrive here through server.py's
    # register() rebinding.
    return Path(override if isinstance(override, str) and override else _resolve_hermes_home())
