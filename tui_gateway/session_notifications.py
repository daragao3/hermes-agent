...
def _notif_dispatch_event(sid: str, session: dict, evt: dict, text: str, *, db_path=None) -> None:
...
def _notif_handle_event(sid, session, evt, emitted, registry, fmt, deferred, completions=None, *, owned=False, db_path=None) -> bool:
...
def _notif_dispatch_completions(sid, session, notifications, registry, deferred, *, db_path=None):
...
def _notif_handle_ready(sid, session, events, emitted, registry, fmt, deferred, *, owned=False, db_path=None):
    ...
    for index, event in enumerate(events):
        if event.get("type", "completion") != "completion":
            ...
            completions = []
        if not _notif_handle_event(sid, session, event, emitted, registry, fmt, deferred, completions, owned=owned, db_path=db_path):
...
def _notification_poller_loop(stop_event: threading.Event, sid: str, session: dict, db_path=None) -> None:
...
def _capture_notification_db_path():
    """Resolve once at session/turn start without creating a store."""
    try:
...
def _start_notification_poller(sid: str, session: dict) -> threading.Event:
    ...
    t = threading.Thread(target=_notification_poller_loop, args=(stop, sid, session, _capture_notification_db_path()), daemon=True, name=f"tui-notif-poller-{sid}")
