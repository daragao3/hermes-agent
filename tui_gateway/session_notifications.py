...
def _notif_dispatch_event(sid: str, session: dict, evt: dict, text: str, *,
                          db_path=None) -> None:
    """Run the claimed (running=True) agent turn for one notification event.

    ``db_path`` is the delegation db captured by a caller whose thread outlives the scope
    that fixed HERMES_HOME (the poller, the post-turn drain). The delivery calls below
    CREATE ``<home>/state.db``, so re-resolving it here would materialise one under
    whatever home is current at claim time. None means "resolve live", which is right for
    a caller still inside the request scope.
    """
    from tools.async_delegation import claim_event_delivery, complete_event_delivery, release_event_delivery
    if (claim := claim_event_delivery(evt, "tui-poller", db_path=db_path)) is None:
        return
    ...
    try:
        ...
        release_event_delivery(evt, claim, db_path=db_path)
        return
    complete_event_delivery(evt, claim, db_path=db_path)


def _notif_handle_event(sid, session, evt, emitted, registry, fmt, deferred, completions=None, *,
                        owned=False, db_path=None) -> bool:
    ...
    if not _notif_claim_turn(session):
        ...
        return True
    _notif_dispatch_event(sid, session, evt, text, db_path=db_path)
    return True
...
def _notif_dispatch_completions(sid, session, notifications, registry, deferred, *,
    ...
    claimed = [(event, text, claim) for event, text in notifications
               if (claim := claim_event_delivery(event, "tui-completion-batch", db_path=db_path)) is not None]
    ...
    try:
        ...
        for event, _text, claim in claimed:
            release_event_delivery(event, claim, db_path=db_path)
        return
    for event, _text, claim in claimed:
        complete_event_delivery(event, claim, db_path=db_path)


def _notif_handle_ready(sid, session, events, emitted, registry, fmt, deferred, *,
                        owned=False, db_path=None):
...
def _notification_poller_loop(
    stop_event: threading.Event, sid: str, session: dict, *, db_path=None,
) -> None:
    ...
    # db_path was captured by _start_notification_poller before this long-lived
    # thread started; resolving here would follow whatever HERMES_HOME is active
    # when the thread eventually enters.
    handle = lambda events, deferred: _notif_handle_ready(  # noqa: E731
        sid, session, events, emitted, process_registry, format_process_notification, deferred, db_path=db_path)
...
def _start_notification_poller(sid: str, session: dict) -> threading.Event:
    ...
    _wire_desktop_sinks()
    # Capture before starting the long-lived thread. Delivery calls create
    # <home>/state.db, so resolving inside the thread can materialize a store
    # under an unrelated later profile; a carried path also arms
    # async_delegation's BoundHomeMissing refusal.
    try:
        from tools.async_delegation import _db_path as _delegation_db_path

        poll_db_path = _delegation_db_path()
    except Exception:
        poll_db_path = None
