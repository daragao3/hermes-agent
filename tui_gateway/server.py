...
from hermes_constants import (
    ...
    get_process_hermes_home, real_executable,
...
# FanoutTransport is a RE-EXPORT, not a local use: session_reaper.py and
# session_transports.py reference it bare and resolve it through this module's globals
# (method_ctx.bind_module). Deleting the "unused" import is a runtime NameError there,
# and silent to every linter.
from tui_gateway.transport import (FanoutTransport, StdioTransport, Transport, bind_transport,  # noqa: F401
                                   current_transport, reset_transport)

logger = logging.getLogger(__name__)

# Resolved per use, NOT baked into a module constant at import. Under pytest the autouse
# ``_hermetic_environment`` fixture redirects ``HERMES_HOME`` to a per-test tempdir only
# AFTER collection has imported this module, so a constant captured here holds the
# developer's REAL ``~/.hermes``. Four use sites below are WRITES — ``_save_cfg`` (the
# user's live config.yaml), the two ``images/`` sites, and ``paste.collapse`` — and nothing
# structural stopped a test from reaching them.
#
# It stays as an override SEAM so existing
...
def _resolve_hermes_home() -> Path:
    ...
    ``get_process_hermes_home()`` rather than ``get_hermes_home()``, on purpose. The
    import-time constant this replaces was evaluated with no context-local override active,
    so the launch home is exactly what it captured, and every use site below runs on the RPC
    dispatch thread — outside the per-turn ``set_hermes_home_override`` scope opened by the
    turn dispatcher. Following the override would be a behaviour CHANGE, not a fix.

    ``_profile_home`` depends on this directly: it compares a candidate profile against the
    LAUNCH home to decide whether an override is needed at all, so resolving it through the
    override would make it answer "already the launch profile" for whatever profile the
    caller was scoped to.
    """
...
# Same reasoning as _hermes_home above, and the same override seam for
# ``monkeypatch.setattr(server, "_CRASH_LOG", ...)``.
...
def _crash_log_path() -> str:
    ...
    ``get_process_hermes_home()`` rather than ``get_hermes_home()``: the panic log is a
    process-level forensic asset for the gateway, and resolving it through the context-local
    override would scatter a turn's crash record into whichever profile home that turn
    happened to be scoped to (the turn dispatcher's ``except`` block runs while its
    ``set_hermes_home_override`` is still active). No override is active at import, so this
    is the exact live equivalent of what the old import-time constant captured.
    """
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # real_executable(), not sys.executable: under Store Python the latter is the MSIX
        # alias, and spawning it starts the child SUSPENDED pending AppX activation — if this
        # gateway dies mid-handoff the worker is stranded forever (see
        # hermes_constants.real_executable).
        argv = [real_executable(), "-m", "tui_gateway.slash_worker", "--session-key", session_key] + (["--model", model] if model else [])
        self._closed = False
...
def _active_config_path() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _resolve_hermes_home()) / "config.yaml"
