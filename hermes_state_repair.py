...
def _db_opens_cleanly(db_path: Path, *, timeout_seconds: Optional[float] = None) -> Optional[str]:
    ...
    from hermes_state_local_fts import StateDbProbeTimeout, _arm_probe_deadline
    ...
    def check_budget():
        if timed_out is not None and timed_out.is_set():
            raise StateDbProbeTimeout("state.db health probe", timeout_seconds)
