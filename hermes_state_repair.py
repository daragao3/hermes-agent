...
def _db_opens_cleanly(
    ...
    *,
    ...
    ``timeout_seconds`` is a wall-clock budget for the whole probe. ``None`` (the default)
    means UNBOUNDED, deliberately: the repair paths gate destructive strategies on this
    result and must not trade thoroughness for latency. When a budget is set and exhausted
    this raises :class:`StateDbProbeTimeout` and never returns a reason string — running out
    of time is not a corruption report and must not be actionable as one.

    ``include_fts_integrity`` additionally runs the rank=1 FTS ``'integrity-check'``. Off by
    default because it re-reads and re-tokenises every indexed row (~12.6s on a 171 MB index;
    production holds roughly eight times that text), so only a deliberate deep check opts in.
    ...
    def _raise_if_timed_out(stage, exc=None):
        """Turn a watchdog-induced abort into the timeout signal, not a corruption reason."""
        ...
        with contextlib.closing(conn):
            ...
            try:
                ...
                if _schema_not_built(exc) or "no such tokenizer: cjk_unicode61" in str(exc).lower():
                    return None
                _raise_if_timed_out("fts write probe", exc)
                return str(exc)

            if include_fts_integrity:
                reason = _fts_integrity_reason(conn)
                if reason is not None:
                    _raise_if_timed_out("fts integrity-check")
                    return reason

            # LAST, on purpose: the only statement here whose cost scales with database
            # size (a page-by-page verification of every b-tree — 116.8s on a compacted
            # snapshot, >12 minutes on the live 5.1 GB file). Run earlier, an exhausted
            # budget would mean none of the targeted probes above ever ran.
