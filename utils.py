...
def atomic_roundtrip_yaml_dump(path: Union[str, Path], document: Any, *, yaml_rt: Any = None) -> None:
    """Write a prepared round-trip document through the common atomic writer."""
    path = Path(path)
    if yaml_rt is None:
        yaml_rt = build_roundtrip_yaml()
    _roundtrip_dump(path, yaml_rt, document)
