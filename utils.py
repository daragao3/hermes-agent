...
# Round-trip YAML preservation: comments, quoting and key order survive a rewrite, and
# _pyyaml_would_retype guards the values PyYAML would silently re-type on the way back.
# (The fork's Windows sharing-violation retry is NOT carried — upstream's atomic_replace
# already detects contention, backs off with jitter, and falls back to an in-place rewrite.)
...
def atomic_roundtrip_yaml_dump(
    ...
    *,
    ...
    A WRAPPER over ``_roundtrip_dump``, not a second writer: upstream extracted the same
    load/dump split the fork did, so keeping both would mean two atomic-write paths that could
    drift on mode preservation or temp-file naming. The public name stays because
    tests/gateway/test_dm_topics.py and the full-document config path use it.
    """
    target = Path(path)
    target.parent.mkdir(parents=True, exist_ok=True)
    _roundtrip_dump(target, yaml_rt if yaml_rt is not None else build_roundtrip_yaml(), document)
