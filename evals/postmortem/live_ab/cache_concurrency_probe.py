...
def patched_stream(self, **kw):
    ...
    # Reconstructed from patched_create below, which is this same function against the OpenAI
    # SDK: without it `m`, `sys_sha`, `tools_sha` and `msg_shas` are undefined and every
    # streamed Anthropic call raises NameError. The one difference is forced by the SDKs —
    # OpenAI carries the system prompt inside `messages` and has to split it out, Anthropic
    # passes it as its own `system=` kwarg, so `msgs` is used whole (as the surrounding
    # call=/n_msgs= already assume) and the system sha comes from the kwarg.
    import re as _re
    ...
    sys_sha, tools_sha, msg_shas = sha(kw.get("system")), sha(kw.get("tools")), [sha(x) for x in msgs]
