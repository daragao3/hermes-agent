...
def main():
    ...
    transcript = []
    result = {'home': str(home), 'command': cmd, 'sha': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=args.repo, text=True, encoding="utf-8").strip()}
    try:
        for _ in range(180):
        ...
        model.server_close()
        log.close()
        (args.out / 'wire.json').write_text(json.dumps(transcript, indent=2), encoding="utf-8")
