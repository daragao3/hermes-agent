...
import {
  ...
  checkRootInstall as checkWorkspaceRoot
} from "../../../scripts/js/assert-workspace-install.mjs"
export {
  checkLockDrift, checkWorkspaceInstall, formatDrift, installLooksCurrent
} from "../../../scripts/js/assert-workspace-install.mjs"
...
export function checkRootInstall(appDir, rootDir) {
  if (rootDir === undefined) return checkWorkspaceRoot(appDir)
...
function main() {
  ...
  const drift = checkWorkspaceInstall(root, app)
  const result = drift.ok ? checkRootInstall(app, root) : drift
