...
export interface SidebarSessionSlice {
  ...
  /** Showable-session count for EVERY profile, regardless of the requested scope.
   *  The only field that can tell a scoped-but-empty profile apart from a broken
   *  load: under a concrete scope the scoped maps carry just that profile's key, so
   *  the comparison number is absent from them by construction. Absent from the
   *  legacy per-slice endpoint (and from any pre-#42651 backend), which leaves the
   *  "your chats are in another profile" empty state inert rather than wrong. */
