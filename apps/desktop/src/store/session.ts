...
/** Showable-session count per profile across ALL profiles, not just the scoped one.
 *  Kept SEPARATE from any scoped map on purpose: the sidebar iterates the scoped maps to
 *  decide which profile catalogs to hydrate, so widening one of those would make a scoped
 *  sidebar re-fetch every profile on every refresh. This map is read only to answer "is
 *  this profile empty while chats exist elsewhere?" — see sidebar/scoped-empty.ts. */
