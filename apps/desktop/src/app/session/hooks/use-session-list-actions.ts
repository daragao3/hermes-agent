    ...
    async (shouldPublish: () => boolean = () => true) => {
      ...
      try {
        ...
        if (
          ...
          // Cross-profile counts, same identity gate as the two maps around it. Only read
          // to decide whether a scoped-but-empty sidebar should say where the chats ARE;
          // an older backend omits the field, which leaves this {} and that state inert.
          setSessionAllProfileTotals(prev => {
            const next = keepFailedProfileMeta(prev, recents.all_profile_totals ?? {}, recentsErrors)
            const prevKeys = Object.keys(prev)
