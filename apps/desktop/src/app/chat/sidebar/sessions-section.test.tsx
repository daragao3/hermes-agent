...
const dateSectionLabels = vi.hoisted(() => ({ today: 'Today', yesterday: 'Yesterday', thisWeek: 'This week', thisMonth: 'This month' }))

vi.mock('@/i18n', () => ({
  useI18n: () => ({
    t: {
      sidebar: {
        dateSections: dateSectionLabels,
        dateDivider: {
...
describe('SidebarSessionsSection memoization & virtualizer stability', () => {
  it('carries local calendar sections into the shared virtual row model', () => {
    mockVirtualListPropsHistory.length = 0
    const now = Math.floor(Date.now() / 1000)
    const sessions = Array.from({ length: VIRTUALIZE_THRESHOLD + 1 }, (_, index) => makeSession(`today-${index}`, now))
    render(<SidebarSessionsSection activeSessionId={null} dateSections emptyState={<div>Empty</div>}
      label="Sessions" onArchiveSession={noop} onDeleteSession={noop} onResumeSession={noop}
      onToggle={noop} onTogglePin={noop} onToggleUnread={noop} open pinned={false} sessions={sessions} />)
    const rows = mockVirtualListPropsHistory.at(-1)!.rows
    expect(rows[0]).toMatchObject({ kind: 'divider', key: 'date:today', label: 'Today' })
    expect(rows.filter(row => row.kind === 'session')).toHaveLength(sessions.length)
  })
