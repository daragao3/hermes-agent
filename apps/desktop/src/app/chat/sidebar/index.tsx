...
export function ChatSidebar({
  ...
  // Preserve the local loaded/known count when this backend provides totals.
  // Pagination remains authoritative on upstream truncation flags above.
  const knownRecentsTotal = Math.max(
    showAllProfiles
      ? Object.values(sessionProfileTotals).reduce((total, count) => total + count, 0)
      : (scopedProfileTotal ?? loadedSessionCount),
    loadedSessionCount
  )
  const recentsMeta = countLabel(displayAgentSessions.length, knownRecentsTotal)
          ...
          >
            ...
            {!trimmedQuery && worktreeGroupingActive && (
              <SidebarSessionsSection
                ...
                dndSensors={dndSensors}
                emptyState={
                  inProject && projectLoadFailed ? null : inProject && projectLoading ? <SidebarSessionSkeletons /> : (
                ...
                grouping={showArchived ? 'none' : 'date'}
                headerAction={
            ...
            {!trimmedQuery && (
              <SidebarSessionsSection
                ...
                footer={
                  // Sessions owns pagination in both scope modes; upstream
                  // profile groups share the same bounded page loader.
