  ...
  useI18n: () => ({
    t: {
      sidebar: {
        ...
        row: {
          bridgeProvider: (provider: string) => `Provider: ${provider}`,
          sessionDriver: (driver: string) => `Driven by ${driver}`,
          ageMin: 'm',
          ageNow: 'now',
...
function makeBridgeSession(overrides: Partial<SessionInfo> = {}): SessionInfo {
  return {
    archived: false,
...
function renderBridgeRow(session: SessionInfo) {
  return render(
...
describe('SidebarSessionRow bridge badges', () => {
  ...
  it.each([
    ...
    renderBridgeRow(makeBridgeSession({ bridge_sidebar_state: state }))
  ...
  it('tints Claude, Codex and Hermes provider badges with distinct hues', () => {
    const classes = (['claude', 'codex', 'hermes'] as const).map(provider => {
      const { unmount } = renderBridgeRow(makeBridgeSession({ bridge_provider: provider, source: provider }))
  ...
  it('keeps an unrecognized provider badge on the neutral tint', () => {
    ...
    renderBridgeRow(makeBridgeSession({ bridge_provider: 'mystery' as SessionInfo['bridge_provider'], source: 'cli' }))
  ...
  it('tints the driver badge with the same per-harness hue', () => {
    renderBridgeRow(makeBridgeSession({ driver: 'claude' }))
