...
import { dateSectionLabel, withDateSections } from './session-date-sections'
...
interface SidebarSessionsSectionProps {
  ...
  /** Preserve the local calendar sections for the flat recency view. */
...
export function SidebarSessionsSection({
  ...
  const flatRows: SidebarListRow[] = useMemo(() => {
    const rows =
      dateSections
        ? withDateSections(displayEntries).map<SidebarListRow>(item => item.kind === 'header'
          ? { kind: 'divider', key: item.id, label: dateSectionLabel(item.bucket, t.sidebar.dateSections) }
          : { kind: 'session', entry: item.entry })
        : grouping === 'date'
  ...
  }, [dateSections, grouping, displayEntries, dotStates, manualOrderIds, statusDividerLabels, t.sidebar.dateSections])
