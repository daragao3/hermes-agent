...
export function CronView({ onClose, onOpenSession, setStatusbarItemGroup: _setStatusbarItemGroup }: CronViewProps) {
  ...
  const refresh = useCallback(async () => {
    const { jobs: refreshedJobs, profileErrors, refreshError, stale } = await refreshCronJobs(profile)
    ...
    if (refreshedJobs !== null) {
      setListErrors(profileErrors ?? [])
    }
  ...
  return (
    <Panel closeLabel={c.close} onClose={onClose}>
      ...
      {loading && jobs.length === 0 ? (
      ...
      ) : totalCount === 0 && visibleBlueprints.length === 0 && listErrors.length === 0 ? (
        <PanelEmpty
          action={
        ...
        <PanelBody>
          ...
          >
            ...
            {visibleJobs.length === 0 && listErrors.length === 0 && (
          ...
          {selectedJob ? (
            ...
            />
          ) : jobs.length === 0 && listErrors.length > 0 ? null : query.trim() ? (
...
function CronJobRuns({
  ...
  }, [c, jobId, jobProfile, reloadNonce, changeEventsAvailable, cronChangeTick])
