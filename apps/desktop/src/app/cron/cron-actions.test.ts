...
describe('triggerAndRefreshCronJobs', () => {
  ...
  it('preserves profile errors alongside the accepted partial job list', async () => {
    const jobs = [{ id: 'available', state: 'scheduled' }]
    const errors = [{ profile: 'locked', error: 'store unavailable' }]
    getCronJobs.mockResolvedValue({ jobs, errors })
    const result = await refreshCronJobs('all')
    expect(result).toMatchObject({ jobs, profileErrors: errors, stale: false })
