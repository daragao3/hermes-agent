...
export function ToolsetConfigPanel({ toolset, onConfiguredChange, profile }: ToolsetConfigPanelProps) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // row was collapsed and a setExpandedProvider(default) was still queued:
      // a click landing in that window enqueued its own setExpandedProvider
      ...
      const initialProvider = defaultExpandedProvider(next, seeded)
      if (!providerChoiceClaimedRef.current && initialProvider) {
        providerChoiceClaimedRef.current = true
        setExpandedProvider(current => current ?? initialProvider)
      }
    } catch (err) {
