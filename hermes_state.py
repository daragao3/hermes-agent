...
from hermes_state_guard import (
    ...
    _set_last_init_error, get_last_init_error, refuse_mock_db_path,
)
from hermes_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for, execute_read_with_retry
from hermes_state_sessions import SessionSessionsMixin
from hermes_state_sessions import detect_session_driver as detect_session_driver
from hermes_state_common import _is_canonical_absolute_cwd as _is_canonical_absolute_cwd
from hermes_state_bridge_schema import BRIDGE_SCHEMA_SQL as BRIDGE_SCHEMA_SQL, SessionBridgeSchemaMixin
from hermes_state_local_fts import (
    SessionLocalFtsMixin, StateDbProbeTimeout as StateDbProbeTimeout,
    check_state_db_fts_integrity as check_state_db_fts_integrity,
    _arm_probe_deadline as _arm_probe_deadline,
    _fts_integrity_check_sql as _fts_integrity_check_sql,
    _fts_indexes_present as _fts_indexes_present,
    _fts_integrity_reason as _fts_integrity_reason,
    _is_interrupted_error as _is_interrupted_error,
)
...
class SessionDB(
    SessionBridgeSchemaMixin, SessionLocalFtsMixin,
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        # A mock is os.PathLike, so it would silently become a real directory tree under CWD.
        refuse_mock_db_path(self.db_path, where="SessionDB")
    ...
    def _execute_read(self, fn: Callable[[sqlite3.Connection], T]) -> T:
        """Materialize a SELECT-only callback through pooled reads with local lock retry."""
        return execute_read_with_retry(
            self._read_ctx, fn,
            max_attempts=getattr(self, "_WRITE_MAX_RETRIES", 15),
            min_delay=self._WRITE_RETRY_MIN_S,
            max_delay=self._WRITE_RETRY_MAX_S,
        )
