...
# Re-exported for `from hermes_state import ...` callers (hermes_cli/doctor.py, tests): the probe
# itself lives in hermes_state_common so the FTS mixin can use it without importing this module,
# which would be a cycle (hermes_state imports the mixins).
...
class SessionDB(
    ...
    def _execute_read(
        ...
        """Run *fn(conn)* as a read with the same jittered retry writes get.

        Public-by-use: session_bridge/catalog.py and session_bridge/store.py call this directly on
        the resume path, so the name is part of the contract, not an internal detail.

        MERGED, not carried. The fork's version held ``self._lock`` and ran ``fn(self._conn)`` —
        correct before the 0.21.1 decomposition, wrong after it: upstream's reads now borrow a
        pooled READ-ONLY connection and take no lock at all under WAL, so re-acquiring the writer
        lock here would serialise every bridge read behind the gateway's writes and undo that.
        It also referenced ``self._WRITE_MAX_RETRIES``, which upstream replaced with a deadline
        model — a straight copy would have raised AttributeError on the first contended read.

        So: upstream's ``_read_ctx()`` for the connection, and upstream's
        ``_sleep_before_write_retry`` for the backoff. The retry is still worth having, because
        ``_read_ctx`` deliberately degrades to the writer connection under ``self._lock`` when the
        pool is unavailable (non-WAL/NFS, open failure, fd ceiling) — on that branch the fork's
        original failure returns, and upstream has no retry of its own. Retrying is what keeps a
        transient WAL collision from surfacing as the desktop's "Resume failed: handler error:
        database is locked".

        *fn* must perform SELECTs only (no transaction, no mutation) and must tolerate being
        replayed, exactly as ``_execute_write``'s callback must.
        """
        if patience_s is None:
            patience_s = self._WRITE_PATIENCE_S
        deadline = time.monotonic() + patience_s
        while True:
            try:
                ...
                if ("locked" in message or "busy" in message) and self._sleep_before_write_retry(
                    deadline, patience_s
                ):
                    continue
                raise
    ...
    _TOKEN_DELTA_ROUTE_FIELDS = (
        ...
        "billing_mode", "source", "model_config",
    )
