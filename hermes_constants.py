...
def _run_version_probe(argv: list[str], **kwargs):
    ...
    try:
        ...
        # DEVNULL, not capture_output=True. `path` is npm.cmd / an npx shim on Windows, so
        # the direct child is cmd.exe and the probe runs in a node GRANDCHILD that inherits
        # the capture pipes. On timeout subprocess.run kills only cmd.exe and then blocks
        # re-draining a pipe the grandchild still holds — so the 10s bound would not hold
        # for exactly the broken wrapper this probe exists to detect. Only returncode is
        # read, so there is no output to preserve.
