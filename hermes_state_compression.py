    ...
    def _publish_child_session_row(self, conn, parent, *, parent_session_id, child_session_id, source,
        ...
        conn.execute(
            ...
            (
                ...
                system_prompt_hash, parent_session_id, selected_cwd, parent["git_branch"] if same_workspace else None,
                parent["git_repo_root"] if same_workspace else None,
    ...
    def publish_compression_child(
        ...
        *compression_lock_holder* refreshes the lease on the same ``conn`` before the expiry check (no
        ...
        def _do(conn):
            ...
            parent = conn.execute(
                """SELECT id, source, ended_at, end_reason, cwd, git_branch, git_repo_root,
