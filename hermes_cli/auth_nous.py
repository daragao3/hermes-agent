...
def _quarantine_nous_pool_entries(
    ...
    dead_refresh_token: Optional[str]) -> bool:
    """Remove singleton-seeded Nous pool entries that carry the DEAD OAuth chain.

    Callers reach here when ``dead_refresh_token`` has just failed terminally. What must be
    reclaimed is every OTHER copy of that same dead chain — singleton-seeded entries are
    re-seeded from ``providers.nous``, so a copy left behind keeps re-driving the doomed
    refresh.

    Matching on the refresh token rather than on ``source`` alone is what makes that precise.
    A pool routinely holds entries on DIFFERENT chains under the same source: an old
    ``manual:device_code`` entry stranded on a revoked lineage alongside a ``device_code``
    entry seeded from a freshly re-authed singleton. Removing by source alone destroyed the
    healthy entry whenever the stale one failed, leaving nothing selectable. See
    ``CredentialPool._quarantine_dead_chain``, the in-memory twin of this function.

    ``dead_refresh_token`` is keyword-only and has NO default deliberately: the defect was an
    unstated assumption about which chain died, so every call site is made to name it.
    ``None``/``""`` means no chain was identified, and reclaims only entries that themselves
    ...
    retained = [
        e for e in entries if not (
            isinstance(e, dict) and e.get("source") in singleton_sources
            and str(e.get("refresh_token") or "").strip() == dead_refresh)]
    ...
    *, client: httpx.Client, auth_store: Dict[str, Any], state: Dict[str, Any],
    ...
    try:
        ...
        if _is_terminal_nous_refresh_error(exc):
            ...
            # The chain that just failed is the one being redeemed here, by construction.
            _quarantine_nous_pool_entries(
                auth_store, exc, reason=reason, dead_refresh_token=refresh_token)
            persist()
        raise
