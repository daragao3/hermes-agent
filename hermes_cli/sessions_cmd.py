...
def _cmd_repair(args):
    ...
    if not db_path.exists():
        ...
        return
    # The DEEP, unbounded path `hermes doctor` points users at ("For the full check (no
    # time limit)"), so it runs the FTS rank=1 integrity-check doctor skips by default.
    # That check is the only one that surfaces an index rowid with no surviving messages
    # row — searches inner-join it away rather than failing — and its repair is strategy
    # #1 below. Without it this function probes exactly what the fast path does and never
    # detects the corruption class it exists to fix.
    print(
        f"Checking {db_path} (deep check — re-reads every indexed message; this can "
        "take minutes on a large database)…"
    )
