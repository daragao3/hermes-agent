...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if is_shallow:
        ...
        _print_update_check_result(_github_compare_behind(head_sha, target_sha), compare_branch, branch=branch)
        return

    rev_result = _git_run(git_cmd, ["rev-list", f"HEAD..{compare_branch}", "--count"], check=True)
    _print_update_check_result(int(rev_result.stdout.strip()), compare_branch, branch=branch)
...
def _print_update_check_tail(branch: str, compare_ref: str, *, update_available: bool = True) -> None:
    """Preserve local apply-gate advice without gating the read-only check."""
    from hermes_cli._update_divergence import check_advisory_lines

    for line in check_advisory_lines(branch, compare_ref=compare_ref, update_available=update_available):
        print(line)


def _print_update_check_result(behind: int | None, compare_branch: str, *, branch: str = "main") -> None:
...
def _pip_install_prefix(uv_bin) -> tuple[list[str], dict | None]:
    ...
    return [real_executable(), "-m", "pip"], None
