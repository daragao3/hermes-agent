...
def _audit_one(npm_bin: str, npm_dir, label: str, audit_extra: list[str], issues: list[str]) -> None:
    ...
    try:
        ...
        # run_text_capture, NOT subprocess.run: on Windows `npm` is `npm.cmd`, so the work
        # runs in a node.exe GRANDCHILD holding the capture pipes, and run()'s timeout kills
        # only the direct child before blocking on a pipe that never reaches EOF. Measured at
        # 6m44s of silent gap across the four targets, with every result swallowed.
        from hermes_cli.doctor import _npm_audit_budget, _run_npm_audit
        from hermes_cli.doctor_report import doctor_request

        audit_result = _run_npm_audit(
            [npm_bin, "audit", "--json", *audit_extra], cwd=str(npm_dir),
            timeout=_npm_audit_budget(on_demand=doctor_request("audit")))
...
def _check_npm_audit(should_fix: bool, f: Finding) -> None:
    ...
    OPT-IN (``--audit``): 40-120s per target across four targets is too slow to finish inside
    any budget a diagnostic can hold, so a default run paid six-plus minutes to print four
    "not checked" warnings. See ``_npm_audit_budget``.
    ...
    """
    from hermes_cli.doctor import PROJECT_ROOT, _audit_npm_target
    from hermes_cli.doctor_report import doctor_request

    # Printed regardless of --audit: these are the tested Termux install path's fallbacks, not a
    # property of the (opt-in, slow) dependency audit below.
    if _is_termux():
        check_info("Termux compatibility fallbacks:")
        for note in _TERMUX_INSTALL_ALL_FALLBACK_NOTES:
            check_info(note)
    if not doctor_request("audit"):
        # Said out loud: a check that silently does not run reads as a check that passed.
        check_info("npm dependency audit skipped (slow) — run: hermes doctor --audit")
        return
    ...
    if npm_bin:
        ...
        for npm_dir, label, audit_extra in (
            ...
            if ((PROJECT_ROOT if audit_extra else npm_dir) / "node_modules").exists():
                # Resolved through hermes_cli.doctor at call time so the per-target renderer
                # stays a single patchable seam; --audit is the on-demand budget.
                _audit_npm_target(npm_bin, npm_dir, label, audit_extra, f.issues, on_demand=True)
