...
def _render_platforms(ctx):
    ...
    try:  # Plugin-registered platforms
        ...
        for entry in platform_registry.plugin_entries():
            ...
            #
            # deps_available_fn, not check_fn: for adapters that defer a heavy SDK, check_fn IS
            # the loader. Feishu's check_feishu_requirements() imports lark_oapi, whose
            # top-level __init__ eagerly pulls ~10k modules — measured 238.2s and 404.9s on two
            # consecutive runs here. Printing one status line must not cost that. Mirrors
