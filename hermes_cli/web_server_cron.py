...
def _cron_profile_targets() -> List[Tuple[str, Path]]:
    """``(name, home)`` pairs for the cron dashboard's cross-profile fan-out.

    ENUMERATION ONLY -- deliberately not ``list_profiles()``. The consumers
    (:func:`_find_cron_job_profile` and :func:`_list_cron_jobs_sync`) read nothing off a
    profile but its *name*, which they use to scope a cron store; they never touch ``model``,
    ``provider``, ``skill_count`` or ``gateway_running``. Gathering that metadata to produce
    a list of names is expensive out of all proportion to what is used, and this is a *polled*
    path: the desktop sidebar refreshes ``/api/cron/jobs?profile=all`` every 30s
    (``CRON_POLL_INTERVAL_MS``), so every one of those polls was parsing every profile's
    ``config.yaml`` and probing the process table for gateway liveness. Measured 2026-09-08
    across this box's 20 profiles: the old ``list_profiles`` + ``_profile_to_dict`` form ran a
    median 76.7ms warm (231ms cold) against ~5ms for the enumeration.

    Same profiles in the same order as ``list_profiles()`` -- ``list_profiles`` is defined in
    terms of :func:`~hermes_cli.profiles.list_profile_targets`, so the two sets cannot drift.
    A broken enumerator falls back to the directory scan rather than blanking the view.
    """
    from hermes_cli.web_server_profiles import _fallback_profile_dicts
    from hermes_cli import profiles as profiles_mod
    try:
        return [(name, Path(home)) for name, home in profiles_mod.list_profile_targets()]
    except Exception:
        ...
        return [
            ...
            for row in _fallback_profile_dicts(profiles_mod)]


def _cron_profile_names() -> List[str]:
    """Profile names for the cron dashboard's cross-profile fan-out (see _cron_profile_targets)."""
    return [name for name, _home in _cron_profile_targets()]
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        for name, home in _cron_profile_targets()]
