...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # Canonical profile-plugin directory -> the exact tool names it registered, and the
        # reverse key -> path map used to refuse a second path for the same key. Set HERE and
        # not lazily: load_profile_tools reads before it writes, and a manager built by
        # another code path would raise on the first cron job entering a foreign profile.
        # Profile jobs are serialized, but the lock also protects direct callers and makes the
        # check/import/commit sequence one operation.
            ...
            for manifest in selected:
                ...
                if loaded is None:
                    self._plugins[key] = LoadedPlugin(
                        manifest=manifest,
                        # Upstream's scope-keyed namer, not the fork's key-hashed one:
                        # _directory_module_name disambiguates by the profile home that
                        # claimed the bare name, which is the collision that matters here.
                        module=sys.modules.get(
                            self._directory_module_name(manifest)
                        ),
                        tools_registered=list(names),
                        enabled=True,
                    )
