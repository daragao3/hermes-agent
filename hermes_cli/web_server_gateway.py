...
# None means RESOLVE LIVE (see _action_log_dir below). It is not a default value: binding
# a home here would freeze it at import time, and this module is imported before any
# profile scope is entered, so every profile's action logs landed in whichever home
# happened to be current then. Tests and callers may still set it to pin a directory.
_ACTION_LOG_DIR: Optional[Path] = None


def _action_log_dir() -> Path:
    """Directory holding the per-action log files, resolved at CALL time."""
...
def _spawn_hermes_action(
    ...
    log_file = open(log_dir / _ACTION_LOG_FILES[name], "ab", buffering=0)
