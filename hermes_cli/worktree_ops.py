...
def _git(args, cwd, timeout: float = 10, check: bool = False, **kwargs):
    """Run ``git *args`` in *cwd* capturing UTF-8 text, with a timeout that is really honoured.

    NOT ``subprocess.run(capture_output=True, timeout=N)``: that does not bound a command whose
    GRANDCHILD inherits the capture pipes. The timeout kills only the direct child, then
    ``communicate()`` blocks forever on an EOF the surviving grandchild never delivers. ``git
    worktree add`` over a ~10k-file checkout spawns exactly that tree, and is the call most
    likely to reach its timeout in the first place (120s here, 113s measured under load).
    ``run_text_capture`` captures into temp files and tree-kills, so N means N.

    ``check`` is accepted and ignored: every caller passes ``False``, which is already the
    default, and raising on a non-zero git exit is not this helper's contract."""
    from hermes_cli._subprocess_compat import run_text_capture

    return run_text_capture(["git", *args], timeout=timeout, cwd=cwd, **kwargs)
...
def _maintain_pack_health(repo_root: str) -> None:
    ...
    try:
        ...
        if os.name == "posix":
            cmd = ["nice", "-n", "19", *cmd]
        # 30 minutes of repacking is precisely long enough for a wedged pipe to matter.
        from hermes_cli._subprocess_compat import run_text_capture

        run_text_capture(cmd, timeout=1800, cwd=repo_root)
