...
from hermes_cli.web_server_files import _default_hermes_root_is_opt_data
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        ...
        # context= is NOT optional here. Without it the engine defaults to "local" and
        # the ENTIRE hosted-console policy — the path allowlist, the `config set` key
        # gate, the stdio-`mcp add` refusal, the `cron create --script` refusal — is
        # inert. That policy is a boundary reached over the network, and it would be
        # switched off here with all of its own tests still passing, because they build
        # the engine directly.
        engine = HermesConsoleEngine(
            output_limit=_CONSOLE_OUTPUT_LIMIT,
            context=_dashboard_console_context(),  # type: ignore[arg-type]
        )
    ...
    await out.prompt(
        type="ready", profile=profile or "current", context=engine.context,
    )
