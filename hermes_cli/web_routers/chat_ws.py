...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        ...
        engine = HermesConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT, context=context)
    ...
    _log.info("console accepted peer=%s mode=%s cred=%s context=%s profile=%s", peer, mode, cred, context, profile or "current")
    await out.prompt(type="ready", profile=profile or "current", context=context)
