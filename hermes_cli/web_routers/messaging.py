...
def _platform_enablement(
    ...
    try:
        ...
        gateway_config = gateway_config_shared if gateway_config_shared is not None else load_gateway_config()
...
def _messaging_platform_payload(
    ...
    scoped: bool = False, profile_home: Optional[Path] = None, gateway_config_shared=None,
) -> dict[str, Any]:
    ...
    enabled, configured, home_channel = _platform_enablement(
        platform_id, entry, env_on_disk, scoped, gateway_config_shared)
...
def _platform_payloads(scoped_dir: Optional[Path], entries) -> list[dict[str, Any]]:
    ...
    # Materialize plugin labels before loading config can extend Platform.
    entries = list(entries)
    shared_config = None
    if scoped_dir is None:
        try:
            from gateway.config import load_gateway_config
            shared_config = load_gateway_config()
        except Exception:
            _log.debug("Messaging config snapshot unavailable", exc_info=True)
    return [_messaging_platform_payload(entry, env_on_disk, runtime, scoped=scoped_dir is not None,
                                       profile_home=scoped_dir, gateway_config_shared=shared_config)
...
def _ensure_whatsapp_bridge_dependencies(bridge_dir: Path) -> None:
    ...
    try:
        ...
        # File-backed capture bounds npm.cmd's descendant lifetime; UTF-8
        # replacement decoding and hidden-window flags are supplied by helper.
        result = run_text_capture(
            [npm, "install", "--silent"], cwd=str(bridge_dir),
