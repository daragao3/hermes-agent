...
def _active_profile_gateway_dir() -> Optional[Path]:
    """Directory whose gateway identity files plain ``/api/status`` should read, or None to
    keep the zero-arg (root) probe.

    Issue #56986: the gateway writes ``gateway.pid`` / ``gateway_state.json`` into the
    directory it was LAUNCHED with, and ``_get_process_hermes_home()`` pins those reads
    there regardless of the config contextvar. This dashboard runs under the ROOT home while
    the real gateway usually runs under the ACTIVE profile, so an unscoped probe checks an
    empty root and reports "gateway not running" for a healthy gateway (the per-profile
    ``gateways[]`` topology already detects it). Point the headline probe at the active
    profile's identity files by EXPLICIT path instead -- the same retargeting an explicit
    ``?profile=`` gets.

    Two guards, both load-bearing:
      * ``default`` (or no ``active_profile`` file) keeps the root path, which is correct
        and is what every unscoped status test relies on;
      * a profile that does NOT own its own ``gateway.pid`` keeps the root path too, because
        in multiplex mode one root gateway serves many profiles.
    Never raises: a status endpoint degrades to the root probe rather than 500.
    """
    try:
        ...
        if not active or active.lower() == "default":
            return None
        target = root / "profiles" / active
        return target if (target / "gateway.pid").exists() else None
    except Exception:
        return None
...
async def get_status(profile: Optional[str] = None):
    ...
    if requested_profile and requested_profile.lower() != "current":
        ...
        profile_dir = _active_profile_gateway_dir()
