...
async def get_health():
    ...
    return JSONResponse(
        {"ok": True, "status": "ok", "version": __version__,
...
def _default_gateway_profile_dir() -> Optional[Path]:
    """Use the selected profile's own gateway; otherwise keep the root probe."""
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        gateway_profile_dir = profile_dir
        if gateway_profile_dir is None:
            gateway_profile_dir = await run_in_threadpool(_default_gateway_profile_dir)
        gateway = await _resolve_gateway_status(gateway_profile_dir, _GATEWAY_HEALTH_URL)
        gateway_running, gateway_state = gateway["gateway_running"], gateway["gateway_state"]
