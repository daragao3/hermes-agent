...
def _trigger_cron_job_sync(job_id: str, profile: Optional[str] = None):
    ...
    try:
        ...
        # 409, not a 200 over a silent un-pause (2026-08-26): run-now on this dashboard used to
        # clear a containment hold as a side effect. The reason travels with the refusal so the
        # operator can see WHAT they would be overriding without leaving the page; resuming is
        # an explicit, separate action.
        raise HTTPException(
            status_code=409,
            detail={"error": str(paused), "paused_reason": paused.paused_reason,
    ...
    # this run even racing across processes.
    ran = _fire_cron_job_for_profile(selected, job["id"])
