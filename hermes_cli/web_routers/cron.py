...
def _trigger_cron_job_sync(job_id: str, profile: Optional[str] = None, reason: Optional[str] = None):
    ...
    # this run even racing across processes. Never force a paused job: an
    # operator must explicitly resume it, including if paused after our read.
    ran = _fire_cron_job_for_profile(selected, job["id"], force=False)
    refreshed = _call_cron_for_profile(selected, "get_job", job["id"])
    if ran:
        # Audit only the fire this caller actually claimed, never a competing
        # scheduler's completion. Preserve the provider's atomic claim/re-arm.
        _call_cron_for_profile(
            selected, "emit_cron_triggered_safe",
            ...
            new_next_run_at=(refreshed or {}).get("next_run_at"),
        )
