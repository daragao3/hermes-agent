...
def _new_oauth_session(provider_id: str, flow: str, profile: Optional[str] = None) -> tuple[str, Dict[str, Any]]:
    ...
    sess = {
        ...
        # Captured HERE, on the request thread, while the scope is still the right one. The
        # poller reads it back through _oauth_session_home minutes later.
    ...
    try:
        ...
        with _oauth_sessions_lock:
            if _codex_cancelled(sess, session_id, " before token save"):
                return
            # hermes_home=: this runs on the poller thread minutes after the request scope exited.
            with _profile_scope(session_profile, hermes_home=_oauth_session_home(session_id)):
