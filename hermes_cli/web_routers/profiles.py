...
from hermes_cli.web_server_sessions import (
    _add_bridge_metadata_to_sessions, _open_session_db_at_path,
)
...
def get_profiles_sessions(
    # ``le`` REFUSES an over-large page with a 422 rather than clamping it: this endpoint
    # fans out across EVERY profile's state.db, and an unbounded limit would drag every
    # session row out of SQLite in one hit, multiplied per profile (#39200 / #74778, pinned
    # by tests/hermes_cli/test_dashboard_param_clamps.py). The bound is
    # MAX_SESSION_CATALOG_PAGE_SIZE so it cannot drift from the per-profile over-fetch cap
    # below. 500 (not 100) because desktop callers use limit=200 and the electron
    # remote-merge over-fetches ``limit + offset``.
    limit: int = Query(20, ge=0, le=MAX_SESSION_CATALOG_PAGE_SIZE), offset: int = Query(0, ge=0),
    min_messages: int = 0,
    ...
    targets = ([_cron_profile_home(profile)] if concrete_profile
    ...
    # A CONCRETE profile is one store, so the page is delegated straight to SQLite — correct
    # at any depth and cheaper. Over-fetching ``limit + offset`` and slicing in Python (what
    # a cross-profile page must do) silently caps a single profile's reachable catalog at
    # MAX_SESSION_CATALOG_PAGE_SIZE rows: offset=500 over a 501-row store returns nothing.
    # Cross-profile pages DO need rows from every profile to build the global window, so
    # they keep the over-fetch, bounded so a huge profile can't blow up the response.
    per_profile = (limit if concrete_profile
                   else min(max(limit + offset, limit), MAX_SESSION_CATALOG_PAGE_SIZE))
    ...
    for name, home in targets:
        def _read(db, name=name):
            rows = db.list_sessions_rich(
                limit=per_profile, offset=per_profile_offset,
    ...
    if concrete_profile:
        # SQLite already ordered and offset this page; only the pinned back-fill (rows the
        # query returned PAST its LIMIT) still needs trimming, hence offset 0 here.
        window = _pinned_window(merged, 0, limit)
    else:
...
def get_profiles_sessions_sidebar(
    ...
    # Showable-session counts. ``recents_totals`` is IN SCOPE (it drives "load more" and the
    # catalogs the desktop hydrates); ``all_recents_totals`` covers EVERY profile regardless
    # of scope, which is the only thing that can tell a scoped-but-empty profile apart from a
    # broken load. They stay separate on purpose -- see the response comment below.
    recents_totals: Dict[str, int] = {}
    all_recents_totals: Dict[str, int] = {}
    errors: List[Dict[str, str]] = []
    now = time.time()

    def _recents_total(db) -> int:
        """Showable recents for one profile: the same filters as the recents slice, minus its
        window. ``exclude_children`` mirrors listing visibility so the count and the rows agree."""
        return db.session_count(
            exclude_sources=recents_exclude_list or None, min_message_count=1,
            include_archived=False, archived_only=False, exclude_children=True)
    ...
    def _build_slices(db, cache_key):
        ...
        slices = {"recents": _slice(db, "recents"), "usage": db.usage_totals(),
                  ...
                  # Counted in SQL, not over the recents window: the window is a page, and a
                  # total that shrank when you scrolled would be worse than no total at all.
                  "recents_total": _recents_total(db)}
        # Same best-effort bridge enrichment as /api/profiles/sessions, so imported
        # Claude/Codex sessions keep their [Claude]/[Codex] provider badges on the batched
        # sidebar route. A perf refactor of this endpoint has now dropped this call twice
        # (40160e2a0, then upstream's 0.21.1 rewrite), each time silently removing every
        # badge while the per-slice endpoint stayed enriched. It sits INSIDE _build_slices
        # so the cached slices carry the metadata too.
        for _key in ("recents", "cron", "messaging"):
            _add_bridge_metadata_to_sessions(db, slices[_key])
        _sidebar_profile_cache_put(cache_key, slices)
        return slices

    for name, home in targets:
        in_scope = recents_scope == "all" or name == recents_scope
        db_path = Path(home) / "state.db"
        if not db_path.exists():
            continue
        fingerprint = _sidebar_db_fingerprint(db_path)
        if not in_scope:
            # Count only, and under its own cache key: an out-of-scope profile must not pay
            # the three-slice + usage build this endpoint skips it for. Errors are swallowed
            # (errors=None) rather than reported -- a 200 carrying errors[] is treated as a
            # failed scan and deliberately not cached, so an unrelated profile's unreadable
            # store would disable the cache AND raise a toast about a profile nobody asked for.
            count_key = (str(db_path), fingerprint, "recents_total", tuple(recents_exclude_list))
            cached_total = _sidebar_profile_cache_get(count_key)
            if cached_total is None:
                cached_total = _read_profile_db(name, home, None, _recents_total)
                if cached_total is not None:
                    _sidebar_profile_cache_put(count_key, cached_total)
            if cached_total is not None:
                all_recents_totals[name] = cached_total
            continue
        ...
        if slices is None:
            ...
            if slices is None:
                continue
        recents_totals[name] = slices["recents_total"]
        all_recents_totals[name] = slices["recents_total"]
    ...
    return {
        "recents": {"sessions": _window("recents"), "profiles_truncated": recents_truncated,
                    ...
                    # Scope echo + whether it matched anything openable. A scope naming a
                    # deleted/ghost profile otherwise answers exactly like a real, empty one.
                    "profile": recents_scope,
                    "profile_matched": recents_scope == "all" or recents_scope in recents_totals,
                    # IN-SCOPE counts. ``total`` drives "load more" for the rows actually
                    # shown, so it must NOT absorb out-of-scope profiles or the footer stays
                    # on forever; ``profile_totals`` is DELIBERATELY still scoped, because the
                    # desktop ITERATES it to pick which profile catalogs to hydrate and
                    # widening it would make a scoped sidebar re-fetch every profile.
                    "total": sum(recents_totals.values()),
                    "profile_totals": recents_totals,
                    # EVERY profile's showable count, scope or no scope. The one field with no
                    # upstream equivalent, and the only one that distinguishes "this profile
                    # really is empty, your chats are over there" from "nothing loaded".
                    # Superset of profile_totals; keep the two separate (see above).
                    "all_profile_totals": all_recents_totals},
