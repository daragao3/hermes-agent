...
from hermes_cli.web_server_sessions import _add_bridge_metadata_to_sessions
...
def _read_profile_db(name: str, home, errors: Optional[List[Dict[str, str]]],
    ...
    db = None
    try:
        db = _open_session_db_at_path(db_path, read_only=True, skip_non_session=True)
...
def get_profiles_sessions(
    ...
    per_profile = min(limit if concrete_profile else max(limit + offset, limit), 500)
    ...
    for name, home in targets:
        def _read(db, name=name):
            rows = db.list_sessions_rich(
                limit=per_profile, offset=per_profile_offset, order_by_last_active=order == "recent",
    ...
    window = _pinned_window(merged, 0 if concrete_profile else offset, limit)
    if not full:
...
def get_profiles_sessions_sidebar(
    ...
    def _build_slices(db, cache_key):
        ...
        slices = {"recents": _slice(db, "recents"), "usage": db.usage_totals(),
                  ...
                  "recents_count": _recents_count(db)}
        _add_bridge_metadata_to_sessions(db, [row for key in slice_scope for row in slices[key]])
    ...
    for name, home in targets:
        ...
        if recents_scope != "all" and name != recents_scope:
            # Report where other chats exist without loading or returning
            # those profiles' session/cron/messaging rows.
            count_key = (str(db_path), _sidebar_db_fingerprint(db_path),
                         "recents-count", tuple(recents_exclude_list))
            counted = _sidebar_profile_cache_get(count_key)
            if counted is None:
                counted = _read_profile_db(name, home, errors, lambda db: {"count": _recents_count(db)})
                if counted is not None:
                    _sidebar_profile_cache_put(count_key, counted)
            if counted is not None:
                all_profile_totals[name] = counted["count"]
            continue
        ...
        recents_profile_totals[name] = slices["recents_count"]
        all_profile_totals[name] = slices["recents_count"]
    ...
    return {
        "recents": {"sessions": _window("recents"), "profiles_truncated": recents_truncated,
                    ...
                    "total": sum(recents_profile_totals.values()),
                    "profile_totals": recents_profile_totals,
                    "all_profile_totals": all_profile_totals,
                    "profile": recents_scope,
                    "profile_matched": recents_scope == "all" or any(name == recents_scope for name, _ in targets)},
