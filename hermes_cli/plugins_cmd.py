...
def _run_plugin_git(git_exe: str, target: Path, *args: str, timeout: int = 60) -> subprocess.CompletedProcess:
    ...
    # File-backed output keeps the deadline enforceable when git helpers outlive the root.
    return run_text_capture(
        [git_exe, *args], timeout=timeout, cwd=str(target), env=noninteractive_git_env())
