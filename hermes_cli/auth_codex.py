...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    ...
    from hermes_cli.auth import clear_codex_refresh_failure_marker
    clear_codex_refresh_failure_marker()


def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Hermes auth, if available."""
    from hermes_cli.auth import _import_codex_cli_tokens, _save_codex_tokens, _codex_cli_import_is_superseded
    ...
    if _codex_cli_import_is_superseded():
        logger.warning("Refusing Codex CLI recovery from a token pair not newer than Hermes (%s).", reason)
        return None
...
def _import_codex_cli_tokens() -> Optional[Dict[str, str]]:
    ...
    from hermes_cli.auth import _codex_access_token_is_expiring, _codex_cli_auth_path
