        ...
        with self._lock:
            ...
            try:
                ...
                if isinstance(exc, AuthError) and _is_terminal_nous_refresh_error(exc):
                    # Read the refresh token BEFORE quarantining the state, which clears it:
                    # the pool-side reclaim needs to know which chain died, not merely that
                    # one did.
        ...
        try:
            with _auth_store_lock():
                ...
                if quarantine_error is not None and quarantine_reason:
                    _quarantine_nous_pool_entries(
                        store, quarantine_error, reason=quarantine_reason,
