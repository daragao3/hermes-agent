...
def run_slash(rest: str) -> str:
    ...
    # split_command_line, not shlex.split: shlex defaults to POSIX mode, where ``\`` is an
    # escape character — so ``/kanban attach t_1 C:\Users\me\upload.txt`` arrived as
    # ``C:Usersmeupload.txt`` and the attach failed with "no such file".
