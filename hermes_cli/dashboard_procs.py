...
def _iter_process_table() -> list[tuple[int, str]]:
    """Read process argv in-process; a failed walk must not look like an empty host."""
    import psutil
    rows = []
    for proc in psutil.process_iter(["pid", "cmdline"]):
        try:
            info = proc.info
            pid, argv = info.get("pid"), info.get("cmdline")
            if pid is not None and argv:
                rows.append((int(pid), subprocess.list2cmdline([str(a) for a in argv])))
        except Exception:
            continue  # Per-process churn/access denial is expected.
    return rows
...
def _kill_pids_windows(pids: list[int], killed: list[int], failed: list[tuple[int, str]]) -> None:
    ...
    from gateway.status import get_process_start_time, pid_exists
    ...
    for pid in pids:
        try:
            ...
            if expected_start_time is None:
                ...
                # A parent kill may already have taken this child down. Message
                # text alone is not proof: retain failures for live/reused PIDs
                # and let probe errors remain failures through the outer handler.
                if result.returncode == 0 or not pid_exists(pid):
                    killed.append(pid)
                else:
...
def _kill_stale_dashboard_processes(
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    ...
    if not _scan_ok(pids):
        ...
        return {**_empty_result(), "scan_ok": False, "scan_error": _scan_error(pids)}
    if not pids:
