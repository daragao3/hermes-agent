...
class _ScannedProcesses(list):
    """Scan rows, plus whether the scan itself succeeded.

    A bare ``[]`` conflates "looked, found nothing" with "could not look at all" — and
    on Windows 11, where wmic no longer exists, that conflation used to be TOTAL.

    Subclassing ``list`` keeps every existing caller and test working unchanged (``== []``,
    ``not scanned``, iteration, tuple unpacking) while giving the user-visible paths a way
    to tell the two apart. Read the flag through ``scan_ok(...)``, never off the attribute:
    tests routinely patch the scan with a plain list, which must keep behaving like a
    successful empty scan.
    """

    def __init__(self, rows=(), *, ok: bool = True, error: str | None = None) -> None:
        super().__init__(rows)
        self.ok = ok
        self.error = error


def scan_ok(scanned) -> bool:
    """True unless *scanned* is a _ScannedProcesses reporting a failed scan."""
    return bool(getattr(scanned, "ok", True))


def scan_error(scanned) -> str | None:
    """Why the scan failed, when it did."""
    return getattr(scanned, "error", None)
...
class _DashboardPids(list):
    ...
    behaving like a successful empty scan.  ``ok``/``error`` mirror
    ``scan_ok``/``scan_error`` so :func:`scan_ok` reads this and
    :class:`_ScannedProcesses` alike.
    """
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(pids)
        self.scan_ok = self.ok = scan_ok
        self.scan_error = self.error = scan_error


def _scan_ok(pids) -> bool:
    """True unless *pids* came from a process-table scan that failed."""
    return bool(getattr(pids, "scan_ok", getattr(pids, "ok", True)))


def _scan_error(pids) -> str:
    """Human-readable reason a scan failed (never empty)."""
    return (
        getattr(pids, "scan_error", None) or getattr(pids, "error", None)
        or "process table unavailable"
    )
...
def _iter_process_table() -> list[tuple[int, str]]:
    """``(pid, cmdline)`` for every process — psutil, else wmic (Windows) or ps.

    Raises on scan failure. That is load-bearing and was not true before: the Windows
    branch used to RETURN an empty list when wmic could not be run, so
    ``_scan_dashboard_processes``'s ``except`` never fired and a failed scan was
    indistinguishable from "nothing is running".
    """
    rows: list[tuple[int, str]] = []

    # psutil FIRST on every platform: it is a hard dependency and reads the process
    # table in-process, so there is no subprocess, no console spawn and no per-OS fork.
    # It is not merely faster — Microsoft REMOVED wmic from Windows 11, so the branch
    # below finds no executable at all on a current host and every scan came back empty:
    # `dashboard --status` printed a false clean, `--stop` stopped nothing, and
    # `hermes update` skipped the stale-backend reaper it exists to run.
    try:
        import psutil
    except Exception as exc:
        # psutil is a hard dependency and the only in-process walk; the subprocess
        # fallbacks below are the very wmic path Windows 11 removed, so its absence is a
        # FAILED scan, never a silently empty one.
    ...
    if psutil is not None:
        for proc in psutil.process_iter(["pid", "cmdline"]):
            try:
                ...
                # NoSuchProcess / AccessDenied on ONE process is normal churn on a busy
                # box, not a failed scan — skip it and keep walking.
                continue
            if pid is None or not cmdline:
                continue
            rows.append((int(pid), " ".join(str(part) for part in cmdline)))
        return rows
    ...
    if sys.platform == "win32":
        ...
        if result is None or result.returncode != 0 or result.stdout is None:
            # RAISE, do not return an empty table: the caller's except clause is what
            # separates "could not look" from "nothing running", and wmic is absent
            # entirely on Windows 11.
            raise OSError("process table scan failed (wmic unavailable or errored)")
        current_cmd = ""
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the Hermes
    ...
    """
    # Exclude the whole ancestor chain, not just os.getpid(): the console script runs as a
    # parent/child pair (`hermes.exe` spawning `python.exe ...hermes.exe`), so this scan
    # executes in the CHILD and the wrapper parent would otherwise match its own invocation --
    # making `--status` list itself and `--stop` kill the process running it. Mirrors
    # `hermes_cli.gateway._scan_gateway_pids` (#13242).
    try:
        from hermes_cli.gateway import _get_ancestor_pids

        self_pids = set(_get_ancestor_pids())
    except Exception:
        self_pids = {os.getpid()}
    skip = {os.getpid(), *self_pids, *(exclude_pids or ())}
    try:
        # Token-aware matcher, not the adjacent-words substrings: the live server runs
        # `python -m hermes_cli.main -p default dashboard --port 9119` (profile selector
        # BETWEEN entrypoint and subcommand) and `--status`/`--stop` must not match themselves.
        found = [(pid, cmd) for pid, cmd in _iter_process_table()
                 if pid not in skip and _looks_like_dashboard_command_line(cmd)]
    except Exception as exc:
        # The walk itself died (or never started): whatever was collected is not a
        # trustworthy answer, so report the scan as failed rather than empty.
        return _ScannedProcesses(ok=False, error=str(exc))
    ...
    return _ScannedProcesses(found)
        ...
        try:
            ...
            if expected_start_time is None:
                ...
                if result.returncode == 0:
                    ...
                    # Already gone. Each dashboard runs as a parent/child pair on Windows
                    # (hermes.exe -> python.exe ...hermes.exe) and the scan matches both, so
                    # killing the parent routinely takes the child down before its own taskkill
                    # lands. That is the outcome we wanted, not a failure to report.
                    killed.append(pid)
                else:
