...
def list_profile_targets() -> List[Tuple[str, Path]]:
    """Same ordered profiles as list_profiles, without metadata or process probes."""
...
def list_profiles() -> List[ProfileInfo]:
    """Return metadata for the profiles enumerated by list_profile_targets."""
    targets = list_profile_targets()
    alias_map = build_alias_map() if any(name != "default" for name, _ in targets) else {}
    profiles = []
    for name, home in targets:
        profiles.append(_profile_info(name, home, is_default=name == "default",
                                      alias_name=alias_map.get(normalize_profile_name(name)) if name != "default" else None))
    return profiles
