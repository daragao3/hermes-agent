...
def list_profile_targets() -> List[Tuple[str, Path]]:
    ...
    The enumeration half of :func:`list_profiles` — the same profiles, in the same order —
    with none of the per-profile metadata that function gathers. ``list_profiles`` is
    DEFINED IN TERMS OF THIS, so the two sets can never drift; that identity is pinned by
    ``test_list_profile_targets_matches_list_profiles_enumeration``.

    Use it wherever a caller only needs to know WHICH profiles exist and where they live.
    Gathering the metadata is what makes ``list_profiles`` expensive, and it is expensive out
    of proportion to its inputs: per profile it parses ``config.yaml`` with
    ``yaml.safe_load`` and probes the process table for gateway liveness. Measured across
    this box's 20 profiles, 5 reps: ``list_profiles`` 0.37-0.70s versus 0.0042-0.0058s here,
    about 100x."""
    targets: List[Tuple[str, Path]] = []
    default_home = _get_default_hermes_home()
    if default_home.is_dir():
        targets.append(("default", default_home))
    # _iter_named_profile_dirs, not a raw iterdir walk: it also skips TOMBSTONED profiles,
    # and because list_profiles is defined in terms of this helper, a second enumeration
    # here would put deleted profiles back into both.
    targets.extend((entry.name, entry) for entry in _iter_named_profile_dirs())
    return targets
...
def list_profiles() -> List[ProfileInfo]:
    ...
    Enumeration comes from :func:`list_profile_targets`; everything below it is the
    per-profile metadata gathering. If you only need the enumeration, call that instead —
    see its docstring for what this costs."""
    profiles = []
    alias_map = None  # built ONCE, lazily: the default-only case needs no wrapper scan
    for name, home in list_profile_targets():
        if name == "default":
            profiles.append(_profile_info("default", home, is_default=True))
            continue
        if alias_map is None:
            alias_map = build_alias_map()  # ONCE, not per profile (was the dominant cost)
        profiles.append(_profile_info(
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    try:
        # real_executable(), not sys.executable: on a Microsoft-Store CPython host that value
        # is an MSIX app-execution alias, and [sys.executable, "-c", ...] is the shape that
        # strands suspended orphans — one per profile seeded.
