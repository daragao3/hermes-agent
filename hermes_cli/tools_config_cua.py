...
def _run_installer(argv: list, *, timeout, capture_output: bool = True,
                   **kwargs) -> subprocess.CompletedProcess:
    """:func:`_run_text` for an INSTALLER, where the timeout has to survive a grandchild.

    A captured run goes through ``run_text_capture``, which captures into temp files rather
    than pipes. ``subprocess.run``'s ``timeout`` does not bound a command whose grandchild
    inherited the capture pipes: the kill reaps only the direct child, and the re-drain then
    waits for an EOF the survivor never sends. Installers fork grandchildren as a matter of
    course — ``uv`` and ``pip`` run a build backend (setuptools, maturin, cmake) for any sdist
    in the resolved set — so a wedged backend hangs setup indefinitely instead of for
    ``timeout`` seconds. A tree-kill alone is not enough: measured against a real wedged npm on
    Windows a nominal 30s budget still cost 75s, all of it in the pipes and their reader
    threads, so the pipes have to go.

    ``capture_output=False`` is live installer output with no redirection at all. There are no
    pipes to wedge on, ``subprocess.run`` is already bounded, and that path must keep its
    console-streaming creationflags or the output is hidden into an invisible console."""
    if not capture_output:
        return _run_text(argv, timeout=timeout, capture_output=False, **kwargs)
    from hermes_cli._subprocess_compat import run_text_capture

    # run_text_capture owns its own stdio, so the console-hiding creationflags are moot.
    kwargs.pop("creationflags", None)
    return run_text_capture(argv, timeout=timeout, **kwargs)
...
def _pip_install(args: List[str], *, timeout: int = 300, capture_output: bool = True):
    ...
    try:  # a failed uv run falls through to pip — it may have failed for a reason pip can handle
        result = uv_bin and _run_installer([uv_bin, "pip", "install", *args], timeout=timeout,
        ...
        pass
    # real_executable(), not sys.executable: on a Microsoft-Store CPython host the latter is an
    # MSIX app-execution alias, which AppX starts SUSPENDED and resumes only once the activation
    # handoff completes — so a short-lived `-m` child whose parent dies mid-handoff is never
    # resumed and survives forever having executed nothing. These three spawns are that shape.
    pip_cmd = [real_executable(), "-m", "pip"]
    try:
        # Probe for pip; bootstrap via ensurepip if missing (uv venv lacks it).
        # Output is discarded either way, so take the pipes away rather than making them safe.
        probe = _run_text(pip_cmd + ["--version"], timeout=15, capture_output=False,
        ...
        try:
            _run_text([real_executable(), "-m", "ensurepip", "--upgrade", "--default-pip"],
                      timeout=120, check=True, capture_output=False,
                      stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                      creationflags=_post_setup_no_window_flags())
    ...
    return _run_installer(pip_cmd + ["install", *args], capture_output=capture_output,
                          timeout=timeout, creationflags=install_flags)
