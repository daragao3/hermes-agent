...
def _pip_install(args: List[str], *, timeout: int = 300, capture_output: bool = True):
    ...
    try:  # a failed uv run falls through to pip — it may have failed for a reason pip can handle
        result = uv_bin and _run_install([uv_bin, "pip", "install", *args], timeout=timeout,
    ...
    try:
        ...
        probe = subprocess.run(pip_cmd + ["--version"], timeout=15,
        ...
        try:
            subprocess.run([real_executable(), "-m", "ensurepip", "--upgrade", "--default-pip"],
                      stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
    ...
    return _run_install(pip_cmd + ["install", *args], capture_output=capture_output, timeout=timeout)
    ...
    try:
        if verbose:
            ...
            try:
                ...
                raise
            returncode, out = proc.returncode, None
        else:
            # File-backed capture cannot be held open by installer grandchildren.
            from hermes_cli._subprocess_compat import run_text_capture
            result = run_text_capture(install_cmd, timeout=timeout, env=installer_env)
            returncode = result.returncode
            out = (result.stdout or "") + (result.stderr or "")
        if out:
            _record_installer_output(out, returncode)
        installed_binary = _resolved_cua_driver_cmd()
        if returncode == 0 and installed_binary:
