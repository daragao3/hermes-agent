...
def _profile_scope(profile: Optional[str], *, hermes_home: Optional[str] = None):
    ...
    with _config_profile_scope(profile, hermes_home=hermes_home) as scoped:
...
@contextmanager
def _config_profile_scope(profile: Optional[str], *, hermes_home: Optional[str] = None):
    ...
    """
    if _is_current_profile(profile):
        # ``hermes_home`` is a home captured earlier by a caller whose work OUTLIVES the
        # scope that fixed it — the device-code OAuth pollers, which finish up to ~15
        # minutes after their request, on their own thread. Resolving live at that point
        # writes credentials into whatever profile is current then. It applies only to
        # this branch: a named profile is a stronger statement of intent and still wins,
        # and a caller on the request thread passes nothing and keeps resolving live.
        if hermes_home:
            with _hermes_home_scope(Path(hermes_home)):
                yield None
            return
        yield None
        return
