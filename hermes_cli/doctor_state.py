...
def _state_db_health(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str) -> None:
    ...
    try:
        ...
        from hermes_cli.doctor import _state_db_probe_budget
        from hermes_cli.doctor_report import doctor_request
        from hermes_state_common import StateDbProbeTimeout
        ...
        # Bounded: the probe ends in PRAGMA integrity_check, a page-by-page verification of
        # the whole database — over 12 minutes on a 5.1 GB state.db, which made `hermes
        # doctor` never terminate. A diagnostic that cannot answer quickly should say so.
        try:
            ...
            # Out of budget is NOT corruption. Leaving _write_reason None keeps --fix from
            # "repairing" a database nothing was found wrong with.
            _db_mb = state_db_path.stat().st_size // (1024 * 1024)
            check_warn(f"{_DHH}/state.db health probe did not finish in budget",
                       f"({_probe_timeout}; {_db_mb} MB database)")
            check_info("This is not a corruption report — the cheap checks passed and only "
                       "the exhaustive scan was cut short.")
            check_info("For the full check (no time limit): hermes sessions repair --check-only")
            _write_reason = None
        if not doctor_request("deep"):
            # Said out loud: a check that silently does not run reads as a check that passed.
            check_info("Full-text index verification was skipped (~70s on a 5 GB state.db) — "
                       "run 'hermes doctor --deep' for it.")
        else:
            _state_db_fts_integrity(f, should_fix, state_db_path, _DHH)
...
def _state_db_fts_integrity(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str) -> None:
    """``--deep`` only: the rank=1 FTS integrity-check, on its own budget.

    The only probe that finds an index rowid whose ``messages`` row is gone. Separate budget
    from the general probe because the two differ by an order of magnitude on the same file
    (~70s CPU-bound and stable, vs >12 min I/O-bound and fragmentation-sensitive) — one shared
    deadline would spend it all on the other check and report "unknown" for this one."""
    from hermes_cli.doctor import _STATE_DB_FTS_PROBE_TIMEOUT_ENV, _state_db_fts_probe_budget
    from hermes_state import check_state_db_fts_integrity
    from hermes_state_common import StateDbProbeTimeout

    try:
        reason = check_state_db_fts_integrity(
            state_db_path, timeout_seconds=_state_db_fts_probe_budget())
    except StateDbProbeTimeout as timeout:
        # Same rule as the general probe: out of budget is "unknown", never "corrupt", and never
        # a trigger for --fix. It is equally not "healthy" -- no green line for a check that
        # never finished.
        check_warn(f"{_DHH}/state.db full-text index check did not finish in budget", f"({timeout})")
        check_info("This is not a corruption report. Raise the budget with "
                   f"{_STATE_DB_FTS_PROBE_TIMEOUT_ENV}, or run 'hermes sessions repair --check-only' "
                   "(no time limit).")
        return
    if reason is None:
        return check_ok(f"{_DHH}/state.db full-text index verifies against its content")
    check_warn(f"{_DHH}/state.db full-text index does not match its content", f"({reason})")
    if not should_fix:
        return f.issues.append(
            "state.db full-text index is out of sync with messages — run 'hermes doctor --deep --fix' "
            "to rebuild it (searches silently omit the affected messages until then)")
    from hermes_state import SessionDB

    _fts_db = SessionDB(db_path=state_db_path)
    try:
        rebuilt = _fts_db.rebuild_fts()
    finally:
        _fts_db.close()
    check_ok("Rebuilt the state.db full-text index", f"({rebuilt} index(es) rebuilt)")
    f.fixed += 1
