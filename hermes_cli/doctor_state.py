...
def _state_db_health(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str, *, deep: bool = False) -> None:
    ...
    try:
        ...
        from hermes_state import StateDbProbeTimeout
        try:
        ...
        if not deep:
            ...
            from hermes_state_local_fts import check_state_db_fts_integrity
...
@doctor_check()
def _check_state_db(should_fix: bool, f: Finding, *, deep: bool = False) -> None:
    """state.db session count, FTS write health, schema repair, stats snapshot, WAL size."""
    from hermes_cli.doctor import HERMES_HOME, _DHH
    state_db_path = HERMES_HOME / "state.db"
    if state_db_path.exists():
        _state_db_health(f, should_fix, state_db_path, _DHH, deep=deep)
