...
from hermes_cli.gateway_diag import (
    SPAWN_SITE_ENV as GATEWAY_SPAWN_SITE_ENV, launch_identity as _launch_identity,
    process_start_age_s as _process_start_age_s, register_exit_hook as _register_exit_hook,
...
def _iter_windows_list_processes(listing: str):
    """Yield Windows WMIC/CIM records, accepting equals and historical colon formats."""
    current_cmd = ""
    for line in listing.splitlines():
        key, separator, value = line.strip().partition("=")
        if key.strip() not in {"CommandLine", "ProcessId"}:
            key, separator, value = line.strip().partition(":")
        if not separator:
            continue
        if key.strip() == "CommandLine":
            current_cmd = value.strip()
        elif key.strip() == "ProcessId":
            with contextlib.suppress(ValueError):
                yield int(value.strip()), current_cmd
            current_cmd = ""
...
def cleanup_gateway_state_files() -> list[str]:
    """Remove only records with a confirmed dead owner; retain live or ambiguous state."""
    from gateway.status import _get_gateway_lock_path, _get_lock_dir, _get_pid_path, _pid_exists, _read_json_file

    pid_path = _get_pid_path()
    paths = [pid_path, _get_gateway_lock_path(pid_path)]
    with contextlib.suppress(OSError):
        paths.extend(_get_lock_dir().glob("*.lock"))
    removed: list[str] = []
    for path in dict.fromkeys(paths):
        try:
            record = _read_json_file(path, bare_pid_ok=path == pid_path, unreadable_locked=True)
            ...
            if isinstance(owner, bool):
                continue
            owner = int(owner)
            if owner <= 0 or _pid_exists(owner):
                continue
            # Re-read after the liveness probe; a replacement owner must keep its claim.
            if _read_json_file(path, bare_pid_ok=path == pid_path, unreadable_locked=True) != record or _pid_exists(owner):
                continue
            path.unlink()
...
def launch_gateway_detached(extra_args: list[str] | None = None) -> int | None:
    ...
    cmd = [arg for arg in cmd if arg != "--replace"]
    if extra_args:
        cmd += list(extra_args)
...
def _make_exit_diag():
    """Compatibility factory for the shared, profile-aware lifecycle recorder."""
    return _gateway_exit_diag
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False,
                replace_reason: str | None = None, ignore_restart_claim: bool = False):
    ...
    _exit_diag(
        ...
        **_launch_identity(), proc_age_s=_process_start_age_s(),
    )
    ...
    if replace:
        ...
        _replace_claim = guard_and_open(
            ...
            incumbent_pids=_safe_find_gateway_pids(), ignore_blocking=ignore_restart_claim,
        )
    ...
    try:
        if _replace_claim is not None:
            _replace_claim.close(outcome="HANDOFF", new_pids=[os.getpid()],
                                 note="Foreground --replace hands off to this gateway; boot health remains to be confirmed.")
...
def _gateway_restart_subcommand(args):
    ...
    if kind is not None:
        swallow = (RuntimeError, OSError, subprocess.TimeoutExpired) if kind == "windows" else ()
        try:
            _service_call(kind, "restart", system)
            return
        except (subprocess.CalledProcessError, *swallow):
            if kind == "windows" and find_gateway_pids():
                print("Gateway process is still running; preserving it after the restart error.")
                return
...
def _cmd_restart(args):
    ...
    try:
        ...
        claim.close(outcome=outcome, new_pids=_safe_find_gateway_pids())
