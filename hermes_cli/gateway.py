...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if not respawn_cwd:
        # A respawn with no cwd runs whatever tree the watcher happened to inherit: `-m` puts the
        # spawning process's cwd on the child's sys.path[0], AHEAD of the editable-install finder,
        # so a stray checkout there would shadow the installed one. Resolved BEFORE the watcher
        # source is formatted below -- the literal is embedded, not read at respawn time.
        try:
    ...
    # real_executable(), not sys.executable: on a Microsoft-Store host the latter is an MSIX
    # alias that AppX starts SUSPENDED, and nothing resumes it if the parent dies mid-handoff.
    watcher_argv = [real_executable(), "-c", watcher, str(old_pid), *run_argv]
    ...
    try:
        with open(log_dir / "gateway.log", "ab") as out:
            subprocess.Popen(
                ...
                # Stamp the site: a detached gateway outlives the parent that started it,
                # so this is the only place the reason can be recorded.
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False,
                replace_reason: Optional[str] = None, ignore_restart_claim: bool = False):
    ...
    ``replace_reason`` is recorded on the restart claim this bounce opens;
    ``ignore_restart_claim`` proceeds past another session's open claim, which is only ever
    right for a claim whose holder died mid-restart."""
    ...
    # Read at entry: the gateway.start record below reuses them, and it must be written FIRST --
    # the losing racer of a double spawn trips _guard_existing_gateway_process_conflict() and
    # sys.exit(1)s long before the old write point (after the ~23s gateway.run import), so it
    # left no trace in any log. Write first, then do the expensive work.
    ...
    # ONE writer for gateway-exit-diag.log: gateway_windows.py already records its
    # gateway.force_kill through this module, and two writers for one file is how the two
    # halves of a kill drift apart. write_diag also pins its log dir at capture time, so an
    # atexit record cannot land in the live home after a test restored HERMES_HOME.
    _exit_diag = _gateway_exit_diag
    _exit_diag(
        "gateway.start", replace=replace, argv=list(sys.argv), stdin_is_tty=stdin_is_tty,
        ...
        proc_age_s=_process_start_age_s(), **_launch_identity(),
    )
    ...
    if replace:
        ...
        _replace_claim = guard_and_open(
            reason=replace_reason, surface="cli:run --replace",
    ...
    # ASCII: the caduceus and the box-drawing characters are not encodable in cp1252, so a
    # stock Windows console raised UnicodeEncodeError here — the gateway died at its own
    # startup banner, before anything it would have logged.
    print("+---------------------------------------------------------+")
    ...
    if _replace_claim is not None:
        # Closed HERE, not at exit: from this point start_gateway(replace=True) owns the
        # incumbent's death, so the claim has done its job. Leaving it open would hold it for
        # the entire life of the gateway.
        _replace_claim.close(
            outcome="HANDOFF", new_pids=[os.getpid()],
            note=(
    ...
    if entry is not None:
        ...
        try:
            if entry.is_connected is not None:
                ...
                # deps_available_fn when the plugin supplies one: WhatsApp's check_fn runs a
                # real probe, which is the wrong shape (and cost) for a status line.
                configured = bool((entry.deps_available_fn or entry.check_fn)())
        except Exception:
            configured = False
...
def _builtin_setup_fn(key: str):
    """Resolve a built-in platform's setup function.

    Nothing here is late-bound any more: every platform that used to come from
    ``hermes_cli.setup`` now registers its own setup_fn from its plugin, so the import that
    dodged that cycle has no remaining reference and is simply dead.
    """
    return {
...
def _cmd_restart(args):
    """``hermes gateway restart``, wrapped in a restart claim.

    Two restarts racing leaves either no gateway or two, and neither is visible from inside
    one of them. The claim records who is restarting, why, and how it ended; the outcome is
    written in a finally so a SystemExit or a crash still closes it."""
    from hermes_cli.gateway_restart_claim import guard_and_open

    _claim = guard_and_open(
        reason=getattr(args, "reason", None), surface="cli:restart",
        incumbent_pids=_safe_find_gateway_pids(),
        ignore_blocking=bool(getattr(args, "ignore_restart_claim", False)),
    )
    outcome = "FAILED"
    try:
        _cmd_restart_inner(args)
        outcome = "DONE"
    except SystemExit as exc:
        outcome = "DONE" if not exc.code else f"EXIT {exc.code}"
        raise
    finally:
        _claim.close(outcome=outcome, new_pids=_safe_find_gateway_pids())


def _cmd_restart_inner(args):
    ...
    if kind is not None:
        # SubprocessError, not CalledProcessError, on Windows: subprocess.TimeoutExpired is a
        # SIBLING of CalledProcessError under SubprocessError, not a subclass, so a taskkill
        # that blew its budget escaped this recovery arm entirely and reached the user as a
        # traceback with the gateway down (2026-08-11).
        swallow = (subprocess.SubprocessError, RuntimeError, OSError) if kind == "windows" else ()
        try:
            ...
            if kind == "windows":
                ...
                # readiness, so a slow boot makes it raise while a perfectly healthy gateway
                # is still coming up. The generic path below assumes nothing is running: it
                # would stop that gateway, delete its lock/pid, and launch a third one
                # (observed as pairs of detached gateways fighting over :8642). Only fall
                # through when the restart really did leave us with nothing.
                surviving = list(find_gateway_pids())
                if surviving:
                    print("\u2713 Gateway is running (PID: "
    ...
    # Windows defaults to detached: the old path tied the new gateway to the terminal that
    # restarted it, so closing that shell took the gateway with it.
...
def _safe_find_gateway_pids() -> list:
    """``find_gateway_pids()`` that never raises — restart-claim bookkeeping must not be able
    to break a restart."""
    try:
        return [int(p) for p in find_gateway_pids()]
    except Exception:  # noqa: BLE001 - bookkeeping, not control flow
        return []
