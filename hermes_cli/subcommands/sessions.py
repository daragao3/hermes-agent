...
def build_sessions_parser(subparsers, *, cmd_sessions: Callable) -> None:
    ...
    sessions_repair = sessions_subparsers.add_parser(
        ...
        description="Recover a state.db whose schema is malformed (e.g. 'table "
            ...
            "preserved and the FTS search index is rebuilt if needed. This is the deep, "
            "unbounded check: unlike `hermes doctor` it also verifies the full-text index "
            "against every message it claims to index, which takes minutes on a multi-GB "
            "database.")
