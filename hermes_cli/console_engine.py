...
# A console command is available in one or both SURFACES: the local `hermes console` REPL and
# the hosted dashboard web-console. They are not the same trust boundary — the hosted one is
# reached over the network — so availability is per-command rather than global.
...
class ConsoleCommand:
    ...
    mutating: bool = False
    confirmation: str = ""
    # Registration defaults to local-only; _mark_hosted widens the allowed paths once,
    # after everything is registered. A NEW command is therefore hosted-DENIED until
    # someone lists it deliberately — a missing command is a support ticket, an
    # accidentally exposed one is an incident.
...
class HermesConsoleEngine:
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            # Defense in depth: a handler called sys.exit() directly, or built a parser
            # that is not our _ArgumentParser. SystemExit is a BaseException, so without
            # this it escapes execute(), slips past the dashboard worker's
            # `except Exception`, and tears down the uvicorn loop.
            code = exc.code
            if code in (0, None):
    ...
    def _register_defaults(self) -> None:
        ...
        # LAST: everything above registers local-only, and this widens exactly the paths
        # the hosted console is allowed to reach. It raises on a path it cannot find, so
        # the table cannot rot into names that no longer exist.
        self._mark_hosted(EXPECTED_HOSTED_PATHS)

    def register(
        self, path: Iterable[str], usage: str, summary: str,
        handler: Callable[["HermesConsoleEngine", list[str]], str], *,
        mutating: bool = False, confirmation: str = "",
        contexts: Iterable[ConsoleContext] = LOCAL_CONTEXTS) -> None:
        key = tuple(path)
        self.commands[key] = ConsoleCommand(
            key, usage, summary, handler, mutating, confirmation, frozenset(contexts))
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        # Filtered too: never suggest a command this context would refuse.
        available = [
            " ".join(path)
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> None:
    ...
    if not db_path.exists():
        ...
        return
    # Deep check, matching `hermes sessions repair` — this is the unbounded path, so it
    # runs the FTS rank=1 integrity-check that `hermes doctor` skips by default. Minutes
    # on a multi-GB database, hence the warning before it starts.
    print(
...
def _cron_pause(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    return _cron_job_action(
        ...
        lambda ref: pause_job(
            ref, reason="paused from hermes console", caller="hermes_console:cron_pause"))
...
# --- hosted `cron run`: a bounded background pool ------------------------------------------
...
_BUILTIN_COMMANDS = (
    ...
    (("cron", "run"), "cron run <job>",
     "Run a job now (the hosted web-console runs it on a background pool).", _cron_run,
