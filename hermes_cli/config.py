...
from typing import Callable, Iterator, Dict, Any, Optional, List, Tuple, Set
...
from hermes_cli.config_defaults import DEFAULT_CONFIG as _STATIC_DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401

# Export a process-start snapshot for legacy callers while keeping
# config_defaults path-neutral. load_config() recomputes this profile-derived
# leaf through effective_default_config() on every routed profile load.
DEFAULT_CONFIG = copy.deepcopy(_STATIC_DEFAULT_CONFIG)
DEFAULT_CONFIG["session_bridge"]["sidebar"]["inbox_cwd"] = (
...
def atomic_config_write(config_path: Path, data: Any, **kwargs: Any) -> None:
    ...
    Held under the CROSS-PROCESS file lock as well as the in-process one: config.yaml is
    shared by the gateway, the CLI and every profile subprocess, and an in-process lock
...
# Fork-only config surface with live callers: mutate_config / mutate_raw_config* (auth,
# auth_commands, session_bridge/cli), effective_default_config (doctor, setup),
# ConfigPersistenceRejected (session_bridge/cli), the unsupported-install warning pair
# (hermes_cli/main), and the cross-process config file lock they persist under.
