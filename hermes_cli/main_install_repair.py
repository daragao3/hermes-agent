...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        ...
        manual = (
            ...
            f'    cd /d "{PROJECT_ROOT}"',
            f'    "{real_executable()}" -m pip install -e ".[all]"',
        ) if self_locked else (
            "  Recover manually with:",
            f"    cd {PROJECT_ROOT}",
            f"    {real_executable()} -m ensurepip --upgrade",
            f"    {real_executable()} -m pip install -e '.[all]'",
        )
        for line in manual:
            print(line)
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    # real_executable(), not sys.executable: the interpreter is reached through the
    # Windows Store app-execution alias here, and that stub is not a usable -m target.
