...
try:
    ...
    pass  # Diagnostics must never block startup.
...
from hermes_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes as _respawn_dashboard_processes,
)
...
from hermes_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError as ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
    _clear_lazy_refresh_incomplete_marker as _clear_lazy_refresh_incomplete_marker,
    _clear_marker_file as _clear_marker_file,
    _clear_update_incomplete_marker as _clear_update_incomplete_marker,
    _install_python_dependencies_with_optional_fallback as _install_python_dependencies_with_optional_fallback,
    _is_termux_env as _is_termux_env,
    _is_windows as _is_windows,
    _is_windows_npm_path as _is_windows_npm_path,
    _lazy_refresh_marker_path as _lazy_refresh_marker_path,
    _pytest_owns_live_checkout as _pytest_owns_live_checkout,
    _reexec_dependency_sync_off_windows_shim as _reexec_dependency_sync_off_windows_shim,
    _repair_venv_via_import_probes as _repair_venv_via_import_probes,
    _resolve_install_target_python as _resolve_install_target_python,
    _resolve_node_runtime_npm as _resolve_node_runtime_npm,
    _resolve_update_branch,
    _run_install_with_heartbeat as _run_install_with_heartbeat,
    _run_package_only_install as _run_package_only_install,
    _update_marker_path as _update_marker_path,
    _venv_scripts_dir as _venv_scripts_dir,
    _verify_console_scripts_installed as _verify_console_scripts_installed,
    _verify_core_dependencies_installed as _verify_core_dependencies_installed,
)
...
from hermes_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed as _desktop_build_needed,
    _desktop_dist_exists as _desktop_dist_exists,
    _desktop_macos_relaunchable_fixup as _desktop_macos_relaunchable_fixup,
    _desktop_packaged_executable as _desktop_packaged_executable,
)
...
from hermes_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui as _build_web_ui,
    _nixos_build_env as _nixos_build_env,
    _record_bytecode_fingerprint as _record_bytecode_fingerprint,
    _run_npm_install_deterministic as _run_npm_install_deterministic,
)
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        ...
        sys.exit(1 if not _scan_ok(remaining) or remaining else 0)
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    # Refuse an occupied fixed port before expensive runtime imports/discovery.
    preflight_host = args.host or "127.0.0.1"
    if args.port and _dashboard_listening(preflight_host, args.port):
        logger.error("Dashboard startup aborted - %s:%s is already in use.", preflight_host, args.port)
        print(f"x Refusing to start: {preflight_host}:{args.port} is already in use.", file=sys.stderr)
        print("  Use the running server, choose --port, or run hermes dashboard --stop.", file=sys.stderr)
        raise SystemExit(1)
