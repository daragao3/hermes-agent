...
# Pre-argparse ``send`` fast path. Deliberately HERE and not earlier: everything above —
# ``_apply_profile_override()``, the early config read, ``setup_logging()`` and the
# force_ipv4 toggle — has run, so a fast-path send inherits byte-identical bootstrap
# semantics from the slow path: same profile, same env, same redaction policy, same
# agent.log, same IPv4 preference.
#
# An earlier revision fired this immediately after ``_apply_profile_override()`` to also
# skip setup_logging's 154 modules. That was wrong twice over: it dropped ``send``'s
# agent.log records and — less obviously — it skipped ``network.force_ipv4``, so a box that
# needs forced IPv4 would have had its sends fail against platform REST endpoints. Do not
# move this call back up.
#
# What it skips is the genuinely send-irrelevant part below: ``model_setup_flows`` and the
# ~60 ``build_*_parser`` subcommand modules (now imported inside ``_build_cli_parser``) with
...
from hermes_cli.main_dashboard import (
    _DashboardPids,  # noqa: F401 -- dashboard-scan surface (tests + plugins)
    _finalize_update_output,
    _find_stale_dashboard_pids,
    _looks_like_dashboard_command_line,  # noqa: F401
    _scan_error,
    _scan_ok,
...
from hermes_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,  # noqa: F401  (frozen re-export; see the block comment)
)
...
from hermes_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,  # noqa: F401  (frozen re-export; see the block comment)
    _UPDATE_REEXEC_ENV,
    _clear_lazy_refresh_incomplete_marker,  # noqa: F401  (frozen re-export; see the block comment)
    _clear_marker_file,  # noqa: F401  (frozen re-export; see the block comment)
    _clear_update_incomplete_marker,  # noqa: F401  (frozen re-export; see the block comment)
    _install_python_dependencies_with_optional_fallback,  # noqa: F401  (frozen re-export; see the block comment)
    _is_termux_env,  # noqa: F401  (frozen re-export; see the block comment)
    _is_windows,  # noqa: F401  (frozen re-export; see the block comment)
    _is_windows_npm_path,  # noqa: F401  (frozen re-export; see the block comment)
    _lazy_refresh_marker_path,  # noqa: F401  (frozen re-export; see the block comment)
    _pytest_owns_live_checkout,  # noqa: F401  (frozen re-export; see the block comment)
    _reexec_dependency_sync_off_windows_shim,  # noqa: F401  (frozen re-export; see the block comment)
    _repair_venv_via_import_probes,  # noqa: F401  (frozen re-export; see the block comment)
    _resolve_install_target_python,  # noqa: F401  (frozen re-export; see the block comment)
    _resolve_node_runtime_npm,  # noqa: F401  (frozen re-export; see the block comment)
    _resolve_update_branch,
    _run_install_with_heartbeat,  # noqa: F401  (frozen re-export; see the block comment)
    _run_package_only_install,  # noqa: F401  (frozen re-export; see the block comment)
    _update_marker_path,  # noqa: F401  (frozen re-export; see the block comment)
    _venv_scripts_dir,  # noqa: F401  (frozen re-export; see the block comment)
    _verify_console_scripts_installed,  # noqa: F401  (frozen re-export; see the block comment)
    _verify_core_dependencies_installed,  # noqa: F401  (frozen re-export; see the block comment)
)
...
from hermes_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed,  # noqa: F401  (frozen re-export; see the block comment)
    _desktop_dist_exists,  # noqa: F401  (frozen re-export; see the block comment)
    _desktop_macos_relaunchable_fixup,  # noqa: F401  (frozen re-export; see the block comment)
    _desktop_packaged_executable,  # noqa: F401  (frozen re-export; see the block comment)
)
...
from hermes_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,  # noqa: F401  (frozen re-export; see the block comment)
    _nixos_build_env,  # noqa: F401  (frozen re-export; see the block comment)
    _record_bytecode_fingerprint,  # noqa: F401  (frozen re-export; see the block comment)
    _run_npm_install_deterministic,  # noqa: F401  (frozen re-export; see the block comment)
)
...
def _clear_bytecode_cache(root: Path) -> int:
    ...
    for dirpath, dirnames, _ in os.walk(root):
        ...
        # ``.claude/worktrees/<name>``, each a FULL checkout of this repo. The old list
        # named ``.worktrees``, which does not exist there — so this walk descended into
        # every sibling worktree and rmtree'd their ``__pycache__``. Measured 2026-08-17:
        # 27 worktrees, 737 cache dirs, i.e. one ``hermes update`` wiped the bytecode of
        # every other live session on the box and forced each into a full recompile.
        #
        # The structural check is the real guard, since a name list can never be
        # exhaustive: never descend into a directory carrying a ``.git`` entry. A nested
        # clone has a ``.git`` DIRECTORY and a git worktree has a ``.git`` FILE, so one
        # ``exists`` covers both — and bytecode inside either belongs to a different
        # checkout, not ours.
        dirnames[:] = [
            d
            for d in dirnames
...
def cmd_dashboard(args):
    ...
    # Fail fast before imports, skill sync, plugin discovery, MCP discovery and
    # web builds. Uvicorn binds last and its non-propagating logger otherwise
    # leaves no Hermes log explaining a redundant launch. Port 0 asks the OS to
    # assign a free port, so it deliberately skips the probe.
    from hermes_cli.main_dashboard import _dashboard_listening
...
def _try_fast_serve_launch() -> bool:
    ...
    # Imported here rather than at module scope: each is used exactly once, to build
    # its subparser, so importing them up top charged every invocation for every
    # subcommand's dependencies before argparse knew which one was being run. That is
    # what put ``hermes send --help`` at 444 modules, and it is what makes the
    # pre-argparse send fast path above worth anything. Plain local imports rather
    # than a helper splicing into globals(), so ruff's F821 can still see them.
...
def _build_cli_parser():
    ...
    from hermes_cli.subcommands.whatsapp import build_whatsapp_cloud_parser, build_whatsapp_parser
    ...
    # =====================================================================
    # overrides — see and revoke active model reroute overrides
    # =====================================================================
    ...
    overrides_parser = subparsers.add_parser(
        ...
        description=(
            "See and revoke active model reroute overrides (Phase 2 rate-limit handling). An "
            "override diverts calls to (provider, model) onto a replacement for a bounded "
            "window; this is the safety valve to see and clear one without needing the "
            "Telegram flow that set it."
        ),
    )
    ...
    overrides_subparsers.add_parser(
        "list",
        aliases=["ls"],
        help="Show active overrides: what is avoided, what it routes to, expiry, who set it "
