...
DOCTOR_CHECKS = (
    ...
    ('Python Environment', _check_python_environment), ('Install Integrity', _check_install_integrity), ('SSL / CA Certificates', _check_certificates),
...
def run_doctor(args):
    ...
    total = Finding()
    options = {
        _check_state_db: {"deep": bool(getattr(args, "deep", False))},
        _check_npm_audit: {"on_demand": bool(getattr(args, "audit", False))},
    }
    for title, check in DOCTOR_CHECKS:
        if title:
            _section(title)
        total.merge(check(should_fix, **options.get(check, {})))
