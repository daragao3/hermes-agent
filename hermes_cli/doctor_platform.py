...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    if venv_bin is None:
        # No `source venv/bin/activate` here: this branch is reached only when NEITHER
        # venv/bin/hermes nor .venv/bin/hermes exists, so the activate script it used to
        # tell you to source was, by construction, not there either. The detected command
        # (pip or uv, whichever will actually run on this box) names its interpreter instead.
        from hermes_cli.doctor import _editable_install_cmd
        _reinstall = _editable_install_cmd("-e '.[all]'")
        check_warn("Venv entry point not found", f"(hermes not in venv/bin/ or .venv/bin/ — reinstall with {_reinstall})")
        return f.manual_issues.append(f"Reinstall entry point: cd {PROJECT_ROOT} && {_reinstall}")
