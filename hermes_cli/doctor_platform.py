...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    if venv_bin is None:
        reinstall = _editable_install_cmd("-e '.[all]'")
        check_warn("Venv entry point not found", f"(reinstall with {reinstall})")
        return f.manual_issues.append(f"Reinstall entry point: cd {PROJECT_ROOT} && {reinstall}")
...
@doctor_check("Install integrity check failed", "({e})")
def _check_install_integrity(should_fix: bool, f: Finding) -> None:
    ...
    for status, text, detail in rows:
        {"ok": check_ok, "warn": check_warn}.get(status, check_fail)(text, detail)
    if remediation:
        f.manual_issues.append(remediation)
