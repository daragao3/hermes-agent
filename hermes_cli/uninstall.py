    ...
    for is_system, scope in ((False, "user"), (True, "system")):
        ...
        try:
            ...
            for verb in ("stop", "disable"):
                # DEVNULL, not capture_output: systemctl can leave a grandchild holding the
                # capture pipe, and only the returncode is read here.
                subprocess.run(cmd + [verb, svc_name], check=False,
                               stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                               stdin=subprocess.DEVNULL)
            unit_path.unlink()
            subprocess.run(cmd + ["daemon-reload"], check=False,
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                           stdin=subprocess.DEVNULL)
