...
_GIT_TIMEOUT = 30
# Forking git ops get a checkout-sized budget; see _git_forking.
...
def _git_forking(cwd: str, args: list[str], *, timeout: int = _GIT_FORKING_TIMEOUT) -> tuple[int, str, str]:
    ...
    ``_run``'s capture PIPES cannot bound such a command: the timeout kills only the direct
    child, and ``communicate()`` then waits forever for an EOF the surviving grandchild never
    sends. ``run_text_capture`` captures into temp files and tree-kills instead, so the budget
    is real — and the budget is checkout-sized, because these are the only calls here that
    legitimately take minutes.

    Deliberately NOT a conversion of :func:`_git`: the temp-file capture costs an extra process
    spawn per call on Windows, and the many leaf ``status`` / ``log`` / ``rev-parse`` queries
    this module makes fork nothing and would pay it for nothing. Hardening is unchanged —
    same ``harden_git_argv`` and ``noninteractive_git_env`` as ``_run``."""
    from hermes_cli._subprocess_compat import run_text_capture

    try:
        proc = run_text_capture(["git", *harden_git_argv(args)], cwd=cwd, timeout=timeout,
...
def _resolve_push_remote(cwd: str, branch: str) -> str | None:
    """The remote a push of ``branch`` should target, or ``None`` when it is ambiguous.

    git's own precedence first, then the branch's own remote, then the TRUNK branches' — the
    config that names this repo's own lineage, which is what makes an ordinary fork checkout
    resolve without the user having configured every branch.

    Every candidate is validated against the real remote list. git will happily store a URL in
    ``branch.<name>.remote``, and passing that through as if it were a remote NAME is how a
    "resolved" remote becomes an unintended push target.

    Deliberately does NOT fall back to ``origin``. In a fork checkout ``origin`` is often the
    UPSTREAM project (``~/.hermes/agent-src`` is exactly that), so guessing it publishes private
    work to a public repo and pins the branch's tracking there. A lone remote is unambiguous;
    anything else is the caller's decision, not ours."""
    ...
    for key in keys:
        configured = _git_line(cwd, ["config", "--get", key])
...
def _branch_base(cwd: str) -> str | None:
    ...
    ``origin/HEAD`` first (upstream's probe, and the right answer in a plain clone), then the
    refs this checkout's OWN configuration names — a fork's trunk usually tracks a remote that
    is not called origin, and diffing against the wrong lineage silently produces the wrong
    "all branch changes". The origin/* literals stay last, so a plain clone is unaffected."""
    head = _origin_head_abbrev(cwd)
    candidates = [head] if head else []
    for trunk in _TRUNK_BRANCHES:
        upstream = _git_line(cwd, ["rev-parse", "--abbrev-ref", "--symbolic-full-name",
                                   f"{trunk}@{{upstream}}"])
        if upstream:
            candidates.append(upstream)
    branch = _git_line(cwd, ["rev-parse", "--abbrev-ref", "HEAD"])
    remote = _resolve_push_remote(cwd, branch) if branch else None
    if remote:
...
def _native_path(path: str) -> str:
    """Re-spell a git-printed path in the platform's own separators.

    ``git worktree list --porcelain`` prints forward slashes even on Windows (``C:/Users/…``),
    while every other path this API emits — the ``/api/fs`` targets, the ``os.path.join``-built
    worktree directories — is native-separator, and callers compare the two by string equality.
    No-op on POSIX, where a backslash is a filename character rather than a separator and
    ``normpath`` leaves it alone."""
...
def worktree_list(cwd: str) -> list[dict]:
    ...
    for line in _git_out(cwd, ["worktree", "list", "--porcelain"]).split("\n"):
        if line.startswith("worktree "):
            trees.append({"path": _native_path(line[9:]), "branch": None, "isMain": not trees,
...
def _worktree_for_existing(root: str, raw_name: str) -> dict:
    ...
    if remote:
        ...
        _git_forking(root, ["fetch", remote, existing])
        _git_forking_ok(root, ["worktree", "add", "--track", "-b", existing, target, requested])
    else:
...
def worktree_add(cwd: str, options: dict) -> dict:
    ...
    if options.get("base"):
        ...
        if base.startswith("origin/"):
            _git_forking(root, ["fetch", "origin", base[len("origin/"):]])
