...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    # DevFlow decision surface. busy_policy=dispatch deliberately: a DDP decision is
    # independent of the running agent and must stay reachable while it is busy. The handlers
    # enforce their own explicit-admin, fail-closed gate before touching the ledger; they never
    # unblock a tool or grant execution authority, so this is not an approval bypass.
    CommandDef("ddp-approve", "Stage a human approval for one TRIAGED DevFlow request", "Session",
               gateway_only=True, busy_policy="dispatch", aliases=("ddp_approve",),
               args_hint="<request-id> <evidence>"),
    CommandDef("ddp-decline", "Stage a human decline for one TRIAGED DevFlow request", "Session",
               gateway_only=True, busy_policy="dispatch", aliases=("ddp_decline",),
               args_hint="<request-id> <evidence>"),
    CommandDef("ddp-approve-confirm", "Confirm one staged DevFlow approval", "Session",
               gateway_only=True, busy_policy="dispatch", aliases=("ddp_approve_confirm",),
               args_hint="<confirmation-token>"),
    CommandDef("ddp-decline-confirm", "Confirm one staged DevFlow decline", "Session",
               gateway_only=True, busy_policy="dispatch", aliases=("ddp_decline_confirm",),
               args_hint="<confirmation-token>"),
    CommandDef("devflow-login", "Get a one-time code to sign in to DevFlow Mission Control (:3040)",
               "Session", gateway_only=True, busy_policy="dispatch", aliases=("devflow_login",)),
