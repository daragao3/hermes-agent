...
# A freshly spawned Windows gateway is not discoverable for a while -- measured 2026-08-17,
# spawn 10:18:06 -> :8642 listening 10:18:34, i.e. ~28s. Every find_gateway_pids() re-check
# inside that window truthfully answers "nothing running" and spawns again; on that day one
# parent produced THREE gateways in 1.4s (pids 24760/44860/35144, all ppid 48796),
# repeatedly. A liveness poll can never be an idempotence guard for something that takes 28s
# to appear -- remembering our OWN spawn is what closes it. The post-spawn readiness poll
# below narrows the race but does not close it: its budget is seconds against a 28s boot, and
# when it times out it raises, leaving nothing to stop the next attempt.
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    with _abort_on_error("Could not re-check Desktop gateway-lifecycle ownership before cold-start"):
        if _desktop_owns_gateway_lifecycle():
            ...
            return True
    # Second guard: WE may have spawned one moments ago, while it is still invisible to
    # find_gateway_pids(). See _COLD_START_SPAWNED_AT.
    global _COLD_START_SPAWNED_AT
    now = _time.monotonic()
    if (_COLD_START_SPAWNED_AT is not None
            and (now - _COLD_START_SPAWNED_AT) < _COLD_START_SUPPRESS_S):
        logger.debug("Skipping cold-start: spawned %.1fs ago, still inside the discovery window",
                     now - _COLD_START_SPAWNED_AT)
        return True
    with _abort_on_error("Could not cold-start Windows gateway after update"):
        # reason=: the diag log has to be able to name this spawn site -- a post-update cold
        # start racing an autostart entry is the classic way two gateways appear at once.
        pid = gateway_windows._spawn_detached(reason="update:windows-cold-start")
    # Stamped BEFORE the readiness poll on purpose: a slow boot that fails the poll is exactly
    # the case the window guard exists for, so it must still suppress the next attempt.
