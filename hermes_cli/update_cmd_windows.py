...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    with _abort_on_error("Could not re-check Desktop gateway-lifecycle ownership before cold-start"):
        if _desktop_owns_gateway_lifecycle():
            ...
            return True
    global _COLD_START_SPAWNED_AT, _COLD_START_PID
    now = _time.monotonic()
    recently_spawned = (_COLD_START_SPAWNED_AT is not None
                        and now - _COLD_START_SPAWNED_AT < _COLD_START_SUPPRESS_S)
    if recently_spawned:
        # Recheck readiness without spawning a second child during discovery lag.
        pid = _COLD_START_PID
    else:
        ...
        _COLD_START_SPAWNED_AT, _COLD_START_PID = now, pid
