...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    ``HERMES_DESKTOP_CRON`` overrides the gateway guard: ``0`` = never tick, ``1`` = legacy
    always-tick, unset/anything else = auto (heartbeat guard). The guard only applies to the
    built-in provider — an external provider (e.g. chronos) arms remote schedules rather than
    looping locally, so it keeps the plain ``start()`` path.
    ...
    # Built-in provider on auto: tick only while no machine gateway is doing it. Both
    # processes take the per-store cron/.tick.lock, so a double tick is not corruption — it
    # is a race for WHICH process fires the job, and this backend has no live adapters, so a
    # job it wins is delivered through the standalone per-platform send path instead.
    from cron.jobs import record_ticker_heartbeat
    from cron.scheduler import tick as cron_tick

    owner_hold_ticks = _GATEWAY_SILENT_OWNER_HOLD_TICKS
    _log.info(
    ...
    while not stop_event.is_set():
        if _machine_gateway_alive():
            ...
            if state != "hold":
                ...
                state = "hold"
            # Neither tick nor ticker-heartbeat write: a hold must look like deferral to the
            # cron-stale monitors, not like a live ticker.
        else:
            if state != "active":
            ...
            try:
                ...
                # Mirror InProcessCronScheduler.start: BaseException so a SystemExit from a
                # provider SDK can't silently kill the thread; shutdown is driven by
                # stop_event, not exceptions.
    ...
    async def _serve():
        ...
        with server.capture_signals():
            try:
                ...
                # uvicorn's Server.startup() handles a failed bind by logging the OSError to
                # its OWN "uvicorn.error" logger and then calling sys.exit(1). uvicorn
                # configures that logger with propagate=False, so the message reaches stderr
                # and NO Hermes log file: during the 2026-08-12 incident, grepping for
                # "10048|error while attempting to bind" returned 0 hits across agent.log,
                # gui.log, errors.log, agent-dashboard.log and errors-dashboard.log while
                # that exact error was plainly on stderr. Whoever spawned us may also have
                # discarded stderr entirely, so the only durable record is this one. Re-log on
                # a propagating Hermes logger, then re-raise so the exit code is unchanged.
                # ASCII only - this must survive a legacy MBCS console (see
                # hermes_logging._safe_stderr).
                _log.error(
