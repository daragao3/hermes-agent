...
from hermes_cli.config import (
    cfg_get, get_hermes_home, get_config_path, get_env_path, load_config, save_config,
...
def _run_setup_wizard_impl(args):
    ...
    if getattr(args, "reset", False):
        # effective_default_config(), not the raw DEFAULT_CONFIG: the two differ in exactly
        # one field, session_bridge.sidebar.inbox_cwd, which the plain default leaves unset —
        # so writing the raw default produces a config whose sidebar inbox points nowhere.
