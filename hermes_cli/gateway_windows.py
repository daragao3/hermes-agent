...
from hermes_cli.gateway_diag import SPAWN_SITE_UNSPECIFIED as GATEWAY_SPAWN_SITE_UNSPECIFIED
from hermes_cli.gateway_diag import parent_chain, raw_cmdline, write_diag
from hermes_cli.install_root import find_installed_package_root, installed_package_root
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, hermes_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"If Len(env.Item({q(GATEWAY_SPAWN_SITE_ENV)})) = 0 Then",
        f"  env.Item({q(GATEWAY_SPAWN_SITE_ENV)}) = {q(_SPAWN_SITE_TASK_SCRIPT)}",
        "End If",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    _prepend_pythonpath(env_overlay, [_preserve_hermes_home_path(p) for p in (installed_package_root(), *extra_pythonpath)])
...
def _start_or_report_running(running_pids: list[int] | None = None, *, reason: str = _SPAWN_SITE_UNSPECIFIED) -> None:
...
def _probe_exit_diag(diag_path: Path) -> None:
    ...
    try:
        ...
        try:
            ...
            tag = event.get("tag", "?")
            _probe(6, tag in ("gateway.start", "gateway.spawn"), f"Last lifecycle event: tag={tag} pid={event.get('pid', '?')} ts={event.get('ts', '?')}")
        except Exception:
    ...
    for pid in pids:
        ...
        try:
            if not _pid_exists(pid):
                continue
            write_diag(
                "gateway.force_kill", victim_pid=pid, killer_pid=own_pid,
                killer_cmdline=raw_cmdline(), killer_parent_chain=parent_chain(),
            )
            ...
            print(f"Kill of PID {pid} timed out; death unconfirmed (continuing)")
        except OSError as exc:
