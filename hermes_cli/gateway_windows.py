...
# Spawn-site labels. Carried to the child in HERMES_GATEWAY_SPAWN_SITE so a detached gateway
# can say who started it long after that parent has exited.
...
# Stop-grace derivation (see _windows_stop_drain_timeout).
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs, by two invariants in priority order.

    1. THE CWD DECIDES WHICH CODE RUNS. Every launcher here invokes ``python -m hermes_cli.main``,
       and ``-m`` puts the cwd on ``sys.path[0]`` — ahead of the editable-install finder, which
       setuptools appends to ``sys.meta_path``. So a gateway spawned with the caller's cwd runs
       whatever copy of the tree that cwd holds; on 2026-08-17 that was an agent worktree ~10
       commits behind main and deletable underneath the running process. The install root names
       the checkout the venv was built from, wherever the operator invoked the CLI from.
    2. THE CWD MUST NOT ROT. Scheduled Task / Startup entries outlive the shell that generated
       them, and a ``cd /d`` naming a deleted directory fails before Python loads, so the on-boot
       self-heal never runs. The install root cannot rot without the install itself being broken;
       with no install record at all (a bare clone), HERMES_HOME still beats a transient checkout.

    ``project_root`` is the last resort. Paths keep their configured spelling rather than being
    resolved, so an AppData install backed by a junction still identifies itself as AppData."""
...
def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    # The INSTALL root, not ``Path(__file__).resolve().parent.parent``: this module may
    # itself have been imported from a worktree (the caller's cwd is ``sys.path[0]`` under
    # ``python -m``), and baking that into a generated launcher DEPLOYS the worktree -- on
    # 2026-08-17 one ~10 commits behind main and deletable underneath the running gateway.
    # The launchers' ``cd /d`` already uses the pinned ``_stable_gateway_working_dir``;
    # PYTHONPATH has to agree with it. Covered by
...
def _build_gateway_cmd_script(python_path: str, working_dir: str, hermes_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        ...
        f'set "{GATEWAY_SPAWN_SITE_ENV}={_SPAWN_SITE_TASK_SCRIPT}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, hermes_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        ...
        "End If",
        # Only when EMPTY: the Startup-folder dropper chains into this same script after
        # stamping its own site, and the outer site is the one that is true.
        f"If Len(env.Item({_quote_vbs_string(GATEWAY_SPAWN_SITE_ENV)})) = 0 Then",
        f"  env.Item({_quote_vbs_string(GATEWAY_SPAWN_SITE_ENV)}) = "
        f"{_quote_vbs_string(_SPAWN_SITE_TASK_SCRIPT)}",
        "End If",
...
def _build_startup_launcher(script_path: Path) -> str:
    ...
    lines = [
        ...
        'Set env = sh.Environment("PROCESS")',
        f"env.Item({_quote_vbs_string(GATEWAY_SPAWN_SITE_ENV)}) = "
        f"{_quote_vbs_string(_SPAWN_SITE_STARTUP_FOLDER)}",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    # This was PROJECT_ROOT, which is __file__-derived and so names whichever copy of the tree
    # THIS process imported — a worktree, when an agent session runs the CLI from one. The child
    # must get the INSTALLED checkout; installed_package_root() falls back to PROJECT_ROOT when
    # there is no install record at all.
    _roots = (installed_package_root(), *extra_pythonpath)
    _prepend_pythonpath(env_overlay, [_preserve_hermes_home_path(p) for p in _roots])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    # Same pin as _build_gateway_argv: a --replace respawn must not inherit the module root of
    # whatever tree the caller happened to be run from.
    _prepend_pythonpath(env_overlay, [str(installed_package_root()), *extra_pythonpath])
...
def _spawn_detached(script_path: Path | None = None, *, reason: str = _SPAWN_SITE_UNSPECIFIED) -> int:
    ...
    ``reason`` names the call site and is carried to the child in ``HERMES_GATEWAY_SPAWN_SITE``,
    which the child echoes into its ``gateway.start`` diag record. Detached gateways outlive their
    parents, so attribution has to travel with the process rather than be reconstructed from a
    parent that has since exited."""
    _assert_windows()
...
def _launch_detached_gateway(*, reason: str) -> None:
    ...
    ``reason`` is required rather than defaulted at every level of this path: a defaulted
    attribution field silently mislabels, and telling a cold start apart from a restart is
    precisely what the diag log has to do when two gateways appear at once."""
    pid = _spawn_detached(reason=reason)
    _report_gateway_start(f"direct spawn (PID {pid})")


def _start_or_report_running(running_pids: list[int] | None = None, *, reason: str) -> None:
    ...
    if running_pids:
        ...
        _launch_detached_gateway(reason=reason)


def _install_startup_fallback(script_path: Path, start_now: bool, detail: str, *, reason: str) -> None:
...
def install(
    ...
    if not start_on_login:
        ...
        if start_now:
            _start_or_report_running(reason="install:no-login-autostart")
        else:
    ...
    if ok:
        ...
        if start_now:
            _start_or_report_running(reason="install:scheduled-task")
        else:
    ...
    if _should_fall_back(1, detail):
        _install_startup_fallback(script_path, start_now, detail,
                                  reason="install:startup-fallback-after-schtasks")
        return
...
def _probe_exit_diag(diag_path: Path) -> None:
    ...
    try:
        ...
        try:
            ...
            tag = event.get("tag", "?")
            # gateway.spawn means the boot is UNDERWAY, not that it failed: the diag record is
            # written by the spawner before the child has had time to write its own start event.
            booting = tag == "gateway.spawn"
            _probe(6, booting or tag in ("gateway.start",),
                   f"Last lifecycle event: tag={tag} pid={event.get('pid', '?')} "
                   f"ts={event.get('ts', '?')}" + ("  (boot in progress)" if booting else ""))
        except Exception:
...
def _windows_stop_drain_timeout() -> float:
    """Bounded stop grace period, DERIVED from the dying gateway's own shutdown budget.

    It has to OUTLAST that budget, so it cannot be picked independently. Until 2026-08-17 it was
    ``min(configured, 30.0)`` while ``agent.restart_drain_timeout`` was 60s and the shutdown
    watchdog leash 120s — so the gateway was force-killed at 30s while its phase-2 drain believed
    it had 60, structurally guaranteed to die mid-drain whenever an in-flight cron needed longer
    (a live LLM turn routinely does). Census over the whole of gateway.log: 10 of 29 stops killed
    that way; restricted to the stops with a trustworthy in-flight signal, 3 of 3 with crons
    running died at shutdown phase 1 and 0 of 2 without did. It also made
    ``gateway/shutdown_watchdog.py`` unreachable, so the one killer that logs CRITICAL and writes
    an all-thread dump could never fire and a genuine wedge left no diagnosis at all.

    Ordering the grace PAST the watchdog leash restores the intended escalation — drain, then the
    loud internal force-exit, and ``taskkill`` only as the last resort it was written to be — and
    deriving it keeps that ordering true if either budget is retuned later. The cost is a longer
    worst-case restart, paid only where it buys something: ``_drain_gateway_pid`` polls until the
    PID exits, so a teardown finishing in 2s still returns in 2s.

    ``0`` is a legal configured value, not a missing one: ``agent.restart_drain_timeout`` defaults
    to 0 and documents it as the deliberate "no drain, interrupt immediately" choice
    (``hermes_cli/config.py``), and every other consumer honours it. Testing it with ``or`` would
    make the fallback an accidental FLOOR — 0 is falsy, so an explicit 0 would be costed at 30s,
    inflating the grace by a drain the gateway was never granted. Only an ABSENT result falls back.
    Honouring 0 does not collapse the ordering: no drain is not no shutdown, so the grace is still
    past the watchdog, which fires on its own leash regardless of the drain budget."""
    ...
    # The CLI must still never wedge: give the watchdog room to fire and be seen, then escalate.
...
def _force_terminate_known_gateway_pids(pids: list[int]) -> int:
    ...
    for pid in pids:
        ...
        try:
            ...
            # A SubprocessError, NOT an OSError — it would otherwise escape this loop entirely
            # and abort the stop with one unkillable PID left, the same escape fixed in
            # gateway/status.py::terminate_pid.
            print(
...
def restart() -> None:
    ...
    replaces it when it exits (a silent outage).

    A failure inside ``stop()`` is deliberately NOT fatal here. By the time stop raises, the old
    gateway is usually already dead — so propagating leaves :8642 with no listener and no
    replacement, strictly worse than any error stop was trying to report. The error is carried
    forward and re-raised only if the relaunch ALSO fails to produce a running gateway. Liveness,
    ...
    # Not start(): its install prompt has no place in a restart, and a concurrent starter that
    # won the race must not be answered with a SECOND gateway — two listeners is its own outage.
