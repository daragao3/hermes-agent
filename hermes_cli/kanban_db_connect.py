    ...
    try:
        ...
        if not acquired:
            _kb._log.warning(
                ...
                "— proceeding without the cross-process lock (in-process lock + idempotent "
...
def _open_configured(
    path: Path, under_lock, *, cross_process_section: Optional[str] = None,
) -> tuple[sqlite3.Connection, Any]:
    """Open/configure ``path``, then run ``under_lock(conn)`` safely.

    WAL setup is serialized between this process's threads but deliberately
    outside the cross-process init lock: attaching a hot ``-shm`` can queue
    behind live writers for seconds. Schema callbacks additionally acquire
    cross-process then in-process locks, matching ``repair_db`` without a lock
    inversion. Closed if anything raises.
    """
    conn = _sqlite_connect(path)
    try:
        ...
        if cross_process_section is None:
            with _INIT_LOCK:
                out = under_lock(conn)
        else:
            with _cross_process_init_lock(path, cross_process_section):
                with _INIT_LOCK:
                    out = under_lock(conn)
    except Exception:
        conn.close()
...
def connect(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> sqlite3.Connection:
    ...
    # The cross-process lock deliberately does NOT cover the open on the hot path.
    # _open_configured runs the WAL/pragma probe, which on a hot board attaches the -shm
    # and takes the first read lock — 1-2s per process behind live writers. Holding the
    # lock across it made each process pay for all its predecessors: with a dozen workers
    # first-connecting at once the serialized total reached 16s against a 10s budget, so
    # the "not acquired" fallback fired on ORDINARY contention rather than a wedged holder.
    #
    # It is still taken when this process has not yet initialized this path — the only case
    # that runs schema work. Order stays cross-process -> _INIT_LOCK, matching repair_db;
    # taking it INSIDE _init_if_needed would invert that (_open_configured already holds
    # _INIT_LOCK) and deadlock until the 10s timeout, firing the warning this silences.
    conn, _ = _open_configured(
        path,
        _init_if_needed,
        cross_process_section=None if resolved in _INITIALIZED_PATHS else "schema",
    )
    return conn
