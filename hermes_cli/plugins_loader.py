...
class PluginLoaderMixin:
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.debug("env-hint derivation failed; resolving eagerly", exc_info=True)
                env_hints = ()
            platform_registry.register_deferred(platform_name, _loader, scope=scope, env_hints=env_hints)
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        ...
        bare_name = _plugin_module_name(manifest_key(manifest), manifest.path)
