...
from hermes_cli.dashboard_procs import (  # noqa: F401 -- re-exported for hermes_cli.main
    _DashboardPids,
    _looks_like_dashboard_command_line,
    _scan_error,
    _scan_ok,
)
...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None) -> _DashboardPids:
    """Return PIDs of stale ``dashboard``/``serve`` processes for update cleanup.

    The result is a ``_DashboardPids`` -- a list that also carries whether the scan itself
    succeeded. On failure the list is empty *and* ``_scan_ok()`` is False, so callers can say
    "couldn't look" instead of reporting a false clean.
    """
    from hermes_cli.dashboard_procs import _scan_dashboard_processes, scan_error, scan_ok
    scanned = _scan_dashboard_processes(exclude_pids=exclude_pids)
    return _DashboardPids(
        [pid for pid, _cmd in scanned], scan_ok=scan_ok(scanned), scan_error=scan_error(scanned))
...
def _report_dashboard_status() -> int:
    ...
    scanned = _scan_dashboard_processes()
    live: list[tuple[int, str, str]] = []
    for pid, command in scanned:
    ...
    if not live:
        if not scan_ok(scanned):
            # Never report a clean we did not actually establish: an operator who reads
            # "none running" stops looking, and --stop then silently stops nothing.
            print("Could not scan the process table, so nothing can be reported: "
                  f"{scan_error(scanned)}")
            return 0
