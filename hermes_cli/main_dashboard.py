...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None):
    from hermes_cli.dashboard_procs import _DashboardPids, _scan_dashboard_processes
    try:
        rows = _scan_dashboard_processes(exclude_pids=exclude_pids)
        return _DashboardPids(pid for pid, _ in rows)
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    from hermes_cli.dashboard_procs import _dashboard_command_subcommand
    mode = _dashboard_command_subcommand(command)
...
def _report_dashboard_status() -> int:
    ...
    try:
        ...
        print(f"⚠ Could not scan the process table ({exc})")
        return -1
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    default_dist = PROJECT_ROOT / "hermes_cli" / "web_dist"
    assets = default_dist / "assets"
    already_built = (default_dist / "index.html").is_file() and assets.is_dir() and any(assets.iterdir())
    if _headless_backend:
        os.environ["HERMES_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "HERMES_WEB_DIST" not in os.environ and not skip_build and already_built:
        print(f"→ Using already-built web dist at {default_dist}")
