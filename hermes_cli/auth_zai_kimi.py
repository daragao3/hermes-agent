    ...
    try:
        ...
        def _first_ready(require_done: bool) -> Optional[Dict[str, str]]:
            ...
            for ep in ZAI_ENDPOINTS:
                # A done Future may not have been yielded by as_completed yet.
                # Only consumed results can prove a higher-priority probe failed.
                if require_done and ep[0] not in consumed:
                    return None
                if ep[0] in results:
                    return results[ep[0]]
            return None
