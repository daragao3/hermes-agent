...
def _terminate_reclaimed_worker(
    ...
    *,
    ...
    try:
        ...
        # Windows raises a BARE OSError (WinError 87, "The parameter is incorrect") for a PID
        # that no longer exists — not the ProcessLookupError POSIX raises for ESRCH — so the
        # branch above never fires there. Fall back to the authoritative cross-platform liveness
        # probe: if the worker is already gone, the termination effectively succeeded. Reporting
        # terminated=False here makes _worker_survived_termination() misread a dead worker as a
        # live one and defer the reclaim, which in detect_stale_running() lets the cruder crash
        # detector steal a stale task and count a circuit-breaker failure against a merely
        # long-running job.
        if not _kb._pid_alive(pid):
            info["terminated"] = True
        return info
...
def _module_hermes_argv() -> list[str]:
    ...
    # real_executable(), not sys.executable: on a Microsoft-Store CPython host that value is
    # an MSIX app-execution alias, and this argv is what every dispatched kanban shim spawns.
