...
from hermes_cli.tools_config_cua import (
    _cua_driver_install_ready, _pip_install, _post_setup_no_window_flags, _run_installer,
    install_cua_driver,
)
...
def _install_chromium(install_cmd: list[str]) -> None:
    ...
    try:
        # npx -> node -> playwright -> downloader: a grandchild tree by construction, so the
        # 600s below only means anything with file-backed capture behind it.
        result = _run_installer(install_cmd, cwd=str(PROJECT_ROOT), timeout=600,
...
def _post_setup_camofox() -> None:
    ...
    if camofox_dir.exists():
        ...
        # timeout=None was no bound at all, on a step that fetches a ~300MB engine through npm's
        # own child processes. Same 600s ceiling as the Chromium install; it is the same work.
        result = _run_installer([_npm_bin, "install", "--silent", "--workspaces=false"],
