...
# _ensure_httpx has production callers (agent/account_usage.py, ai_usage/collector.py).
# The codex refresh-failure sentinel exists because the lazy-refresh path used to log at
# DEBUG and simply exhaust the entry, so a dead single-use refresh-token chain went unnoticed
# for 33 days; agent/credential_pool.py writes the marker and the health probe reads it.
