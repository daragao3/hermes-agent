...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    for dep in meta.get("external_dependencies", []):
        ...
        if check_cmd:
            try:
                # DEVNULL, not capture_output: an npm/npx shim spawns a grandchild that
                # inherits the capture pipes, and on Windows that defeats the timeout the
                # probe depends on. Only success/failure is used.
                subprocess.run(shlex.split(check_cmd), check=True, timeout=5,
