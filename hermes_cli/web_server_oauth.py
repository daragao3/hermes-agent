...
def _oauth_session_home(session_id: str) -> Optional[str]:
    """Home captured at request time; absent for legacy in-memory sessions."""
    with _oauth_sessions_lock:
        sess = _oauth_sessions.get(session_id)
...
def _nous_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    with _profile_scope(_oauth_session_profile(session_id), hermes_home=sess.get("hermes_home")):
