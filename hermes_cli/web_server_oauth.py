...
def _oauth_session_home(session_id: str) -> Optional[str]:
    """The HERMES_HOME captured when an OAuth session was created.

    None means "resolve live" — the session predates the capture, or the home could not
...
def _nous_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    with _profile_scope(
        _oauth_session_profile(session_id), hermes_home=_oauth_session_home(session_id)
    ):
