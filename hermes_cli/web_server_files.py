...
def _default_hermes_root_is_opt_data() -> bool:
    ...
    # Compare like for like. ``root`` has been through ``resolve()``, so the constant must be
    # too, or the check is asymmetric and silently answers "not hosted" for a root that IS
    # /opt/data:
    #   - Linux: a symlinked /opt/data (bind-mount style layouts) resolves to its target, which
    #     never equals the bare constant -- so the managed file browser would fail to lock and
    #     expose the whole container FS.
