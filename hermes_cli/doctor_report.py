...
# The flags `hermes doctor` was invoked with. Checks run from DOCTOR_CHECKS as
# ``check(should_fix)`` and never see ``args``, so the two opt-in checks read their flag from
# here. Set once by run_doctor; empty (every flag False) for any other caller, which is what a
# programmatic doctor run should get.
_REQUEST: dict = {}


def set_doctor_request(args) -> None:
    """Record the parsed CLI args so opt-in checks can consult their flag."""
    _REQUEST.clear()
    if args is not None:
        _REQUEST.update(vars(args))


def doctor_request(flag: str, default: bool = False) -> bool:
    """True when ``hermes doctor`` was invoked with ``--<flag>``."""
    return bool(_REQUEST.get(flag, default))
