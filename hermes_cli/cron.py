...
def cron_list(show_all: bool = False):
    ...
    for job in jobs:
        ...
        for label, value in _job_rows(job):
            print(f"    {label + ':':<11}{' ' if len(label) >= 10 else ''}{value}")
...
def _job_rows(job: Dict[str, Any]) -> List[tuple[str, str]]:
    ...
    if effective_job_state(job) == "paused":
        reason, clipped = _clip_pause_reason(job.get("paused_reason") or "(no reason recorded)")
        since = f" (since {job['paused_at']})" if job.get("paused_at") else ""
        optional.insert(0, ("Paused", reason + since))
        if clipped:
            optional.insert(1, ("Full reason", f"hermes cron show {job.get('id', '?')}"))
    return [
...
def _job_api_kwargs(args) -> Dict[str, Any]:
    ...
    values = {api_key: getattr(args, attr, None) for api_key, attr in _JOB_ARG_FIELDS}
    if values.get("provider") is None:
        values["provider"] = getattr(args, "provider", None)
    return values


_JOB_DETAIL_LINES = (
    ("model", "  Model: {}"), ("provider", "  Provider: {}"), ("base_url", "  Base URL: {}"),
    ("script", "  Script: {}"),
...
def cron_edit(args):
    ...
    for field, label in (("model", "Model"), ("provider", "Provider")):
        arg_name = "model_provider" if field == "provider" else field
        supplied = getattr(args, arg_name, getattr(args, field, None))
        if supplied == "" and not updated.get(field):
            print(f"  {label}: (inherited from env/config)")
    return 0
...
def cron_resume(args) -> int:
    ...
    if not run_at and not run_now:
        return _job_action("resume", args.job_id, "Resumed", caller="hermes_cli:cron_resume")
    from cron.jobs import AmbiguousJobReference, ResumeBarrierError, _hermes_now, rearm_oneshot
    try:
        job = rearm_oneshot(args.job_id, _hermes_now().isoformat() if run_now else run_at)
    except (AmbiguousJobReference, ResumeBarrierError, ValueError) as exc:
...
def cron_pause(args) -> int:
    stop = bool(getattr(args, "stop_inflight", False))
    reason = _clean_reason(getattr(args, "reason", None))
    if stop and reason is None:
        reason = _existing_paused_reason(args.job_id)
    return _job_action("pause", args.job_id, "Paused", reason=reason,
                       caller="hermes_cli:cron_pause", stop_inflight=stop)
...
_CRON_SUBCOMMANDS = {
    ...
    "run": lambda a: _job_action("run", a.job_id, "Triggered",
        reason=_clean_reason(getattr(a, "reason", None)), caller="hermes_cli:cron_run"),
...
def cron_command(args):
    ...
    print(f"Unknown cron command: {subcmd}\n"
          "Usage: hermes cron [list|show|create|edit|pause|resume|barrier|run|remove|status|runs|incidents|notepad|doctor|tick]")
    sys.exit(1)
