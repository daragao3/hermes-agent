...
def _job_rows(job: Dict[str, Any]) -> List[tuple[str, str]]:
    ...
    ] + _pin_rows(job) + [(label, value) for label, value in optional if value]


def _paused_rows(job: Dict[str, Any]) -> List[tuple[str, str]]:
    """The pause reason (+ when), for a job the scheduler treats as paused.

    Storing the reason is only useful if an audit can see it. A long reason is clipped to one
    line here and points at ``cron show`` for the rest."""
    from cron.jobs import effective_job_state
    if effective_job_state(job) != "paused":
        return []
    suffix = f" (since {job['paused_at']})" if job.get("paused_at") else ""
    raw_reason = job.get("paused_reason")
    if not raw_reason:
        return [("Paused", f"(no reason recorded){suffix}")]
    shown, clipped = _clip_pause_reason(raw_reason)
    hint = color(f"  (full reason: hermes cron show {job.get('id', '?')})", Colors.DIM) if clipped else ""
    return [("Paused", f"{shown}{suffix}{hint}")]


_PIN_ROWS = (("Model", "model"), ("Provider", "provider"), ("Base URL", "base_url"))


def _pin_rows(job: Dict[str, Any]) -> List[tuple[str, str]]:
    """The per-job inference pin. It is the highest-precedence and most durable model override
    on the box -- it beats HERMES_MODEL and config.yaml, is re-read from storage every tick, and
    lives in the untracked cron store, so no git checkout can revert it. Rendering it is the only
    way an operator can SEE which jobs are pinned; base_url rides along because provider+base_url
    is a single security-relevant pair, not two independent fields."""
    return [(label, job[key]) for label, key in _PIN_ROWS if job.get(key)]


def _print_pin_lines(job_data: Dict[str, Any]) -> None:
    for label, key in _PIN_ROWS:
        if job_data.get(key):
            print(f"  {label}: {job_data[key]}")
...
def cron_create(args):
    ...
    _print_pin_lines(result.get("job", {}))
...
def cron_edit(args):
    ...
    # Echo the inference pin whenever THIS edit touched it, even when the new value is empty: a
    # clear that printed nothing would be indistinguishable from a no-op, and this is the pin that
    # survives a git checkout -- the operator needs to read back what the store now holds.
    if getattr(args, "model", None) is not None or updated.get("model"):
        print(f"  Model: {updated.get('model') or '(inherited from env/config)'}")
    if getattr(args, "model_provider", None) is not None or updated.get("provider"):
...
def _job_action(action: str, job_id: str, success_verb: str, *,
                reason: Optional[str] = None, caller: Optional[str] = None,
                stop_inflight: bool = False) -> int:
    ...
    if action == "pause":
        ...
        # Pause stops future FIRES; a run already executing keeps going, and until this
        # nothing said so.
        _report_inflight(
            job.get("job_id") or job_id, reason=paused_reason,
            stop_inflight=stop_inflight, caller=caller or "hermes_cli:cron_pause")
    return 0
...
_CRON_SUBCOMMANDS = {
    ...
    "pause": lambda a: _cmd_pause(a),
    "resume": lambda a: cron_resume(a),
    "run": lambda a: _job_action(        "run", a.job_id, "Triggered", reason=_clean_reason(getattr(a, "reason", None)),        caller="hermes_cli:cron_run"),
    "show": lambda a: cron_show(a.job_id),
    "detail": lambda a: cron_show(a.job_id),
    "barrier": lambda a: cron_barrier(a),
...
# --- paused jobs: the reason, what is still running, and the resume fence ---------------------
...
def _cmd_pause(args) -> int:
    """``hermes cron pause`` — with the reason, and a report of what is still running."""
    stop_inflight = bool(getattr(args, "stop_inflight", False))
    reason = _clean_reason(getattr(args, "reason", None))
    if stop_inflight and reason is None:
        # pause_job OVERWRITES paused_reason, and this is the very command the in-flight
        # report tells the operator to run — so read the recorded reason back rather than
        # wiping it.
        reason = _existing_paused_reason(args.job_id)
    return _job_action(
        "pause", args.job_id, "Paused", reason=reason,
        caller="hermes_cli:cron_pause", stop_inflight=stop_inflight)
