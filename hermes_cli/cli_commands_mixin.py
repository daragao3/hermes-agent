...
def _is_windows_executable(command: str) -> bool:
    ...
    Windows has no execute bit: cmd.exe decides by extension (``PATHEXT``). Handing it a
    file with an unregistered extension — a bash script, say — does not fail; it blocks
    indefinitely in the shell-association handler. Callers use this to refuse that fallback
    rather than hang."""
    import shutil
...
class CLICommandsMixin:
    ...
    def _handle_branch_command(self, cmd_original: str) -> None:
        ...
        try:
            ...
            self._session_db.create_session(
                session_id=new_session_id, source=child_source,
                ...
                # Narrow metadata copy, not cwd discovery: returns a value only when the
                # requested parent identity, the returned row identity and the local source
                # all agree and the cwd is a canonical absolute native path.
    ...
    def _cron_list(self, subcommand: str, opts: dict) -> None:
        ...
        for job in jobs:
            ...
            if job.get("last_run_at"):
                ...
                # last_run_at is stamped when the run FINISHES; "Last run" reads as when it
                # started, which makes a long job look like it fired late.
                print(f"  Last completed: {job['last_run_at']} ({status})")
            print()
    ...
    def _cron_job_action(self, subcommand: str, opts: dict) -> None:
        ...
        # An explicit caller per action: without one the cronjob tool defaults to
        # "llm:cronjob_tool", which would attribute an interactive /cron pause to a model that
        # did not issue it.
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        try:
            ...
            try:
                # posix=False on Windows: POSIX mode treats a backslash as an escape, so
                # ``C:\Program Files\...\editor.exe`` is split into an unusable token.
                subprocess.call([*shlex.split(editor, posix=os.name != "nt"), path])
            except (OSError, ValueError) as exc:
                # Narrower than bare Exception — these are what shlex.split and
                # subprocess.call actually raise — so a real bug is not swallowed into a
                # silent shell fallback.
                if os.name == "nt" and not _is_windows_executable(editor):
                    # cmd.exe has no execute bit and dispatches by PATHEXT; handing it a
                    # file with an unregistered extension BLOCKS in the shell-association
                    # handler instead of failing, so refuse rather than hang.
                    raise RuntimeError(
