...
请参阅完整的 **[安装指南](/getting-started/installation)**，了解安装程序的具体操作、按用户与 root 的目录布局以及 Windows 相关说明。如需完整的平台支持矩阵，请参阅 **[平台支持](/getting-started/platform-support)**。

:::tip 上手可用智能体的最快路径
安装完成后，运行 `hermes setup --portal`——一次 OAuth 即可覆盖一个模型以及全部四项 Tool Gateway 工具（网络搜索、图像生成、TTS、浏览器）。参见 [Nous Portal](/integrations/nous-portal)。
:::
...
|                                                                       |                                                                    |
| --------------------------------------------------------------------- | ------------------------------------------------------------------ |
| 🚀 **[安装](/getting-started/installation)**                          | 在 Linux、macOS、WSL2、原生 Windows、Nix 与 NixOS 或 Android 上 60 秒完成安装 |
...
| 🤖 **[Bot 模式](/user-guide/bot-mode)**                              | 拥有独立模型、记忆、技能、例行任务和聊天的具名 Bot                 |
| 🔧 **[工具与工具集](/user-guide/features/tools)**                     | 60+ 内置工具及其配置方式                                           |
...
- **随处运行，不限于本地** — 7 种终端后端：本地、Docker、SSH、Daytona、Singularity、Modal、Vercel Sandbox。Daytona 和 Modal 提供 serverless 持久化——环境闲置时休眠，几乎零成本
...
- **[Bot 模式](/user-guide/bot-mode)** — 组建一支长期存在的专家 Bot 团队，它们在群聊中并通过 `@mentions` 协同工作
...
- **完整的 Web 控制** — 搜索、提取、浏览、视觉、图像生成、TTS——通过 [Nous Portal](/integrations/nous-portal) 一个订阅即可打包获得全部功能
