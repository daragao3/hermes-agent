...
| `coding` | 复合（`file` + `terminal` + `search` + `web` + `skills` + `browser` + `todo` + `memory` + `session_search` + `clarify` + `code_execution` + `delegation` + `vision`） | 面向编码的软件工作套件：文件编辑、终端、搜索、网页文档、skill、浏览器、委派和代码执行。 |
...
少数工具在工具集归属之外还有额外的可用性检查，**不会**仅凭 `all`/`*` 就被开启：

- **能力门控（Capability-gated）** 工具（浏览器、`computer_use`、`code_execution`、飞书、Home Assistant、cronjob）仅在其后端/凭据前置条件已配置时出现。
- **工作流门控（Workflow-gated）** 工具——即 `kanban` 工具集——刻意设计为需要显式启用。`all`/`*` **不会**启用 kanban；你必须显式列出 `kanban`（或作为设置了 `HERMES_KANBAN_TASK` 的调度器生成 worker 运行）。Kanban 工具会改变共享的看板状态，因此即使在 `all` 之下也默认保持关闭。

## 与 `hermes tools` 的关系

`hermes tools` 命令提供基于 curses 的 UI，用于按平台切换单个工具的启用/禁用状态。该操作在工具级别进行（比工具集更细粒度），并持久化到 `config.yaml`。即使工具集已启用，被禁用的工具也会被过滤掉。

另见：[工具参考](./tools-reference.md)，其中列出了所有单个工具及其参数的完整清单。
