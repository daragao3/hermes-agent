...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`computer-use`](/user-guide/skills/bundled/computer-use/computer-use-computer-use) | 在后台驱动用户桌面——点击、输入、滚动、拖拽——不抢占光标、键盘焦点，也不切换虚拟桌面 / Space。跨平台：macOS、Windows、Linux。适用于任何支持工具调用的模型。 | `computer-use` |
