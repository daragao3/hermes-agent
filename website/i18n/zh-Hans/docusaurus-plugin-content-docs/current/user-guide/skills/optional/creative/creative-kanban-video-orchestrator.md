...
8. **触发前验证 API 密钥。** 外部 API（TTS、图像生成、图像转视频）需要在 `${HERMES_HOME:-~/.hermes}/.env` 或用户密钥存储中配置密钥。遇到缺少密钥错误的 worker 会浪费一个任务槽。安装脚本的 `check_key` 辅助函数在缺少必要密钥时会干净地中止。
