...
| 相关 skill | [`design-md`](/docs/user-guide/skills/bundled/creative/creative-design-md), [`popular-web-designs`](/docs/user-guide/skills/bundled/creative/creative-popular-web-designs), [`excalidraw`](/docs/user-guide/skills/bundled/creative/creative-excalidraw), [`architecture-diagram`](/docs/user-guide/skills/bundled/creative/creative-architecture-diagram) |
...
## 表面优先：在触碰 token 之前先确定构图

这是杠杆最高的反糟粕规则。大多数 AI 设计糟粕是**构图性的，而非外观性的**——模型对*每一个*界面都伸手去拿居中英雄区加三张等权重功能卡片，然后再做装饰。重新配色或重新设定样式永远修不好它，因为在选定任何一种颜色之前，布局就已经错了。

在写下任何颜色、字号阶梯或组件之前，**明确地口头确定唯一一个表面原型**。这会先用高层计划来约束生成，从而收缩产出内容的熵——与思维链步骤能提升推理质量是同一个道理。

七种表面：

1. **监控（Monitor）**——用户在观察状态变化（仪表盘、状态页、可观测性）。密度、可一眼扫视的层级，不要营销式框架。
2. **操作（Operate）**——用户在对事物采取行动（控制台、管理面板、队列、收件箱）。行动可供性和选中状态占主导。
3. **比较（Compare）**——用户在权衡各个选项（定价、套餐、规格表、搜索结果）。对齐的列、结构上的对等、强调唯一一个差异点。
4. **配置（Configure）**——用户在做设置（设置项、表单、向导、上手引导）。渐进式披露、清晰的保存/校验状态、低装饰。
5. **决策/学习（Decide / Learn）**——用户在被说服或被教学（落地页、文档、营销）。每个区块只落地一个想法；这是**唯一**通常适合使用英雄区的表面。
6. **探索（Explore）**——用户在浏览一个开放空间（画廊、地图、搜索与筛选、目录）。筛选器、结果网格以及缩放/预览就是构图本身。
7. **命令/检视（Command / Inspect）**——用户在用键盘驱动，或深入到单个对象（命令栏、检视器、详情面板、属性编辑器）。速度和聚焦优先于广度。

规则：

- 在设计之前用一句话说明表面（例如"这是一个**监控**表面，因此密度和可扫视性优先于英雄区"）。
- 仪表盘是监控表面，不是决策表面——不要给它居中英雄区加三张功能卡片。
- 如果某个屏幕确实横跨两种表面，指明**主要**的那一个，把另一个当作次要；不要把它们平均成一团糊。
- 英雄区加三卡片的构图**只对决策/学习**是正确的。在其他任何地方伸手去拿它，都是第一号破绽。

这一条约束消除的通用感 UI，比下面任何一条美学规则都多。
...
AI 设计糟粕的失败分布极小且可预测——让设计师去标注 AI 界面时，"这是 AI"的信号会收敛到大约十个破绽。在打磨或修复制品之前，把这一步作为显式的自查来运行，并写一份简短报告。**先诊断，后治疗**——把自查和修复放在同一口气里做会失败，因为模型的先验会压过指令，于是它重复同样的错误（明明需要重新布局却去重新配色，明明是构图问题却去打磨字体）。

十个破绽（每命中一个记 1 分糟粕；分数越低越好）：

1. **科技渐变**——所有东西上都是蓝/紫/靛蓝的光泽渐变。
2. **通用科技色**——默认强调色是靛蓝/紫罗兰（不是为品牌选的，只是模型的偏好色）。
3. **功能磁贴网格**——图标 + 标题 + 一句话 × 3，全部等权重，没有任何优先级。
4. **强调色标条**——卡片上的彩色左侧色条：假装成组织结构的装饰。
5. **无由来的模糊**——毛玻璃效果背后没有真实的深度/层级体系。
6. **纪念碑式数字**——超大数字填满了本该承载产品叙事的空间。
7. **图标顶饰**——每个标题上方居中放一个圆角方形图标（Tailwind 模板填充物）。
8. **居中堆叠**——因为没有确定真正的构图，所以什么都居中。
9. **默认字体**——默认用了 Inter（或 system-ui），而不是经过挑选。
10. **表面错误**——构图与表面不匹配（例如在监控表面上放英雄区）。这是其他大多数破绽背后的根本原因。

如何运行：

- 给制品打一个 10 分制的分数（10 = 糟粕最严重）。用一份简短报告说明分数并列出命中了哪些破绽。
- 把这份报告当作**上下文，而不是待办清单**——它告诉你修复精力该花在*哪里*，而不是规定要做哪些编辑。
- 然后对症修复：
  - 破绽 3、8、10 → **重新布局/重新构图**（回头审视表面选择——不要去重新配色）。
  - 破绽 1、2、9 → **重新配色/重新排版**（这里颜色和字体确实就是问题所在）。
  - 破绽 4、5、6、7 → **去掉装饰**；用真正的层级（字号、字重、间距）来替代。
- 修复后重新打分。只要构图类破绽（3、8、10）还在命中，就不要宣布完成——它们是原因，其余通常只是症状。

把诊断与治疗分开的意义在于：先让自查提出抱怨，然后只修复它抱怨的内容，并且用抱怨所要求的方式去修。
