...
Signal 消息以**原生格式**渲染，而非显示原始 markdown 字符。适配器将 markdown（`**bold**`、`*italic*`、`` `code` ``、`~~strike~~`、`||spoiler||`、标题）转换为 Signal `bodyRanges`，使文本在接收方客户端以真实样式显示，而非可见的 `**` 或 `` ` `` 字符。
...
Signal 不支持编辑已发送的消息。因此，即使启用了 `/verbose` 并为该平台保存了非 `off` 模式，Hermes 也会在 Signal 上抑制 gateway 的工具进度气泡。

你仍然可以在 CLI 中查看工具活动，而最终的 Signal 回复可以包含正常的助手输出。如果你需要在聊天中查看实时的逐工具进度，请使用支持消息编辑的消息平台。
