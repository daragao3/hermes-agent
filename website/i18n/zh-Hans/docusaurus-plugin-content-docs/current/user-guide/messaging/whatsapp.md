...
whatsapp:
  ...
  # reply_prefix: "🤖 *My Bot*\n──────\n"  # 自定义前缀（支持 \n 换行）
  send_read_receipts: false                 # 将已接受的入站消息标记为已读（蓝色勾号）
```

当 `send_read_receipts` 为 `true` 时，适配器会在私聊/群组/提及过滤通过后，将策略接受的入站消息标记为已读。被拒绝的消息（例如来自不在白名单中的发送者）不会被标记为已读。出于隐私考虑，默认禁用。更改此设置会在下次连接时自动重启桥接子进程。
...
### 原生投票、以投票形式澄清与位置 {#native-polls-clarify-as-poll-and-locations}

Baileys 桥接适配器（bot 模式）支持多种 WhatsApp 原生消息类型：

- **投票** —— agent 可以通过桥接的 `/send-poll` 端点发送 WhatsApp 原生投票（问题 + 选项）。投票结果会回流到对话中。
- **以投票形式发出澄清问题** —— 当 agent 提出多选澄清问题时，会渲染为原生单选投票；点选某个选项即回答该问题。如果投票发送失败，适配器会回退为纯文本问题。审批提示**绝不会**映射为投票——投票仅用于真正的多选澄清。
- **位置标记** —— agent 可以通过 `/send-location` 发送原生位置标记（纬度/经度，可选名称/地址），收到的共享位置（包括实时位置）也会以位置消息的形式传递给 agent。

以上功能在 bot（Baileys）模式下开箱即用，无需配置。
