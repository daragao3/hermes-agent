...
:::note 流式传输与"正在输入"提示
WeCom 适配器将每条响应作为一条完整消息投递——它**不会**逐 token 流式输出响应，
也**不会**显示"正在输入"提示。下文所说的"回复关联"只是把响应与其入站请求串成
一个会话线程，并不是实时流式传输。
:::
...
完整响应会作为一条消息投递——适配器不会增量地流式输出 token。如果入站请求上下文已过期或不可用，适配器将回退到通过 `aibot_send_msg` 主动发送消息。
