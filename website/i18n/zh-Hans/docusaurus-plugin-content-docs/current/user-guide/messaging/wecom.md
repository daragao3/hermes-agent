...
另请参阅：[WeCom 回调](./wecom-callback.md)，了解入站 webhook 的设置方法。
...
:::note 流式传输与"正在输入"提示
WeCom 适配器通过企业微信的 `msgtype: "stream"` 协议原生流式输出响应：
一轮对话开始时，客户端会立即显示"思考中/正在输入"气泡，
随后回复会随模型生成逐 token 渲染在同一个气泡中。工具调用进度也会合并到
同一个气泡里。原生流式默认开启（`config.yaml` 中的
`display.platforms.wecom.streaming: true`）；将其设为 `false` 可恢复一次性投递。
:::
...
| `groups` | `{}` | 按群组配置（见下文） |
| `stream_keepalive_enabled` | `false` | 在长时间的对话轮次中定期发送保活帧，以刷新企业微信约 6 分钟的回复流窗口 |
| `stream_keepalive_interval_seconds` | `120` | 启用时保活帧的发送间隔 |
| `stream_safe_duration_seconds` | `330` | 流存续超过此时长后，收尾时优先使用更可靠的主动发送 |
...
当机器人通过 WeCom 回调接收到消息时，适配器会记住入站请求 ID。如果在请求上下文仍然有效期间发送响应，适配器将使用 WeCom 的回复模式（`aibot_respond_msg`）将响应直接与入站消息关联。这在 WeCom 客户端中提供了更自然的对话体验。

当原生回复流处于活动状态时，响应会通过回复模式的 `msgtype: "stream"` 帧增量流式输出。如果入站请求上下文已过期或不可用（或某个流帧发送失败），适配器将回退到通过 `aibot_send_msg` 主动发送消息。
