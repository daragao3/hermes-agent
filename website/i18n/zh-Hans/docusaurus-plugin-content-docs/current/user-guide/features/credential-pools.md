...
:::warning 密钥轮换会重置 prompt 缓存
提供商侧的 prompt（提示词）缓存（Anthropic、OpenAI、OpenRouter）以发起请求的账号/API 密钥为作用域。当池在会话中途轮换到另一个密钥时，新密钥没有你这段对话的缓存前缀——下一次请求会以无折扣的输入价格重新读取完整历史；之后再轮换回来又是一次完整重读，除非先前那个密钥的缓存 TTL 仍然有效。轮换让你的会话得以继续运行，这正是它的意义所在，但在长对话中，每次轮换都要付出一次全价通读上下文的代价。
:::

:::tip
凭证池主要面向 API 密钥类提供商（OpenRouter、Anthropic）。单个 [Nous Portal](/integrations/nous-portal) OAuth 即可覆盖 300 多个模型，因此使用 Portal 的大多数用户并不需要凭证池。
:::
