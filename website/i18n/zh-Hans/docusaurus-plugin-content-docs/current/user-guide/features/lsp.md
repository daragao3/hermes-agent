...
lsp:
  ...
  # 未使用的语言服务器客户端保持存活的时长（秒）。
  # 空闲服务器会被自动关闭，并在下一次相关文件操作时
  # 重新启动。设为 0 可禁用空闲回收，让服务器在
  # 进程整个生命周期内保持运行。低于 30s 的值会被
  # 钳制为 30，确保回收扫描绝不会在操作中途回收客户端。
  idle_timeout: 600
...
服务器在被使用期间保持运行，并在无文件活动达 `lsp.idle_timeout` 秒（默认 600）后
关闭——一个触及许多 worktree 的长时间运行的 gateway，不再会为每个工作区永久累积
一个语言服务器进程。被回收的服务器会在下一次相关文件操作时自动重新启动。设置
`idle_timeout: 0` 可禁用回收，让每个服务器的索引在进程整个生命周期内保持预热。

支持多根工作区的服务器（目前为 pyright）在每个 Hermes 进程中以**单个进程**运行：
第一个 Python 项目启动它，之后的每个项目根目录——例如由并行 subagent 编辑的
同级 git worktree——都会作为额外的工作区文件夹挂接到同一个服务器上，而不是再启动
一个副本。
