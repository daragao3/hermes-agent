...
设置 → 外观中有一个 **Roam（漫游）** 开关：启用后，当智能体空闲时，宠物会自己在
窗口中四处游走——沿着界面表面行走、停顿，并在不同位置之间跳跃。漫游只在宠物位于窗口内、
处于活动状态且智能体空闲时运行；任何由智能体驱动的状态（工作中、庆祝）
都会立即接管。该开关默认关闭，并在重启后保持。

### Alt + 滚轮调整大小 {#altwheel-resizing}

按住 **Alt** 并在宠物上滚动鼠标滚轮，即可就地调整其大小——
在应用窗口和弹出浮层中都适用。浮层会朝光标位置缩放，得到的比例会被持久化，
因此重启后依然保留，并与应用内的宠物保持同步。

### 氛围反应 {#vibe-reactions}

对智能体说句好话——"good bot"、"thank you"、"ily"、`<3` 或一个
爱心表情——宠物就会以漂浮的爱心（桌面端）或爱心闪烁（CLI/TUI）作出反应。检测基于一套精选的、
不消耗 token 的词表，在本地对每条用户消息进行匹配（不调用模型）；它只对针对智能体的喜爱和感谢触发，
而不是一般的正面情绪。所有界面——CLI 宠物、TUI、桌面浮动宠物以及弹出浮层——都基于同一信号作出反应。
...
- [`hermes-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent.md)
  可让智能体按你的要求替你安装和切换宠物（参见其
