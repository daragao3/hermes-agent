...
- Auto-pruning (**on by default** since #54189): when `sessions.auto_prune` is `true`, ended sessions inactive for `sessions.retention_days` (default 90) are pruned at CLI/gateway startup
- Pruning does not shrink the database file. The CLI and gateway auto-maintenance callers pass `vacuum=False` to avoid an exclusive rewrite of the shared live database. See [Reclaiming disk space](#reclaiming-disk-space).
...
:::caution Automatic pruning does not vacuum
`vacuum_after_prune` does not enable VACUUM in the CLI or gateway startup
paths: both explicitly pass `vacuum=False`. The maintenance API's vacuum
interval and free-page gates apply only when a caller enables vacuuming.
Use the offline procedure below to reclaim file space.
:::
