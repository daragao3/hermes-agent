...
6. FRONTMATTER   -- every page, BOTH locales, must declare a non-empty ``title``
                    and ``description``. Gate 5 only inspects a description it
...
def frontmatter_value(text, key):
    """A top-level scalar from a page's YAML frontmatter, or None.

    None means "no usable value" for either reason a caller cares about: the
    page has no frontmatter block at all, or the block does not carry the key.
    An empty or whitespace-only value reads as "" rather than None, so a caller
    that wants to reject it must test falsiness, not identity.
    """
    lines = text.split("\n")
...
# --------------------------------------------------------------------- gate 6
# A page may carry a frontmatter block that is PARTIAL -- typically just
# `sidebar_position:` -- and every gate above passes it. Gate 1 tests page
# existence, gates 2-4 test body structure, and gate 5 only inspects a
# description it has already found, so a page with no `description:` at all is
# invisible to all five. Two such pages sat in user-guide/messaging until
# 2026-09-09 (554309cc81); four more had no frontmatter whatsoever (d4e4ea083f)
# and were found only by enumerating the directory by hand.
#
# WHAT IS AND IS NOT REQUIRED. `title` sets the sidebar label and the browser
# tab; `description` becomes the page's meta description and its social-card
# text. Both have a visible effect on every page, so both are required.
#
# `sidebar_position` is deliberately NOT required, and that is a measured call
# rather than leniency: website/sidebars.ts is a hand-written explicit sidebar
# that lists all 361 pages by id, and the string "autogenerated" appears in it
# ZERO times. Docusaurus only consults sidebar_position for autogenerated
# sidebar items, so on this site the key is inert -- 184 of 361 English pages
# omit it with no effect. The falsifier is in the built HTML: the rendered
# messaging sidebar runs ... matrix, bluebubbles, photon ... with bluebubbles at
# position 21 ahead of photon at 18, i.e. sidebars.ts order, not position order.
# Requiring it would mean 184 findings of pure noise. If a future sidebar
# section is ever switched to autogenerated, revisit this -- for THAT subtree.
...
def gate_frontmatter(en_pages, zh_pages, findings):
    """Every page must declare a non-empty `title` and `description`."""
    bad = 0
    for locale, root, pages in (
        ...
        for rel in sorted(pages):
            ...
            for key in FRONTMATTER_REQUIRED:
                if (rel, locale, key) in FRONTMATTER_ALLOW:
                    continue
                if not frontmatter_value(text, key):
                    bad += 1
                    findings.append(
                        "frontmatter   %s [%s]  no %s: in the frontmatter -- "
                        "gates 1-5 are all blind to a missing key"
                        % (rel, locale, key)
                    )
    return bad
