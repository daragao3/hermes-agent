...
def _standalone_send(
    t: _TargetDelivery, content: str, media_files: list, *, buttons=None) -> tuple[Any, Optional[str]]:
...
def _deliver_result(
    job: dict, content: str, adapters=None, loop=None, skip_cron_framing: bool = False,
    *, for_failure: bool = False, buttons=None
) -> Optional[str]:
    ...
    for target in targets:
        ...
        if not delivered:
            _deliver_standalone(
                t, cleaned_delivery_content, media_files, target_errors, delivery_errors, buttons=buttons)
