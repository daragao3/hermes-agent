...
def _iter_home_target_platforms(name_filter=None):
    ...
    *name_filter* is an optional ``name -> bool`` predicate applied BEFORE a deferred plugin
    platform is resolved. Resolving runs that plugin's loader, which imports the adapter module
    and the whole vendor SDK behind it, so a caller that already knows it will discard the name
    must be able to say so up front rather than paying for an import it throws away. See
    ...
    with contextlib.suppress(Exception):
        ...
        for name in platform_registry.known_names():
            # Filter by NAME before resolving. ``plugin_entries()`` would run every deferred
            # loader -- importing all ~18 bundled adapters and their vendor SDKs -- only for
            # this loop to discard the 11 whose names are already built-ins above. Resolving
            # just the survivors yields the identical set: ``get()`` runs one loader, and a
...
def cron_delivery_targets() -> list[dict]:
    ...
    # Gate on ``connected`` INSIDE the iterator, not after it yields. The membership test is
    # name-only and already known here, but applying it afterwards meant every unconnected
    # plugin platform was resolved -- and its vendor SDK imported -- purely to be discarded on
    # the next line. Measured with only ``matrix`` connected: 26.5s and 72 extra module roots
    # (microsoft_teams -> jwt -> cryptography -> bcrypt) for a one-item result. Same
    # resolve-then-discard shape as the built-in pre-filter inside the iterator; see
    # tests/cron/test_home_target_import_cost.py.
...
def _standalone_send(
    t: _TargetDelivery, content: str, media_files: list, *, buttons=None,
    ...
    def _send():
        # Inside the closure, so BOTH the direct asyncio.run path and the
        # ThreadPoolExecutor retry below carry the buttons.
        return _send_to_platform(
            t.platform, t.pconfig, t.chat_id, content, thread_id=t.thread_id,
            media_files=media_files, buttons=buttons)
...
def _deliver_standalone(
    t: _TargetDelivery, content: str, media_files: list, target_errors: list,
    delivery_errors: list, *, buttons=None,
) -> None:
    ...
    if t.is_relay:
        ...
        return
    result, err = _standalone_send(t, content, media_files, buttons=buttons)
...
def _deliver_result(
    ...
    skip_cron_framing: bool = False, buttons=None,
) -> Optional[str]:
    ...
    ``skip_cron_framing``: omit the "Cronjob Response" header/footer. Event-bus subscribers
    pass this because the cron framing is misleading for a digest or an escalation.
    ``buttons``: the serializable spec from ``events.override_buttons.buttons_for``, forwarded
    only to the standalone sender (the verified production path for event-bus deliveries,
    which pass adapters=None); inert otherwise, and None leaves behaviour unchanged.
    """
    # Strip internal iteration-tracking markers before they reach any user-facing channel.
    # The event-bus extractors already consumed them from the raw final_response upstream;
    # left in the delivered body they break Telegram HTML parsing and surface as JSON noise.
    content = _sched._strip_iteration_markers(content)
    ...
    # Per-job override beats the global knob; an explicit skip_cron_framing beats both.
