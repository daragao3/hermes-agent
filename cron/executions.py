...
# Terminal rows retained per ledger. 10000 holds several days of a busy box's history
# (the census and post-mortems read it); 1000 was under a day on this fleet.
...
def _owner_is_live(pid: int, started_at: Optional[int]) -> bool:
    try:
        ...
        # Fail safe: inability to prove death must not rewrite state. Keep that behaviour --
        # but SAY so. Silently returning True makes the caller skip recovery, so an interrupted
        # attempt keeps a stale 'running' row forever and jobs.json never learns the run ended,
        # with nothing anywhere recording why. This is also the ONLY branch that can make
        # recover_interrupted_executions() return 0 for a genuinely dead owner (a recycled PID
        # is rejected by the process_started_at comparison below, which is centisecond
        # resolution), so it is the first thing to look for when that count is unexpectedly 0.
        logger.warning(
            "Could not probe liveness of pid %s; treating its execution as still owned and "
            "skipping recovery (fail-safe).", pid, exc_info=True)
        return True
...
def recover_interrupted_execution_records() -> List[Dict[str, Any]]:
    ...
    Callers need the rows, not just a count, to carry the verdict onto records this module
    must not know about -- jobs.json's ``last_run_at`` / ``last_status``, which are otherwise
    written only by the owning process at the end of a run and are therefore lost exactly when
    that process dies. The ledger stays storage-only; the scheduler layer does the
    propagating."""
    now = _hermes_now().isoformat()
    changed = 0
    ...
    for record in recovered:
        _emit_execution_state(record)
    # `changed` and len(recovered) can differ only if a row updated inside this still-open
    # transaction cannot be read back in it, which sqlite does not do.
    assert len(recovered) == changed, (len(recovered), changed)
    return recovered
...
# --- owner liveness: census, and the cross-process admission probe ---------------------------
...
def _readonly_connection(path: Path) -> sqlite3.Connection:
    """Open ANOTHER profile's ledger read-only, without creating or migrating it.

    ``open_ledger`` always creates (it runs ``_ensure_cron_dir`` first), and creating another
    profile's ledger as a side effect of reading it is how a census invents the thing it is
    counting. ``mode=ro`` also means the schema migration cannot run — which is the correct
    constraint, not an obstacle: a census must never migrate a database it does not own."""
...
def _nonterminal_rows(conn) -> list:
    return conn.execute(
...
def nonterminal_execution_census() -> List[Dict[str, Any]]:
    """Every claimed/running row in THIS profile's ledger, with its owner's liveness."""
    with _transaction() as conn:
        rows = _nonterminal_rows(conn)
...
def cross_profile_nonterminal_execution_census() -> List[Dict[str, Any]]:
    """A read-only, lossless census from every profile ledger that already exists."""
    rows: List[Dict[str, Any]] = []
    for ledger in _cross_profile_execution_ledgers():
        with _lock:
            conn = _readonly_connection(ledger)
            try:
                raw = _nonterminal_rows(conn)
            finally:
                conn.close()
        for row in _classify_nonterminal_rows(raw):
