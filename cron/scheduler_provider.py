...
#: ``cron_<job_id>_<YYYYmmdd_HHMMSS>``. Only ids of this shape are ever considered by the
#: orphan-session recovery below; anything else is left alone.
_CRON_SESSION_ID_RE = re.compile(r"^cron_(?P<job_id>.+)_\d{8}_\d{6}$")

#: ``end_reason`` stamped on a cron session whose owning process died before ``run_job``'s own
#: ``end_session(..., "cron_complete")`` could run. Distinct from ``cron_complete`` (the run
#: finished) and from the 24h ``reaped_stale`` reaper, so a consumer can tell "interrupted,
#: closed by the successor" from either.
...
class CronScheduler(ABC):
    ...
    def fire_due(
        ...
        # ONE retained section spans the store-level fire claim through to the durable
        # running execution row. run_one_job releases this exact admission at that handoff
        # rather than holding it through the model run — but the finally below stays armed
        # regardless, so a call that fails to BIND (signature skew between a partially
        # deployed scheduler and this call site) cannot leave the section held by nobody.
        # Both releases are the same idempotent one; see RetainedDispatchAdmission.
        ...
        try:
            ...
            return self.fire_claimed(
                claimed_job, adapters=adapters, loop=loop, _dispatch_admission=admission)
        finally:
            admission.release()
    ...
    def fire_claimed(
        ...
        cancel_event: Any = None, _dispatch_admission: Any = None,
    ) -> bool:
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        # Startup recovery must NEVER kill this thread. It runs BEFORE the loop, so the
        # loop's own `except BaseException` does not cover it — on 2026-08-11 a transiently
        # half-applied checkout made this raise ImportError, the daemon thread died before
        # its first tick, and the gateway ran 5h08m with NO scheduler while logging a clean
        # "started" line one statement earlier. Zero of 69 jobs fired. A failed recovery
        # pass only costs interrupted runs their `unknown` stamp; losing the ticker stops
        # cron entirely, so degrade to the former and always enter the loop.
        try:
            recovered = self.recover_interrupted()
        except BaseException as e:
            logger.error(
                "Cron startup recovery failed — continuing WITHOUT it so the ticker still "
                "runs (interrupted runs keep their stale status): %s", e, exc_info=True)
            recovered = 0
        if recovered:
            logger.warning(
        ...
        while not stop_event.is_set():
            # Publish the preceding wait's oversized gap BEFORE cron_tick enters catch-up.
            record_ticker_state(
                "dispatching",
            ...
            record_ticker_state(
                ...
                last_success_at_epoch=last_success_at,
                **get_ticker_authority_counts())
            # Subtract the wait we actually ASKED for, not `interval`: upstream's EMFILE
            # backoff can deliberately wait far longer, and charging that to host-suspend
            # would report every backoff as a resume gap. Windows monotonic time advances
            # through Modern Standby while some platforms' does not, so take the larger of
            # wall and monotonic. Timing only the wait excludes a slow cron_tick.
            _wait_s = _backoff_wait_seconds(interval, consecutive_failures)
            _wait_wall = time.time()
            _wait_monotonic = time.monotonic()
            stop_event.wait(_wait_s)
            resume_gap = max(0.0, max(
                time.time() - _wait_wall, time.monotonic() - _wait_monotonic) - _wait_s)
