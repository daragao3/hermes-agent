...
def _resolve_bash() -> Optional[str]:
    ...
    NOT ``shutil.which("bash")``. On Windows PATH order routinely puts the WSL launcher
    (System32\\bash.exe) or its WindowsApps stub ahead of Git Bash. WSL bash runs the script
    inside the Linux VM, where a ``C:\\...`` script path does not resolve -- it exits 127 with a
    mangled-path "No such file or directory". ``resolve_windows_git_bash`` prefers Git Bash
    explicitly and refuses the WSL launcher, honouring HERMES_GIT_BASH_PATH and Hermes'
    portable Git install -- the same resolution the terminal tool uses.
    """
    if sys.platform != "win32":
...
def _run_job_script(
    ...
    cancel_event: Optional[_CancelEventLike] = None, timeout_s=None,
) -> tuple[bool, str]:
    ...
    if path is None:
        return False, err
    # Per-job ``script_timeout_seconds`` beats the env/config default: a long deterministic
    # job (the nightly test gate) needs more than 120s without raising the ceiling for every
    # script. Non-numeric or non-positive values fall back rather than disabling the bound.
    script_timeout = None
    ...
    # Re-read .env fresh for every script run. Script subprocesses inherit the GATEWAY's
    # os.environ, and build_subprocess_env() snapshots it without consulting dotenv, so
    # without this a rotated secret in profiles/<active>/.env is invisible until a gateway
    # restart. Bit for real on 2026-07-11: HERMES_GITHUB_TOKEN was rotated but
    ...
    try:
        ...
        # A broken reload must not block a script that may not need env at all; stale env is
        # the pre-2026-07-11 status quo, not a new failure.
    ...
    try:
        ...
        if proc.returncode != 0:
            parts = [
                ...
                f"{_sched.describe_exit_code(proc.returncode)}"
            ]
            if stderr:
...
def _run_job_script_with_claim_heartbeat(
    ...
    # Honour the per-job script_timeout_seconds override on EVERY branch of this wrapper.
    ...
    if not (isinstance(schedule, dict) and schedule.get("kind") == "once" and owner):
        return _run_job_script(
            script_path, workdir=workdir, cancel_event=cancel_event, timeout_s=_timeout_s)
