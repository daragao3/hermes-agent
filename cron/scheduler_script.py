...
def _run_job_script(
    ...
    if path is None:
        return False, err
    try:
        script_timeout = _positive_int(timeout_s) if timeout_s is not None else None
    ...
    try:
        ...
        if proc.returncode != 0:
            parts = [f"Script exited with code {proc.returncode}{describe_exit_code(proc.returncode)}"]
            if stderr:
...
def _run_job_script_with_claim_heartbeat(
    ...
    if not (isinstance(schedule, dict) and schedule.get("kind") == "once" and owner):
        return _run_job_script(script_path, workdir=workdir, cancel_event=cancel_event, timeout_s=job.get("script_timeout_seconds"))
