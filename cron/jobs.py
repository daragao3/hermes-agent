...
def claim_job_for_fire(
    ...
    manual: bool = False, return_job: bool = False, preserve_schedule: bool = False,
    ...
    def apply(jobs, _i, job):
        ...
        # ``preserve_schedule`` (the fork's event wakes) is the same story from the other
        # direction: the wake is off-tick, so it must not claim the scheduled slot either.
        # NOTE the local is ``manual_fire``: ``manual`` is now a PARAMETER, and reusing the
        # name here would shadow the caller's value before it is read.
        manual_fire = (force or manual or preserve_schedule
