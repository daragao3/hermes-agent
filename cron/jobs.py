...
# ``resume_barrier`` is deliberately immutable here: the barrier is set and cleared only by the
# dedicated ceremony helpers (set_resume_barrier / clear_resume_barrier), never as an ordinary
# field update -- otherwise any caller with update_job could smuggle one in or wipe one out.
...
def _normalize_job_record(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Fork: per-job Hermes profile. Normalized on read so a hand-edited or legacy
    # record with "" reads the same as one with no profile at all.
...
def create_job(
    ...
    profile: optional Hermes profile name -- the job runs with that profile's HERMES_HOME so
    its config, credentials, scripts, skills and memory paths resolve consistently;
    ``default`` selects the root profile, empty/None keeps the scheduler's behaviour."""
...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def apply(jobs, i, job):
        ...
        # Validate / normalize profile if present in updates. Empty string, None and
        # False all mean "clear the field".
...
def trigger_job(
    ...
    """Schedule a job for the next tick (ID or name), only after entering the canonical
    dispatch boundary.

    The boundary is not decoration: ``_trigger_job_admitted`` is where the fork emits
    CRON_TRIGGERED, and every trigger path must reach it. Upstream 0.21 rewrote this
    function to write the job record directly; adopting that verbatim would recreate the
    2026-07-13 regression, where a rerouted run path fired unaudited for four days.

    ``extra_prompt`` (upstream) is stamped as ``manual_run_prompt`` for that single fire
    only; ``mark_job_run`` clears it.
    """
    with default_control_store().dispatch_section(boundary="trigger-job"):
        return _trigger_job_admitted(
            job_id, caller=caller, reason=reason, extra_prompt=extra_prompt
        )
...
def _trigger_job_admitted(
    ...
    # Upstream 0.21 guard, moved here with the rest of trigger_job's body: a terminal job
    # is finished, not paused, so it needs a new occurrence rather than a re-arm.
    ...
    updated = update_job(
        job["id"],
        {
            ...
            # Upstream 0.21 run-now intent: keeps the cron-expression / TZ repair guards
            # from reading this as stale state, and carries a single-fire prompt that
            # ``_record_run_outcome`` clears.
...
def _record_run_outcome(
    ...
    if success:
        ...
        # Fork alias, same value: artifact_surface/data_api.py and four sites in
        # cron/scheduler.py read ``consecutive_errors`` for alert severity.
        ...
        job["consecutive_errors"] = job["failure_streak"]
...
def mark_job_run(
    ...
    *,
    ...
    Returns the run timestamp it stamped, or None when the fence can't be taken, the job is
    missing, or ``expected_fire_owner`` no longer holds the fire claim. The fork returns the
    timestamp rather than True because cron/scheduler.py stores it as
    ``deadline_last_run_at``; a non-empty string keeps every ``if mark_job_run(...)`` call
...
def claim_job_for_fire(
    ...
    def apply(jobs, _i, job):
        if is_terminal_job(job) and not _is_recoverable_error_job(job):
            return False
        # Admission-time barrier re-check. Everything else here reads state that an un-pause
        # CLEARS, so it can only answer "is this job paused right now" -- not "was it allowed
        # to stop being paused". The barrier is the one field an un-pause does not touch, so
        # re-reading it HERE, on the fire path, is what makes it a fence rather than a flag: a
        # job whose pause was lifted by any means, sanctioned or not, still does not run.
        # Above the ``force`` bypass on purpose -- ``force`` IS an un-pause.
        barrier = _resume_barrier(job)
        if barrier is not None:
            logger.error(
                "REFUSED fire of job_id=%s name=%s: resume barrier is set (%s). The job was "
                "un-paused without the barrier being cleared -- see clear_resume_barrier.",
                job_id, job.get("name"), barrier.get("reason"),
            )
            return False
        ...
        # Layer 2: the claim is stale or absent, so the clock says "go". Before overwriting it,
        # ask the durable execution ledger whether a run of this job is STILL ALIVE -- a 20-40
        # minute run is otherwise indistinguishable from a tick that died at minute 5.
        # Cross-process and PID-recycle-safe, and it refuses only on positive proof of life, so
        # a dead owner (or one whose liveness cannot be determined) still yields the job.
        #
        # Known, accepted false-refusal window: _run_one_job_admitted calls mark_job_run
        # (clearing the claim) immediately before finish_execution (closing the row), so a fire
        # landing between those two statements is refused. One statement wide, and it costs a
        # missed run rather than a duplicate. Swapping them would trade it for the 2026-07-27
        # defect where a crash in between loses the jobs.json write-back entirely.
...
class _DueScan:
    ...
    # Missed recurring fires the policy below refused. Recorded as data under the jobs
    # lock and emitted by the caller once it is released -- an event-bus emit is a SQLite
    # transaction and must never run inside this critical section.
...
def _fast_forward_missed_recurring(d: _DueJob, grace: int) -> bool:
    """Decide whether a recurring job past its scheduled instant fires this tick.

    Within grace, it simply fires: the sequential tick always observes a due job some seconds
    late, and treating that as a missed window made weekly and ``skip_only`` crons permanently
    unable to fire (security-audit-weekly, skipped at 12s and 27s late on 2026-06-01/06-08).

    Past grace the accumulated misses are collapsed -- ``next_run_at`` fast-forwards so a
    backlog cannot burst-fire on restart -- and the eligibility tree decides whether the job
    fires ONCE now (the perpetual-defer fix, #33315) or is skipped and recorded. The
    fast-forward is persisted immediately rather than left to advance_next_run/mark_job_run:
    it covers the crash window before mark_job_run and the external fire_due path, which never
    calls advance_next_run. mark_job_run re-anchors on completion, so the value is provisional.
    """
    missed_seconds = int((d.scan.now - d.next_run_dt).total_seconds())
    if missed_seconds <= grace:
        return True

    # ``recovery_policy`` is a flag-style string; ``skip_only`` is the only recognized value and
    # anything else falls through to the period / miss-size check. If it ever needs richer
    # shape, add an explicit shape check here rather than widening this comparison.
    period_seconds = _compute_period_seconds(d.schedule)
    if d.job.get("recovery_policy") == "skip_only":
        reason = "skip_only"
    elif period_seconds is None or period_seconds > 86400:
        reason = "default_period_cap"        # weekly or unknown period -- always skip
    elif missed_seconds > 86400:
        reason = "miss_exceeded_24h_cap"     # daily cron missed for more than a full period
    else:
        reason = None                        # daily-or-shorter, missed within 24h -> fire once
    ...
    if reason is not None:
        logger.info(
            "Job '%s' missed at %s (by %ds, reason=%s) -- skipping; advanced next_run_at to %s",
            d.label, d.next_run, missed_seconds, reason, new_next)
        d.scan.persist(d.job["id"], next_run_at=new_next)
        d.scan.skipped.append({
            "job_id": d.job["id"],
            "name": d.job.get("name", d.job["id"]),
            "missed_at": d.next_run,
    ...
    logger.info(
        "Job '%s' missed its scheduled time (%s, by %ds, grace=%ds). Running once on "
        "recovery; next run provisionally set to %s (re-anchored on completion)",
        d.label, d.next_run, missed_seconds, grace, new_next)
    d.scan.persist(d.job["id"], next_run_at=new_next)
    # Stamped, not emitted: this runs under _jobs_lock(). _emit_recovery_fire_triggers turns
    # the marker into a cron_triggered once the lock is released, and only for jobs that
    # actually survived the rest of the scan into ``due``.
    d.job[_RECOVERY_FIRE_MARKER] = {"missed_at": d.next_run}
...
def _evaluate_due_job(job: Dict[str, Any], scan: _DueScan, run_claim_ttl: float) -> bool:
    ...
    if not manual_run and recurring and not _fast_forward_missed_recurring(d, grace):
        return False
    if kind == "once":
...
def _get_due_jobs_locked() -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Inner implementation of get_due_and_skipped_jobs(); call with _jobs_lock held."""
    raw_jobs = load_jobs()
    ...
    for job in jobs:
        ...
        try:
            ...
            if not job.get("enabled", True):
                continue
            # Second half of the admission barrier re-check; see claim_job_for_fire for why the
            # barrier is re-read on the fire path rather than trusted from the pause flags. Both
            # gates are needed: this one keeps a barriered job out of the due list at all (so it
            # never reaches dispatch, and no CRON_TRIGGERED is emitted for a job that will only be
            # refused later), while the claim covers the manual and recovery paths that reach a
            # fire without going through here.
            if _resume_barrier(job) is not None:
                logger.warning(
                    "Skipping due job_id=%s name=%s: resume barrier is set (%s).",
                    job.get("id"), job.get("name"), (_resume_barrier(job) or {}).get("reason"))
                continue
