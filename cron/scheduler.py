...
# --- Fork imports upstream's header does not carry --------------------------
...
from cron.executions import amend_execution_after_abandon
from cron.jobs import amend_late_outcome_after_abandon
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    def _delivered(reason: str) -> str:
        """Wrap a compact reason in the standard notification header.

        A bare "Cron 'x' failed:" line carries no RAG indicator and does not match the
        CRON_FAILED message the alerts topic gets for the same run, so the two read as
        unrelated events. The bare line stays as the fallback when formatting is
        unavailable.
        """
        return _format_cron_delivery(
            job,
    ...
    if lower.startswith("script timed out"):
        return _delivered(
            "script timed out. No model was invoked. "
    ...
    if provider_reachable and (
        ...
        if "weekly usage limit" in lower:
            ...
            reason = "quota limit"
        return _delivered(
            f"provider {reason}. {_fallback_chain_phrase()} "
            "Full details saved in cron output."
        )
    ...
    if re.search(r"idle for \d+s\s*\(limit \d+s\)", lower):
        return _delivered(
            "the job itself stalled — no tool/API activity for the configured "
            "inactivity window. Not a provider or fallback-chain issue; check what "
            "the job was doing when it went quiet. Full details saved in cron output."
        )
    ...
    if provider_reachable and (
        ...
        return _delivered(
            f"provider timeout. {_fallback_chain_phrase()} "
            "Full details saved in cron output."
        )
    ...
    # Skew hint is appended to the REASON, so it lands inside the notification body
...
from cron.jobs import (
    ...
    clear_run_claim, get_due_and_skipped_jobs, heartbeat_fire_claim, heartbeat_run_claim, mark_job_run,
...
class _ReadWriteLock:
    ...
    Guards the process-global overrides a cron job applies for the whole of its
    run (see ``_job_mutates_process_globals``).  Such jobs are writers: they
    mutate shared state and need exclusive access.  Every other job is a reader:
    it only observes that state (indirectly, via ``_get_hermes_home()`` and the
    tools), so any number of them may run concurrently with each other, but none
    may run alongside a writer — that is exactly what stops a concurrently-firing
    job from resolving its ``script:`` slot under another job's profile.

    Historically the override was ``os.environ["TERMINAL_CWD"]`` for workdir jobs;
    since 0.21.1 a workdir is passed as the subprocess cwd / ``_SESSION_CWD``
    ContextVar (context-local), so only ``profile`` jobs still write.

    Writer preference bounds the wait for a writer (dispatched on the
    single-thread sequential pool) so a stream of readers cannot starve it.
    """
...
def _job_mutates_process_globals(job: dict) -> bool:
    ...
    * ``profile`` jobs go through :func:`_job_profile_context`, which sets the
    ...
    ``workdir`` jobs used to count too (they overrode ``os.environ["TERMINAL_CWD"]``);
    since 0.21.1 the workdir is per execution (subprocess cwd / the ``_SESSION_CWD``
    ContextVar), so a workdir alone is a pure reader and stays on the parallel pool.
    """
...
def _get_sequential_pool() -> concurrent.futures.ThreadPoolExecutor:
    """Persistent single-thread pool for jobs that mutate process-global state.

    One worker guarantees they never overlap, even ACROSS ticks: a job queued by a newer
    tick waits for the previous tick's sequential jobs instead of corrupting their
    os.environ state. Persistent (not per-tick) so a long job does not starve the ticker.
    """
...
def _run_no_agent_job(
    ...
    if not str(script_path or "").strip():
        ...
        _stamp_activity(activity, "blocked", "script_missing")
    ...
    if not ok:
        ...
        _stamp_activity(activity, "failed", "script_failed")
    ...
    if not _parse_wake_gate(output):
        ...
        _stamp_activity(activity, "no_work", "wake_gate_false")
        return True, f"{header}**Status:** silent (wakeAgent=false)\n", SILENT_MARKER, None

    if not output.strip():
        logger.info("Job '%s' (no_agent): empty stdout — silent run", job_id)
        _stamp_activity(activity, "no_work", "empty_stdout")
        return True, f"{header}**Status:** silent (empty output)\n", SILENT_MARKER, None

    # Process-level success only: the script produced output. Whether that output was
    # semantically correct, or was delivered, is not evidenced here.
    _stamp_activity(activity, "succeeded", "script_completed")
    ...
    try:
        ...
        for entry in get_fallback_chain(jc.cfg):
            ...
            try:
                ...
                # Preserve the attempted primary identity for the creation-snapshot
                # drift guard. A declared fallback activating after auth/network
                # failure is not a global config change and must not be blocked as one.
                runtime = dict(runtime)
                runtime["_cron_primary_provider"] = requested or ""
                runtime["_cron_primary_model"] = model
                logger.info(
...
def _hard_stop_agent(agent, thread_ident, job_name: str, message: str, label: str,
                     reason: str) -> dict:
    """Stop a live cron agent as hard as we can, and return its activity summary.

    Three escalating steps, because each reaches a case the previous one cannot:
    ``interrupt()`` only sets a flag the agent polls; a tool blocked in a SUBPROCESS never
    polls it, so the subprocess is killed by worker-thread ident; and a tool call that is
    neither polling nor in a subprocess (HTTP, MCP, an async handler) is cancelled by call
    frame. Every step is a no-op when nothing is in flight.
    """
    activity = {}
    if hasattr(agent, "get_activity_summary"):
        with contextlib.suppress(Exception):
    ...
    with contextlib.suppress(Exception):
        kill_run_tool_subprocesses(agent, thread_ident, label=label)
    with contextlib.suppress(Exception):
        cancel_run_tool_calls(agent, thread_ident, reason=reason, label=label)
    return activity

def _run_agent_with_watchdog(
    agent, prompt: str, job: dict, job_id: str, job_name: str, task_id: str, cancel_event,
    worker_state: Optional[dict] = None, session_id: str = "",
) -> dict:
    ...
    # Wall-clock (hard) bound on TOTAL duration, default 0 = unlimited. Upstream bounds
    # inactivity only, so a job that keeps making progress can still starve the queue:
    # 2026-04-26, jobflow-scout ran 22.8 min actively and blocked every other due job
    # because tick() runs profile jobs sequentially.
    ...
    _cron_start_time = time.monotonic()
    # Time the poll loop did not run AT ALL (host suspend, VM pause, severe starvation).
    # Not the job's to pay for -- both limits below charge ACTIVE time.
    ...
    # The worker's thread ident lets an operator stop or a timeout kill a tool subprocess
    # blocked on it, without relying on the agent to propagate its interrupt there.
    _run_thread: dict = {}
    ...
    def _idle_seconds() -> float:
        """Idle seconds with host-suspend credit removed.

        The agent's last-activity stamp predates a suspend, so raw idle is inflated by the
        full suspend duration and a slept laptop reads as a hung run. Fresh activity
        retires the credit: only a suspend since the last activity can be inflating this.
        """
        nonlocal _suspend_since_activity, _prev_idle
        ...
        try:
            ...
            return 0.0
        if _prev_idle is not None and raw < _prev_idle:
            _suspend_since_activity = 0.0
        _prev_idle = raw
        return max(0.0, raw - _suspend_since_activity)
    ...
    try:
        if _cron_inactivity_limit is not None:
            ...
            _watch_thread.start()
        # ALWAYS poll. Upstream short-circuits to a bare future.result() when no limit and
        # no claim applies, but the operator stop is serviced from this loop -- and a run
        # with every limit disabled is exactly the one an operator most needs to stop.
        result = None
        _last_poll = time.monotonic()
        while True:
            done, _ = concurrent.futures.wait({_cron_future}, timeout=_POLL_INTERVAL)
            _now = time.monotonic()
            # A gap far larger than the poll interval means THIS LOOP did not run.
            _gap_suspend = suspended_seconds(_now - _last_poll, _POLL_INTERVAL)
            if _gap_suspend > 0:
                _suspended_total += _gap_suspend
                _suspend_since_activity += _gap_suspend
                logger.warning(
                    "Job '%s': poll loop stalled %gs (host suspend or starvation) -- not "
                    "charging it to the run budget (total discounted %gs)",
            ...
            # Checked before the limits so a stop lands within one poll interval.
            _operator_stop = consume_stop_request(job_id, session_id)
            if _operator_stop is not None:
                break
            if wallclock_exceeded(_now - _cron_start_time, _suspended_total,
                                  _cron_hard_limit):
    ...
    if _operator_stop is not None:
        ...
        _label = f"Job '{job_name}' operator stop"
        _activity = _hard_stop_agent(
            agent, _run_thread.get("ident"), job_name, "Cron run stopped by operator",
            _label, f"operator stop (by={_stop_by})")
        logger.warning(
            "Job '%s' (session %s) stopped by operator request (by=%s at=%s reason=%s) "
            "| last_activity=%s | iteration=%s/%s | tool=%s",
            job_name, session_id, _stop_by, _stop_at, _stop_reason,
            _activity.get("last_activity_desc", "unknown"),
            _activity.get("api_call_count", 0), _activity.get("max_iterations", 0),
            _activity.get("current_tool") or "none")
        _stop_msg = (
            f"Cron job '{job_name}' stopped by operator "
            f"(session {session_id}, requested by {_stop_by} at {_stop_at}")
        if _stop_reason:
            _stop_msg += f", reason: {_stop_reason}"
        raise CronRunStoppedByOperator(_stop_msg + ")")

    if _wallclock_timeout:
        # Report ACTIVE elapsed, not raw: the 2026-08-25 incident logged a raw 24269s
        # against a 3600s limit, which read as a hung API call when the host had simply
        # been suspended for 6.67h. Raw is reported alongside so a real overrun stays
        # legible.
        _wc_raw = time.monotonic() - _cron_start_time
        _wc_elapsed = _wc_raw - _suspended_total
        _label = f"Job '{job_name}' wall-clock timeout"
        _activity = _hard_stop_agent(
            agent, _run_thread.get("ident"), job_name,
            "Cron job timed out (wall-clock)", _label, "wall-clock timeout")
        _last_desc = _activity.get("last_activity_desc", "unknown")
        logger.error(
            # %g, not %.0f: HERMES_CRON_HARD_TIMEOUT uses 0 for UNLIMITED, so a sub-second
            # limit rounded to "0s" would read as "no limit was set" exactly as it fired.
            "Job '%s' exceeded wall-clock limit %gs (active %gs of %gs raw; %gs "
            "discounted as host suspend) | last_activity=%s | iteration=%s/%s | tool=%s",
            job_name, _cron_hard_limit, _wc_elapsed, _wc_raw, _suspended_total,
            _last_desc, _activity.get("api_call_count", 0),
    ...
    if _inactivity_timeout:
        _label = f"Job '{job_name}' inactivity timeout"
        _activity = _hard_stop_agent(
            agent, _run_thread.get("ident"), job_name,
            "Cron job timed out (inactivity)", _label, "inactivity timeout")
        _last_desc = _activity.get("last_activity_desc", "unknown")
        _secs_ago = _activity.get("seconds_since_activity", 0)
        logger.error(
            # %g for the same sentinel hazard: HERMES_CRON_TIMEOUT documents 0 = unlimited.
            "Job '%s' idle for %gs (inactivity limit %gs) | last_activity=%s "
            "| iteration=%s/%s | tool=%s",
            job_name, _secs_ago, _cron_inactivity_limit, _last_desc,
            _activity.get("api_call_count", 0), _activity.get("max_iterations", 0),
            _activity.get("current_tool") or "none")
        raise TimeoutError(
            f"Cron job '{job_name}' idle for "
...
def _stamp_activity(activity: Optional[dict], process: str,
                    evidence_key: Optional[str] = None) -> None:
    """Record which terminal path a pre-agent gate took, for the activity recorder.

    An out-parameter rather than a return-value change: upstream tests call these gates
    directly, and ``activity=None`` leaves every caller behaving exactly as before.
    """
    if activity is None:
        return
    activity["process"] = process
    if evidence_key is not None:
        activity["evidence"] = _ACTIVITY_EVIDENCE[evidence_key]


def _prepare_job_prompt(
    job: dict, job_id: str, job_name: str, extra_prompt: Optional[str], cancel_event,
    activity: Optional[dict] = None, run_state: Optional[dict] = None,
    ...
    if not job.get("no_agent"):
        ...
        try:
            ...
            _stamp_activity(activity, "failed")
            return (False, f"# Cron Job: {job_name}\n\nError: {exc}\n", "", str(exc)), None

    # no_agent short-circuits BEFORE importing run_agent / opening SessionDB.
    if job.get("no_agent"):
        return _run_no_agent_job(job, job_id, job_name, cancel_event, activity=activity), None
    ...
    if script_path:
        ...
        if not _ran_ok and run_state is not None:
            # The agent turn still runs and reports the script error; only the recorded
            # status changes. Without this a dead pre-run script leaves last_status "ok".
            run_state["prerun_failure"] = _prerun_failure_error(script_path, _script_output)
    ...
    try:
        ...
        _stamp_activity(activity, "blocked", "prompt_injection_blocked")
        return (False, blocked_doc, "", str(block_exc)), None
    if prompt is None:
        logger.info("Job '%s': script produced no output, skipping AI call.", job_name)
        _stamp_activity(activity, "no_work", "empty_prompt")
...
def _resolve_cron_agent_setup(job: dict, job_id: str, job_name: str, jc) -> _CronAgentSetup:
    ...
    # No drift BLOCK here any more: _snapshot_pin (see _load_cron_job_config) already keeps an
    # unpinned job on the provider/model it was created under, so a global change cannot bill it
    # -- and cron.model / cron.model_provider can still route the fleet deliberately. Refusing on
    # top of that only stopped runs the snapshot pin was keeping alive. See #44585.
...
def _construct_cron_agent(AIAgent, job: dict, _cfg: dict, setup: _CronAgentSetup, *, workdir,
                          session_id, session_db, activity_recorder=None):
    ...
    return AIAgent(
        ...
        # Fork: the agent writes its runtime telemetry onto the activity row this fire
        # already opened (_open_cron_activity) and closes in _finish_cron_activity.
...
def _run_job_impl(
    ...
    # Observational telemetry opens BEFORE any branch, so deterministic and no-work fires
    # are recorded too -- otherwise it only ever sees jobs that reached a model. Unmapped
    # jobs and every telemetry failure execute exactly as they did before this existed.
    _activity_recorder = None
    _activity_outcome: dict = {}
    _activity_policy = _resolve_cron_activity_policy(job)
    if _activity_policy is not None:
        try:
            _activity_run_id = str(uuid.uuid4())
    ...
    def _close_activity(process: str, evidence: Optional[str] = None) -> None:
        refs = (evidence,) if evidence else ()
        _finish_cron_activity(_activity_recorder, process=process, evidence_refs=refs)

    _run_state: dict = {}
    early, prompt = _prepare_job_prompt(
        job, job_id, job_name, extra_prompt, cancel_event, activity=_activity_outcome,
        run_state=_run_state)
    if early is not None:
        _close_activity(
            _activity_outcome.get("process", "failed"),
            _activity_outcome.get("evidence"))
        return early
    from run_agent import AIAgent

    _cron_session_id = f"cron_{job_id}_{_hermes_now().strftime('%Y%m%d_%H%M%S')}"

    # A stop request on disk at this point was aimed at an EARLIER run: the CLI only files
    # one against a session it saw open, and this session did not exist yet. Drop it so it
    # cannot fire on this run. consume_stop_request's session match is the second guard.
    try:
    ...
    # Inference is about to start, so this run now crosses the wake/prompt boundary and has
    # a real session. The deterministic and no-work branches above deliberately never reach
    # here and stay session-less.
    ...
    try:
        ...
        _cfg = jc.cfg
        model = jc.model
        # The scheduler process discovers its own profile at startup, so a NAMED profile may
        # enable a tool-only plugin that was absent from that first sweep. Load the routed
        # profile's plugin tools before the agent is built, or the job silently runs without
        # them. Additive and fail-closed; profile jobs are already serialized because their
        # .env loading mutates process-global state.
        if active_profile is not None:
            # Bare, NOT suppressed: a profile whose tool contract fails must fail the run, not
            # run it without the tools it was routed to that profile for.
        ...
        agent = _construct_cron_agent(
            ...
            session_db=_session_db, activity_recorder=_activity_recorder)
        ...
        result = _run_agent_with_watchdog(
            ...
            worker_state=_worker_state, session_id=_cron_session_id)
        ...
        _close_activity("succeeded", f"session:{_cron_session_id}")
        _prerun_failure = _run_state.get("prerun_failure")
        if _prerun_failure is not None:
            # output and final_response are returned untouched, so delivery is identical.
            # Only the recorded status changes — which is what makes a dead pre-run script
            # visible to last_status and cron_failed.
            logger.error(
                "Job '%s' ran the agent but its pre-run script failed: %s",
                job_name, _prerun_failure)
            return False, output, final_response, _prerun_failure
        ...
        _close_activity("failed", f"session:{_cron_session_id}")
...
def run_job(
    ...
    """
    job_id = job["id"]
    # Serialize this job's process-global overrides against every other running cron job:
    # writers (see _job_mutates_process_globals) get the lock exclusively, everything else
    # holds it as a reader. The sequential pool only stops writers overlapping EACH OTHER;
    # the lock is what keeps a parallel-pool job from resolving its script: slot under this
    # job's profile home while the override is live. Acquire precedes the try so it cannot
    # leak; a leaked writer would deadlock every future job on acquire_*.
    _holds_global_write = _job_mutates_process_globals(job)
    if _holds_global_write:
...
def _run_one_job_admitted(
    ...
    _release_dispatch_admission=None, _abandoned=None, _deadline_box=None,
) -> bool:
    ...
    # Same-job concurrency guard (Guard #3). The built-in ticker enforces this inside
    # tick's per-job closure; registering here too puts the external-provider fire_due
    # path and direct API fires on the SAME registry, so a provider fire racing a tick
    # fire of one job_id is refused with CRON_SKIPPED_DUPLICATE rather than running twice.
    # Registered BEFORE the external-worker handoff below: a handoff that returns True is
    # still a fire, so the slot has to cover it, and the release lives in a finally.
    _job_id = job["id"]
    ...
    if _prior_in_flight is not None:
        _emit_cron_skipped_duplicate(
            _get_event_emitter(), _job_id, _cron_job_name, _prior_in_flight)
        try:
            finish_execution(
                execution_id, success=False,
                error="Duplicate fire blocked; prior run still in flight.")
        except Exception:
            logger.debug("finish_execution failed for duplicate-blocked %s", _job_id)
        return False

    # Guard #5: a live run of this job in ANOTHER process. Guard #3 above is per-process
    # only. Never raises — a ledger fault degrades to Guard #3 alone rather than costing
    # the tick its jobs; an unreachable ledger lets the fire proceed, so this can never
    # leave a job permanently unfireable.
    try:
        _live_elsewhere = live_execution_for_job(_job_id)
    except Exception:
        logger.warning(
            "Cross-process liveness check failed for job %s; running on the in-process "
            "guard alone", _job_id, exc_info=True)
        _live_elsewhere = None
    if _live_elsewhere is not None:
        with contextlib.suppress(Exception):
            _emit_cron_skipped_cross_process(
                _get_event_emitter(), _job_id, _cron_job_name, _live_elsewhere)
        # Release the slot Guard #3 just acquired, or the rejected fire's own slot blocks
        # the next legitimate one.
        _release_in_flight(_job_id)
        with contextlib.suppress(Exception):
            finish_execution(
                execution_id, success=False,
                error="Duplicate fire blocked; a run is live in another process.")
        return False

    # Min-interval fire guard: per-job ``min_seconds_between_fires``, else
    # HERMES_CRON_MIN_SECONDS_BETWEEN_FIRES, else 0 = off. Like Guards #3 and #5 above it
    # must RELEASE the slot it is rejecting from, or the rejected fire's own in-flight slot
    ...
    if _min_interval > 0 and _last_run_at_str:
        try:
            _elapsed = (_hermes_now() - _cron_ensure_aware(
                datetime.fromisoformat(_last_run_at_str))).total_seconds()
        except (TypeError, ValueError) as _parse_err:
            # Malformed last_run_at must never wedge the scheduler; let the job proceed.
            logger.debug(
                "Min-interval guard: failed to parse last_run_at=%r for job_id=%s (%s); "
                "proceeding without check", _last_run_at_str, _job_id, _parse_err)
            _elapsed = float("inf")
        if _elapsed < _min_interval:
            logger.warning(
                "Cron min-interval fire blocked: job_id=%s job_name=%s elapsed=%.1fs "
                "min=%ds last_run=%s",
                _job_id, _cron_job_name, _elapsed, _min_interval, _last_run_at_str)
            # Emit at the point the fire is ACTUALLY refused: a log line leaves a suppressed
            # fire unattributable on the event bus; the skip event is what the audit reads.
            _emitter = _get_event_emitter()
            if _emitter:
                try:
                    _emitter.on_job_skipped_min_interval(
                        job_id=_job_id, job_name=_cron_job_name, last_run_at=_last_run_at_str,
                        elapsed_since_last_seconds=round(_elapsed, 1),
                        min_seconds_between_fires=_min_interval)
                except Exception as ee:
                    logger.warning("Event emit failed for cron_skipped_min_interval: %s", ee)
            _release_in_flight(_job_id)
            with contextlib.suppress(Exception):
                finish_execution(
                    execution_id, success=False,
                    error="Fire blocked by the min-interval guard.")
            return False

    try:
        return _run_one_job_registered(
            job, adapters=adapters, loop=loop, verbose=verbose, extra_prompt=extra_prompt,
            cancel_event=cancel_event, execution_id=execution_id,
            _release_dispatch_admission=_release_dispatch_admission,
            _abandoned=_abandoned, _deadline_box=_deadline_box)
    finally:
        # Every exit path: duplicate refused above returns before this, handoff-returned,
        # completed, failed, or raised. EXCEPT when the deadline handler abandoned this run:
        # it already released the slot (start-time-checked), and a successor fire may have
        # registered a fresh record that must not be popped here.
        if not _deadline_has_elapsed(_abandoned, _deadline_box):
            _release_in_flight(_job_id)


def _run_one_job_registered(
    job: dict, *, adapters=None, loop=None, verbose: bool = False,
    extra_prompt: Optional[str] = None, cancel_event: Optional[_CancelEventLike] = None,
    execution_id: str = "", _release_dispatch_admission=None,
    _abandoned=None, _deadline_box=None,
) -> bool:
    """Body of _run_one_job_admitted, holding the fork's in-flight slot (Guard #3)."""
...
def run_one_job(
    ...
    *,
    ...
    ``_abandoned`` / ``_deadline_box`` come from the ticker's per-job soft-deadline wrapper
    (``_run_callable_with_deadline``); direct callers leave them unset."""
...
def _compose_run_delivery(
    ...
    if blocked_config and not success:
        ...
        # Script jobs are operational output landing in ops topics beside event-bus
        # notifications, so they get the same header; agent prose is left alone.
        deliver_content = _format_cron_output_for_delivery(job, final_response)
    else:
...
class _RunDelivery:
    ...
    # Set when the soft-deadline owner abandoned this run before its output was saved: the
    # caller amends the durable records with the real verdict and suppresses every other
    # side effect (see _run_callable_with_deadline).
    late_deadline: bool = False


def _save_compose_deliver(
    d: _RunDelivery, fence: _FireOwnership, final_response: str, output: str, *,
    adapters, loop, verbose: bool, execution_token, _abandoned=None, _deadline_box=None,
) -> None:
    ...
    # Output persistence is the one late side effect that still runs after the deadline
    # owner abandoned this worker; everything from here (compose, deliver, mark, events)
    # is suppressed and the caller amends the provisional verdict instead.
    if _deadline_has_elapsed(_abandoned, _deadline_box):
        d.late_deadline = True
        return
...
def _run_one_job_body(
    ...
    execution_token: Optional[object] = None, _release_dispatch_admission=None,
    _abandoned=None, _deadline_box=None,
) -> bool:
    ...
    # One span per cron fire, covering run + delivery + event emission. Placed here
    # because upstream made this the single shared body, so it covers the ticker and
    # external providers alike.
    _cron_cm = _start_cron_span(f"cron.run_job:{_job_name}")
    try:
        _cron_span = _cron_cm.__enter__()
    except Exception:
        # start_as_current_span returns a LAZY context manager, so an SDK/API version
        # skew surfaces HERE, outside _start_cron_span's own guard. OTel must never
        # break a fire: degrade to the no-op span.
        _cron_cm = _NoopCronSpan()
        _cron_span = _cron_cm.__enter__()
    with contextlib.suppress(Exception):
        _cron_span.set_attribute("cron.job_id", job["id"])
        _cron_span.set_attribute("cron.job_name", _job_name)
        _cron_span.set_attribute("cron.schedule", str(job.get("schedule_display", "")))
    # SR-535: session.id groups a day's fires of one job; user.id is its runtime profile.
    _stamp_cron_langfuse(_cron_span, job, _job_name)
    try:
        ...
        # cron_started. The event id is recorded on the in-flight slot so a later
        # duplicate-skip can reference the live cron_started for cross-event correlation.
        if emitter:
            try:
                _attach_started_event_id(
                    job["id"],
                    emitter.on_job_started(
                        job_id=job["id"], job_name=_job_name,
        ...
        # The attempt is now durable, so the settlement census can see this work
        # independently and dispatch admission must not stay held through the model run
        # and delivery -- an incident barrier has to be able to exclude NEW work while
        # this one finishes. Same release point the fork used.
        ...
        try:
            ...
            if _deadline_box is not None:
                # The deadline owner finalizes a first-failure firecrawl event on abandon.
        ...
        # A run can exit successfully while the agent's own structured workload block says
        # it failed. Demote BEFORE composing the notice, so the delivered message and
        # last_status agree with the workload rather than with the process exit.
        ...
        try:
            _save_compose_deliver(
                ...
                execution_token=execution_token, _abandoned=_abandoned,
                _deadline_box=_deadline_box)
        ...
        if d.late_deadline:
            # The deadline owner already emitted its alert, released the slot, and suppressed
            # delivery. Preserve those race boundaries, but replace its PROVISIONAL failure in
            # the durable records with the worker's real terminal verdict.
            if d.success and _is_interrupted(job["id"], execution_token):
                d.success = False
                d.error = (
                    "Interrupted by gateway shutdown before the run finished "
            ...
            amended = _amend_late_deadline_outcome(
                job, success=d.success, error=d.error, deadline_box=_deadline_box)
            logger.warning(
                "Job '%s': finished %.1fs after deadline abandon -- "
                "delivery suppressed; durable outcome amended=%s (success=%s)",
                job["id"], time.monotonic() - _run_started_monotonic, amended, d.success)
            with contextlib.suppress(Exception):
        ...
        _finished = _finish_completed_run(d, fire_owner, execution_id)
        _run_success = d.success

        # cron_completed / cron_failed. consecutive_errors is the severity field
        # artifact_surface and the scheduler's alerting read, so it is re-read from the
        # stored record rather than inferred here.
        if emitter:
            try:
                _current = next(
                ...
                emitter.on_job_completed(
                    ...
                    output_summary=(
                        _summarize_for_event_bus(final_response) if d.success else None),
                    error=d.error,
                    consecutive_errors=(
                        _current.get("consecutive_errors", 0) if _current else 0),
                )
            except Exception as ee:
                logger.warning(
                    "Event emit failed for cron_completed/cron_failed: %s", ee)

        # Tailor / generic agent-iteration structured events (gated inside the helpers).
        if d.success and emitter:
        ...
        if _deadline_has_elapsed(_abandoned, _deadline_box):
            # A deadline-abandoned worker can fail by raising instead of returning a
            # structured failure. Preserve that real cause too; the deadline owner already
            # marked, finished and alerted, so no delivery or bookkeeping runs here.
            _amend_late_deadline_outcome(
                job, success=False, error=_err_text, deadline_box=_deadline_box)
        ...
        # Emitted on EVERY exit, the raised path included — that is where it is most
        # diagnostic. Never allowed to mask the outcome, and the firecrawl reset runs even
        # if the emit raises, so crawl state cannot leak into the next run on this thread.
        with contextlib.suppress(Exception):
            _cron_span.set_attribute("cron.success", bool(_run_success))
            _cron_span.set_attribute(
                "cron.duration_s", round(time.monotonic() - _run_started_monotonic, 3))
        with contextlib.suppress(Exception):
            _cron_cm.__exit__(None, None, None)
        try:
            try:
                # Unconditional, deadline or not: the deadline owner finalizes any credits already
                # recorded at abandonment, and if the 402 arrives LATER this worker performs the
                # same failure-only attempt. The run-state claim lock (_claim_credits_iteration)
                # makes the two paths one-shot -- gating this on the deadline instead loses the
                # event entirely whenever the credits failure lands after abandonment.
                _finalize_agent_iteration_event(
                    emitter, job, final_response or "", success=_run_success,
            ...
            logger.debug("agent-iteration finalize failed", exc_info=True)
...
def _process_due_job(
    job: dict, adapters, loop, verbose: bool, *, _abandoned=None, _deadline_box=None,
) -> bool:
...
def _submit_with_guard(job: dict, pool: concurrent.futures.ThreadPoolExecutor, process_job):
    ...
    if not try_register_running_job(job_id):
        ...
        # Emit at the point the duplicate is ACTUALLY blocked. Upstream's running-set
        # guard only logs, which leaves a suppressed fire unattributable on the event bus;
        # the fork treats a blocked duplicate as an observable outcome, not a log line.
        with contextlib.suppress(Exception):
            with _in_flight_lock:
                _prior = _in_flight.get(job_id)
            if _prior is not None:
                _emit_cron_skipped_duplicate(
                    _get_event_emitter(), job_id, job_label, _prior)
        return None
    ...
    # The fork's per-job SOFT DEADLINE: the submitted callable runs under
    # _run_callable_with_deadline on a "cron-job-<id>" worker thread.
    #   - parallel-safe jobs: abandon_on_timeout=True -- a runaway worker is marked failed
    #     and its slot released so the next fire is not blocked.
    #   - writers (see _job_mutates_process_globals): abandon_on_timeout=False -- alert-only,
    #     because abandoning one mid os.environ snapshot/restore would corrupt the next
    #     job's environment.
    _abandon_on_timeout = not _job_mutates_process_globals(job)

    def _run_and_release(j=dispatched_job, ctx=_ctx, abandon=_abandon_on_timeout):
        try:
            return _run_callable_with_deadline(j, process_job, abandon, ctx)
        finally:
...
def _tick_admitted(
    ...
    try:
        ...
        # Emit CRON_SKIPPED for any job whose missed fire was fast-forwarded. Before
        # due-job processing, so visibility is immediate even on a tick with zero due
        # jobs. Failures emitting events must never block tick progress.
        if skipped_jobs:
            try:
                _emitter = _get_event_emitter()
                if _emitter is not None:
                    for _s in skipped_jobs:
                        _emitter.on_job_skipped(
                            job_id=_s["job_id"],
                            job_name=_s["name"],
                            missed_at=_s["missed_at"],
                            missed_seconds=_s["missed_seconds"],
                            schedule_kind=_s["schedule_kind"],
                            reason=_s["reason"],
                        )
            except Exception:
                logger.exception("Failed to emit cron_skipped events")

        # Event-driven wakes, drained BEFORE the no-work early return so a tick with zero
        # SCHEDULED jobs still runs work an event asked for.
        woken_jobs = _collect_woken_jobs(exclude_ids={j["id"] for j in due_jobs})

        if not due_jobs and not woken_jobs:
        ...
        # Woken jobs execute alongside the due ones but are appended AFTER the advance
        # loop on purpose: an event says "there is work now", not "the schedule moved",
        # so their next_run_at is left exactly where it was.
        ...
        def _process_job(job: dict, _abandoned=None, _deadline_box=None) -> bool:
            # ``_abandoned`` / ``_deadline_box`` are set by the soft-deadline wrapper when
            # this run was already declared failed at its deadline (see _submit_with_guard).
        ...
        # Jobs that override process-global state for their whole run (a ``profile`` whose
        # context snapshots/restores os.environ and installs a Hermes-home override; a
        # workdir is per execution since 0.21.1) must not run beside the parallel pool: while one is active,
        # _get_hermes_home() returns ITS home for every thread, so a concurrent job resolves
        # its script: slot under the wrong profile. That failed silently — 8 'Script not
        # found' failures across 6 jobs on 2026-08-19/20, every one concurrent with a
        # financier-profile job, all with last_status 'ok'.
        _sequential_jobs = [j for j in due_jobs if _job_mutates_process_globals(j)]
        _parallel_jobs = [j for j in due_jobs if not _job_mutates_process_globals(j)]

        def _dispatch(job, pool) -> None:
            fut = _submit_with_guard(job, pool, _process_job)
            if fut is None:
                return
            _all_futures.append(fut)
            # A wake is consumed only once its job is really dispatched.
            _wake = job.get("_durable_wake")
            if _wake is not None and not wake_channel.ack_wake(_wake):
                logger.warning(
                    "cron wake for %s changed before submit acknowledgement; "
                    "replacement retained", job["id"])
            if not sync:
                _results.append(True)  # optimistically counted

        # Sequential pass first: a single worker, so these never overlap each other either.
        if _sequential_jobs:
            _seq_pool = _get_sequential_pool()
            for job in _sequential_jobs:
                _dispatch(job, _seq_pool)
        if _parallel_jobs:
            _par_pool = _get_parallel_pool(_max_workers)
            for job in _parallel_jobs:
                _dispatch(job, _par_pool)

        # Submission is the acting boundary. Waiting for execution to COMPLETE must not
        # retain dispatch admission: an incident barrier needs to exclude new submissions,
        # not wait for work the settlement census can already see.
        if _release_dispatch_admission is not None:
            _release_dispatch_admission()
...
# --- Re-exports -------------------------------------------------------------
# Upstream 0.21 split these functions into sibling modules. It re-imports some of
# them here for its own use, but not all -- and 45 fork test files (plus
# cron/suggestions.py) still import them from cron.scheduler. A missing one is an
# ImportError at COLLECTION time: the whole test file never runs, rather than fails.
# Only the names upstream does not already import are listed; adding the rest would
# be an F811 redefinition. Bound at the BOTTOM, mirroring the late sibling-to-parent
# binding those modules do at their own end, so this module is fully defined first.
