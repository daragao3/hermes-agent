...
def _shlex_tokens(line: str) -> list[str]:
    """POSIX-tokenize one shell line, preserving native Windows path tokens.

    POSIX shlex consumes backslashes in an unquoted drive path. Convert only
    drive-path-shaped runs to forward slashes first; POSIX escapes elsewhere
    retain their shell meaning, and Windows accepts the normalized path.
    """
    if os.name == "nt":
        line = re.sub(
            r'(?<![\w])([A-Za-z]:\\[^\s;"&|()]+)',
