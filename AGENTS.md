...
Gateway logs also include `gateway-forensics.log` (unfiltered INFO+ catch-all);
`gateway.log` contains gateway-component records only. Override the forensics path
with `HERMES_GATEWAY_LOG_FILE` in root `~/.hermes/.env` (not profile-scoped;
see `hermes_cli/env_loader.py`).
