...
def _visible_env_keys() -> set:
    ...
    if scope is not None:
        # Match the actual credential reader: scoped secrets plus permitted
        # process settings, without loading a sibling profile's dotenv file.
        return set(scope) | {
            name for name in os.environ if _get_secret(name, None) is not None
        }
    keys = set(os.environ)
    try:
