...
def shared_loop_targets(registry) -> list:
    ...
    from gateway.config import _platform_registry_names
    for name in _platform_registry_names(registry, plugin_only=True) if registry is not None else ():
...
def apply_plugin_yaml_hooks(yaml_cfg: dict, gateway_platforms: Any, platforms_data: dict, registry) -> None:
    ...
    while pending:
        ...
        for extra_name in _platform_registry_names(registry, plugin_only=False):
            if extra_name not in seen and extra_name not in pending:
                pending.append(extra_name)
