...
from gateway.config import (
    Platform, _dict_slot, _normalize_choice, _platform_registry_names)
...
def shared_loop_targets(registry) -> list:
    ...
    # Names only — plugin_entries() would import every bundled adapter's SDK to answer this.
    for name in (_platform_registry_names(registry, plugin_only=True)
...
def apply_plugin_yaml_hooks(yaml_cfg: dict, gateway_platforms: Any, platforms_data: dict, registry) -> None:
    ...
    if registry is None:
        return
    # Plugin-owned YAML→env config bridges (#24836). See ``PlatformEntry.apply_yaml_config_fn``
    # for the hook contract. Order: shared-key loop (above) → this dispatch → legacy hardcoded
    # blocks (below; no-op when a hook already set their env var) → ``_apply_env_overrides()``
    # after ``GatewayConfig.from_dict``.
    #
    # A WORKLIST rather than all_entries(). The hook can only fire for a platform the user
    # actually configured, and the unconfigured case already continues — so testing the config
    # block BEFORE materializing the entry is behaviour-preserving, and it means an unconfigured
    # platform's SDK is never imported just to discover it has no hook to run. It is a worklist
    # because resolving one platform can register SECONDARY platforms under names no manifest
    # declared (wecom's loader also registers 'wecom_callback'); those used to be covered because
    # all_entries() resolved everything first. None of them currently define
    # apply_yaml_config_fn, but re-scanning after each resolve costs nothing and keeps that from
    # becoming a silent gap.
    pending = _platform_registry_names(registry, plugin_only=False)
    seen: set = set()
    while pending:
        name = pending.pop(0)
        if name in seen:
        ...
        if not isinstance(platform_cfg, dict):
            continue
        entry = registry.get(name)   # resolves only a CONFIGURED platform
        for extra in _platform_registry_names(registry, plugin_only=False):
        ...
        try:
            ...
            logger.debug("apply_yaml_config_fn for %s raised: %s", name, e)
            continue
        if isinstance(seeded, dict) and seeded:
            _dict_slot(_dict_slot(platforms_data, name), "extra").update(seeded)
