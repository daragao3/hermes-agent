...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        # real_executable(), not sys.executable: on a Microsoft-Store CPython host that is an
        # MSIX app-execution alias, whose child starts SUSPENDED pending an activation handoff
        # — one stranded process per webhook if the gateway restarts mid-handoff.
        from hermes_constants import real_executable
        interpreter = (shutil.which("bash") or ("/bin/bash" if os.path.isfile("/bin/bash") else None)) if is_shell else real_executable()
        if interpreter is None:
            logger.warning("[webhook] script ignored webhook: bash not found")
            return False, None
        try:
            from tools.environments.local import build_subprocess_env
            # run_text_capture, not subprocess.run(capture_output=True): a filter script is
            # arbitrary user code, so a grandchild inheriting the capture pipes is not a risk
            # but an expectation — and this runs inside the gateway, where the resulting
            # unbounded hang is silent. ``input`` is staged through a temp file for the same
            # reason (a pipe needs communicate()'s writer thread, which parks the same way),
            # and the helper sets CREATE_NO_WINDOW itself, replacing the creationflags below.
            from hermes_cli._subprocess_compat import run_text_capture
            result = run_text_capture(
                [interpreter, str(path)], input=json.dumps(payload),
                timeout=self.script_timeout_seconds, cwd=str(path.parent), env=build_subprocess_env(),
            )
