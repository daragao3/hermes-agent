...
class BasePlatformAdapter(ABC):
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        from gateway.status import (
            _get_scope_lock_path, _read_json_file,
        ...
        # Who held it before us, read BEFORE the acquire overwrites the record.
        _prior_record = _read_json_file(_get_scope_lock_path(scope, identity))
        _prior_pid = (int(_prior_record.get("pid", 0))
                      if isinstance(_prior_record, dict) else 0) or None
        # pid_recheck_after_seconds: on Windows a process that has ALREADY been terminated
        # still answers the liveness probe as running until it actually exits, so a restart
        # reads the outgoing gateway as a live foreign holder and declares FATAL. Re-probing
        # after a short sleep distinguishes 'dying' from 'alive'.
        acquired, existing = acquire_scoped_lock(
            scope, identity, metadata=lock_meta, pid_recheck_after_seconds=2.0)
        if acquired:
            self._synthesize_previous_gateway_stopped_if_needed(_prior_pid, scope)
            return True
        if (self._platform_lock_takeover_allowed and not self._platform_lock_takeover_attempted
            ...
            if owner_pid is not None:
                ...
                # Same recheck, and here it matters most: we have just TERMINATED the
                # holder, which is exactly when a killed PID still reports alive.
                acquired, existing = acquire_scoped_lock(
                    scope, identity, metadata=lock_meta, pid_recheck_after_seconds=2.0)
                if acquired:
                    logger.info("[%s] Acquired %s after taking over PID %d", self.name, resource_desc, owner_pid)
                    self._synthesize_previous_gateway_stopped_if_needed(owner_pid, scope)
                    return True
    ...
    def _synthesize_previous_gateway_stopped_if_needed(
        self, prev_pid: Optional[int], scope: str,
    ) -> None:
        """Emit GATEWAY_STOPPED on behalf of a dead previous lock holder.

        Called only on the takeover-success path: we just acquired a lock previously held by
        ``prev_pid``, and the liveness recheck confirmed that PID is now dead. WMI Terminate
        (Windows ``TerminateProcess``) bypasses the signal-handler / atexit /
        graceful-shutdown paths, so that gateway never emitted its own STOP and the bus is
        left with a START that has no STOP. This closes that audit gap.

        ``source`` differs from a normal stopped emit so a consumer can distinguish
        self-emitted from synthesized — we do not impersonate the dead process, we declare
        ourselves the synthesizer. Skipped for a clean acquire (no previous owner) or when
        the prior PID is our own. Best-effort; never raises."""
        if not prev_pid or prev_pid == os.getpid():
            return
        try:
            ...
            logger.warning(
                "[%s] Synthesized GATEWAY_STOPPED for dead previous PID %d (took over %s lock)",
                self.name, prev_pid, scope)
        except Exception:
            logger.debug("synthesize_previous_gateway_stopped failed", exc_info=True)
    ...
    def extract_local_files(content: str) -> Tuple[List[str], str]:
        ...
        path_re = re.compile(
            # ``~[/\\]``, not ``~/``: a Windows home-relative path is written ``~\Documents``,
            # and every other alternation here already accepts both separators.
