...
def _parse_docker_volume_mounts() -> List[Tuple[Path, PurePosixPath]]:
    ...
    for entry in parsed if isinstance(parsed, list) else ():
        ...
        # A forward-slash Windows drive prefix is part of the host path.
        sep = spec.find(":/", 2 if len(spec) > 1 and spec[1] == ":" else 0)
        if sep <= 0:
            continue
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    container_candidate = PurePosixPath(candidate.as_posix())
    ...
    for host_root, container_root, _score in sorted(matched, key=lambda m: -m[2]):
        translated = _resolve_path(host_root / container_candidate.relative_to(container_root), strict=True)
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    try:
        ...
        return None
    # Docker paths are absolute in the container's POSIX domain, not necessarily
    # in the host's Windows path domain. Only a verified mount may translate one.
    resolved = _translate_docker_container_media_path(expanded, session_key=session_key)
    if resolved is None:
        if not expanded.is_absolute():
            return None  # Never reinterpret an unmapped container path on the current drive.
...
class BasePlatformAdapter(ABC):
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        from gateway.status import (
            ...
            _read_json_file, _get_scope_lock_path, _pid_from_record, pid_exists)
        self._platform_lock_scope, self._platform_lock_identity = scope, identity
        lock_meta = {"platform": self.platform.value}
        prior_pid = _pid_from_record(_read_json_file(_get_scope_lock_path(scope, identity)))
        acquired, existing = acquire_scoped_lock(
            scope, identity, metadata=lock_meta, pid_recheck_after_seconds=2.0)
        if acquired:
            # Staleness may also mean PID reuse or non-gateway ownership, not death.
            if prior_pid and prior_pid != os.getpid() and not pid_exists(prior_pid):
                self._synthesize_previous_gateway_stopped(prior_pid, scope)
            return True
