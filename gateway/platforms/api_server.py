...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    from hermes_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config as _get_model_config
    try:
...
if AIOHTTP_AVAILABLE:
    ...
    async def security_headers_middleware(request, handler):
        ...
        for k, v in _SECURITY_HEADERS.items():
            # Only the locally served MiniApp document may be embedded by Telegram.
            # API responses and missing-document errors keep the strict defaults.
            if k == "X-Frame-Options" and response.get("hermes.miniapp_document", False):
                continue
...
try:
    from cron.jobs import (
        ...
        trigger_job as _cron_trigger, JobPaused as _CronJobPaused)
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    async def _handle_miniapp_index(self, request: "web.Request") -> "web.StreamResponse":
        ...
        response["hermes.miniapp_document"] = True
        response.headers["Content-Security-Policy"] = (
            "default-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'; "
            "connect-src 'self'; img-src 'self' data:; font-src 'self'; "
            "base-uri 'none'; form-action 'none'; frame-ancestors https://web.telegram.org"
        )
        return response
    ...
    def _job_response(self, fn, job_id: str, *, notify: bool) -> "web.Response":
        ...
        try:
            ...
            return web.json_response(
                {"error": str(paused), "paused_reason": paused.paused_reason,
                 "paused_at": paused.paused_at}, status=409)
        except Exception as e:
    ...
    async def _handle_pause_job(self, request: "web.Request") -> "web.Response":
        ...
        return await self._job_lookup_or_mutate(request, lambda jid: _cron_pause(jid, caller="http_api:api_server"), notify=True)

    async def _handle_resume_job(self, request: "web.Request") -> "web.Response":
        """POST /api/jobs/{job_id}/resume — resume a paused cron job."""
        return await self._job_lookup_or_mutate(request, lambda jid: _cron_resume(jid, caller="http_api:api_server"), notify=True)
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        return self._job_response(
            lambda jid: _cron_trigger(jid, extra_prompt=extra_prompt, caller="http_api:api_server", reason=request.query.get("reason") or None), job_id, notify=False)
