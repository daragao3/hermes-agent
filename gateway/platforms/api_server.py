...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        routes: List[tuple] = [
            ...
            # Devflow: profile-scoped, so these DO ride the /p/<profile>/ mirror loop —
            # unlike the MiniApp routes registered directly in connect().
    ...
    def _job_response(self, fn, job_id: str, *, notify: bool) -> "web.Response":
        ...
        try:
            ...
            # 409 and not the generic 500 below: a held job's whole point is to explain
            # itself, and a 500 says nothing at all. 409 rather than 404 so a client can
            # tell "held" from "gone", and rather than 403 because the refusal is about the
            # job's state, not the caller's rights — the same request succeeds once someone
            # resumes the job on purpose.
            return web.json_response(
                {
                    "error": str(paused),
    ...
    async def _handle_pause_job(self, request: "web.Request") -> "web.Response":
        ...
        return await self._job_lookup_or_mutate(
            request, lambda jid: _cron_pause(jid, caller="http_api:api_server"), notify=True)

    async def _handle_resume_job(self, request: "web.Request") -> "web.Response":
        """POST /api/jobs/{job_id}/resume — resume a paused cron job."""
        return await self._job_lookup_or_mutate(
            request, lambda jid: _cron_resume(jid, caller="http_api:api_server"), notify=True)

    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        """POST /api/jobs/{job_id}/run — trigger immediate execution.

        Threads ``caller="http_api:api_server"`` plus an optional ``?reason=`` query string
        into ``trigger_job`` so every off-schedule fire from this route lands in the audit
        log via the cron_triggered event WITH attribution — the postmortem gap the
        2026-04-30 sentinel-vip-morning triple-fire investigation opened
        (sentinel-vip-burst-rc-2026-04-30.md §5).

        A paused job answers 409 carrying ``paused_reason``/``paused_at`` (raised through
        ``_job_response``), NOT 200 having quietly un-paused it (2026-08-26)."""
        ...
        return self._job_response(
            lambda jid: _cron_trigger(
                jid, extra_prompt=extra_prompt, caller="http_api:api_server", reason=reason),
            job_id, notify=False)
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # MiniApp routes. Registered directly, NOT through _http_route_table() — that
            # table is mirrored under /p/{profile} by the loop above, and the MiniApp surface
            # is a WebUI convenience API that is not profile-scoped. Routing it through the
            # table would silently mint a second, profile-prefixed copy of every route here.
            ...
            # Static assets for the MiniApp SPA — always at ~/.hermes/miniapp/ (not
            # profile-scoped, since the assets are shared across profiles).
...
class _CronJobPaused(Exception):
    """Stand-in so ``_job_response``'s except clause still binds when cron is unavailable.

    Unreachable in practice — ``_cron_request_guard`` has already short-circuited the
    request by then."""
