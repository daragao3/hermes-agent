...
import atexit  # gateway exit paths register a GATEWAY_STOPPED fall-through
...
# NOT ``get_hermes_home()``. Evaluated at import, that constant froze the launch
# home, so a test or embedder re-pointing HERMES_HOME afterwards still read -- and
# WROTE -- the user's real ~/.hermes. This is now an override SLOT, default None,
# and every use site goes through _resolve_hermes_home() below.
...
# cannot leak a stale home. The name is kept only because seven tests
# monkeypatch.setattr it, which raises when the attribute is absent. Do NOT add a
# runtime read -- that would make it a second import-time snapshot, which is the
# whole defect this seam removes.
...
# Consumed on the very next line, before any override can be installed, so the
# process home at import IS the correct home here. Do NOT reuse this name later --
# a deferred read would make it a second snapshot.
...
def _gateway_config_home() -> Path:
    ...
    return Path(override) if override else _resolve_hermes_home()
...
def _resolve_hermes_bin() -> Optional[list[str]]:
    ...
    try:
        ...
        if importlib.util.find_spec("hermes_cli") is not None:
            # real_executable(), not sys.executable: this argv re-execs Hermes for an
            # update, and a shim/alias interpreter path would not re-exec what it names.
...
class GatewayRunner(
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        # Secondary adapters are authorized against their OWN immutable startup config,
        # not the default profile held in self.config. Populated alongside
        # _profile_adapters and consulted by every source-profile-aware slash decision.
    ...
    # Evaluated in the class body this bound the REAL ~/.hermes under pytest, so
    # _save_voice_modes() wrote the user's live voice state instead of the per-test
    # home. Kept as a plain overridable class attribute rather than a property,
    # because existing tests set it on the INSTANCE and a data descriptor would
    # break them.
    # Platforms whose boot-time connect is FIRED rather than awaited. WhatsApp's bridge
    # (localhost:3000) is frequently down and its connect blocks for the full 30s platform
    # timeout on every boot; awaiting it inline gated the api_server :8642 bind, and so every
    # gateway restart and watchdog recovery, on that 30s (boot forensics 2026-07-10).
    #
    # Telegram joined for a second reason (death forensics 2026-07-16): a NordVPN/DNS flap
    # made all name resolution fail, so its inline boot connect blocked on the reconnect
    # ladder and the gateway could not reach running until DNS recovered — a ~10-minute blip
    # became a ~27-minute restart storm. Backgrounding lets the gateway boot (api_server
    # bound, cron and event bus up) during the outage; the reconnect watcher wires the
    # platform in once the network returns.
    ...
    async def _connect_platform_in_background(
        ...
        # Delivery-ledger seam (v0.19.0 merge): startup's
        # _redeliver_pending_obligations() sweep only claims rows for
        # platforms already present in ``self.adapters``, and backgrounding
        # this platform's connect means it was almost certainly NOT connected
        # when that sweep ran. Re-sweep now that the adapter is live so final
        # responses owed on this platform are redelivered this boot instead
        # of waiting out the stale cutoff. Runs BEFORE the resume scheduling
        # below (same ordering contract as startup: a session whose output we
        # hold must not be re-run). Safe against the startup sweep racing us:
        # sweep_recoverable() skips rows owned by a live process and claims
        # atomically, so a row is claimed at most once per boot — no
        # double-send.
        try:
            await self._redeliver_pending_obligations()
        except Exception:
            logger.debug(
                "post-background-connect delivery-ledger sweep failed",
                exc_info=True,
            )
...
def _clear_planned_stop_marker_quiet() -> None:
    """Best-effort: scoped to one target, so a stale marker griefs an unrelated shutdown."""
    try:
...
async def _drain_incumbent_via_marker(pid: int, timeout: float) -> bool:
    ...
    # Upstream's waiter is async and takes (attempts, delay); the fork's took a timeout.
    # Awaiting it keeps the loop responsive during the drain instead of blocking it.
    return await _wait_for_pid_exit(pid, max(1, int(timeout / 0.5)), 0.5)


async def _request_incumbent_shutdown(existing_pid: int, *, timeout: float) -> bool:
    ...
    """
    if _IS_WINDOWS:
        return await _drain_incumbent_via_marker(existing_pid, timeout)
    ...
    return await _wait_for_pid_exit(existing_pid, max(1, int(timeout / 0.5)), 0.5)
...
async def _start_gateway_replace_existing_instance(existing_pid: int, replace: bool) -> bool:
    ...
    if _replace_target_belongs_to_other_profile(existing_pid):
        ...
        return False
    # Refuse to signal a gateway that is still coming up: it has not bound :8642 yet, so
    # killing it leaves the port dead through our own boot too and the operator sees one
    # outage spanning two launches. BEFORE the takeover marker, so a refusal leaves no
    # state behind for the incumbent to misread. Fails open on an unknown age.
    ...
    if _incumbent_age is not None:
        ...
        logger.error(
            "Refusing --replace: gateway PID %d is only %.0fs old and is still booting "
            "(grace %.0fs). Something else is already starting a gateway; wait for it, "
            "or set %s=0 to override.",
            existing_pid, _incumbent_age, _grace, _REPLACE_MIN_AGE_ENV)
        print(
            f"\n❌ Refusing to replace gateway PID {existing_pid} — it is only "
            f"{_incumbent_age:.0f}s old and still booting.\n"
            f"   Another launch is already in flight. Wait for it to bind :8642,\n"
    ...
    # ASK, then wait, then escalate. On Windows a SIGTERM is TerminateProcess -- no drain,
    # in-flight cron work lost -- so this writes the planned-stop marker the incumbent's own
    # watcher polls, which drives its normal shutdown handler and drain. Either way it gets
    # at least its OWN shutdown budget, instead of a fixed wait short enough to guarantee a
    # mid-drain kill whenever a drain actually took time.
    _drain_budget = _replace_drain_timeout()
    logger.info(
        "Asking gateway PID %d to shut down gracefully (up to %.0fs) so in-flight cron "
        "work can drain.", existing_pid, _drain_budget)
    try:
        old_gateway_exited = await _request_incumbent_shutdown(
            existing_pid, timeout=_drain_budget)
    except (PermissionError, OSError):
        logger.error("Permission denied killing PID %d. Cannot replace.", existing_pid)
        # Both markers are scoped to this target; a stale one griefs an unrelated shutdown.
        _clear_takeover_marker_quiet()
        _clear_planned_stop_marker_quiet()
        return False
    if not old_gateway_exited:
        logger.warning(
            "Old gateway (PID %d) did not exit after a graceful stop request within %.0fs, "
            "sending SIGKILL. In-flight cron work may be lost.", existing_pid, _drain_budget)
        try:
...
def _start_gateway_configure_logging(verbosity: Optional[int]) -> None:
    ...
    def _security_audit() -> None:
        ...
        log_startup_security_warnings(
            hermes_home=_resolve_hermes_home(), config=_best_effort(_raw_cfg))
...
def _start_gateway_make_shutdown_signal_handler(runner, _signal_initiated_shutdown: list):
    ...
    def shutdown_signal_handler(received_signal=None):
        ...
        if planned_takeover:
            ...
            # Caller #3, defence in depth (M1, 2026-04-30). _stop_impl runs via
            # create_task(runner.stop()) below and emits the canonical event with full
            # context; this covers the rare loop-teardown race where that task is never
            # scheduled, so the cause is still recorded. Idempotent — a no-op if _stop_impl
            # wins. Unplanned signals only: a planned stop or takeover takes a branch above
            # and reaches _stop_impl normally.
            try:
        ...
        if _shutdown_ctx is not None:
            ...
            def _diagnostic() -> None:
                ...
                spawn_async_diagnostic(
                    _resolve_hermes_home() / "logs" / "gateway-shutdown-diag.log",
                    _shutdown_ctx["signal"], timeout_seconds=5.0)
...
def _start_gateway_claim_pid_file() -> bool:
    ...
    if not acquire_gateway_runtime_lock():
        # Name the holder. A bare 'lock is already held' cannot say WHO won, and one stray
        # line still appears per reboot after the boot-race fix; this pairs with the
        # [gateway-launch] tag in laptop-start.ps1 so the next reboot's err.log
        # self-identifies the loser. Fully guarded — this path is already failing.
        _holder_pid = None
        _holder_raw = None
        try:
        ...
        logger.error(
            ...
            "[lock-reject my_pid=%d my_ppid=%d argv=%r holder_pid=%s holder_raw=%r]",
            os.getpid(), os.getppid(), sys.argv, _holder_pid, _holder_raw)
        return False
    try:
        write_pid_file()
...
async def _start_gateway_shutdown_tail(
    ...
    def _shutdown_event_bus() -> None:
        # Upstream's tail never stops the bus, so the subscriber loop outlives the gateway it
        # reports on and a restart leaves the previous life's subscribers polling the shared
        # SQLite bus.
        from events.gateway_integration import shutdown as eventbus_shutdown
        eventbus_shutdown()

    _best_effort(_shutdown_event_bus)
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Nothing writes a heartbeat between here and runner.start() bringing up the EventBus
    # subscriber that normally does. A gateway slow to get there looks dead to the watchdog,
    # which kills it and relaunches into the same gap — the boot wedge that sank the 0.18.2
    # cutover on 2026-07-09. Keep it fresh on a daemon thread until the real writer starts.
    _early_hb_stop = threading.Event()
    start_early_boot_heartbeat(_early_hb_stop)

    # Three independent paths leave the gateway: graceful _stop_impl (best informed), atexit
    # fall-through, and the signal handlers. The emit is idempotent — the first wins, the
    # rest no-op — so registering here only adds coverage for the paths that never reach a
    # graceful stop. After the PID claim, so stop-state is recorded while the pid metadata is
    # still on disk.
    ...
    def _retire_boot_scaffolding() -> None:
        """Stop everything start_gateway started that the shutdown tail would otherwise own.

        Every early return below skips _start_gateway_shutdown_tail, so without this the nous
        keepalive (a 60s loop that re-resolves HERMES_HOME and touches the auth store) and the
        planned-stop watcher (an unbounded 0.5s poll that unlinks stale markers) both run for
        the life of the process. One helper, so a future early return cannot miss one.
        """
        _early_hb_stop.set()
    ...
    try:
        ...
        _retire_boot_scaffolding()
        _shutdown_gateway_health_export(runner)
        raise
    # runner.start() brought up the EventBus subscriber — the normal heartbeat writer — so the
    # boot-time stand-in can retire even on the paths that continue.
    _early_hb_stop.set()
    if not success:
        _retire_boot_scaffolding()
        _shutdown_gateway_health_export(runner)
...
def main():
    ...
    # Defined before the try so a path that raises before the assignment still exits with a code.
    exit_code = 1
    try:
        ...
        # Ctrl-C / SIGINT is a clean shutdown path, not a fatal error. It needs its own arm
        # because it is a BaseException: without one it would be reported as a crash and
        # would write a .fatal sidecar the watchdog then acts on.
        exit_code = 0
        ...
        # Uncaught exception: the gap asyncio.run leaves, where the traceback goes to stderr
        # with no FATAL marker to grep for. Every step below is individually guarded because
        # this runs exactly when logging may already be broken.
        tb = traceback.format_exc()
        try:
            logger.critical("Gateway FATAL — uncaught %s: %s\n%s",
                            exc.__class__.__name__, exc, tb)
        except Exception:
            pass                      # logging itself failed; fall through to the sidecar
        # Emitted BEFORE the sidecar write so a bus consumer sees the cause without reading
        # the file. Idempotent: no-ops if the graceful path, signal handler or atexit already
        # emitted.
        try:
            from events.gateway_integration import emit_gateway_stopped
            emit_gateway_stopped({
            ...
            pass
        # Marker next to the PID file, so the same cleanup path picks it up. gateway_watchdog.py
        # reads it to tell a caught crash (do not restart-loop) from a SIGKILL/sleep death.
        try:
            from gateway.status import _get_pid_path
            sidecar = _get_pid_path().with_suffix(".fatal")
            sidecar.write_text(f"{exc.__class__.__name__}: {exc}\n\n{tb}", encoding="utf-8")
        except Exception:
