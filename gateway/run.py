...
from hermes_constants import get_hermes_home, get_hermes_home_override, get_process_hermes_home, real_executable
_hermes_home: Optional[Path] = None


def _resolve_hermes_home() -> Path:
    """Process launch home, resolved live; explicit test/embedding override wins.

    Per-turn multiplex ContextVars must not redirect gateway-owned state.
    _gateway_config_home explicitly layers profile scope where required.
    """
    return Path(_hermes_home) if _hermes_home is not None else get_process_hermes_home()
...
def _reload_runtime_env_preserving_config_authority() -> None:
    ...
    if not is_multiplex_active():
        load_hermes_dotenv(
            hermes_home=_resolve_hermes_home(), project_env=Path(__file__).resolve().parents[1] / '.env')
...
def _bridge_config_to_env(_cfg: dict) -> None:
    ...
    for _key, _val in _cfg.items():  # top-level scalars: fallback only, never override .env
        if not isinstance(_val, (str, int, float, bool)):
            continue
        env_shaped = isinstance(_key, str) and _key.isupper() and _key.replace("_", "").isalnum()
        bridge_log = logging.getLogger(__name__)
        if _key in os.environ:
            if env_shaped and str(_val) != os.environ[_key]:
                # Configuration scalars can be credentials: diagnose precedence without their values.
                bridge_log.warning("config.yaml top-level %s is IGNORED because an environment value is already set", _key)
            continue
        os.environ[_key] = str(_val)
        if env_shaped and _key.endswith("_HOME_CHANNEL"):
            bridge_log.info("%s was bridged from config.yaml because no environment value is set", _key)
...
# The import-time bridge has consumed this path; do not retain a stale home.
del _config_path
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from hermes_cli.runtime_provider import (
        resolve_runtime_provider, format_runtime_provider_error, _get_model_config as _get_model_config)
...
class GatewayRunner(
    ...
    def _init_session_db(self) -> None:
        ...
        # Preserve opt-in startup archiving. Pruning runs after startup in the
        # supervised online maintenance watcher, without an exclusive VACUUM.
...
async def _drain_incumbent_via_marker(pid: int, timeout: float) -> bool:
    """Write the Windows graceful-stop request, then asynchronously confirm exit."""
    import math
    from gateway.status import write_planned_stop_marker
    try:
        write_planned_stop_marker(pid)
    except Exception:
        logger.debug("planned-stop marker write failed", exc_info=True)
    return await _wait_for_pid_exit(pid, max(1, math.ceil(timeout / 0.5)), 0.5)


async def _request_incumbent_shutdown(existing_pid: int, *, timeout: float) -> bool:
    """Ask for a drain and await confirmed exit without blocking the loop."""
    import math
    ...
    try:
        ...
        return True
    return await _wait_for_pid_exit(existing_pid, max(1, math.ceil(timeout / 0.5)), 0.5)
...
async def _start_gateway_replace_existing_instance(existing_pid: int, replace: bool) -> bool:
    ...
    if _replace_target_belongs_to_other_profile(existing_pid):
        ...
        return False
    incumbent_age = _booting_incumbent_blocks_replace(existing_pid)
    if incumbent_age is not None:
        logger.error("Refusing --replace: PID %d is %.0fs old (boot grace %.0fs); set %s=0 to override.",
                     existing_pid, incumbent_age, _replace_min_incumbent_age_s(), _REPLACE_MIN_AGE_ENV)
        return False
    ...
    drain_budget = _replace_drain_timeout()
    try:
        old_gateway_exited = await _request_incumbent_shutdown(existing_pid, timeout=drain_budget)
    except ProcessLookupError:
        old_gateway_exited = True
    except (PermissionError, OSError):
        logger.error("Permission denied killing PID %d. Cannot replace.", existing_pid)
        _clear_replace_markers_quiet()
        return False
    # Escalate only after the incumbent has had its drain and teardown budget.
    if not old_gateway_exited:
        logger.warning("Old gateway (PID %d) did not exit after %.0fs drain; force-stopping.", existing_pid, drain_budget)
...
def _start_gateway_configure_logging(verbosity: Optional[int]) -> None:
    ...
    def _security_audit() -> None:
        ...
        log_startup_security_warnings(hermes_home=_resolve_hermes_home(), config=_best_effort(_raw_cfg))
...
def _start_gateway_make_shutdown_signal_handler(runner, _signal_initiated_shutdown: list):
    ...
    def shutdown_signal_handler(received_signal=None):
        ...
        if planned_takeover:
            ...
            _emit_gateway_stopped_quietly({"exit_reason": "signal", "signal_initiated": True})
        ...
        if _shutdown_ctx is not None:
            ...
            def _diagnostic() -> None:
                ...
                spawn_async_diagnostic(
                    _resolve_hermes_home() / "logs" / "gateway-shutdown-diag.log", _shutdown_ctx["signal"], timeout_seconds=5.0)
...
def _start_gateway_claim_pid_file() -> bool:
    ...
    atexit.register(_emit_gateway_stopped_quietly, {"exit_reason": "atexit"})
    return True
...
def _emit_gateway_stopped_quietly(payload) -> None:
    try:
        from events.gateway_integration import emit_gateway_stopped
        emit_gateway_stopped(payload)
    except Exception:
        logger.debug("Gateway stopped event failed (non-fatal)", exc_info=True)


def _stop_owned_gateway_eventbus(runner) -> None:
    if not getattr(runner, "_eventbus_cleanup_needed", False):
        return
    runner._eventbus_cleanup_needed = False
    try:
        from events.gateway_integration import shutdown as eventbus_shutdown
        eventbus_shutdown()
    except Exception:
        logger.debug("EventBus shutdown failed (non-fatal)", exc_info=True)


async def _stop_gateway_cron_resources(cron_stop, cron_provider, cron_thread, housekeeping_thread):
    """Cooperatively drain the scheduler workers on every exit path."""
    cron_stop.set()
...
async def _start_gateway_shutdown_tail(
    ...
    failure_verdict = _exit_with_failure_verdict(runner)
    ...
    await _stop_gateway_cron_resources(cron_stop, cron_provider, cron_thread, housekeeping_thread)

    _stop_owned_gateway_eventbus(runner)
    ...
    return False if failure_verdict else _resolve_gateway_exit_verdict(runner, _signal_initiated_shutdown[0])
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    _control_server = None
    _cron_resources = None
    _mcp_cleanup_needed = False
    try:
        ...
        try:
            ...
            _early_hb_thread = None
            try:
                _early_hb_thread = start_early_boot_heartbeat(_early_hb_stop)
            except Exception:
                logger.debug("Early boot heartbeat did not start", exc_info=True)
            try:
                ...
                _early_hb_stop.set()
                if _early_hb_thread is not None:
                    _early_hb_thread.join(timeout=2)
            if not success:
                _shutdown_gateway_health_export(runner)
                return False
            ...
            _cron_resources = _start_gateway_start_cron_and_housekeeping(runner)
            cron_stop, cron_provider, cron_thread, housekeeping_thread = _cron_resources
            ...
            _mcp_cleanup_needed = False  # normal tail already closed it
            _cron_resources = None  # normal tail already drained it
            _control_server = None  # normal tail already stopped it
            return verdict
        finally:
            _stop_nous_keepalive_quietly()
    except BaseException as exc:
        _emit_gateway_stopped_quietly({
            "exit_reason": "startup_or_runtime_exception",
            "error_class": type(exc).__name__, "error_message": str(exc)[:500],
        })
        raise
    finally:
        if _cron_resources is not None:
            try:
                await _stop_gateway_cron_resources(*_cron_resources)
            except Exception:
                logger.debug("Exceptional-exit cron cleanup failed", exc_info=True)
        _stop_owned_gateway_eventbus(runner)
        if _mcp_cleanup_needed:
            with suppress(Exception):
                await _shutdown_mcp_servers_nonblocking()
        _planned_stop_watcher_stop.set()
        _planned_stop_watcher_thread.join(timeout=2)
        if _control_server is not None:
            try:
                ...
                logger.debug("Early-exit control socket cleanup failed", exc_info=True)
        _shutdown_gateway_health_export(runner)
