...
class GatewayInboundMixin:
    ...
    async def _hm_offer_pairing_code(self, source: SessionSource) -> None:
        ...
        if adapter:
            # Log the send, not just the decision. This is the record that answers 'where
            # did that code go' — the question the 2026-04-19 group leak was diagnosed by.
            logger.info(
                "Sending pairing %s to unauthorized %s DM: chat_id=%s user_id=%s",
                "code" if code else "rate-limit notice",
    ...
    async def _hm_admit_event(
        ...
        if not self._is_user_authorized_for_source(source):
            if source.user_id is None:
                ...
                return None
            # chat_type and chat_id belong in this line: after the 2026-04-19 leak the first
            # question asked of the logs was WHICH CHAT a pairing response went to, and
            # user_id alone cannot answer it.
            logger.warning(
            ...
            # HARD STOP: never send any auto-response to group chats. On 2026-04-19 a pairing
            # code leaked into the "Ted" WhatsApp group; the downstream DM guard was not
            # enough, and this early return is the load-bearing invariant. Do not replace it
            # with an elif below. The dm/pair check further down is an ALLOWLIST and so
            # already excludes groups on today's code — this is deliberate defence in depth
            # against that branch being refactored, which is the event it was written for.
            if source.chat_type == "group":
                return None
            # WhatsApp-specific: even in DMs, never send pairing unless the operator opts in
            # via HERMES_WHATSAPP_ALLOW_PAIRING. Default silent, so an unknown WhatsApp DM (a
            # stranger who guessed the number) gets no response at all — the number is a
            # personal one and a pairing prompt confirms it is live and agent-backed.
    ...
    def _hm_write_update_response(response_text: str) -> Optional[str]:
        ...
        try:
            ...
            (_resolve_hermes_home() / ".update_prompt.json").unlink(missing_ok=True)
        except OSError as e:
            return str(e)
        return None
    ...
    def _hm_evict_idle_stale_agent(self, _quick_key: str) -> None:
        ...
        if _should_evict:
            logger.warning(
                # %g for the same sentinel hazard: HERMES_AGENT_TIMEOUT 0 = unlimited.
