...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Keep hints with loader generations so CAS restoration restores their metadata.
        # Weak keys do not retain unloaded plugins. Unsupported callable keys fail open.
        self._loader_env_hints = weakref.WeakKeyDictionary()
    ...
    def register_deferred(self, name: str, loader: _Loader, *, scope: Optional[str] = None,
                          env_hints: tuple[str, ...] | list[str] | None = None) -> None:
        ...
        with self._lock:
            ...
            if name not in entries:
                ...
                try:
                    self._loader_env_hints.setdefault(loader, {})[(scope, name)] = tuple(env_hints or ())
                except TypeError:
                    pass  # Unknown hints mean resolve normally, never skip a platform.

    def deferred_names(self) -> list[str]:
        """Pending names visible in this profile, without importing adapters."""
        with self._lock:
            entries, deferred = self._scope_maps(self.current_scope_key())
            names = dict.fromkeys(n for n in self._deferred if n not in entries)
            names.update(dict.fromkeys(deferred))
            return list(names)

    def deferred_env_hints(self, name: str) -> tuple[str, ...]:
        """Hints for the visible pending loader; unknown means resolve normally."""
        with self._lock:
            ...
            if loader is None:
                return ()
            try:
                return self._loader_env_hints.get(loader, {}).get((scope, name), ())
            except TypeError:
                return ()

    def loaded_entries(self) -> list[PlatformEntry]:
        """Materialized entries in this profile, without resolving pending loaders."""
        with self._lock:
            entries, _ = self._scope_maps(self.current_scope_key())
            return list({**self._entries, **entries}.values())

    def known_names(self) -> list[str]:
        """Known names without importing adapters."""
        with self._lock:
            return list(dict.fromkeys([e.name for e in self.loaded_entries()] + self.deferred_names()))
