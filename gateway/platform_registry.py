...
class PlatformEntry:
    ...
    # CHEAP passive probe, distinct from ``check_fn``: for adapters that defer a heavy SDK or
    # shell out to a version probe, ``check_fn`` IS that expensive work. Config load and status
    # displays only ask whether the platform could work, so they use
    # ``deps_available_fn or check_fn``. WhatsApp's ``check_whatsapp_requirements`` runs a
    # ``node --version`` subprocess, which is why it supplies the cheap answer separately
    # (pinned by tests/gateway/test_whatsapp_node_probe_budget.py). This is NOT
    # ``ensure_deps_fn``: both of these are passive, that one installs.
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # (scope, name) -> env var names / prefixes that could configure that deferred
        # platform, so a caller can skip resolving — and thus importing — a platform the
        # user could not possibly have configured. Keyed by load key like the other
        # deferred maps: one profile's teardown must not erase another's hints.
        self._deferred_env_hints: dict[_LoadKey, tuple[str, ...]] = {}
    ...
    def register_deferred(
        self, name: str, loader: _Loader, *, scope: Optional[str] = None,
        ...
        *env_hints* is an optional collection of env var names / name prefixes that could
        configure this platform (e.g. ``("FEISHU_APP_ID", "FEISHU_")``). Callers that only
        want platforms the user could plausibly have configured consult
        :meth:`deferred_env_hints` and skip the resolve entirely when nothing matches. An
        empty or omitted value means no hint is available — callers must then fail OPEN and
        resolve, so a plugin that cannot express hints never silently disappears.
        """
        with self._lock:
            ...
            if name not in entries:
                ...
                self._deferred_env_hints[(scope, name)] = tuple(env_hints or ())

    def deferred_names(self) -> list[str]:
        """Names with a pending deferred loader, current scope and process-global.

        Resolves NOTHING — this is the accessor for callers that need the set of known
        platform names, or want to gate resolution themselves, without paying for ~20
        platform SDK imports.
        """
        with self._lock:
            _entries, deferred = self._scope_maps(self.current_scope_key())
            names = list(deferred)
            names.extend(n for n in self._deferred if n not in deferred)
            return names

    def deferred_env_hints(self, name: str) -> tuple[str, ...]:
        """Env var names / prefixes that could configure deferred *name*.

        An empty tuple means unknown — callers must fail open and resolve. Resolves nothing.
        """
        with self._lock:
            hints = self._deferred_env_hints.get((self.current_scope_key(), name))
            if hints is None:
                hints = self._deferred_env_hints.get((None, name))
            return hints or ()
    ...
    def loaded_entries(self) -> list[PlatformEntry]:
        ...
        with self._lock:
            return list({**self._entries,
                         **self._scoped_entries.get(self.current_scope_key(), {})}.values())

    def known_names(self) -> list[str]:
        """Every known platform name WITHOUT resolving deferred loaders.

        A sorted wrapper over :meth:`registered_names`, which already returns exactly this
        union, rather than a second implementation of it. Kept under this name because
        ``cron/scheduler.py`` calls it. Use it instead of :meth:`all_entries` when the caller
        only needs to filter names and can then :meth:`get` the few survivors: that accessor
        resolves every deferred loader, which imports every bundled adapter.
        """
        return sorted(self.registered_names())

    def snapshot_registration(        self, name: str, *, scope: Optional[str] = None
    ) -> tuple[Optional[PlatformEntry], Optional[_Loader]]:
        ...
        with self._lock:
            ...
            if event is None and name not in entries:
                ...
                if loader is not None:
                    self._deferred_env_hints.pop(scoped_key, None)
            if event is None and loader is None and name not in entries:
                ...
                if event is None:
                    ...
                    if loader is not None:
                        self._deferred_env_hints.pop(global_key, None)
    ...
    def register(self, entry: PlatformEntry, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            self._deferred_env_hints.pop((scope, entry.name), None)
    ...
    def unregister(self, name: str, *, scope: Optional[str] = None) -> bool:
        ...
        with self._lock:
            ...
            if inferred_scope is not None or name in entries or name in deferred:
                ...
                self._deferred_env_hints.pop((active_scope, name), None)
                removed = entries.pop(name, None) is not None
                self._prune_scope(active_scope)
                return removed
            self._deferred.pop(name, None)
            self._deferred_env_hints.pop((None, name), None)
