...
class GatewayBusySessionMixin:
    ...
    def _compose_busy_ack_message(
        ...
        from gateway.run import (
            _AGENT_PENDING_SENTINEL, _resolve_hermes_home, _load_gateway_config, _platform_config_key
        )
        from gateway.display_config import resolve_display_setting
    ...
    _PLAIN_COMMANDS = (
        ...
        # DevFlow decisions: reachable idle AND mid-run (their CommandDefs declare
        # busy_policy="dispatch", which resolves the handler through THIS table).
    ...
    def _check_slash_access(self, source: SessionSource, canonical_cmd: str) -> Optional[str]:
        ...
        if config is None:
            # Distinguish the two ways the resolver yields None. Only the multiplex case —
            # a stamped FOREIGN profile whose config has not loaded yet — is a policy gap
            # that must fail closed. When the runner has no base config at all,
            # _gateway_config_for_source hands that None straight back, and the historical
            # contract is that policy_for_source treats it as a disabled (allow-all)
            # policy; denying there would gate slash commands on an unrelated condition.
            if getattr(self, "config", None) is None:
                return None
            logger.warning(
                "Slash command /%s denied for stamped source with no profile config: %s",
                canonical_cmd, getattr(source, "profile", None))
