...
class GatewayBusySessionMixin:
    ...
    _PLAIN_COMMANDS = (
        ...
        "ddp-approve", "ddp-decline", "ddp-approve-confirm", "ddp-decline-confirm", "devflow-login",
    )
    ...
    def _gateway_config_for_source(self, source: SessionSource):
        """Resolve slash authority from the immutable adapter-owner config."""
        config = getattr(self, "config", None)
    ...
    def _check_slash_access(self, source: SessionSource, canonical_cmd: str) -> Optional[str]:
        ...
        if config is None:
            if getattr(self, "config", None) is None:
                return None  # Preserve the historical no-policy context.
            logger.warning(
