...
def _enable_plugin_platform(config: GatewayConfig, entry) -> None:
    ...
    try:
        ...
        # Prefer ``deps_available_fn`` when the plugin supplies one: WhatsApp's
        # ``check_whatsapp_requirements`` runs a real probe, which is both slow and the wrong
        # shape for a deps gate, and the registry already carries a cheap probe beside it.
        deps_probe = entry.deps_available_fn or entry.check_fn
        deps_ok = bool(deps_probe())
    except Exception as e:
...
def _enable_plugin_platforms_from_env(config: GatewayConfig) -> None:
    ...
    try:
        ...
        # Not plugin_entries(): that resolves EVERY bundled adapter, importing ~20 platform
        # SDKs on a call that merely reads config. The candidate pass resolves only the
        # platforms whose declared env hints (or config) suggest they could be configured.
