...
def _spawn_detached_update(hermes_cmd, output_path, exit_code_path) -> None:
    ...
    if sys.platform == "win32":
        ...
        subprocess.Popen(
            [real_executable(), "-c", _WINDOWS_UPDATE_HELPER, str(output_path), str(exit_code_path),
             real_executable(), "-m", "hermes_cli.main", "update", "--gateway"],
...
class GatewaySlashCommandsMixin(
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        async def _write_marker(name: str, build, label: str) -> None:
            try:
                await asyncio.to_thread(atomic_json_write, _resolve_hermes_home() / name, build(), indent=None)
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        if config is None:
            return "Slash access policy unavailable for this profile."
