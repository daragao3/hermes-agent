...
def _spawn_detached_update(hermes_cmd, output_path, exit_code_path) -> None:
    ...
    if sys.platform == "win32":
        ...
        # real_executable(), not sys.executable: on a Microsoft-Store host that is an MSIX
        # alias which AppX starts SUSPENDED, resuming it only once the activation handoff
        # completes — and this helper is DETACHED precisely so it outlives the gateway
        # restarting underneath it, so nothing is left to complete that handoff. Both the
        # helper's interpreter and the one it re-spawns for `-m hermes_cli.main`.
        _python = real_executable()
        subprocess.Popen(
            [_python, "-c", _WINDOWS_UPDATE_HELPER, str(output_path), str(exit_code_path),
             _python, "-m", "hermes_cli.main", "update", "--gateway"],
...
class GatewaySlashCommandsMixin(
    ...
    async def _handle_whoami_command(self, event: MessageEvent) -> str:
        ...
        source = event.source
        # self.config is the GATEWAY's config; under the multiplexer that is not the config
        # governing the profile this message arrived for. Unresolvable fails CLOSED.
        ...
        async def _write_marker(name: str, build, label: str) -> None:
            try:
                await asyncio.to_thread(
                    atomic_json_write, _resolve_hermes_home() / name, build(), indent=None)
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        # Per-profile, and unresolvable fails CLOSED: this mutates profile-wide security
        # policy, so answering from the wrong config is worse than refusing.
        config = self._gateway_config_for_source(event.source)
        if config is None:
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        # Persist before mutate. Gateway-local writer honors the active
        # multiplex profile scope; cli.save_config_value intentionally targets
        # the process home and would overwrite the default profile instead.
        if not self._save_gateway_config_key("display.busy_input_mode", arg):
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        import json
        from gateway.run import _resolve_hermes_bin, _resolve_hermes_home
        ...
        # Resolved per call: under the multiplexer these markers are profile-scoped, and a
        # frozen import-time home writes one profile's update state into another's.
        _home = _resolve_hermes_home()
        pending_path = _home / ".update_pending.json"
        output_path = _home / ".update_output.txt"
        exit_code_path = _home / ".update_exit_code"
        pending = {
    ...
    # --- DevFlow delegation: staged, actor-bound lifecycle decisions ---------------------------
