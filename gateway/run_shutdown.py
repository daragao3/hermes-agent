...
class GatewayShutdownMixin:
    ...
    async def _stop_begin_teardown(self, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        self._draining = True
        # Emit before disconnecting adapters or closing the bus; the emitter deduplicates.
        try:
            from events.gateway_integration import emit_gateway_stopped
            payload = {"exit_reason": "restart" if self._restart_requested else "graceful"}
            if self._restart_requested:
                payload["restart_detached"] = bool(self._restart_detached)
                payload["restart_via_service"] = bool(self._restart_via_service)
            try:
                from cron.inflight import current_inflight_correlation_ids
                payload["inflight_cron_correlation_ids"] = list(current_inflight_correlation_ids())
            except Exception:
                payload["inflight_cron_correlation_ids"] = []
            emit_gateway_stopped(payload)
        except Exception:
            logger.debug("emit_gateway_stopped failed (non-fatal)", exc_info=True)
    ...
    async def _stop_finalize_agents_and_adapters(self, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        getattr(self, "_profile_gateway_configs", {}).clear()
