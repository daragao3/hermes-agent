...
class GatewayShutdownMixin:
    ...
    def _stuck_loop_counts_path(self) -> Path:
        ...
        return _resolve_hermes_home() / self._STUCK_LOOP_FILE
    ...
    async def _stop_begin_teardown(self, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        self._draining = True
        # Caller #1 of the three emit_gateway_stopped paths (M1, 2026-04-30). This is the
        # most-informed caller — it alone knows graceful vs restart — and the emitter keeps
        # the FIRST payload, so it must run before the atexit hook's "atexit" or the signal
        # handler's coarser one. Emitted here, at the top of teardown, so the event lands
        # while the EventBus thread is still polling: after adapter disconnect and bus close
        # there is nothing left to publish it.
        try:
            ...
            # cron_started event_ids for every cron still in flight, so a consumer can tell
            # which runs this shutdown cut short. Landed 2026-08-16, when cron/inflight.py
            # began reading the Guard #3 _in_flight registry; before that the import always
            # failed and this key shipped as [] on every GATEWAY_STOPPED since 2026-04-30.
            # The try/except stays: this is the shutdown path, and a failed import must not
            # abort the emit.
            try:
    ...
    def _stop_release_runtime_state(self, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        self.adapters.clear()
        # Released WITH the adapters it describes: a profile removed and re-added inside one
        # process would otherwise be authorized against its previous config.
    ...
    def _stop_persist_exit_state(self, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        from gateway.run import _resolve_hermes_home, _planned_restart_notification_path, _shutdown_gateway_health_export
        from utils import atomic_json_write
