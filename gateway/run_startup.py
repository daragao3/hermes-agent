...
class GatewayStartupMixin:
    ...
    async def _start_recover_previous_run(self) -> None:
        ...
        self._previous_shutdown_was_clean = _clean_marker.exists()
    ...
    async def _finish_background_startup_connect(self, connect_task, platform, adapter):
        """Settle a slow boot connection under the normal shutdown task owner."""
        pending = self._startup_background_connects
        try:
            result = await connect_task
            while not self._running and not self._startup_should_abort():
                await asyncio.sleep(0.05)
            if self._startup_should_abort():
                await self._startup_teardown_adapter(adapter, platform)
                return
            retryable, fatal = [], []
            connected = await self._start_aggregate_connect_results([result], retryable, fatal)
            if connected:
                ...
                try:
                    ...
                    logger.debug("background delivery recovery failed", exc_info=True)
                if not getattr(self, "_startup_restore_in_progress", False):
                    try:
                        self._schedule_resume_pending_sessions(platform=platform)
                    except Exception:
                        logger.debug("background pending-session resume failed", exc_info=True)
            pending.discard(asyncio.current_task())
            if fatal and not self.adapters and not self._failed_platforms and not pending:
                self._startup_fail_fatal_config("; ".join(fatal))
    ...
    async def _start_connect_pending(self, _pending_connects: list) -> Optional[list]:
        ...
        takeover = bool(getattr(self, "_platform_lock_takeover_on_start", False))
        background_platforms = {Platform.TELEGRAM, Platform.WHATSAPP}
        ...
        async def _connect_one_startup(p, p_cfg, adp):
            ...
            try:
                if p in background_platforms:
                    ok = await self._connect_initial_adapter_with_timeout(adp, p, takeover_allowed=takeover)
                else:
        ...
        background = getattr(self, "_startup_background_connects", None)
        if background is None:
            background = self._startup_background_connects = set()
        for task, (platform, _config, adapter) in list(_task_map.items()):
            if platform in background_platforms:
                # Own the connector too: shutdown can cancel its settlement wrapper
                # before that wrapper has entered its first await/finally.
                self._retain_background_task(task)
                owner = self._retain_background_task(asyncio.create_task(
                    self._finish_background_startup_connect(task, platform, adapter)))
                background.add(owner)
                owner.add_done_callback(self._late_failure_callback(
                    "background startup connection settlement failed"))
                del _task_map[task]
    ...
    def _start_handle_no_connections(
        ...
        from gateway.run import _write_runtime_status_quiet
        if connected_count != 0 or getattr(self, "_startup_background_connects", None):
            return False
    ...
    async def _start_post_connect_services(self, connected_count: int) -> None:
        ...
        try:
            from events.gateway_integration import get_bus, startup as eventbus_startup
            # Claim cleanup before startup: a partial initialization may raise after opening the bus.
            self._eventbus_cleanup_needed = get_bus() is None
            eventbus_startup(adapters=self.adapters)
        except Exception as exc:
            logger.warning("EventBus initialization failed (non-fatal): %s", exc)
