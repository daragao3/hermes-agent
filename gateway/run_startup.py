...
class GatewayStartupMixin:
    ...
    def _start_log_systemd_timing_alignment(self) -> None:
        ...
        with _log_suppressed(logging.DEBUG, "check_systemd_timing_alignment failed: %s"):
            ...
            if _alignment is not None and _alignment.get("mismatch"):
                logger.warning(
                    # %g, not %.0f: agent.restart_drain_timeout documents 0 = no drain,
                    # so a sub-second window printed as "0s" reads as "no drain is
                    # configured" — the opposite of what is being warned about. Rounding
                    # also made the numbers incoherent to a reader comparing them
                    # ("TimeoutStopSec=30s ... expected >=31s" for a 0.5s window), so all
                    # of them move together.
                    "Stale systemd unit detected: %s has TimeoutStopSec=%gs but drain_timeout=%gs "
                    "cron_drain_timeout=%gs (expected >=%gs). systemd may SIGKILL the gateway "
    ...
    async def _start_recover_previous_run(self) -> None:
        ...
        # Stash it NOW, for the GATEWAY_STARTED payload: by the time that is emitted the
        # marker has been consumed, and without the boolean a bus consumer would have to
        # reach into the filesystem to tell an operator restart from crash recovery.
        # Distinct from _booted_from_restart, which tracks the /restart notification marker.
    ...
    async def _start_connect_pending(self, _pending_connects: list) -> Optional[list]:
        ...
        # Peel the fire-and-retry platforms off BEFORE the task map is built. Upstream's
        # abort path below cancels pending tasks and tears down already-connected ones, and
        # that logic assumes every connect is in _task_map — a background task fired inside
        # it would escape the teardown. These carry their own abort guard instead:
        # _connect_platform_in_background re-checks _startup_should_abort() before wiring in.
        _deferred = [t for t in _pending_connects if self._should_connect_in_background(t[0])]
        _pending_connects = [
            t for t in _pending_connects if not self._should_connect_in_background(t[0])]
        for _p, _cfg, _adp in _deferred:
            logger.info(
                "Connecting to %s in the background — not blocking startup", _p.value)
            _bg = asyncio.create_task(
                self._connect_platform_in_background(_adp, _p, _cfg))
            self._background_tasks.add(_bg)
            _bg.add_done_callback(self._background_tasks.discard)
    ...
    async def _start_post_connect_services(self, connected_count: int) -> None:
        ...
        # Bring the event bus UP. Upstream's tree starts it nowhere, while the shutdown
        # path stops it and the exit paths emit GATEWAY_STOPPED — so without this the
        # gateway tears down and reports the stop of a bus that never came up.
        # Non-fatal: a gateway that cannot publish events must still serve messages.
        try:
        ...
        if not notified and any(
            (_resolve_hermes_home() / name).exists()
    ...
    def _start_spawn_background_watchers(self) -> None:
        ...
        # Periodic ONLINE state.db prune. Through the same supervision surface as its
        # siblings so it lands in _background_tasks and is cancelled with the rest during
        # stop() — its 6h sleep must never block the shutdown drain. A clean return (no
        # _session_db) is not respawned by the supervisor.
        self._spawn_supervised(
    ...
    async def start(self) -> bool:
        ...
        # Carries operator-relevant boot context so a future restart-cluster investigation
        # does not have to triangulate from gateway.pid mtime + agent.log boot lines +
        # watchdog probe gaps. Best-effort; the helper swallows bus failures itself, and
        # this guard covers an emitter that is unavailable or refactored to raise.
        try:
