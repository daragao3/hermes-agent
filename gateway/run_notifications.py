...
class GatewayNotificationsMixin:
    ...
    async def _deliver_media_from_response(
        ...
        with _log_suppressed(logging.WARNING, "Post-stream media extraction failed: %s"):
            ...
            if image_paths:
                try:
                    images = [(Path(p).absolute().as_uri(), "") for p in image_paths]
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_resolve_hermes_home() / ".update_pending.json",
            claimed=_resolve_hermes_home() / ".update_pending.claimed.json", output=_resolve_hermes_home() / ".update_output.txt",
            exit_code=_resolve_hermes_home() / ".update_exit_code",
            prompt=_resolve_hermes_home() / ".update_prompt.json", response=_resolve_hermes_home() / ".update_response",
        )
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _resolve_hermes_home, _non_conversational_metadata
