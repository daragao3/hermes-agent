...
class GatewayAgentCacheMixin:
    ...
    def _extract_honcho_cache_busting_config(cls) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                stat = path.stat()
                # Size participates in the key, not just mtime. Windows advances a file's
                # last-write time on the ~15.6ms system tick, so a rewrite landing in the
                # same tick as the previous one is invisible to an mtime-only key and the
                # stale memo is served. A real (if narrow) staleness window in production,
                # and it made the memo's own test fail roughly 1 run in 3.
