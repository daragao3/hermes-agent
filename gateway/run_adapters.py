...
class GatewayAdapterLifecycleMixin:
    ...
    async def _connect_initial_adapter_with_timeout(self, adapter, platform, *, takeover_allowed=None) -> bool:
        ...
        adapter._platform_lock_takeover_allowed = bool(
            self._platform_lock_takeover_on_start if takeover_allowed is None else takeover_allowed
        )
        try:
    ...
    async def _start_one_profile_adapters(
        ...
        # Keep authority bound to the adapter's owner, including later retries.
