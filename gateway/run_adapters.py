...
class GatewayAdapterLifecycleMixin:
    ...
    async def _start_one_profile_adapters(
        ...
        # Keep the parsed config WITH the secondary adapters. Authorization happens after
        # adapter construction and must not fall back to the multiplexer default profile's
        # policy for a stamped source. Guarded like the sibling read sites because tests
        # build GatewayRunner via object.__new__, bypassing the __init__ that seeds it.
