...
# taskkill's own budget, and how long we then wait to see the target actually die.
...
def _terminate_caller() -> str | None:
    ...
    A bare "a kill happened" record is not actionable when a dozen call sites can produce
    it; the frame is what turns the log into a name."""
    try:
        import traceback
...
def terminate_pid(
    ...
    ``reason`` is recorded on the ``process.terminate`` diag record. A bare caller + force
    flag does not distinguish "the incumbent refused to drain inside its budget" from an
    ordinary kill, and that distinction is the one a postmortem needs: the first means
    in-flight work was lost on purpose after being asked nicely, the second means it was
    never asked. Optional and keyword-only so no existing caller changes.
    """
    try:
        from hermes_cli.gateway_diag import write_diag

        write_diag(
            "process.terminate", victim_pid=pid, force=force, killer_pid=os.getpid(),
            caller=_terminate_caller(), reason=reason,
        )
    except Exception:
        pass
    ...
    try:
        result = subprocess.run(
            ...
            encoding="utf-8", errors="replace", timeout=_TASKKILL_TIMEOUT_S,
        ...
        # taskkill's exit status is NOT the source of truth here — liveness is. A timeout
        # says only that we stopped waiting for taskkill to report back; on a loaded box the
        # kill routinely lands well before it is scheduled to say so. Treating it as fatal
        # took the gateway down on 2026-08-11: TimeoutExpired is a SubprocessError, so both
        # `except OSError` and `except CalledProcessError` handlers missed it, and it
        # propagated out of stop() into restart(), which died before spawning the
        # replacement and left :8642 with no listener.
        if _wait_for_pid_death(pid, _TASKKILL_VERIFY_TIMEOUT_S):
            logger.warning(
                "terminate_pid(%s): taskkill exceeded its %.0fs budget but the process is "
                "gone — treating the kill as successful.", pid, _TASKKILL_TIMEOUT_S)
            return
        raise OSError(
...
def _read_json_file_ex(
    path: Path, *, bare_pid_ok: bool = False
    ...
    try:
        ...
        # PermissionError. A lock we cannot read BECAUSE IT IS HELD is positive proof the
        # owning process is alive, so surface that to callers as a sentinel instead of
        # reporting "no record" (the regression that produced a /api/status HTTP 500).
        # Scoped to the PID/lock readers (bare_pid_ok); generic callers keep "absent".
        # PermissionError is an OSError SUBCLASS, so this branch must precede the catch below.
        return ({"locked": True} if bare_pid_ok else None), "unreadable"
...
def _read_json_file(path: Path, *, bare_pid_ok: bool = False) -> Optional[dict[str, Any]]:
    ...
    return _read_json_file_ex(path, bare_pid_ok=bare_pid_ok)[0]
...
# Public alias: six production modules import ``pid_exists`` without the underscore
# (cron/executions.py, gateway/run.py, hermes_cli/gateway_windows.py, hermes_cli/goals.py,
# plugins/platforms/whatsapp/adapter.py, scripts/check-windows-footguns.py). One
# implementation, two names — never a second body.
...
def get_process_age_seconds(pid: int) -> Optional[float]:
    ...
    Deliberately NOT derived from :func:`_get_process_start_time`. That value is ``/proc``
    clock-ticks-since-boot on Linux and psutil centiseconds-since-epoch everywhere else — two
    origins in two units, meaningful only for same-source EQUALITY (its own docstring says
    so). Subtracting it from a wall clock is a unit error on every platform. psutil's
    ``create_time()`` is epoch seconds uniformly, which is the only reading age arithmetic
    can use.

    ``None`` means "don't know" — a dead PID, a denied handle, or psutil unavailable.
    Callers must fail open rather than treat it as young."""
    try:
        import psutil  # type: ignore
...
def acquire_scoped_lock(
    ...
    ``pid_recheck_after_seconds`` defends against the Windows race where a process that has
    ALREADY been terminated still answers ``WaitForSingleObject`` as running until it
    actually exits — so a restart sees the outgoing gateway as alive and declares FATAL even
    though it is dying. When > 0, an alive reading is re-checked after that sleep; a second
    reading of dead means the incumbent was merely on its way out and the lock is taken over.
    Default 0.0 preserves the fast-fail path for every other caller.
    """
    ...
    if existing:
        ...
        if not _scoped_lock_record_is_stale(existing, existing_pid):
            if not (pid_recheck_after_seconds > 0 and existing_pid is not None):
                return False, existing
            time.sleep(pid_recheck_after_seconds)
            if _pid_exists(existing_pid):
                return False, existing
            logger.warning(
                "acquire_scoped_lock(%s): PID %d initially reported alive but was reaped "
                "after a %.1fs recheck — taking over the lock.",
...
def verify_pid_file_matches_self() -> bool:
    """True when the PID file names THIS process.

    Added after observing a mismatch in the wild (the PID file held a dead PID while the
    real gateway was a different live PID). Returns a bool rather than auto-rewriting so a
    caller can decide whether a mismatch is expected (a ``--replace`` handoff) or a bug.
    """
    record = _read_pid_record()
    if record is None:
        return False
    try:
        return int(record["pid"]) == os.getpid()
    except (KeyError, TypeError, ValueError):
        return False
