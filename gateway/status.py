...
def terminate_pid(
    ...
    # Record only after the identity guard admits the operation.
    try:
        write_diag("process.terminate", victim_pid=pid, force=force,
                   killer_pid=os.getpid(), caller=_terminate_caller(), reason=reason)
    except Exception:
        pass
...
def _read_pid_record(pid_path: Optional[Path] = None) -> Optional[dict]:
    return _read_json_file(pid_path or _get_pid_path(), bare_pid_ok=True, unreadable_locked=True)


def _read_gateway_lock_record(lock_path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    return _read_json_file(lock_path or _get_gateway_lock_path(), bare_pid_ok=True, unreadable_locked=True)
...
def _pid_exists(pid: int) -> bool:
    ...
    try:
        ...
        pass  # Missing/failed psutil falls through to the native liveness probe.
    if _IS_WINDOWS:
...
def _pid_exists_win32_ctypes(pid: int) -> bool:
    ...
    try:
        ...
        if not handle:
            # ERROR_INVALID_PARAMETER (87) proves absence; denied/unknown is alive.
            # Never delete gateway metadata on an indeterminate liveness probe.
            return kernel32.GetLastError() != 87
        try:
            # WAIT_OBJECT_0 proves exit; WAIT_TIMEOUT and unknown/failure stay alive.
            wait_result = kernel32.WaitForSingleObject(handle, 0)
            return wait_result == WAIT_TIMEOUT or wait_result != 0
        finally:
...
def write_pid_file() -> None:
    ...
    _clear_running_pid_cache()
    logger.info("gateway.pid written: pid=%d argv=%r path=%s", record["pid"], record["argv"], path)
...
def acquire_scoped_lock(
    scope: str, identity: str, metadata: Optional[dict[str, Any]] = None, *, pid_recheck_after_seconds: float = 0.0
    ...
    if existing:
        ...
        if not _scoped_lock_record_is_stale(existing, existing_pid):
            if pid_recheck_after_seconds <= 0:
                return False, existing
            time.sleep(pid_recheck_after_seconds)
            refreshed = _read_json_file(lock_path)
            # A different starter may have acquired the identity during our wait.
    ...
    if is_gateway_runtime_lock_active(resolved_lock_path):
        ...
        for record in records:
            ...
            if _record_matches_live_gateway_pid(record, pid):
                return pid
        if any(isinstance(record, dict) and record.get("locked") for record in records):
            # Runtime lock was confirmed active. Unreadable identity is not stale.
            return None
...
def pid_exists(pid: int) -> bool:
    """Public liveness probe; shares the safe native/zombie-aware owner."""
