...
class SessionSessionsMixin:
    ...
    def create_session(self, session_id: str, source: str, **kwargs) -> str:
        ...
        # Stamp which agent drove this session, for the sources where the driver is the
        # local process (cli/tui). Gateway-spawned sessions inherit the gateway's
        # environment rather than the driver's, so they are deliberately not stamped.
    ...
    def _list_row(cls, row: sqlite3.Row) -> Dict[str, Any]:
        ...
        # Surface the driver as a top-level field for listings (derived from origin_json).
        # Here rather than in list_sessions_rich so every projection path gets it.
        cls._attach_session_driver(s)
    ...
    def _upsert_session_row(
        ...
        model_config: Dict[str, Any] = None, system_prompt: str = None, user_id: str = None,
        session_key: Optional[str] = None, chat_id: str = None, chat_type: str = None,
        thread_id: str = None, parent_session_id: str = None, cwd: str = None,
        profile_name: Optional[str] = None, git_repo_root: str = None,
        origin_json: str = None, display_name: str = None,
        # session_bridge/store.py supplies the origin timestamp when replaying an external
        # transcript, so the row keeps the event's own start time instead of now.
        started_at: Optional[float] = None,
    ) -> None:
        """:meth:`_insert_session_row` inside the CALLER's transaction (takes a live ``conn``).

        Public-by-use: session_bridge/store.py upserts inside its own write txn, so it cannot go
        through _insert_session_row, which opens one via _execute_write.

        Shares upstream's statement rather than the fork's older copy. The fork's version wrote 14
        columns — a strict SUBSET of upstream's 18, missing system_prompt_hash (and its dedupe),
        git_repo_root, origin_json and display_name, plus the model_config ``_reset_from``
        preservation and the parent-metadata inheritance. Keeping it would have meant a session row
        created through the bridge silently carrying less than one created anywhere else.
        """
