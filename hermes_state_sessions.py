...
class SessionSessionsMixin:
    ...
    def _inherit_parent_session_metadata(conn, session_id: str) -> None:
        """NULL-fill a child's workspace/profile from its parent (profile_name only within the same
        ...
        Local cli/tui cwd inheritance requires matching source and canonical native path. Explicit cwd
        remains authoritative; parent Git metadata only describes the same selected workspace."""
        child = conn.execute("SELECT source, parent_session_id, cwd, git_repo_root FROM sessions WHERE id = ?",
                             (session_id,)).fetchone()
        parent = None if child is None else conn.execute(
        ...
        if child is not None and parent is not None:
            ...
            conn.execute(
                ...
                "git_repo_root = COALESCE(git_repo_root, ?), git_branch = COALESCE(git_branch, ?) WHERE id = ?",
                (selected_cwd, parent["git_repo_root"] if same_workspace else None,
                 parent["git_branch"] if same_workspace and same_repo else None, session_id),
            )
    ...
    def _upsert_session_row(
        self, conn: sqlite3.Connection, session_id: str, source: str, model: str = None, model_config: Dict[str, Any] = None,
        ...
        conn.execute(
            """INSERT INTO sessions (
               ...
               ON CONFLICT(id) DO UPDATE SET
                   source = CASE
                       WHEN sessions.source = 'unknown' AND excluded.source != 'unknown'
                       THEN excluded.source ELSE sessions.source
                   END,
...
""" + _UPSERT_KEEP_EXISTING_SQL,
            (
                session_id, normalized_source, user_id, session_key, chat_id, chat_type, thread_id, model,
    ...
    def _insert_session_row(
        ...
        """Upsert with transcript write patience; caller-owned writes use _upsert_session_row."""
        def _do(conn):
    ...
    def list_open_session_ids(self, source: str) -> List[str]:
        """Source-scoped open IDs, oldest first, for the caller's guarded recovery API."""
        rows = self._read_all(
            "SELECT id FROM sessions WHERE source = ? AND ended_at IS NULL ORDER BY started_at, id",
            (source,),
        )
        return [str(row[0]) for row in rows]

    def get_inheritable_local_child_cwd(self, parent_session_id: str, child_source: str) -> Optional[str]:
        """Read exact local parent metadata; never discover a substitute process cwd."""
        if (not isinstance(parent_session_id, str) or not parent_session_id
                or not isinstance(child_source, str) or child_source not in _LOCAL_PERSISTED_CWD_SOURCES):
            return None
        parent = self._read_one("SELECT id, source, cwd FROM sessions WHERE id = ?", (parent_session_id,))
