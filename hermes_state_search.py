...
class SessionSearchMixin:
    ...
    def merge_fts(self, pages: Optional[int] = None) -> int:
        """Do a BOUNDED amount of FTS5 segment merging. Public API (tests pin the name).

        Delegates to ``_merge_fts_incrementally``, upstream's engine, rather than issuing its
        own ``'merge'`` commands: that version also lowers ``usermerge`` to its minimum of 2
        (persisted in ``%_config``) so a merge acts on any level with >= 2 segments — at the
        default of 4 a fragmented index cannot converge — and stops on a no-progress signal.

        RETURN VALUE CHANGED MEANING in the 0.21.1 merge: this used to count the INDEXES it
        merged into, and now returns the number of merge COMMANDS executed. Both are positive
        whenever an index exists and a command runs, so ``>= 1`` callers are unaffected, but do
        not read the number as an index count.
        """
        return self._merge_fts_incrementally(
            max_pages=pages if pages is not None else self._FTS_MERGE_MAX_PAGES_PER_INDEX)
