...
class SessionUsageMixin:
    ...
    def queue_token_counts(self, session_id: str, **kwargs) -> None:
        ...
        # Freeze mutable call-time attribution before the writer can consume it.
        if kwargs.get("model_config") is not None:
            kwargs["model_config"] = copy.deepcopy(kwargs["model_config"])
    ...
    def update_token_counts(
        ...
        self._insert_session_row(session_id, source or "unknown", model=model, model_config=model_config)
    ...
    def record_auxiliary_usage(
        ...
        self._insert_session_row(session_id, source or "unknown", model_config=model_config)
