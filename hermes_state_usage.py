...
class SessionUsageMixin:
    ...
    def _coalesce_token_deltas(self, batch: List[Tuple[str, Dict[str, Any]]]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        for session_id, kwargs in batch:
            ...
            if not kwargs.get("absolute"):
                # Route identity includes first-touch attribution. model_config is a dict,
                # so freeze nested containers before using it in the tuple key; adjacent
                # deltas with different attribution must never merge and silently retain
                # only the first delta's source/config.
                ...
                key = (
                    session_id,
                    *(_freeze(kwargs.get(f)) for f in self._TOKEN_DELTA_ROUTE_FIELDS),
                )
    ...
    def update_token_counts(
        ...
        # Attribution for the usage row a first-touch write creates. Without these the row is
        # stamped source="unknown" with no model config, which is where auxiliary spend loses
        # its provenance (#23270) — the same chain as agent/turn_context.py's auto-title call.
        source: Optional[str]=None, model_config: Optional[Dict[str, Any]]=None,
    ) -> None:
        ...
        self._insert_session_row(
            session_id, source or "unknown", model=model, model_config=model_config)
    ...
    def record_auxiliary_usage(
        ...
        self._insert_session_row(
            session_id, source or "unknown", model_config=model_config)
