...
if sys.platform == "win32":
    # File locking needs no optional Redis/OTel import chain.
    if "portalocker.redis" not in sys.modules:
        sys.modules["portalocker.redis"] = None
...
def setup_logging(
    *,
    ...
    Safe to call multiple times; handlers are idempotent per path. Level and
    rotation defaults come from config.yaml ``logging.*``. ``mode="gateway"`` adds
    ``gateway.log`` plus unfiltered forensics; ``mode="gui"`` adds ``gui.log``.
    Daemon roles select ``agent-<role>.log`` and ``errors-<role>.log``.
    """
    ...
    handler_specs = (
        (f"agent{role_suffix}.log", level, max_bytes, backups, None),
        (f"errors{role_suffix}.log", logging.WARNING, 2 * 1024 * 1024, 2, None),
    ...
    if mode == "gateway":
        try:
            ...
            logging.getLogger(__name__).warning(
                "gateway-forensics handler failed to attach: %s", exc,
            )
