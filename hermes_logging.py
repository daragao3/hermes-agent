...
if sys.platform == "win32":
    # concurrent_log_handler needs portalocker's cross-process FILE lock and never touches its
    # optional Redis-backed one. portalocker's __init__ nonetheless runs `from .redis import
    # RedisLock` inside a try/except ImportError — which SUCCEEDS whenever `redis` is installed
    # (it always is here), eagerly importing redis -> redis.observability ->
    # opentelemetry.sdk.metrics -> psutil: 10-14s of pure import cost on EVERY hermes
    # invocation, `hermes --version` included, and enough to blow past pytest's 30s cap for the
    # tests that subprocess-spawn the CLI. A None sentinel makes that import raise ImportError,
    # which portalocker already catches and answers with RedisLock = None — its own documented
    # no-redis fallback. Guarded so a real submodule already imported is never clobbered.
...
# Bounded retry, then a cooldown, for a rotation blocked by another process's open handle.
...
def setup_logging(
    *,
    ...
    ``role`` scopes the catch-all files: ``agent-<role>.log`` / ``errors-<role>.log`` instead
    of the shared pair, so a long-lived daemon owns its own files and Windows rotation is
    never blocked by a sibling's open handle. ``None`` (transient cli/cron) keeps the shared
    files; ``mode="gateway"`` defaults it to ``"gateway"``.
    """
    ...
    handler_specs = (
        (f"agent{suffix}.log", level, max_bytes, backups, None),
        (f"errors{suffix}.log", logging.WARNING, 2 * 1024 * 1024, 2, None),
    ...
    if mode == "gateway":
        # Unfiltered catch-all, gateway-process-scoped: a stable grep target for forensics
        # that neither competes with agent.log's CLI activity nor depends on whatever wrapper
        # spawned the gateway capturing stdout/stderr. A bad override costs one warning, not
        # the whole logging setup.
        try:
            _add_rotating_handler(
                _gateway_forensics_path(log_dir), level=logging.INFO,
            ...
            logging.getLogger(__name__).warning(
                ...
                os.environ.get("HERMES_GATEWAY_LOG_FILE"), exc)
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Rotation deferral state; see doRollover / _rotate_lock_safe.
    ...
    def doRollover(self):
        """Rotate the BASE file first, so a locked file never costs the backup chain.

        The stdlib shuffles and removes backups BEFORE the base->.1 rename. On Windows that
        rename fails with PermissionError [WinError 32] whenever any process or reader holds
        the file open, and the stdlib then raises — so logging dumps a full traceback on every
        emit once the file is pinned at maxBytes — and leaves the stream closed, with the
        backups it already shuffled gone. Observed live: agent.log pinned at 5MB, .1 and .2
        erased."""
        if self.stream:
            try:
                self.stream.close()
        ...
        if not self.delay:
            try:
                ...
                self.stream = None
        self._chmod_if_managed()
        # Our own rollover writes a new baseFilename; refresh the snapshot so the next emit
        # doesn't mistake it for external rotation.
        self._record_stream_stat()
        if rotated:
            self._rollover_blocked_until = 0.0
            self._rollover_warned = False
