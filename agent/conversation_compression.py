...
def _publish_rotated_compaction(
    ...
    from agent.context_compressor import _DB_PERSISTED_MARKER
    _child_source = agent.platform or os.environ.get("HERMES_SESSION_SOURCE", "cli")
    # Two sources for the child's cwd, vetted value first. get_inheritable_local_child_cwd
    # copies the PARENT ROW's persisted cwd, and only when the requested parent identity, the
    # returned row identity and the local source all agree and the value is a canonical
    # absolute native path — so it can never write a remote, relative or malformed cwd. When
    # it declines there was simply no vetted parent value to copy, and the live attribute is
    # better than persisting nothing.
    agent._session_db.publish_compression_child(
        parent_session_id=old_session_id, child_session_id=new_session_id,
        source=_child_source, model=agent.model,
        model_config=agent._session_init_model_config, system_prompt=new_system_prompt, messages=compressed,
        cwd=(agent._session_db.get_inheritable_local_child_cwd(old_session_id, _child_source)
             or getattr(agent, "working_directory", None)), profile_name=_profile_for_child,
