...
def _publish_rotated_compaction(
    ...
    # Local rotation inherits only the exact persisted parent cwd, checked by
    # publish_compression_child inside its transaction. Do not substitute the
    # process/agent working directory for missing or ineligible parent metadata.
    child_cwd = None if child_source in {"cli", "tui"} else getattr(agent, "working_directory", None)
    agent._session_db.publish_compression_child(
        parent_session_id=old_session_id, child_session_id=new_session_id,
        source=child_source, model=agent.model,
        model_config=agent._session_init_model_config, system_prompt=new_system_prompt, messages=compressed,
        cwd=child_cwd, profile_name=_profile_for_child,
