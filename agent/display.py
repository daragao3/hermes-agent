...
def _clean_shell_segment(segment: str, *, keep_redirects: bool = True) -> str:
    ...
    (``> /dev/null``) are pure noise and always go. Every other redirect is KEPT by
    default: where a command's output landed is the single fact a reader needs to
    attribute a written file, and hiding it once made a stray ``> NUL`` write
    unattributable from the transcript.

    ``keep_redirects=False`` yields the fully-stripped form used for CLASSIFYING a segment,
    so that classification is unaffected by text inside a redirect target — a file named
    ``echo``, or a target containing ``--``, must not change what the segment is judged to be."""
    words = _split_shell_words(segment)
    out: list[str] = []
    i = 0
    while i < len(words):
...
def summarize_shell_command(command: str) -> str:
    ...
    for segment in segments:
        ...
        # Classify on the fully-stripped form and DISPLAY the form that keeps redirects:
        # a redirect target must never decide what the segment is judged to be.
