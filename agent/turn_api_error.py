...
def _is_local_validation_error(api_error: Any) -> bool:
    ...
    if isinstance(api_error, TypeError) and "nonetype" in _text and "not iterable" in _text:
        return False
    # Broader than the message match above, which names one shape: this asks WHERE the
    # exception was raised. A ValueError/TypeError from inside an LLM SDK is
    # backend-contract drift, not a bug in our code, so it must retry and fall back rather
    # than abort the turn (the R57 openai parse_response TypeError on output=None).
    from agent.error_classifier import is_library_exception

    return not is_library_exception(api_error)
