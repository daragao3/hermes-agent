...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # An attribute so a test can drive request()'s deadline without sleeping through it.
    ...
    def initialize(
        ...
        # Re-check after the reply: a cancel that arrives while `initialize` is in flight must
        # not be answered by marking the client initialized and sending `initialized` anyway.
    ...
    def request(
        ...
        With a ``cancel_event`` the wait is sliced so the event can be observed; without one it
        is a single full-length get, exactly as before — the common path must not start waking
        ten times a second to poll something nobody passed."""
        if cancel_event is not None and cancel_event.is_set():
            raise CodexRequestCancelled("codex app-server request cancelled")
        ...
        try:
            ...
            # BaseException, not Exception: a KeyboardInterrupt mid-write leaks the pending
            # entry just as effectively as an OSError would.
            with self._pending_lock:
...
import time  # noqa: F401,F811,E402
# ---- END PLUGIN-COMPAT ----
