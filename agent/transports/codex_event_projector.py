...
class CodexEventProjector:
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        return self.project_item((notification.get("params", {}) or {}).get("item") or {})

    def project_item(self, item: dict) -> ProjectionResult:
        """Project one completed app-server item using the live event semantics.

        Split out of :meth:`project` so ``session_bridge.codex_adapter`` can replay a completed
        item that never arrived as a notification, without synthesizing a fake envelope."""
