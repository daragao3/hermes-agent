...
def _fire_post_api_request_hook(
    ...
    # Attribute the sanitized response usage. Deliberately OUTSIDE the post_api_request
    # block below: accounting must not depend on whether that hook is installed.
    agent._record_activity_response(response)
    try:
