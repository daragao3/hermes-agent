...
from agent.anthropic_credentials import (
    _is_oauth_token, refresh_anthropic_oauth_pure, resolve_anthropic_token)
...
# Lazy auth shims preserve the historical module patch surface without importing
# hermes_cli.auth (and its config/plugin graph) during collector startup.
def resolve_codex_runtime_credentials(**kwargs: Any) -> dict:
...
# --- lazy httpx, and the cooperative timeout budget -------------------------------------------
#
# httpx is NOT imported at module scope: this module is imported far more often than it is used,
# and the eager import is measurable on every CLI start (pinned by
# tests/agent/test_account_usage_import_cost.py).
...
def __getattr__(name: str):
    """Resolve heavyweight compatibility attributes on first access (PEP 562)."""
    if name == "httpx":
...
def _resolve_codex_usage_credentials(
    ...
    # Import the exception class only after the explicit-credential fast path.
    # Module attribute access still exposes this exact class through __getattr__,
    # preserving account_usage.AuthError construction in tests/plugins.
    from hermes_cli.auth import AuthError
...
def _usage_windows(
    ...
    fraction: bool = False, label_fn: Optional[Callable[[dict, str], str]] = None,
) -> list[AccountUsageWindow]:
    """Build windows from ``source[key][used_key]``; ``fraction`` scales values <= 1 to percent.

    ``label_fn(window, fallback)`` lets a provider name a window from its OWN payload instead of
    from its position in *mapping*. Codex needs it: the backend does not guarantee that
    ``primary_window`` is the 5-hour one, so trusting the order mislabels both rows."""
    windows: list[AccountUsageWindow] = []
    for key, label in mapping:
        ...
        if fraction and used <= 1:
            used *= 100
        windows.append(AccountUsageWindow(
            label=label_fn(window, label) if label_fn else label,
            used_percent=used, reset_at=_parse_dt(window.get(reset_key))))
    return windows
...
def _fetch_codex_account_usage(
    ...
    payload = _get_json(_codex_backend_urls(resolved_base_url)[0], _codex_headers(token, account_id),
                        timeout=timeout)
    # label_fn: the backend does not guarantee primary_window is the 5-hour one.
    windows = _usage_windows(payload.get("rate_limit") or {},
...
def _fetch_anthropic_account_usage(
    ...
    """The primary subscription's usage.

    Prefers the PINNED isolated profile ``~/.claude-anthropic1`` when it exists.
    ``resolve_anthropic_token()`` ultimately reads ``~/.claude``, which follows whichever
    account the desktop app is signed into — so this row used to change SUBJECT on every
    account switch. On 2026-08-23 14:18 the primary flipped to the second subscription's
    account and this row silently became a duplicate of "Claude 2". Pinning fixes the subject;
    the fallback below keeps the row working on a box that never created the profile."""
    ...
    payload = _get_json("https://api.anthropic.com/api/oauth/usage", headers, timeout=timeout)
...
def _fetch_openrouter_account_usage(
    ...
    # Keep runtime_provider (and its model catalog -> models_dev -> requests
    # dependency chain) off the 5-minutely collector's module-import path.
    ...
    with _ensure_httpx().Client(timeout=timeout) as client:
        def _data(path: str, *, budgeted: bool = False) -> dict:
            kwargs = {}
            if budgeted:
                # The budget covers BOTH calls, so this one only gets what the first left.
                # Overrunning here is the failure the clamp exists to prevent; by this point
                # the balance is already in hand and this payload is optional, so a spent
                # budget degrades to omitting the quota window rather than losing the row.
                remaining = timeout - (time.monotonic() - started)
                if remaining <= 0:
                    raise TimeoutError("usage budget spent on the credits call")
                kwargs["timeout"] = remaining
            resp = client.get(f"{normalized}/{path}", headers=headers, **kwargs)
            resp.raise_for_status()
            return (resp.json() or {}).get("data") or {}
        credits = _data("credits")
        try:
            key_data = _data("key", budgeted=True)
        except Exception:
            key_data = {}
...
# (base_url, api_key, timeout) -> snapshot. The timeout is part of the contract, not an extra:
# it is how ai_usage.collector's cooperative budget reaches every provider uniformly.
_UsageFetcher = Callable[[Optional[str], Optional[str], float], Optional[AccountUsageSnapshot]]
_USAGE_FETCHERS: dict[str, _UsageFetcher] = {
    "openai-codex": lambda b, k, t: _fetch_codex_account_usage(base_url=b, api_key=k, timeout=t),
    "anthropic": lambda b, k, t: _fetch_anthropic_account_usage(b, k, timeout=t),
    "anthropic2": lambda b, k, t: _fetch_anthropic2_account_usage(base_url=b, api_key=k, timeout=t),
    "kimi": lambda b, k, t: _fetch_kimi_account_usage(base_url=b, api_key=k, timeout=t),
    "deepseek": lambda b, k, t: _fetch_deepseek_account_usage(base_url=b, api_key=k, timeout=t),
    "opencode-go": lambda b, k, t: _fetch_opencode_go_account_usage(base_url=b, api_key=k, timeout=t),
    "xai": lambda b, k, t: _fetch_grok_account_usage(base_url=b, api_key=k, timeout=t),
}
...
def fetch_account_usage(
    ...
    ``budget_seconds`` is the cooperative budget from ``ai_usage.collector.collect()``: the
    wall-clock this call may spend before the collector's own deadline expires. DECLARING it is
    what makes ``collector._supports_budget`` return True, so the collector passes the remaining
    time instead of falling back to an unbounded call — and the value is clamped onto the HTTP
    timeout below, so the bound is real rather than merely declared. ``None`` (the CLI and
    /usage paths) keeps each fetcher's own default."""
    normalized = str(provider or "").strip().lower()
    timeout = _budgeted_timeout(_DEFAULT_USAGE_TIMEOUT, budget_seconds)
    try:
        # Two providers do not fit the table's shape and stay explicit rather than being
        # forced into it: openrouter has its own (longer) default budget, and gemini makes
        # more than one call so it needs the raw budget, not a per-request clamp.
        ...
        return fetcher(base_url, api_key, timeout) if fetcher else None
    except Exception:
        return None


# --- providers and helpers this fork adds -----------------------------------------------------
