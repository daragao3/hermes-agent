...
def auto_title_session(
    ...
    runtime_validator: Optional[RuntimeValidator] = None,
    # Accounting attribution for the titler's OWN llm call. NOT the same `source` as the
    # title provenance below (llm/derived/user) — that local is renamed title_source so the
    # two meanings can never be confused again.
    ...
    try:
        ...
        set_accounting_context(session_db, session_id, source=source, model_config=model_config)
        title, title_source = generate_title(
        ...
        if not title:  # the inline attempt declined collisions; off the critical path the lineage scan is affordable
            title, title_source = derive_title(user_message), "derived"
        if not title:
            return
        try:
            persisted = _persist_session_title(session_db, session_id, title, source=title_source)
        except Exception as e:
            logger.debug("Failed to set auto-generated title: %s", e)
            return
        if persisted is not None:
            logger.debug("Auto-generated session title: %s", persisted)
            _notify_title(title_callback, persisted, title_source, "Auto-title")
    except Exception as e:
...
def maybe_auto_title(
    ...
    threading.Thread(
        ...
        kwargs=dict(failure_callback=failure_callback, main_runtime=main_runtime,
                    title_callback=title_callback, runtime_validator=runtime_validator,
                    source=source, model_config=model_config),
        daemon=True,
        name="auto-title",
    ).start()
