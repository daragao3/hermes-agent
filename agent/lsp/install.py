...
def _run_installer(tool: str, pkg: str, cmd: list, *, timeout: int, env: Optional[dict] = None) -> bool:
    ...
    # run_text_capture, not subprocess.run(capture_output=True): `go install` and
    # `npm install` both fork a build toolchain, and the grandchild inherits the capture
    # pipes and holds their write end open past the direct child's death. subprocess.run
    # kills only that child and then blocks re-draining a pipe that can never reach EOF, so
    # the TimeoutExpired caught below would never be raised. The helper captures into temp
    # files, tree-kills, and sets CREATE_NO_WINDOW itself.
    from hermes_cli._subprocess_compat import run_text_capture
    try:
        proc = run_text_capture(cmd, timeout=timeout, env=env, stdin=subprocess.DEVNULL)
