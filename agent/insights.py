...
def _has_billable_usage(session: Dict[str, Any]) -> bool:
    ...
    Counts every token bucket, not just input/output: a cache-read-only turn is real
    billable usage. ``api_call_count`` is included as well so a call that somehow reported
    no tokens still registers as usage rather than being silently written off as free."""
...
def _estimate_cost(session_or_model: Dict[str, Any] | str, input_tokens: int = 0, output_tokens: int = 0, *, cache_read_tokens: int = 0,
                   cache_write_tokens: int = 0, provider: Optional[str] = None, base_url: Optional[str] = None) -> tuple[Optional[float], str]:
    """Estimate the USD cost for a session row or a model/token tuple.

    Returns ``(None, "unknown")`` when the route cannot be priced. None and 0.0 are
    DIFFERENT answers and callers must keep them apart: None means "nobody could price
    these tokens", 0.0 means "these tokens are genuinely free" (a subscription route,
    status "included"). This used to return ``float(amount or 0.0)``, which collapsed the
    two — an unpriceable route then contributed $0 to a total the reader takes for
    complete, silently under-reporting spend rather than declaring it unknown."""
...
class InsightsEngine:
    ...
    def _compute_overview(self, sessions: List[Dict], message_stats: Dict, models: Optional[List[Dict]] = None) -> Dict:
        ...
        # Tokens on sessions nobody could price. total_cost sums only the sessions that WERE
        # priced, so a non-zero figure here means the total is a FLOOR, not a complete number
        # — surface it alongside the total rather than letting unpriced tokens read as $0.
        unpriced_tokens = 0
        ...
        for s in sessions:
            ...
            if estimated is None:
                unpriced_tokens += sum(int(s.get(k) or 0) for k in _TOKEN_KEYS)
            else:
                total_cost += estimated
            actual_cost += s.get("actual_cost_usd") or 0.0
            status_counts[status] += 1
            if status == "unknown" and _has_billable_usage(s):
                # A session that consumed NOTHING is not unpriced spend, it is NO spend, and
                # counting it is a false positive in the one counter that exists to raise
                # suspicion about the total. Measured 2026-09-06: the only three sessions this
                # reported were cron fires that never called a model (api_call_count 0, every
                # bucket 0), so they have no route to resolve and never will. Deliberately
                # asymmetric with included_cost_sessions below, which is descriptive rather
                # than a warning and is harmless when a zero-usage session inflates it.
        ...
        if models:
            ...
            # Recompute from the same source as the total it caveats, or the two disagree:
            # the breakdown covers auxiliary rows and residuals the session counters do not.
            unpriced_tokens = sum(int(m.get("unpriced_tokens") or 0) for m in models)
        ...
        return {
            ...
            # > 0 means "estimated_cost" is a FLOOR: these tokens were served by a route
            # with no pricing data and contribute no dollars to it.
    ...
    def _compute_model_breakdown(self, sessions: List[Dict], cutoff: float, source: str = None) -> List[Dict]:
        ...
        model_data = defaultdict(lambda: {"sessions": set(), **dict.fromkeys(_TOKEN_KEYS, 0), "reasoning_tokens": 0, "total_tokens": 0,
                                          "api_calls": 0, "tool_calls": 0, "cost": 0.0,
                                          "actual_cost": 0.0, "unpriced_tokens": 0})
        ...
        def _accumulate(model, provider, base_url, session_id, counts: Dict[str, int], *,
            ...
            if estimate is None:
                # Same rule as the overview: an unpriceable route adds no dollars, so its
                # tokens are recorded as unpriced instead of silently costing $0.
                d["unpriced_tokens"] += sum(counts[k] for k in _TOKEN_KEYS)
            else:
                d["cost"] += estimate
