"""Shared auxiliary router with local automatic fallback policy.

Text uses the main provider, then local/custom and API-key resolution.
Vision uses a capable main provider, then DeepInfra. Other supported
providers remain available by explicit selection.
"""
...
def _try_openrouter(explicit_api_key: str = None, model: str = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if not or_key:
        _mark_provider_unhealthy(
            "openrouter", ttl=60, reason=_describe_openrouter_unavailable(model=or_model)
        )
        return None, None
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if runtime is None and not nous:
        ...
        _mark_provider_unhealthy("nous", ttl=60, reason="no usable credential")
        return None, None
...
def _mark_provider_unhealthy(
    provider: str, ttl: Optional[float] = None, reason: Optional[str] = None,
    *, base_url: Optional[str] = None,
) -> None:
    """Quarantine one endpoint, retaining the cause and quieting repeated reports."""
...
def _auth_refresh_provider_for_route(resolved_provider: Optional[str], client_base_url: str) -> str:
    ...
    if normalized and normalized != "auto":
        return normalized
    return _provider_for_host(client_base_url, _AUTH_REFRESH_PROVIDER_BY_HOST) or _concrete_provider_for_route(resolved_provider, client_base_url)
...
def _quarantine_fallback_candidate(
    ...
    _mark_provider_unhealthy(fb_provider or fb_label, reason="stale or unrefreshable credential", base_url=base_url)
...
_VISION_AUTO_PROVIDER_ORDER = ("deepinfra",)
...
def _deepinfra_strict_vision_backend(model: Optional[str]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if not vision_model:
        logger.debug("Vision auto-detect: no deepinfra vision model resolved: credential missing or rejected, or catalog returned no vision-tagged chat models — skipping")
        return None, None
...
def get_available_vision_backends() -> List[str]:
    """Report available vision providers using the runtime capability decision."""
    available: List[str] = []
    main_provider, main_model, runtime = _vision_main_selection(_normalize_main_runtime(None))
    if main_provider and main_provider not in {"auto", "", "moa"}:
        client, _ = _vision_main_provider_client(main_provider, main_model, runtime, None, None)
        if client is not None:
            available.append(main_provider)
    for provider in _VISION_AUTO_PROVIDER_ORDER:
        if provider != main_provider and _resolve_strict_vision_backend(provider)[0] is not None:
...
def _vision_main_provider_client(
    ...
    block = _main_provider_vision_block(main_provider, vision_model)
    if block:
        logger.debug("Vision auto-detect: skipping main provider %s (%s)", main_provider, block)
        return None, None
...
def _vision_auto_route(
    ...
    """Auto-detect order: main provider + model, then the configured automatic provider order."""
    main_provider, main_model, runtime = _vision_main_selection(runtime)
    if main_provider and main_provider not in {"auto", "", "moa"}:
...
def _validate_llm_response(
    ...
    billing_provider = _concrete_provider_for_route(provider, base_url or "")
    if billing_provider == "auto":
...
def _vision_main_selection(runtime: Dict[str, Any]) -> Tuple[str, str, Dict[str, Any]]:
    """Resolve the same main provider identity for discovery and runtime."""
    ...
    return main_provider, main_model, runtime
