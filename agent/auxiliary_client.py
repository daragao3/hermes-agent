...
def _mark_provider_unhealthy(
    ...
    The reason travels with the endpoint-scoped mark because missing credentials,
    rate limits, and payment failures require different operator actions. Repeated
    marks for the same reason are the steady state for an unavailable provider and
    log at DEBUG rather than warning on every auxiliary call.
    """
    ...
    ttl_seconds = _AUX_UNHEALTHY_TTL_SECONDS if ttl is None else ttl
    repeat = _is_provider_unhealthy(label, base_url) and _aux_unhealthy_reason.get(key) == reason
...
def _log_skip_unhealthy(
    ...
    if now - _aux_unhealthy_logged_at.get(key, 0.0) >= 60:
        ...
        logger.info(
            ...
            task or "call", label,
            _aux_unhealthy_reason.get(key, _AUX_UNHEALTHY_DEFAULT_REASON),
...
def get_available_vision_backends() -> List[str]:
    ...
    This REPORTS on vision availability; resolve_vision_provider_client ACTS on it. The two
    must never disagree, so the main-provider verdict comes from _main_provider_vision_block
    and probes the provider's dedicated VISION model, not its chat model. On 2026-08-25 they
    did disagree and `hermes setup` advertised vision the runtime then returned nothing for.

    The sole production caller is hermes_cli/setup.py (the Tool Availability Summary line);
    the vision tools call resolve_vision_provider_client directly. An earlier docstring here
    claimed 'setup, tool gating, and runtime auto-routing' — only the first was ever true, and
    that overstatement is what made the disagreement look harmless.

    An empty result is a supported state, not an error: callers report vision as unconfigured.
    """
    available: List[str] = []
    main_provider = _read_main_provider()
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_STRICT_BACKENDS:
            ...
            # Mirror resolve_vision_provider_client's auto path exactly: the provider's
            # dedicated vision model wins over its chat model, and the shared capability
            # guard decides. Reporting on a different question than the runtime answers is
            # what made this advertise vision no vision call could serve.
            vision_model = _resolve_provider_vision_default(main_provider) or _read_main_model()
            main_ok = (
                not _main_provider_vision_block(main_provider, vision_model)
                and resolve_provider_client(main_provider, vision_model)[0] is not None
            )
        if main_ok:
...
def _vision_main_provider_client(
    ...
    if block_reason := _main_provider_vision_block(main_provider, vision_model):
        logger.debug(
            "Vision auto-detect: skipping main provider %s (%s) — falling through to aggregator chain",
            main_provider, block_reason,
        )
        return None, None
...
def _validate_llm_response(
    ...
    # An auto-routed call arrives with *provider* still the literal "auto", which no billing
    # route recognises — those rows priced as unknown. Pass the concrete backend as
    # billing_provider SEPARATELY, because `provider` also selects the usage-normalisation
    # shape and the two answers differ: the adapters hand back OpenAI-shaped usage
    # (prompt_tokens/completion_tokens) whatever vendor served the call, so billing wants
    # "anthropic" while normalisation must NOT have it, or it reads the unset input_tokens
    # and the row is dropped as zero rather than merely mis-attributed.
    #
    # Only override when a concrete backend was actually recovered: the resolver normalises a
    # missing provider to "auto", and the fallback paths that omit the route entirely must keep
    # recording an EMPTY route rather than inventing a literal "auto" that reads like real config.
...
def _ladder_provider_fallback(first_err: Exception, route: _LadderRoute):
    ...
    if reason == "payment error":
        ...
        _mark_provider_unhealthy(
            ...
            or resolved_provider, base_url=route.base_info, reason="payment error")
...
# ── Carried from the fork (0.21.1 merge) ─────────────────────────────────────
# Fork-only surface with live callers: _resolve_auto (hermes_cli/web_server.py),
# build_codex_probe_client (obs/backend_conformance_canary.py), and the probe/vision/
# unhealthy-reason helpers the fork's own tests pin.
