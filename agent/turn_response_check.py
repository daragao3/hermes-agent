...
def check_api_response(
    ...
    # The call SUCCEEDED. If this provider/model had an open rate-limit episode, close it
    # and emit RECOVERED. This is the ONLY recovery signal: every other hook fires on
    # failure, so inferring recovery from silence would mistake an idle window for a healthy
    # provider. clear() is a cheap no-op when no episode is open — the common case.
    try:
        from events.rate_limit_signal import clear as _clear_rate_limit_episode

        _clear_rate_limit_episode(provider=agent.provider, model=agent.model)
    except Exception:
        pass
