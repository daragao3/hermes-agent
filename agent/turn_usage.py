    ...
    if agent._session_db and agent.session_id:
        try:
            ...
            agent._session_db.queue_token_counts(
                ...
                model=agent.model,
                api_call_count=1,
                # source/model_config attribute the row to the SESSION, not just the model.
                # Without them a FIRST-TOUCH write stamps source="unknown" with no model
                # config and the spend loses its provenance (#23270) -- the cron case, whose
                # very first token write is that first touch. The codex runtime passes the
                # same pair through its billing() helper.
