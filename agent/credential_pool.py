...
class CredentialPool(CredentialPoolAdminMixin):
    ...
    def _quarantine_terminal_chain(self, entry: PooledCredential, sources: Set[str], exc: Exception) -> None:
        """Retire only this dead chain; preserve unrelated fresh credentials."""
        with self._lock:
            removed_ids = self._quarantine_dead_chain(entry, sources)
            self._mark_dead_from_terminal_refresh(entry, exc)
            current = self._find(lambda item: item.id == self._current_id)
            if self._current_id in removed_ids or (current is not None and current.id == entry.id and current.last_status == STATUS_DEAD):
                self._current_id = None
            self._persist(removed_ids=removed_ids)
        ...
        *,
        ...
        if status_code in (402, 429):
            try:
                ...
                record(provider=self.provider, model=f"{self.provider}:pool",
    ...
    def _recover_failed_refresh(self, entry: PooledCredential, exc: Exception) -> Optional[PooledCredential]:
        ...
        if self.provider == "anthropic":
            ...
            if getattr(auth_mod, terminal_fn_name)(exc):
                ...
                self._quarantine_terminal_chain(entry, {"device_code"}, exc)
                return None
        elif self.provider == "nous":
            synced = self._sync_nous_entry_from_auth_store(entry)
            if synced.refresh_token != entry.refresh_token:
            ...
            if auth_mod._is_terminal_nous_refresh_error(exc):
                ...
                self._quarantine_terminal_chain(
                    entry,
            ...
            with _auth_store_lock():
                ...
                if not store_refresh or store_refresh == str(entry.refresh_token or "").strip():
                    ...
                    auth_mod._quarantine_nous_pool_entries(auth_store, exc, reason="credential_pool_refresh_failure",
                                                         dead_refresh_token=entry.refresh_token)
    ...
    def try_refresh_matching(
        ...
        with self._lock:
            ...
            if api_key_hint and (entry is None or entry.runtime_api_key != api_key_hint):
                rotated = self._entry_rotated_from(api_key_hint)
                if rotated is not None and (entry is None or entry.id == rotated.id):
...
# Reuse within one profile only; root fallback changes must also expire a hit.
_POOL_CACHE: Dict[Tuple[str, str], tuple] = {}
...
def _pool_store_signature() -> tuple:
    root_path = auth_mod._global_auth_file_path()
    root_stamp = None
    if root_path is not None:
        try:
            stat = root_path.stat()
            root_stamp = (str(root_path.resolve()), stat.st_mtime_ns, stat.st_size)
        except FileNotFoundError:
            root_stamp = (str(root_path.resolve()), None, None)
    return _auth_json_mtime(), root_stamp

def load_pool(provider: str) -> CredentialPool:
    """Reuse a pool only while its profile and both credential stores match."""
    provider = (provider or "").strip().lower()
    key = (str(get_hermes_home().resolve()), provider)
    now = time.time()
    try:
        signature = _pool_store_signature()
    except OSError:
        return _load_pool_uncached(provider)
    with _POOL_CACHE_LOCK:
        cached = _POOL_CACHE.get(key)
        if cached is not None:
            cached_at, cached_signature, pool = cached
            if signature == cached_signature and now - cached_at < _POOL_CACHE_TTL_SECONDS:
                return pool
    pool = _load_pool_uncached(provider)
    before_load = signature
    try:
        signature = _pool_store_signature()
    except OSError:
        return pool
    # A loader or peer changed a store: do not certify this pool against a
    # signature it was not loaded from. The next call must reload.
    if signature != before_load:
        return pool
    with _POOL_CACHE_LOCK:
        _POOL_CACHE[key] = (time.time(), signature, pool)
    return pool

def invalidate_pool_cache(provider: Optional[str] = None) -> None:
    """Invalidate the selected provider in every scope, or clear all scopes."""
    with _POOL_CACHE_LOCK:
        if provider is None:
            _POOL_CACHE.clear()
        else:
            selected = provider.strip().lower()
            for key in list(_POOL_CACHE):
                if key[1] == selected:
