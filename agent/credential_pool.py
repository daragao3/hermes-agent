...
class CredentialPool(CredentialPoolAdminMixin):
    def __init__(self, provider: str, entries: List[PooledCredential]):
        ...
        # Runtime api keys this pool has rotated away from, mapped to the entry that
        # superseded them. See _record_superseded_api_key.
    ...
    def _quarantine_sources(self, entry: PooledCredential, sources: Set[str]) -> None:
        """Drop entries seeded from *sources* that carry ENTRY's dead refresh chain.

        Atomic read-modify-write of ``self._entries``: this runs on the DEFERRED refresh path
        (outside the pool lock), so take the RLock here; still-locked callers re-enter safely.

        Matching on the refresh token as well as the source is what makes this precise, and
        removing by source ALONE is a real defect, not a simplification. A pool routinely holds
        entries on DIFFERENT chains: an old ``manual:device_code`` stranded on a revoked lineage
        alongside a ``device_code`` seeded from a freshly re-authed singleton. Source-only removal
        destroyed the healthy entry whenever the stale one failed, leaving nothing selectable, so
        the caller's agent session never started (``profiles/financier``, 2026-07-22..27).

        An entry with no refresh token matches only when *entry* has none either, so a chain that
        was never identified reclaims just the tokenless rows.
        """
        dead_refresh = str(entry.refresh_token or "").strip()

        def _same_dead_chain(item) -> bool:
            return (item.source in sources
                    and str(item.refresh_token or "").strip() == dead_refresh)

        with self._lock:
            removed_ids = [item.id for item in self._entries if _same_dead_chain(item)]
    ...
    def _mark_exhausted(
        ...
        *,
        ...
        # A credential burned. The MODEL is unchanged; the KEY ran out. Only the
        # rate-limit-shaped status codes are reported: a 401 is an auth problem, not a
        # limit, and must not read as one.
    ...
    def _refresh_entry_impl(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            ...
            # A codex refresh failure must NOT be silent. This path used to log at DEBUG and
            # simply exhaust the entry, so a dead single-use refresh-token chain went unnoticed
            # for 33 days. Surface it and drop a sentinel the health probe reads.
            if self.provider == "openai-codex":
        ...
        if self.provider == "openai-codex":
            # Healthy refresh — clear any prior failure sentinel.
            with contextlib.suppress(Exception):
    ...
    def _recover_failed_refresh(self, entry: PooledCredential, exc: Exception) -> Optional[PooledCredential]:
        ...
        if self.provider == "anthropic":
            ...
            if getattr(auth_mod, terminal_fn_name)(exc):
                ...
                # Singleton-seeded rows must disappear with their dead authority,
                # while manual rows remain briefly as a terminal audit record.
        ...
        *,
        ...
        with self._lock:
            ...
            if entry is None:
                ...
                if entry is None and api_key_hint:
                    # A hint matching no live entry is not necessarily unknown: a concurrent
                    # caller sharing this pool instance may have already rotated it away. That
                    # caller spent the single-use refresh token, so re-running the refresh earns
                    # a refresh_token_reused 4xx, while returning None strands a caller whose
                    # usable credential is sitting right here. Adopt the successor instead.
                    rotated = self._entry_rotated_from(api_key_hint)
                    if rotated is not None:
                        logger.debug(
                            "credential pool: hint for %s already rotated by a concurrent "
                            "caller — adopting the refreshed entry",
    ...
    def _mark_dead_from_terminal_refresh(
        ...
        """Mark a retained manual entry dead after its refresh chain is revoked.

        Singleton-seeded rows are removed so they cannot be re-seeded from dead
        provider state. Manual rows remain as an audit record until the existing
        dead-entry retention window prunes them, so terminal branches must stamp
        their status before returning.
        """
        current = next((item for item in self._entries if item.id == entry.id), None)
        if current is None or current.refresh_token != entry.refresh_token:
            return
...
# Upstream's loader, renamed: load_pool below is the cached wrapper over it (the fork's
# split). Upstream's body is kept — the fork's copy of this loader had diverged.
...
# The pool CACHE layer (agent/auxiliary_client.py calls invalidate_pool_cache in production)
# and the superseded-key history the dead-chain quarantine reads.
