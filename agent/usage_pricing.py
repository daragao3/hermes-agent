...
# Fork corrections to the Gemini rate card (2026-09-04 reconciliation against
# https://ai.google.dev/gemini-api/docs/pricing; agent-src dad2ef2549). Two
# deliberate departures from the upstream rows above, both erring HIGH -- the
# only direction a cost estimate is allowed to err here:
#   * gemini-2.5-flash was booked at $0.15/$0.60 against Google's published
#     $0.30/$2.50 -- a 2x input / 4.2x output UNDER-report.
#   * 3.8/3.7-flash book the POST-promo $1.50/$7.50, not the $0.75/$3.75 Google
#     charges through 2026-12-31, so no edit is needed at the new year.
# The cache-read rates keep upstream's 10%-of-input ratio. Sourced from the
# current docs page, not the retired ai.google.dev/pricing behind _GOOGLE_URL.
_GOOGLE_DOCS_URL = "https://ai.google.dev/gemini-api/docs/pricing"
_OFFICIAL_DOCS_PRICING[("google", "gemini-2.5-flash")] = _snap(
    "0.30", "2.50", "0.03", url=_GOOGLE_DOCS_URL, version="google-pricing-2026-09-04",
)
for _gemini_model in ("gemini-3.8-flash", "gemini-3.7-flash"):
    _OFFICIAL_DOCS_PRICING[("google", _gemini_model)] = _snap(
        "1.50", "7.50", "0.15", url=_GOOGLE_DOCS_URL, version="google-pricing-2026-09-04",
    )
del _gemini_model
# Rows the upstream snapshot does not carry.
#
# Moonshot/Kimi: the subscription surface (kimi-for-coding) and the k3 family bill at the
# same published rates, so they share one row rather than three copies that can drift.
_OFFICIAL_DOCS_PRICING[("google", "gemini-2.5-flash-lite")] = _snap(
    "0.10", "0.40", url=_GOOGLE_URL, version="google-pricing-2026-09-04",
)
for _kimi_model in ("kimi-k3", "k3", "kimi-for-coding"):
    _OFFICIAL_DOCS_PRICING[("moonshot", _kimi_model)] = _snap(
        "3.00", "15.00", "0.30", url="https://platform.kimi.ai/docs/pricing/chat-k3", version="moonshot-pricing-2026-09-04",
    )
del _kimi_model
_OFFICIAL_DOCS_PRICING[("moonshot", "kimi-k2.6")] = _snap(
    "0.95", "4.00", "0.16", url="https://platform.kimi.ai/docs/pricing/chat-k26", version="moonshot-pricing-2026-09-04",
)
_OFFICIAL_DOCS_PRICING[("moonshot", "kimi-k2.7-code")] = _snap(
    "0.95", "4.00", "0.19", url="https://platform.kimi.ai/docs/pricing/chat-k27-code", version="moonshot-pricing-2026-09-04",
)
_OFFICIAL_DOCS_PRICING[("moonshot", "kimi-k2.7-code-highspeed")] = _snap(
    "1.90", "8.00", "0.38", url="https://platform.kimi.ai/docs/pricing/chat-k27-code", version="moonshot-pricing-2026-09-04",
)
...
# Subscription route -> the vendor that sells the same tokens per-token. Lets
# ``ignore_subscription=True`` answer "what would these tokens have cost at
# list"; without it a subscription workload reads as free and wins every cost
...
def resolve_billing_route(
    ...
    # Kimi Coding Plan (api.kimi.com/coding): a flat monthly subscription with a
    # refreshing quota, never dollars -- same shape as openai-codex, so calls are
    # "included" at $0 rather than "unknown" at None. Matched on the provider slug
    # ONLY, deliberately: the underlying-vendor re-resolution in get_pricing_entry
    # re-enters this function with the same base_url, and a base_url branch here
    # would match again and pin the route to "included", making the
    # ignore_subscription counterfactual unreachable. The pay-as-you-go Kimi Open
    # Platform (api.moonshot.ai) is a different surface and prices as "moonshot".
...
def get_pricing_entry(
    ...
    if route.billing_mode == "subscription_included" and ignore_subscription:
        # Price the tokens as if they had been bought at list (the api-equivalent
        # counterfactual activity telemetry records beside the $0 actual).
