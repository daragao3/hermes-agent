...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    ...
    def billing(**extra):
        # source/model_config attribute the row to the SESSION, not just the model: without
        # them a first-touch write stamps source="unknown" with no model config and the
        # spend loses its provenance (#23270). One site here because upstream funnels both
        # _queue_token_counts calls through this helper; the fork had to patch two.
        return dict(model=agent.model, billing_provider=agent.provider, billing_base_url=agent.base_url,
                    api_call_count=1, source=agent._session_source_for_persistence(),
                    model_config=agent._session_init_model_config, **extra)
