...
def _revert_stale_init_override(agent) -> bool:
    ...
    ``agent_init._apply_model_override`` runs BEFORE the ``_primary_runtime`` snapshot, so for
    a process that started while an override was active the snapshot IS the replacement. A
    lookup keyed on that snapshot asks ``get_override(replacement…)``, which by construction
    has no record and always returns None — structurally inert for exactly this case.

    ``_apply_model_override`` therefore stashes ``agent._override_origin`` keyed by the
    ORIGINAL (provider, model). This re-checks THAT key:

    * still active  -> False, leave everything alone (the replacement is still the route).
    * gone/expired  -> restore ``_primary_runtime`` to the pre-override snapshot, return True.
    * lookup failed -> False (fail open; retried next turn).
...
def restore_primary_runtime(agent) -> bool:
    ...
    Also enforces the `hermes overrides` 24h TTL cap against a cached, long-lived agent. TWO
    lookups are needed, not one: ``_revert_stale_init_override`` handles an agent whose
    snapshot IS a replacement (diverted at init — and it must run BEFORE the
    ``_fallback_activated`` gate, since such an agent has no fallback activated and would
    return there every turn forever), while the ``rt``-keyed lookup below handles an agent
    diverted mid-session by ``try_activate_fallback``. With no override active both are
    no-ops and the restore is byte-identical to before."""
    ...
    # `not reverted`: a cooldown armed against the REPLACEMENT must not keep the agent
    # pinned to it once the override that installed it is gone.
    ...
    if primary_model and _is_entitlement_rejected(agent, primary_provider, primary_model):
        ...
        return False
    # The other enforcement read: this agent was created BEFORE the override existed, so
    # ``rt`` really is the configured primary. Still active means that primary is still
    # diverted — stay on the current fallback rather than bouncing back onto it.
    try:
        from events import model_override
        _override = model_override.get_override(rt.get("provider", ""), rt.get("model", ""))
    except Exception:
        _override = None
    if _override:
        return False
...
def extract_reasoning(agent, assistant_message) -> Optional[str]:
    ...
    def _add(text, *, allow_blocks: bool = False) -> None:
        from agent.message_content import _text_from_part, flatten_message_text

        # `reasoning` and `reasoning_content` are scalar protocol fields. A list
        # there is malformed data, not a content-block sequence; accepting it
        # serializes arbitrary values into the transcript. Only callers that
        # explicitly read a block-bearing field may opt into list flattening.
        if text is not None and not isinstance(text, str):
            if not (allow_blocks and isinstance(text, list)) and not _text_from_part(text):
                logger.warning(
                    "extract_reasoning: ignoring unreadable %s reasoning value; coercing it "
                    "would write repr text into the transcript", type(text).__name__)
                return
            if isinstance(text, list) and not allow_blocks:
                logger.warning(
                    "extract_reasoning: ignoring list in scalar reasoning field"
                )
                return
