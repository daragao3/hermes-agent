...
def _build_primary_runtime(agent) -> Dict[str, Any]:
    """The runtime snapshot dict, built but NOT installed.

    Shared by :func:`_snapshot_primary_runtime` (which blesses the post-override state as
    primary) and by :func:`_apply_model_override` (which captures the PRE-override state as
    ``agent._override_origin['runtime']``, so a long-lived agent can be put back on its
    truly-configured primary once the override expires). One definition, so the two cannot drift."""
    _cc = agent.context_compressor
    rt = {
        "model": agent.model,
...
def _snapshot_primary_runtime(agent):
    """Build the snapshot AND install it as ``agent._primary_runtime``; returns it."""
    agent._primary_runtime = _build_primary_runtime(agent)
...
_PASSTHROUGH_PARAMS = (
    ...
    "load_soul_identity", "pass_session_id", "log_prefix_chars", "activity_recorder",
...
def init_agent(
    ...
    # BEFORE the snapshot, and that ordering is the whole point: a cron run is a fresh
    # process, so an override applied AFTER would be recorded against a snapshot naming the
    # rate-limited primary, and the next turn's restore_primary_runtime() would undo the
    # reroute straight back onto the model that is still 429ing.
