...
from agent.shell_tokens import split_command as split_command_line
from hermes_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, run_text_capture, windows_hide_flags
...
def _spawn(spec: ShellHookSpec, stdin_json: str) -> Dict[str, Any]:
    ...
    if IS_WINDOWS:
        # File-backed capture survives inherited handles; the shared helper still
        # kills the owned tree on timeout, without a second pipe drain.
        try:
            proc = run_text_capture(argv, input=stdin_json, timeout=spec.timeout,
