...
def _record_fallback_rate_limit(reason, provider, model, outcome, **details):
    """Attribution is best effort and never participates in cooldown decisions."""
    if reason not in _RATE_LIMIT_CLASS_REASONS:
        return
    try:
        from events.rate_limit_signal import record
        record(provider=provider or "", model=model or "", reason=reason.value,
               detector="runtime", outcome=outcome, **details)
    except Exception:
        pass
...
def _fallback_has_open_episode(provider, model):
    ...
    try:
        ...
        return _episode_key(provider, model) in _load_state()
    except Exception:
        return False


def try_activate_fallback(agent, reason: "FailoverReason | None" = None, *,
                          telemetry_reason: "FailoverReason | None" = None) -> bool:
    """Switch to the next fallback model/provider in the chain; False when exhausted. Swaps client,
    model slug and provider in place so the retry loop continues on the new backend; client
    construction goes through resolve_provider_client (no duplicated provider→key mappings).
    ``telemetry_reason`` labels events only; ``reason`` alone controls cooldowns."""
    ...
    while True:
        if agent._fallback_index >= len(agent._fallback_chain):
            _record_fallback_rate_limit(
                attributed_reason, getattr(agent, "provider", ""), getattr(agent, "model", ""),
                "chain_exhausted" if agent._fallback_chain else "no_fallback",
            )
        ...
        if _should_skip_fallback_candidate(agent, fb, fb_key, fb_provider, fb_model, unavailable):
            continue
        if _fallback_has_open_episode(fb_provider, fb_model):
        ...
        try:
            ...
            _record_fallback_rate_limit(
                attributed_reason, old_provider, old_model, "diverted",
