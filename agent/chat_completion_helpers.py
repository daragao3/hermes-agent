...
def _fallback_chain_exhausted(
    agent, reason: "FailoverReason | None",
    attributed_reason: "FailoverReason | None" = None,
) -> bool:
    ...
    ``attributed_reason`` is telemetry only (see :func:`try_activate_fallback`) and never
    reaches the cooldown decision above it."""
    ...
    # Nothing absorbed the traffic. This is the ACT case: runs stop working rather than
    # silently degrading, and it is invisible in every other surface today.
    if (attributed_reason or reason) in _RATE_LIMIT_FAILOVER_REASONS:
        _record_rate_limit_signal(
            agent, attributed_reason or reason,
            outcome="chain_exhausted" if agent._fallback_chain else "no_fallback")
    return False


def _record_rate_limit_signal(agent, reason, *, outcome: str, provider: str = "",
                              model: str = "", fallback_provider: str = "",
                              fallback_model: str = "") -> None:
    """Fire-and-forget rate-limit episode signal. A telemetry defect must never break a call."""
    try:
        from events.rate_limit_signal import record
        extra = {}
        if fallback_provider or fallback_model:
            extra = {"fallback_provider": fallback_provider, "fallback_model": fallback_model}
        record(
            provider=provider or (getattr(agent, "provider", "") or ""),
            model=model or (getattr(agent, "model", "") or ""),
            reason=getattr(reason, "value", reason),
            detector="runtime", outcome=outcome, **extra)
    except Exception:
        pass
...
def _should_skip_fallback_candidate(agent, fb: dict, fb_key: tuple, fb_provider: str, fb_model: str, unavailable: set) -> bool:
    ...
    if _is_entitlement_rejected(agent, fb_provider, fb_model):
        ...
        return True
    # A candidate with an OPEN rate-limit episode is skipped so failover does not walk
    # straight into a backend already known to be limited. Deliberately NOT added to
    # `unavailable`: an episode closes on its own, so the entry is re-checked next time
    # rather than suppressed for the session. Fails OPEN on any lookup error — a broken or
    # missing state file must never block failover, which is the whole point of the chain.
    # With no open episodes this is a no-op and chain-walking is unchanged.
    try:
        from events.rate_limit_signal import _episode_key, _load_state
        if _episode_key(fb_provider, fb_model) in _load_state():
            logger.warning("Fallback skip: %s/%s has an open rate-limit episode", fb_provider, fb_model)
            return True
    except Exception:
        pass
...
def try_activate_fallback(
    ...
    ``reason`` is BEHAVIOURAL. A rate-limit-class value arms ``agent._rate_limited_until``,
    which ``restore_primary_runtime()`` reads to decide whether the agent stays on the
    fallback — i.e. passing it changes which model answers the next call.

    ``telemetry_reason`` is ATTRIBUTION ONLY. It labels the failover for the rate-limit
    detector and is deliberately invisible to every behavioural branch here. A caller that
    merely SUSPECTS a rate limit (an empty/malformed response) can therefore make the
    incident visible without silently pinning the agent to its fallback for 60s. Do not
    "simplify" the two into one parameter — that coupling is the bug this split prevents."""
    from agent.fallback_cooldown import _RATE_LIMIT_FAILOVER_REASONS, _arm_rate_limit_cooldown
    cooldown_seconds = _arm_rate_limit_cooldown(agent, reason)
    attributed_reason = telemetry_reason or reason
    while True:
        if agent._fallback_index >= len(agent._fallback_chain):
            return _fallback_chain_exhausted(agent, reason, attributed_reason)
        ...
        try:
            ...
            # The swap succeeded: old_provider/old_model got limited and fb_provider/fb_model
            # is absorbing the traffic.
            if attributed_reason in _RATE_LIMIT_FAILOVER_REASONS:
                _record_rate_limit_signal(
                    agent, attributed_reason, outcome="diverted",
                    provider=old_provider or "", model=old_model or "",
                    fallback_provider=fb_provider, fallback_model=fb_model)
