...
# Local path: same shape as gateway extract_local_files(), lookbehind skips matches inside
# URLs. TWO alternatives, because anchoring only to ``~/`` or ``/`` means a Windows absolute
# path is not recognised as an image reference at all — the drive-letter form accepts either
# separator, since both reach the filesystem on Windows.
_WINDOWS_ABS_PATH = r"[A-Za-z]:[\\/](?:[\w.\-]+[\\/])*[\w.\-]+"
_POSIX_ABS_PATH = r"(?:~/|/)(?:[\w.\-]+/)*[\w.\-]+"
# URL: strict ``http(s)://`` so ``file://`` and other schemes are not grabbed; optional query.
