...
def _capable_for(provider: WebSearchProvider, capability: str) -> bool:
    """Whether *provider* serves *capability* ("search" | "extract").

    Module level so ``_resolve`` and ``get_fallback_provider`` share ONE definition — two
    copies of this test is how the two resolution paths drift apart."""
    return bool(
        provider.supports_search() if capability == "search"
        else provider.supports_extract() if capability == "extract" else False
    )
...
def _resolve(configured: Optional[str], *, capability: str) -> Optional[WebSearchProvider]:
    ...
    def _capable(p: WebSearchProvider) -> bool:
        return _capable_for(p, capability)
...
def get_fallback_provider(
    capability: str, *, excluded: frozenset[str] = frozenset(),
) -> Optional[WebSearchProvider]:
    """Resolve one available provider for a POST-FAILURE capability fallback.

    Distinct from :func:`_resolve`, which answers "who is active". This answers "who else
    can serve this" after a live call failed, so ``excluded`` — the provider that just
    failed — is honoured even against an explicit config pointing at it. Returning the
    known-bad provider would make the fallback a no-op.

    The keyless free tier is deliberately NOT in this ladder. It exists so a keyless install
    can still search; reaching it after a KEYED provider failed would silently downgrade a
    working paid setup to an anonymous public endpoint on a transient error."""
    if capability not in {"search", "extract"}:
        raise ValueError(f"Unsupported web capability: {capability}")

    def _usable(provider: WebSearchProvider) -> bool:
        return _capable_for(provider, capability) and is_available_safe(
            provider, logger, "provider %s.is_available() raised %s")

    snapshot = _registry.merged()
    explicit = _configured_backend(capability)
    if explicit and explicit not in excluded:
        provider = snapshot.get(explicit)
        if provider is not None and _usable(provider):
            return provider

    eligible = [p for p in snapshot.values() if p.name not in excluded and _usable(p)]
    if len(eligible) == 1:
        return eligible[0]
