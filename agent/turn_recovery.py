...
def nonretryable_client_error_result(
    ...
    # The alert half: emit a loud AGENT_LOOP_FAULT for this abort, IGNORING the
    # non-retryable classification — silence on this path is the defect, since the abort is
    # precisely the event nobody sees. Best-effort; the helper never raises.
    try:
        ...
        emit_agent_loop_fault(
            ...
            phase="non_retryable_abort", provider=provider, model=model,
...
def route_classified_error(
    ...
    if _should_fallback and agent._fallback_index < len(agent._fallback_chain):
        ...
        if not pool_may_recover:
            ...
            # telemetry_reason only labels the failover; `reason` still decides routing.
            if agent._try_activate_fallback(
                    reason=classified.reason, telemetry_reason=classified.reason):
