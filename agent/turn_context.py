...
def _maybe_title_session_at_turn_start(agent: Any, messages: List[Any]) -> None:
    ...
    try:
        ...
        # Accounting attribution for the titler's OWN llm call (task='title_generation',
        # #23270): `source` here is the session's PLATFORM, not the title provenance that
        # `_persist_session_title` calls source. Both reach set_accounting_context, and
        # without them the title spend is billed against the wrong session/model rather than
        # not billed at all. getattr-guarded: agents built by bare test doubles have neither.
        _title_source_fn = getattr(agent, "_session_source_for_persistence", None)
        _title_source = (
            _title_source_fn() if callable(_title_source_fn)
