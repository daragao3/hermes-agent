...
# Windows known-folder paths required by allowlisted helper environments.
...
def run_secret_cli(argv: Sequence[str], *, allow_env: Sequence[str] = (), extra_env: Optional[Dict[str, str]] = None,
    ...
    base_keep = ("PATH", "HOME", "USERPROFILE", "SYSTEMROOT", "TMPDIR", "TEMP",
                 "LANG", "LC_ALL", "XDG_CONFIG_HOME", "XDG_DATA_HOME", *WINDOWS_OS_PATH_ENV_VARS)
    source_env = get_source_environment()
    env = {k: source_env[k] for k in (*base_keep, *allow_env) if k in source_env}
    if extra_env:
