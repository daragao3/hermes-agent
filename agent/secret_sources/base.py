...
# OS path / machine-identity vars, NOT credentials. A secret-manager child on Windows needs
# these to function at all: without SYSTEMDRIVE it cannot expand a %SystemDrive%-templated
# known-folder path and writes a literal "%SystemDrive%" DIRECTORY under its inherited cwd;
# without COMSPEC/PATHEXT it cannot resolve a command. Every name here is a path or a machine
# identity, which is what makes widening the minimal env safe. No-ops on POSIX.
...
def run_secret_cli(argv: Sequence[str], *, allow_env: Sequence[str] = (), extra_env: Optional[Dict[str, str]] = None,
    ...
    full post-dotenv ``os.environ``): PATH/HOME/locale basics, the OS path and
    machine-identity vars in :data:`WINDOWS_OS_PATH_ENV_VARS` (paths, not credentials),
    plus ``allow_env`` and ``extra_env``. ``NO_COLOR=1`` + ANSI-scrubbed stderr; stdin is
    /dev/null so a prompting helper fails fast. Pass user refs AFTER a ``--`` terminator."""
