...
_OP_ENV_ALLOWLIST = (
    ...
    # OS path/identity vars, not credentials. Without SYSTEMDRIVE a Windows child cannot
    # expand %SystemDrive%\ProgramData and writes a literal "%SystemDrive%" directory under
    # its inherited cwd — see the comment on WINDOWS_OS_PATH_ENV_VARS in base.py. No-ops on
    # POSIX.
    *WINDOWS_OS_PATH_ENV_VARS,
)
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Path to the on-disk cache (compatibility surface for direct callers)."""
    return _STORE.disk.path(home_path)
