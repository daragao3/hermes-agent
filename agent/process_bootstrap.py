...
# One stock-default SSLContext for the whole process. httpx builds a FRESH one for every
# ``verify=True``, re-parsing certifi's ~150-cert bundle each time — 1.4-2.5s apiece on a
# Windows dev box, and a client asks for up to three (both no-proxy mounts plus the client),
# so ~5-7s of pure TLS setup per auxiliary client, paid again on every construction.
...
def build_keepalive_http_client(base_url: str = "", *, async_mode: bool = False, verify: Any = True) -> Optional[Any]:
    ...
    try:
        ...
        # process-wide instance is exactly equivalent — and an SSLContext is designed to be
        # shared across connections, which is already how one httpx client reuses one.
        # Deliberately narrow: False, a custom SSLContext, and a CA path (per-provider
        # ssl_ca_cert or HERMES_CA_BUNDLE) are forwarded untouched, so no custom-CA or
        # verification-disabling behaviour changes. It also makes every verify=True caller
        # collide on one _shared_transport_key identity instead of each minting its own.
        if verify is True:
