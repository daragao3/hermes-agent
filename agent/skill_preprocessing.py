...
# run_text_capture applies CREATE_NO_WINDOW itself, so the hide-flags import that used to
# serve run_inline_shell is no longer needed here.
...
def run_inline_shell(command: str, cwd: Path | None, timeout: int) -> str:
    ...
    try:
        # run_text_capture, not subprocess.run(capture_output=True): `command` is an
        # arbitrary snippet, so whatever it launches is a GRANDCHILD that inherits the
        # capture pipes and holds their write end open after bash itself is killed. On
        # Windows subprocess.run then blocks re-draining a pipe that never reaches EOF,
        # so the TimeoutExpired branch below — the whole point of the 'one bad snippet
        # can't wreck the message' contract — would never be reached. Temp-file capture
        # keeps the bound real, and the helper applies CREATE_NO_WINDOW itself (so the
        # windows_hide_flags() kwargs are no longer needed here). It is text-mode by
        # definition, which is why text/encoding/errors/check are dropped.
