...
def _by_status(c: _Ctx) -> Optional[Verdict]:
    ...
    if status is None:
        return None
    # Preserve local Kimi Coding Plan throttle recovery; other vendors' 492
    # responses retain the generic non-retryable 4xx classification.
    if status == 492 and ("kimi" in c.provider_slug or "moonshot" in c.provider_slug):
