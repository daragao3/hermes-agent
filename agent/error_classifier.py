...
def _status_492(c: _Ctx) -> Verdict:
    """Kimi-for-Coding's non-standard 492: plan-level usage/concurrency throttling.

    The subscription surface (api.kimi.com/coding) uses 492 where the pay-as-you-go API uses
    429 for every rate/concurrency/quota limit. It is transient and clears with backoff or
    lower concurrency. Unhandled it lands in the unlisted-4xx bucket as a NON-retryable
    format_error, so the retry loop treats a temporary throttle as a permanent bad request:
    no backoff-retry, and only the last-ditch client-error fallback instead of the eager
    rate-limit failover.

    Scoped to Kimi/Moonshot so an unrelated provider's custom 492 is not misread — a
    non-matching provider falls through to the same format_error verdict it gets today."""
    provider = (c.provider_slug or c.provider or "").lower()
    if "kimi" in provider or "moonshot" in provider:
        return _V_RATE_LIMIT
    return _V_FORMAT_ERROR
...
def is_library_exception(exc: BaseException) -> bool:
    ...
    (openai / anthropic / httpx / httpcore.) Such an exception — e.g. the R57 openai
    ``parse_response`` ``TypeError`` on ``output=None`` — is backend-contract drift, not a bug
    in Hermes's own code, and must NOT be auto-bucketed as a non-retryable programming bug.
    The loop should retry and fall back, failing loudly at the retry-exhaustion terminal if it
    persists, rather than silently aborting on the first hit. (SR-493, ADR-0024 §4; SR-471's
    AGENT_LOOP_FAULT fires on the non-retryable abort path, which such an exception no longer
    enters.)"""
