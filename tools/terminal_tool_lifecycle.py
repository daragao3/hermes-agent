...
def ensure_task_env(task_id: Optional[str] = None):
    ...
    with task_lock:
        ...
        with _env_lock:
            from tools.terminal_tool import _capture_env_scratch_dir
...
def cleanup_all_environments(*, scratch_dir: Optional[Path] = None, _sweep_orphans: bool = True):
    ...
    # No captured root at interpreter exit means no orphan sweep.
    if not _sweep_orphans:
        return cleaned
