...
def _scratch_paths(scratch_dir=None):
    """``hermes-*`` entries under *scratch_dir*, or under a freshly resolved root.

    A resolved-now root is correct for a caller still running inside the environment it was
    set up in; anything that can outlive that environment must pass its captured root."""
    root = _get_scratch_dir() if scratch_dir is None else scratch_dir
    return glob.glob(str(root / "hermes-*"))
...
def cleanup_all_environments(*, scratch_dir: Optional[Path] = None):
    ...
    :func:`tools.terminal_tool.resolve_scratch_dir`, instead of resolving HERMES_HOME at call
    time. Pass it from any caller that can outlive the environment it was set up in — an
    ``atexit`` hook above all, whose fire happens after pytest's ``monkeypatch`` has restored
    the real HERMES_HOME. Without it an exiting test process creates
    ``<real home>/sandboxes/singularity/`` and then rmtrees every ``hermes-*`` entry under it,
    including the overlay directory of a LIVE process."""
    ...
    if scratch_dir is not None and not scratch_dir.exists():
        # The captured root is gone (a pytest tmp home, typically). There is nothing to sweep,
        # and recreating it would leave litter behind — while falling back to a live resolve
        # would reintroduce the very bug the capture exists to prevent.
        if cleaned > 0:
            logger.info("Cleaned %d environments", cleaned)
        return cleaned
    for path in _scratch_paths(scratch_dir):
