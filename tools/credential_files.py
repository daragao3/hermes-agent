...
def _reject_backslash(relative_path: str, source: str) -> bool:
    """True when *relative_path* must be refused for containing a backslash.

    Declared credential paths — skill frontmatter ``required_credential_files`` and
    ``terminal.credential_files`` in config.yaml — are POSIX-relative by contract. They are
    joined against HERMES_HOME on the HOST and also interpolated into ``container_path``,
    which is a path inside the LINUX sandbox (``tools/environments/modal.py`` hands it to
    ``Mount.from_local_file(remote_path=...)``). A backslash cannot satisfy both ends on any
    host:

    * On Windows ``creds\\x.json`` resolves to the real nested host file and then yields the
      container path ``/root/.hermes/creds\\x.json`` — one oddly-named file at the sandbox
      root instead of ``creds/x.json``.
    * On POSIX ``creds\\x.json`` is a single ordinary filename, so it never matches the nested
      file and the declaration silently registers nothing (``is_file()`` fails, logged only at
      debug level).

    So one declaration is broken differently on the two hosts and correct on neither. This
    refuses it loudly rather than normalising it, because normalisation is the riskier change:
    on POSIX ``..\\..\\.ssh\\id_rsa`` is currently inert data — one contained filename that
    the containment check passes and ``is_file()`` then rejects. Rewriting ``\\`` to ``/``
    would turn it into a live traversal attempt whose safety rests entirely on the containment
    check running first and being correct. Rejecting removes the input class instead of adding
    that ordering coupling.

    Matches the reject-and-warn style already used here for absolute paths and traversal,
    rather than silently repairing a security-relevant declaration."""
    if "\\" in relative_path:
        logger.warning(
            "credential_files: rejected %s path %r — declared paths must use '/' separators; "
            "container_path is a POSIX path inside the sandbox", source, relative_path)
        return True
    return False
...
def register_credential_file(relative_path: str, container_base: str = "/root/.hermes") -> bool:
    ...
    # The container side is POSIX even when the host-relative path carries os.sep (Windows).
    container_path = f"{container_base.rstrip('/')}/{str(relative_path).replace(os.sep, '/')}"
