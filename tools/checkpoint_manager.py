...
def _checkpoint_base() -> Path:
    """Resolve the checkpoint base dir at call time (see CHECKPOINT_BASE)."""
...
def _git_subprocess(cmd: List[str], env: dict, timeout: int, cwd: Optional[str] = None):
    # File capture bounds inherited-pipe waits; -z output must preserve exact paths.
    binary = "-z" in cmd
    result = run_text_capture(cmd, timeout=timeout, env=env, cwd=cwd,
                              stdin=subprocess.DEVNULL, text=not binary)
    if binary:
