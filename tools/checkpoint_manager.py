...
# Override slot, default None. Resolved through _checkpoint_base() at CALL time, never
# read directly: evaluating get_hermes_home() here would snapshot HERMES_HOME at import,
# so a later profile switch (or a test fixture pointing at a temp home) would still write
# the real user's checkpoints. Same seam, and the same reason, as gateway/run.py's
# _resolve_hermes_home().
CHECKPOINT_BASE: Optional[Path] = None


def _checkpoint_base() -> Path:
    """Resolve the checkpoint base dir at call time (see _checkpoint_base())."""
...
def _git_subprocess(cmd: List[str], env: dict, timeout: int, cwd: Optional[str] = None):
    """Run one git command. ``-z`` commands go through subprocess.run in BINARY mode;
    everything else goes through run_text_capture, which is the only spawn here whose
    ``timeout`` is a real bound.

    subprocess.run(capture_output=True, timeout=N) does NOT bound a git command that
    leaves a grandchild alive: CPython's timeout path kills the direct child, then on
    Windows re-enters communicate() with no timeout, blocking until the capture pipe hits
    EOF — impossible while a grandchild still holds the inherited write handle. Measured
    on a Windows host 2026-08-15: a 2s budget against a detached 25-ping child returned
    in 24.3s. run_text_capture captures into temp files rather than pipes and tree-kills
    on timeout, so the budget holds; it suppresses the conhost flash too.

    Probed with psutil the same day, ``add -A`` / ``write-tree`` / ``reflog expire`` spawn
    no descendants, but ``gc --prune=now`` spawns a git.exe child — so the reclamation
    paths (``_enforce_size_cap`` on every ``_take``, and ``prune_checkpoints`` from the
    daily hook) are the live hang vector, and they carry no ``-z``. The two commands that
    do (``diff --name-only -z``, ``ls-files --cached -z``) are plumbing that spawns
    nothing, and they need raw bytes: their NUL-separated pathnames are decoded with
    os.fsdecode, which text mode with errors=replace would have already corrupted.

    Callers should size against ``timeout + ~10s`` on Windows — run_text_capture's
    tree-kill runs synchronously and ``taskkill /T /F`` carries its own 10s cap.
    """
    if "-z" in cmd:
        ...
        return result
    return run_text_capture(cmd, timeout=timeout, env=env, cwd=cwd, stdin=subprocess.DEVNULL)
...
def _project_refs(working_dir: str) -> _ProjectRefs:
    ...
    store, dir_hash = _store_path(_checkpoint_base()), _project_hash(abs_dir)
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        ...
        try:
            ...
            if digest is None:
                return
            store, dir_hash = _store_path(_checkpoint_base()), _project_hash(self.get_working_dir_for_path(str(path)))
    ...
    def ensure_checkpoint(self, working_dir: str, reason: str = "auto") -> bool:
        ...
        # Compare a RESOLVED path against a resolved home, and detect a filesystem root
        # structurally (a root is its own parent) rather than by matching the literal "/".
        # On Windows ``Path("/").resolve()`` is the *current drive's* root — "C:\\" — which
        # never equals "/", so a bare "/" used to sail past this guard into _take(), whose
        # first act is ``_dir_file_count()``: an rglob over the entire drive that only stops
        # after _MAX_FILES (50,000) entries. That runs on the agent's pre-mutation hot path
        # (agent/tool_executor.py) for any edit whose project root resolves to a drive root.
    ...
    def list_all_checkpoints(self) -> List[Dict]:
        ...
        to.
        """
        store = _store_path(_checkpoint_base())
...
def _rmtree_git(path: Path) -> None:
    ...
    Git marks object files (loose objects and packs) read-only, and on Windows deleting a
    read-only file raises ``PermissionError`` — so a plain ``rmtree`` over any directory
    holding a git store always fails there. Without this, pruning is a silent no-op on
    Windows: the caller catches the OSError and merely counts it as an error.
    """
