...
def _telegram_format(message):
    ...
    if re.search(r'</?(b|strong|i|em|u|ins|s|strike|del|a|code|pre|span|tg-spoiler|tg-emoji|blockquote)(\s[^>]*)?>', message, re.IGNORECASE):
...
async def _send_telegram(token, chat_id, message, media_files=None, thread_id=None, disable_link_previews=False, force_document=False, *, buttons=None):
    ...
    try:
        ...
        for index, chunk in enumerate(text_chunks):
            # Keep shared thread fallback state; markup appears only on the final bubble.
