...
def _telegram_format(message):
    ...
    # Only the tags Telegram actually renders. The old ``<[a-zA-Z/][^>]*>`` matched any
    # angle-bracket pair starting with a letter, so ``a < b and c > d``, a generic in prose,
    # or a bare ``<placeholder>`` flipped the whole message into HTML mode — which Telegram
    # then rejects, dropping it to plain text with its Markdown unrendered.
    if re.search(
...
async def _send_telegram(token, chat_id, message, media_files=None, thread_id=None,
                         disable_link_previews=False, force_document=False, *, buttons=None):
    ...
    ``events.override_buttons.buttons_for`` (rows of ``{"label", "callback_data"}`` dicts).
    It is converted to a real ``telegram.InlineKeyboardMarkup`` here — the one place in this
    send path that already imports telegram types — and attached as ``reply_markup`` on the
    LAST text chunk (repeating it per chunk would give the reader N copies). Defaults to
    ``None``, in which case no ``reply_markup`` is ever added and behaviour is unchanged
    ...
    try:
        ...
        if buttons:
            try:
                ...
                # The text is the payload and the buttons are an affordance: a malformed
                # spec must not cost the delivery.
                logger.warning(
                    "Failed to build InlineKeyboardMarkup from buttons spec, sending without buttons",
                    exc_info=True)
                reply_markup = None
        ...
        _chunks = list(BasePlatformAdapter.truncate_message(
            formatted, 4096, len_fn=utf16_len)) if formatted.strip() else []
        if send_parse_mode == ParseMode.MARKDOWN_V2:
            try:
                # The chunker appends indicators like ``(1/3)``; on the MarkdownV2 path those
                # characters are metacharacters, so an unescaped indicator is itself a parse
                # failure that drops the whole chunk to plain text. Best-effort.
                from plugins.platforms.telegram.adapter import escape_chunk_indicators
                _chunks = escape_chunk_indicators(_chunks)
            except Exception:
                pass
        for _idx, chunk in enumerate(_chunks):
            _kwargs = dict(text_kwargs)
            if reply_markup is not None and _idx == len(_chunks) - 1:
                _kwargs["reply_markup"] = reply_markup
            last_msg = await _telegram_send_text_chunk(
                bot, int_chat_id, chunk, send_parse_mode, _has_html, _kwargs)
