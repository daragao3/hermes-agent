...
_FIRECRAWL_CREDIT_CODES = frozenset({"provider_credits_exhausted", "provider_circuit_open"})
...
def _merge_credit_fallback_results(
    ...
    for fallback_index, primary_index in enumerate(unresolved):
        ...
        candidate = fallback[fallback_index] if fallback_index < len(fallback) else None
        if isinstance(candidate, dict) and candidate.get("url") == url:
            replacement = dict(candidate)
        else:
            replacement = _incomplete_extract_result(url)
        merged[primary_index] = replacement
    return merged



async def _apply_credit_fallback(results, fetch_urls, format):
    unresolved = [
    ...
    if unresolved:
        ...
        if fallback_provider is not None:
            ...
            fallback_urls = [fetch_urls[index] for index in fallback_indices]
            if fallback_urls:
                try:
...
async def _dispatch_extract(provider, fetch_urls: List[str], format: Optional[str]) -> List[dict]:
    ...
    if credit_indices:
        results = await _apply_credit_fallback(results, fetch_urls, format)
    if not credit_indices and results and all(r.get("error") for r in results) and _rescue_eligible(provider):
    ...
    for index, (url, fetched) in enumerate(zip(fetch_urls, results)):
        if index in credit_indices:
            continue  # Alternate content must never be indexed as the primary provider.
