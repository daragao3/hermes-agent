...
class CuaDriverBackend(_CaptureMixin, _InputMixin, ComputerUseBackend):
    ...
    def move_cursor(self, x: int, y: int) -> ActionResult:
        """Move the agent-cursor overlay without moving the OS pointer."""
        return self._action("move_cursor", {"x": int(x), "y": int(y)})

    def get_cursor_position(self) -> tuple[int, int]:
        """Return the real OS cursor position in screen points."""
        out = self.call_tool("get_cursor_position")
        structured = out.get("structuredContent") or {}
        return int(structured.get("x", 0)), int(structured.get("y", 0))

    def get_screen_size(self) -> Dict[str, Any]:
        """Return the logical main-display size and scale factor."""
        return self.call_tool("get_screen_size").get("structuredContent") or {}

    def zoom(
        self, *, window_id: int, x: float, y: float, w: float, h: float,
        factor: float = 1.0, format: str = "jpeg", quality: int = 85,
    ) -> Dict[str, Any]:
        """Capture and optionally scale a sub-region of a window."""
        return self.call_tool("zoom", {
            "window_id": int(window_id),
            "x": float(x), "y": float(y), "w": float(w), "h": float(h),
            "factor": float(factor), "format": format, "quality": int(quality),
        })

    def set_agent_cursor_motion(
        self, *, glide_ms: Optional[float] = None,
        dwell_ms: Optional[float] = None,
        idle_hide_ms: Optional[float] = None,
        cursor_id: Optional[str] = None,
    ) -> ActionResult:
        """Update only the supplied cursor-overlay motion settings."""
        args: Dict[str, Any] = {}
        for key, value in (
            ("glide_ms", glide_ms), ("dwell_ms", dwell_ms),
    ...
    def set_agent_cursor_style(
        ...
        """Update only the supplied cursor-overlay style settings."""
        args: Dict[str, Any] = {}
    ...
    def get_agent_cursor_state(
        ...
        """Return state for this run's cursor or an explicitly named cursor."""
        args = {"cursor_id": cursor_id} if cursor_id else None
        return self.call_tool("get_agent_cursor_state", args).get("structuredContent") or {}

    def start_recording(
        self, *, output_dir: str, record_video: bool = False,
    ) -> Dict[str, Any]:
        """Start trajectory recording owned by this run."""
        return self.call_tool("start_recording", {
            "output_dir": output_dir, "record_video": bool(record_video),
        }).get("structuredContent") or {}

    def stop_recording(self) -> Dict[str, Any]:
        """Stop trajectory recording and return its final state."""
        return self.call_tool("stop_recording").get("structuredContent") or {}

    def get_recording_state(self) -> Dict[str, Any]:
        """Return the current trajectory recorder state."""
        return self.call_tool("get_recording_state").get("structuredContent") or {}

    def replay_trajectory(
        self, *, trajectory_dir: str, dry_run: bool = False,
        speed_factor: float = 1.0,
    ) -> Dict[str, Any]:
        """Replay a recorded trajectory, optionally without firing actions."""
        return self.call_tool("replay_trajectory", {
    ...
    def install_ffmpeg(self) -> Dict[str, Any]:
        """Install ffmpeg for video recording where the driver requires it."""
        return self.call_tool("install_ffmpeg")

    def get_config(self) -> Dict[str, Any]:
        """Return the current cua-driver configuration."""
        return self.call_tool("get_config").get("structuredContent") or {}

    def get_accessibility_tree(self) -> Dict[str, Any]:
        """Return the current application/window accessibility snapshot."""
        out = self.call_tool("get_accessibility_tree")
        return out.get("structuredContent") or {"data": out["data"]}

    def page(self, *, pid: int, action: str, **page_args: Any) -> Dict[str, Any]:
        """Invoke a browser-page action for an application process."""
        return self.call_tool("page", {
            "pid": int(pid), "action": action, **page_args,
        })
