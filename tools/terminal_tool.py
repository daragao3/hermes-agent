...
from tools.terminal_tool_lifecycle import (
    ...
    _evict_environment_for_task, cleanup_all_environments,
)
from tools.terminal_tool_lifecycle import ensure_task_env  # noqa: F401  re-exported for
# tools/image_source.py, which imports it from THIS module.
...
# --- Windows device-name redirects under MSYS bash --------------------------------------------
#
# `re` is imported at the FOOT of this file too, inside the revert-scheduled PLUGIN-COMPAT
# block — i.e. AFTER this point, so the module-level regexes below cannot rely on it. Imported
# here instead of at the top of the file so that removing the compat block (which is scheduled
# for a clean revert) leaves this code with its own import rather than depending on that
# block's. `Path` is never needed at module scope here — resolve_scratch_dir only annotates.
import re  # noqa: E402  -- deliberately not at the top; see above
...
def _rewrite_windows_device_redirects(command: str) -> tuple[str, int]:
    ...
    while i < n:
        ...
        # Imported here rather than at module scope: terminal_tool_sudo imports back into
        # this module, and base.py takes the same local-import route for its sibling
        # helper _rewrite_compound_background.
        from tools.terminal_tool_sudo import _read_shell_token
...
import sys  # noqa: F401,F811,E402
