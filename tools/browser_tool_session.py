...
def _agent_browser_argv(browser_cmd: str) -> list:
    ...
    return [_bt._resolve_batch_shim(browser_cmd)]
...
def _create_cloud_session_or_fallback(task_id: str, provider) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(session_info, dict):  # mark degraded for observability
            session_info = {**session_info, "fallback_from_cloud": True, "fallback_reason": fallback_reason,
...
def _run_browser_command(
    ...
    cmd_parts = _agent_browser_argv(browser_cmd) + backend_args + ["--json", command] + _bt._cmd_safe_browser_args(command, args)
