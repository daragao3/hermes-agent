...
def _resolve_batch_shim(path: str) -> str:
    ...
    ``CreateProcess`` cannot run a batch file directly — Windows routes it through
    ``cmd.exe``, which RE-PARSES the whole command line. ``subprocess.list2cmdline`` quotes
    only arguments containing spaces or quotes, so a space-free argument reaches cmd.exe bare
    and its ``>``, ``&``, ``|`` and ``^`` are interpreted as shell operators. For the browser
    tools that meant a compact JS one-liner's ``=>`` arrow became an output redirection: the
    script was truncated (SyntaxError) and a cwd-relative junk file was created holding the
    CLI's JSON error response.

    npm's Windows shim is a two-line wrapper around a real executable, so spawning that
    executable directly keeps cmd.exe out of the picture and restores normal argv semantics.

    Falls back to ``path`` unchanged whenever the shim cannot be read or its target is
    missing, so a layout we do not recognise never breaks discovery."""
...
def _cmd_safe_browser_args(command: str, args: list) -> list:
    """Encode ``eval`` payloads so no shell can misread them.

    agent-browser ships ``eval -b <base64>`` precisely because "shell escaping with nested
    quotes and special characters is error-prone". Base64 output is restricted to
    ``A-Za-z0-9+/=``, which contains no cmd.exe or POSIX shell metacharacter, so the
    expression survives intact no matter what wrapper ends up between us and the CLI —
    including the npx fallback, which :func:`_resolve_batch_shim` cannot unwrap because
    there the shim IS npx rather than a thin wrapper around one binary."""
...
def _agent_browser_argv(browser_cmd: str) -> list:
    ...
    # Unwrap an npm .cmd/.bat shim so cmd.exe never re-parses our argv (see the helper).
    return [_resolve_batch_shim(browser_cmd)]
...
def _create_cloud_session_or_fallback(task_id: str, provider) -> Dict[str, Any]:
    ...
    try:
        ...
        from agent.firecrawl_run_state import (
            FirecrawlCircuitOpenError, FirecrawlCreditsExhaustedError)

        # The CONFIGURED provider name when there is one; the class name is a fallback, not
        # the identity an operator recognises.
        _configured = getattr(provider, "name", None)
        provider_name = (_configured if isinstance(_configured, str) and _configured
                         else type(provider).__name__)
        # A stable token for the two conditions with dedicated types, so a consumer can match
        # on the reason instead of parsing an exception repr. Anything else keeps str(e).
        if isinstance(e, FirecrawlCreditsExhaustedError):
            _fallback_reason = "provider_credits_exhausted"
        elif isinstance(e, FirecrawlCircuitOpenError):
            _fallback_reason = "provider_circuit_open"
        else:
        ...
        if isinstance(session_info, dict):  # mark degraded for observability
            session_info = {**session_info, "fallback_from_cloud": True, "fallback_reason": _fallback_reason,
...
def _run_browser_command(
    ...
    cmd_parts = (_agent_browser_argv(browser_cmd) + backend_args
                 + ["--json", command] + _cmd_safe_browser_args(command, args))
