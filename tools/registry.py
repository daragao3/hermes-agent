...
class ToolRegistry:
    ...
    def transaction(self):
        """Hold the registry lock and restore all mutable state if the body raises.

        Profile-scoped plugin loading imports UNTRUSTED registration code inside this block.
        Without the rollback, a plugin that raises halfway leaves its earlier tools installed
        in the process-global registry, visible to every other profile."""
        registry = self
    ...
    def register_batch_if_absent(self, entries: List[dict]) -> None:
        ...
        Profile-scoped plugin loading imports untrusted registration code before committing
        its tool definitions. A whole-batch preflight prevents a late collision from leaving
        the earlier tools partially installed in the process-global registry.

        The self-collision check is separate because the per-name ``in self._tools`` test
        cannot see two entries in the SAME batch claiming one name."""
        with self._lock:
    ...
    def dispatch(
        ...
        try:
            # The frame marks this thread as "inside tool `name`" for exactly the handler's
            # lifetime, so an operator stop or timeout on the cron scheduler can cancel it
            # (tools.inflight_call) without a thread bit that could outlive the call. INSIDE
            # the try: a cancel surfaces as InterruptedError and becomes a normal
