...
class ToolRegistry:
    ...
    def transaction(self):
        """Rollback tools, aliases, checks and authorization without erasing provenance.

        Module-to-profile attribution intentionally survives: delayed callbacks from
        a failed import must remain confined even after its override policy is gone.
        ToolEntry and policy identities are preserved; only registry containers copy.
        """
        registry = self
        ...
        class _Transaction:
            def __enter__(self):
                ...
                self._state = (
                    ...
                    {scope: dict(entries) for scope, entries in registry._scoped_tools.items()},
    ...
    def register_batch_if_absent(self, entries: List[dict]) -> None:
        """Register the whole batch or nothing, checking each effective profile scope."""
        ...
        with self.transaction():
            ...
            if duplicates:
                raise ValueError("Duplicate tool names in registration batch: " + ", ".join(map(repr, duplicates)))
            planned = []
            conflicts = []
            for entry, name in zip(entries, names):
                item = dict(entry)
                owner = caller_owner or self._plugin_owner_of(item["handler"])
                scope = item.get("scope")
                if scope is None and owner is not None:
                    scope = self._plugin_scope_of(owner)
                item["scope"] = scope
                visible = self._tools if scope is None else self._merged_tools(scope)
                if name in visible:
                    conflicts.append(name)
                planned.append(item)
            if conflicts:
                raise ValueError("Tool name already registered: " + ", ".join(map(repr, sorted(conflicts))))
            for item in planned:
                self.register(**item)
                installed = self._slot(item["scope"]).get(item["name"])
                if installed is None or installed.handler is not item["handler"]:
                    raise ValueError(f"Tool registration was refused: {item['name']!r}")
