...
_PUBLIC_ERROR_INFO_FIELDS = ("code", "provider", "scope", "retryable")


def _public_error_info(value) -> Optional[dict]:
    """The machine-readable half of a provider error, and nothing else.

    An allowlist rather than a copy: this function exists inside the trimming pass whose whole
    job is keeping raw provider payloads out of the model's context, so anything a provider
    tucks into error_info must not become a hole in that."""
    if not isinstance(value, dict):
        return None
    allowed = {k: value[k] for k in _PUBLIC_ERROR_INFO_FIELDS if k in value}
    return allowed or None


def _trim_results(results: List[dict]) -> List[dict]:
    """Keep only url/title/content/error per entry (+ blocked_by_policy / error_info).

    ``error_info`` is kept — filtered — because without it the model sees "an error" and cannot
    tell an out-of-credit ACCOUNT from a dead page, which is the difference between telling the
    user to top up and trying another URL."""
    return [
        {
            "url": r.get("url", ""), "title": r.get("title", ""), "content": r.get("content", ""),
            "error": r.get("error"),
            **({"blocked_by_policy": r["blocked_by_policy"]} if "blocked_by_policy" in r else {}),
            **({"error_info": _info} if (_info := _public_error_info(r.get("error_info"))) else {}),
        }
        for r in results
    ]
