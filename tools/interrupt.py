...
def is_interrupted() -> bool:
    """Whether an interrupt has been requested for the CURRENT thread.

    Two signals feed this: the per-thread bit above, which only the owning agent sets and
    clears, and the per-CALL cancel in ``tools.inflight_call`` — set by the cron scheduler's
    operator-stop and timeout paths on the run's threads, and swept automatically when the
    cancelled tool call unwinds. Folding the second in here means every existing poll site
    (terminal wait loop, MCP wait loop, code execution, web providers) honours a scheduler
    cancel with no per-tool change, and without anyone but the agent ever touching
    ``_interrupted_threads``.

    Deliberately NOT folded into ``is_thread_interrupted``: that is the raw-bit query used
    when a wait moves onto a deadline worker, and it must keep answering about the bit alone.
    """
