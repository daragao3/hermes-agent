...
def is_thread_interrupted(thread_id: int | None) -> bool:
    ...
    with _lock:
        if thread_id in _interrupted_threads:
            return True
    # Call-scoped cancellation is swept on unwind; never set a persistent
    # thread interrupt bit on behalf of the scheduler or another owner.
    return _is_call_cancelled(thread_id)
