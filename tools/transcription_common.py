...
def _get_stt_section(stt_config: Dict[str, Any], name: str) -> Dict[str, Any]:
    """Return an stt sub-section if it is a dict, else an empty dict.

    Carried from the fork's transcription_tools.py in the 0.21.1 merge: upstream's
    transcription_tools.py imports this name FROM this module, unguarded, and defines it
    nowhere — so the whole STT surface fails to import on a clean upstream checkout.
    """
