...
def _join_home(home: str, rest: str) -> str:
    ...
    ``get_subprocess_home()`` may return a POSIX path (``/opt/data/profiles/<name>/home``)
    even when Hermes runs on a Windows host: profile and container homes are consumed on the
    far side of a Docker/SSH boundary. ``os.path.join`` would splice a backslash in there,
    and the container branch of path resolution hands the result to ``posixpath.normpath``,
    which does not treat ``\\`` as a separator — so ``~/scratch/file.txt`` would reach Docker
    as a single component literally named ``home\\scratch``. Choose the separator already
    used by *home* instead of the host's."""
    if "\\" in home:
        sep = "\\"
    elif "/" in home:
        sep = "/"
