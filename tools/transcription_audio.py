...
def _run_quiet(command: list, *, timeout: float, env: Optional[dict] = None) -> subprocess.CompletedProcess:
    """Run one STT helper binary: checked, captured, utf-8 text, no stdin, hidden window.

    run_text_capture, not subprocess.run(capture_output=True): ``whisper`` shells out to
    ``ffmpeg``, so the real work is a GRANDCHILD that inherits the capture pipes and holds
    their write end open past whisper's death. subprocess.run kills only the direct child and
    then blocks re-draining a pipe that can never reach EOF, leaving ``timeout`` — 300s on the
    local-STT path — unenforceable on Windows. The helper captures into temp files,
    tree-kills, and sets CREATE_NO_WINDOW itself.

    It takes no ``check=``, so the raise is explicit; callers catch the same
    ``CalledProcessError`` and read ``.stdout``/``.stderr`` off it, which
    ``check_returncode()`` populates identically."""
    from hermes_cli._subprocess_compat import run_text_capture

    completed = run_text_capture(
        command, timeout=timeout, env=env, stdin=subprocess.DEVNULL)
