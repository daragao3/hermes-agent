...
class ProcessCheckpointMixin:
    ...
    def _write_checkpoint(self, extra_entries: Optional[List[Dict[str, Any]]] = None,
        ...
        ``checkpoint_path`` is the path captured when the session was spawned. Callers on the
        spawning thread pass nothing and resolve live, which is correct — the process still
        holds the home it was launched with. The reader/poller threads pass
        ``session.checkpoint_path``, because they finish whenever the CHILD does, long after
        the scope that started them is gone."""
        ...
        if checkpoint_path is not None and not checkpoint_path.parent.is_dir():
            # The captured home was deleted (a torn-down tmp_path). Recreating it would
            # leave litter, and there is nothing left to checkpoint for.
            return
        try:
            with self._lock:
                entries = []
    ...
    def recover_from_checkpoint(self) -> int:
        ...
        _cp = _checkpoint_path()
        if not _cp.exists():
            return 0
        try:
            entries = json.loads(_cp.read_text(encoding="utf-8"))
        except Exception:
            return 0
        recovered = 0
