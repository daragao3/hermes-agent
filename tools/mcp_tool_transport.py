    ...
    async def _discover_tools(self):
        ...
        if not self._advertises_tools():
            ...
            async with self._rpc_lock:
                ...
                # A half-open transport after initialize must reach the reconnect
                # loop instead of waiting forever for tools/list.
                self._tools = await asyncio.wait_for(
                    ...
                    timeout=float(self._config.get("connect_timeout", _core._DEFAULT_CONNECT_TIMEOUT)),
                )
