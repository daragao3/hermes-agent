...
class MCPServerTransportMixin:
    ...
    def _tools_list_timeout(self) -> float:
        """Deadline for one ``tools/list`` round-trip.

        A server that handshakes and then never answers tools/list would otherwise park this
        task on the shared MCP loop forever. Discovery is bounded by _run_discovery_pass, but
        the reconnect and list_changed paths are not, so they carry their own budget — the
        server's own ``connect_timeout``, the same one the connect paths use."""
        config = getattr(self, "_config", None) or {}
        try:
            return float(config.get("connect_timeout", _core._DEFAULT_CONNECT_TIMEOUT))
        except (TypeError, ValueError):
            return float(_core._DEFAULT_CONNECT_TIMEOUT)
    ...
    async def _discover_tools(self):
        ...
        if not self._advertises_tools():
            ...
            async with self._rpc_lock:
                ...
                # Bounded: this runs on EVERY reconnect, outside the discovery-pass deadline.
                self._tools = await asyncio.wait_for(
                    _core._paginate_full_list(
                        self.session.list_tools, "tools", self.name,
                        cache_meta_out=self._list_cache_meta),
                    timeout=self._tools_list_timeout())
