...
# OVERRIDE SLOT, not the resolved path: ``None`` means "resolve live". Kept under this name
# because tests and evals patch it directly (``monkeypatch.setattr(pr, "CHECKPOINT_PATH",
# tmp_path / ...)``); every read goes through ``_checkpoint_path()`` so those still work.
...
def _capture_checkpoint_path() -> Optional[Path]:
    ...
    Resolving only — never creating, and never raising: a spawn must not fail because the
    home could not be resolved (``get_hermes_home()`` falls back to ``Path.home()``, which
    raises when the environment has no home at all). ``None`` means "no binding", and the
    reader thread falls back to resolving live — the pre-existing behaviour."""
    try:
...
class ProcessSession:
    ...
    # The checkpoint file this session's reader/poller thread must write to. Captured at
    # spawn because that thread outlives the scope that started it; None = resolve live.
...
class ProcessRegistry(ProcessCheckpointMixin):
    ...
    def _pty_is_alive(pty: Any) -> bool:
        ...
        Both pywinpty's ``PtyProcess`` and POSIX ``ptyprocess.PtyProcess`` expose
        ``isalive()``. A missing method or a backend error is treated as "not known to be
        alive" so the kill path does not escalate on a false positive. Used by
        ``_signal_kill`` to decide whether a PTY ``terminate(force=True)`` actually reaped."""
    ...
    def _track_started(self, session: ProcessSession, reader_target, reader_name: str, extra_args=()) -> None:
        ...
        # Bind the checkpoint to the home we are spawning under: this thread finishes whenever
        # the CHILD does, by which point HERMES_HOME may have moved, and it would otherwise
        # write processes.json under an unrelated profile.
        ...
        self._write_checkpoint(checkpoint_path=session.checkpoint_path)
    ...
    def _signal_kill(self, session: ProcessSession, session_id: str, consume_output: bool) -> Optional[dict]:
        ...
        if session._pty:
            ...
            # terminate(force=True) can report failure — or report success while the child
            # is still there — on Windows. Escalate to the tree kill the sibling branches
            # already use; host_start_time lets it refuse a recycled PID.
            if _IS_WINDOWS and session.pid and (
                    reaped is False or self._pty_is_alive(session._pty)):
    ...
    def close_stdin(self, session_id: str) -> dict:
        ...
        return self._stdin_op(
            # A Windows console ignores Ctrl-D (pywinpty's sendeof); its EOF is Ctrl-Z + Enter at line start.
            session_id,
            lambda pty: pty.write("\x1a\r\n") if _IS_WINDOWS else pty.sendeof(),
            lambda stdin: stdin.close(), {"status": "ok", "message": msg})
