...
class ProcessRegistry(ProcessCheckpointMixin):
    ...
    def _move_to_finished(self, session: ProcessSession, *, checkpoint_path: Optional[Path] = None):
        ...
        self._write_checkpoint(checkpoint_path=checkpoint_path or session.checkpoint_path)
    ...
    def _signal_kill(self, session: ProcessSession, session_id: str, consume_output: bool) -> Optional[dict]:
        ...
        if session._pty:
            ...
            if _IS_WINDOWS and session.pid and (reaped is False or self._pty_is_alive(session._pty)):
    ...
    def write_stdin(self, session_id: str, data: str) -> dict:
        ...
        def via_pty(pty):
            ...
            if _IS_WINDOWS:
                ...
                # ConPTY accepts Unicode, not POSIX surrogateescaped bytes. Validate
                # before pywinpty's native boundary, where lone surrogates can panic.
