...
def _forward_relay_fronted_run(job: Dict[str, Any], extra_prompt: Optional[str] = None, *, caller=None, reason=None) -> Optional[str]:
    ...
    try:
        ...
        resp = httpx.post(
            ...
            json={**({"prompt": extra_prompt} if extra_prompt else {}), "caller": caller, "reason": reason}, timeout=10.0)
    except Exception:
        resp = None
...
def _claim_for_manual_run(job_id: str, log_label: str, *, caller=None, reason=None, previous_next_run_at=None):
    ...
    try:
        ...
        if isinstance(claimed_job, dict):
            emit_cron_triggered_safe(
                job_id=job_id, job_name=claimed_job.get("name") or job_id,
                caller=caller, reason=reason, previous_next_run_at=previous_next_run_at,
        ...
        if refreshed is None:
            ...
            detail = refreshed.get("paused_reason") or "no reason recorded"
            since = refreshed.get("paused_at")
            reason = f"Job is paused/disabled{f' since {since}' if since else ''} — {detail}. Resume it before running."
        else:
...
def _execute_job_now(job: Dict[str, Any], extra_prompt: Optional[str] = None, *, caller=None, reason=None) -> Dict[str, Any]:
    """Claim and run under one retained admission until the durable-running handoff."""
    from jobflow_dispatch.quarantine_control import default_control_store, retain_dispatch_admission
    admission = None
    claimed = False
    try:
        admission = retain_dispatch_admission(default_control_store(), boundary="manual-immediate-fire")
        claimed_job, err = _claim_for_manual_run(
            job["id"], "immediate run", caller=caller, reason=reason,
            previous_next_run_at=job.get("next_run_at"))
        claimed = claimed_job is not None
        return err if err is not None else _run_claimed_job(
            claimed_job, extra_prompt=extra_prompt, _dispatch_admission=admission)
    except Exception as exc:
        return {"claimed": claimed, "success": False, "error": str(exc)}
    finally:
...
def _run_claimed_job(job: Dict[str, Any], extra_prompt: Optional[str] = None, *, _dispatch_admission=None) -> Dict[str, Any]:
    ...
    try:
        if admission is None:
            from jobflow_dispatch.quarantine_control import default_control_store, retain_dispatch_admission
            admission = retain_dispatch_admission(default_control_store(), boundary="manual-background-fire")
...
def _try_dispatch_background_run(
    ...
    *, caller=None, reason=None,
    ...
    # A retained thread-local admission cannot cross into the executor. Guard
    # claim/audit here, then re-admit on the worker before execution. A fence
    # installed while queued prevents the worker from starting the run.
    from jobflow_dispatch.quarantine_control import default_control_store
    with default_control_store().dispatch_section(boundary="manual-background-claim"):
        claimed_job, err = _claim_for_manual_run(
            job_id, "background run", caller=caller, reason=reason,
            previous_next_run_at=job.get("next_run_at"))
    if err is not None:
        if err["claimed"]:
...
def _action_run(job: Dict[str, Any], a: Dict[str, Any]) -> str:
    ...
    bg = _try_dispatch_background_run(job, session_id=a["session_id"], extra_prompt=extra_prompt, caller=a["caller"] or "llm:cronjob_tool", reason=a["reason"])
    ...
    if bg is not None:
        ...
        forwarded = _forward_relay_fronted_run(job, extra_prompt=extra_prompt, caller=a["caller"] or "llm:cronjob_tool", reason=a["reason"])
        if forwarded is not None:
            return forwarded
        exec_result = _execute_job_now(job, extra_prompt=extra_prompt, caller=a["caller"] or "llm:cronjob_tool", reason=a["reason"])
...
_JOB_ACTIONS = {
    **{key: lambda job, a: _job_state_result(set_resume_barrier(job["id"], reason=a["reason"], caller=a["caller"]))
       for key in ("barrier_set", "barrier-set", "set_barrier")},
    **{key: lambda job, a: _job_state_result(clear_resume_barrier(job["id"], reason=a["reason"], caller=a["caller"]))
       for key in ("barrier_clear", "barrier-clear", "clear_barrier")},
    "remove": _action_remove, "update": _action_update,
    "run": _action_run, "run_now": _action_run, "trigger": _action_run,
    "pause": lambda job, a: _job_state_result(pause_job(job["id"], reason=a["reason"], caller=a["caller"] or "llm:cronjob_tool")),
    "resume": lambda job, a: _job_state_result(resume_job(job["id"], caller=a["caller"] or "llm:cronjob_tool")),
}
