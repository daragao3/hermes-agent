...
def _claim_for_manual_run(job_id: str, log_label: str):
    ...
    try:
        ...
        if refreshed is None:
            ...
            # The stored reason is right here on the refreshed record, and a paused job with
            # no reason is indistinguishable from one paused because it is broken — so say
            # which, and say "none recorded" rather than silently omitting it.
            paused_reason = refreshed.get("paused_reason")
            paused_at = refreshed.get("paused_at")
            detail = f' — "{paused_reason}"' if paused_reason else " (no reason recorded)"
            since = f" since {paused_at}" if paused_at else ""
            reason = f"Job is paused/disabled{since}{detail}. Resume it before running."
        else:
...
def _execute_job_now(job: Dict[str, Any], extra_prompt: Optional[str] = None, *,
    ...
    A WON CLAIM IS AN OFF-SCHEDULE FIRE, so it emits CRON_TRIGGERED (caller/reason
    attribution) BEFORE the run: a hung or crashed run must not lose the trigger's
    attribution. A LOST claim emits nothing — no fire happened on this caller's behalf.

    Dispatch admission is retained across BOTH the claim and the run. ``run_one_job`` would
    enter its own, but only after the claim — and claiming a job for fire is itself dispatch.
    The same handle is passed down so the section is entered once and released once."""
    from jobflow_dispatch.quarantine_control import (
        default_control_store, retain_dispatch_admission)

    admission = retain_dispatch_admission(default_control_store(),
                                          boundary="manual-immediate-fire")
    try:
        # Read BEFORE the claim: claiming advances next_run_at, and the event is supposed to
        # carry the schedule movement this trigger caused.
        ...
        emit_cron_triggered_safe(
            job_id=job["id"],
            job_name=claimed_job.get("name") or job.get("name") or job["id"],
            caller=caller,
            reason=reason,
...
def _run_claimed_job(job: Dict[str, Any], extra_prompt: Optional[str] = None, *,
                     _dispatch_admission=None) -> Dict[str, Any]:
        ...
        try:
            ...
            with _run_heartbeat(str(job.get("name") or job_id)):
                processed = run_one_job(job, adapters=adapters, loop=gateway_loop,
                                        ...
                                        _dispatch_admission=_dispatch_admission)
        finally:
            _registered = False
...
def _caller(a: Dict[str, Any]) -> str:
    """Who asked, for the audit trail. ``llm:cronjob_tool`` only when nobody said."""
    return a.get("caller") or "llm:cronjob_tool"
...
def _action_barrier_set(job: Dict[str, Any], a: Dict[str, Any]) -> str:
    """Attach an authorization barrier. Does NOT pause: barriering and pausing are separate."""
    if not a.get("reason"):
        return tool_error("A barrier needs a reason: what authorization is being withheld.",
                          success=False)
    return _job_state_result(set_resume_barrier(job["id"], reason=a["reason"],
                                               caller=_caller(a)))


def _action_barrier_clear(job: Dict[str, Any], a: Dict[str, Any]) -> str:
    """Lift a barrier. ``reason`` here is the JUSTIFICATION — the evidence it may be lifted."""
    if not a.get("reason"):
        return tool_error("Lifting a barrier needs a reason: the evidence that it may be "
                          "lifted.", success=False)
    return _job_state_result(clear_resume_barrier(job["id"], reason=a["reason"],
                                                 caller=_caller(a)))
...
def _action_run(job: Dict[str, Any], a: Dict[str, Any]) -> str:
    ...
    if bg is not None:
        ...
        exec_result = _execute_job_now(job, extra_prompt=extra_prompt,
                                       caller=_caller(a), reason=a.get("reason"))
...
_JOB_ACTIONS = {
    ...
    "pause": lambda job, a: _job_state_result(
        pause_job(job["id"], reason=a["reason"], caller=_caller(a))),
    "resume": lambda job, a: _job_state_result(resume_job(job["id"], caller=_caller(a))),
    # The barrier is an authorization control, so cron/jobs.py requires both a reason and a
    # caller; there is no useful default for either and none is invented here.
    "barrier_set": _action_barrier_set, "barrier-set": _action_barrier_set,
    "set_barrier": _action_barrier_set,
    "barrier_clear": _action_barrier_clear, "barrier-clear": _action_barrier_clear,
    "clear_barrier": _action_barrier_clear,
}
...
def cronjob(
    ...
    # Who asked. Without it every state change and every off-schedule fire is recorded as an
    # anonymous event — a row saying something happened and nothing about who asked for it.
    caller: Optional[str] = None,
    # NOT model-facing (absent from _HANDLER_FORWARDED_ARGS): a profile selects which Hermes
    # home, credentials and channels an unattended job runs under.
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "profile": {
                "type": "string",
                "description": "Optional Hermes profile name to run the job under. The scheduler applies a context-local Hermes home override plus that profile's config, credentials and environment for the run, bridges HERMES_HOME into subprocesses, then restores process state. Use 'default' for the root profile; named profiles must exist. On update, '' clears."
            },
            "attach_to_session": {
                "type": "boolean",
...
_HANDLER_FORWARDED_ARGS = (
    ...
    "script", "context_from", "continuity", "enabled_toolsets", "workdir", "profile", "no_agent", "attach_to_session",
    "paused_reason")
