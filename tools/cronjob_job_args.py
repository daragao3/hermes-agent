...
_FORMAT_JOB_OPTIONAL_KEYS = (
    ...
    # A barriered job looks identical to an ordinary paused one without these, and the
    # histories are what let a reader tell a routine pause from a contested one. `profile`
    # names which Hermes home the job runs under, which changes what it can reach.
    "resume_barrier", "resume_barrier_history", "paused_history", "profile")
