...
class BoundHomeMissing(RuntimeError):
    ...
    Raised only for an explicitly carried ``db_path`` (see ``_connect``). The long-lived
    consumers that carry one degrade to "delivery not claimed" rather than materialising a
    state.db under a torn-down directory."""
...
def _connect(db_path: Optional[Path] = None) -> sqlite3.Connection:
    ...
    ``db_path`` is a path captured by a caller whose worker outlives the scope that started
    it (the TUI notification poller). Resolving ``_db_path()`` on such a thread re-reads
    ``HERMES_HOME`` at tick time and CREATES a state.db under whatever home the env was
    restored to. Callers on the live thread pass nothing and resolve live, which stays
    correct — and keeps the mkdir, since there the home is by definition the current one.
    """
...
def _update_delivery(sql: str, params: tuple, db_path: 'Optional[Path]' = None) -> bool:
    """Run one UPDATE on the ledger; True iff exactly one row changed.

    ``db_path`` forwards a caller's CARRIED path (see ``_connect``). Without it a public
    entry point that takes ``db_path`` and then delegates here would silently fall back to
    live HERMES_HOME resolution — the exact defect the parameter exists to prevent."""
...
def claim_completion_delivery(
    delegation_id: str, claim_id: str, *, db_path: Optional[Path] = None) -> bool:
...
def claim_event_delivery(
    evt: Dict[str, Any], consumer: str, *, db_path: Optional[Path] = None) -> Optional[str]:
...
def complete_completion_delivery(
    ...
    return _update_delivery("""UPDATE async_delegations SET delivery_state='delivered',
                  ...
                  delivery_claimed_at=NULL
           WHERE delegation_id=? AND delivery_state='pending'
             AND delivery_claim=?""", (now, now, delegation_id, claim_id), db_path)


def complete_event_delivery(
    evt: Dict[str, Any], claim_id: str, *, db_path: Optional[Path] = None) -> None:
    _event_delivery(complete_completion_delivery, evt, claim_id, db_path)


def release_event_delivery(
    evt: Dict[str, Any], claim_id: str, *, db_path: Optional[Path] = None) -> None:
    _event_delivery(release_completion_delivery, evt, claim_id, db_path)


def _event_delivery(fn, evt: Dict[str, Any], claim_id: str,
                    db_path: Optional[Path] = None) -> None:
    if claim_id and evt.get("type") == "async_delegation":
        try:
            fn(str(evt.get("delegation_id") or ""), claim_id, db_path=db_path)
        except BoundHomeMissing:
            # The carried home is gone; there is no ledger left to acknowledge against.
            pass
