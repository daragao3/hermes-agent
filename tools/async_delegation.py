...
class BoundHomeMissing(RuntimeError):
    """A captured database home no longer exists; never recreate it."""
...
def _connect(db_path: Optional[Path] = None) -> sqlite3.Connection:
    if db_path is not None and not db_path.parent.is_dir():
...
def _transaction(db_path: Optional[Path] = None) -> Iterator[sqlite3.Connection]:
    ...
    """
    conn = _connect() if db_path is None else _connect(db_path)
    try:
        with conn:
            yield conn
    finally:
...
def claim_completion_delivery(delegation_id: str, claim_id: str, *, db_path: Optional[Path] = None) -> bool:
...
def claim_event_delivery(evt: Dict[str, Any], consumer: str, *, db_path: Optional[Path] = None) -> Optional[str]:
    ...
    try:
        return claim_id if claim_completion_delivery(delegation_id, claim_id, db_path=db_path) else None
    except BoundHomeMissing:
        return None


def release_completion_delivery(delegation_id: str, claim_id: str, *, db_path: Optional[Path] = None) -> bool:
...
def complete_completion_delivery(delegation_id: str, claim_id: str, *, db_path: Optional[Path] = None) -> bool:
    ...
    return _update_delivery("""UPDATE async_delegations SET delivery_state='delivered',
                  ...
                  delivery_claimed_at=NULL
           WHERE delegation_id=? AND delivery_state='pending'
             AND delivery_claim=?""", (now, now, delegation_id, claim_id), db_path=db_path)


def complete_event_delivery(evt: Dict[str, Any], claim_id: str, *, db_path: Optional[Path] = None) -> None:
    _event_delivery(complete_completion_delivery, evt, claim_id, db_path=db_path)


def release_event_delivery(evt: Dict[str, Any], claim_id: str, *, db_path: Optional[Path] = None) -> None:
    _event_delivery(release_completion_delivery, evt, claim_id, db_path=db_path)


def _event_delivery(fn, evt: Dict[str, Any], claim_id: str, *, db_path: Optional[Path] = None) -> None:
