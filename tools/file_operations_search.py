...
class SearchMixin:
    ...
    def _native_search_enabled(self) -> bool:
        """The no-shell lanes in THIS module (native rg, native exists) are POSIX-only:
        `_run_rg_native` reaps with `os.getpgid` and `_path_exists` stats the raw tool
        path, which on Windows may be MSYS-form. `_native_read_enabled` is wider on
        purpose (read_file translates its path first), so it cannot be the gate here.
        """
        return sys.platform != "win32" and self._native_read_enabled()
 ...
 lane cannot express (the multi-root ``cd`` prefix); ``shell_prefix`` is shell-only."""
        if native_ok and self._native_search_enabled():
    ...
    def _search_files_rg(self, pattern: str, path: str | List[str], limit: int, offset: int,
        ...
        # that otherwise listed fine); the paths it did print are real, so only a
        # failure that produced nothing is surfaced as an error.
        partial = result.exit_code == 2 and bool(all_files)
        if result.exit_code not in {0, 1, 124} and not bounded_sigpipe and not partial:
            if order == "modified":
                return SearchResult(error=(
