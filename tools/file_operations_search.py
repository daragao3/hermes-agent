...
class SearchMixin:
    ...
    # --- native rg transport (local host) --------------------------------------
    ...
    def _run_rg_native(self, argv: List[str], fetch_limit: int, timeout: int,
        ...
        from tools.environments.local import _kill_process_group_posix, _kill_process_windows, _make_run_env
 ...
 lane cannot express (the multi-root ``cd`` prefix); ``shell_prefix`` is shell-only."""
        # Direct argv also avoids MSYS collapsing doubled regex backslashes.
    ...
    def _search_files_rg(self, pattern: str, path: str | List[str], limit: int, offset: int,
        ...
        result = self._run_rg_bounded([rg_cmd], fetch_limit, timeout=60, native_ok=not scoped_common, merge_stderr=True,
                                      shell_prefix=f"set -o pipefail; {cd_prefix}")
        stdout, limit_reason = _search_stdout_and_limit(result)
        diagnostics, all_files = _split_rg_files_output(stdout)
        if scoped_common:
            all_files = [
        ...
        if result.exit_code not in {0, 1, 124} and not bounded_sigpipe and not (result.exit_code == 2 and all_files and order == "discovery"):
            ...
            return SearchResult(error=f"File search failed while running ripgrep: {diagnostics.strip() or stdout.strip() or result.exit_code}")
        return SearchResult(
            files=[self._native_search_result_path(f) for f in all_files[offset:offset + limit]], total_count=len(all_files),
    ...
    def _native_search_result_path(self, path: str) -> str:
        # Remote POSIX paths belong to their backend, even on a Windows controller.
    ...
    def _run_search_pipeline(self, cmd_parts: List[str], output_mode: str, limit: int,
        ...
        parsed = _parse_search_output(result, output_mode, limit, offset, context, warning=warning)
        parsed.files = [self._native_search_result_path(path) for path in parsed.files]
        parsed.counts = {self._native_search_result_path(path): count for path, count in parsed.counts.items()}
        for match in parsed.matches:
            match.path = self._native_search_result_path(match.path)
        return parsed
