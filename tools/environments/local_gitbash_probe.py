...
def _bash_starts(bash: str) -> bool:
    ...
    try:
        # MSYS grandchildren can retain capture pipes after their parent times
        # out. File-backed capture keeps this diagnostic bounded in that case.
        from hermes_cli._subprocess_compat import run_text_capture

        result = run_text_capture(
            [bash, "--noprofile", "--norc", "-c", _BASH_EXTERNAL_PROGRAM_PROBE],
            timeout=15, stdin=subprocess.DEVNULL)  # #78820: never hand the TUI gateway's stdin pipe to MSYS bash
