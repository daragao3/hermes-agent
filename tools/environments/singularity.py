...
def _get_scratch_dir(*, create: bool = True) -> Path:
    ...
    ``tools.terminal_tool.resolve_scratch_dir``, which uses it to capture the root at
    registration time for a sweep that runs at interpreter exit."""
    custom_scratch = os.getenv("TERMINAL_SCRATCH_DIR")
    if custom_scratch:
        scratch_path = Path(custom_scratch)
    else:
        from tools.environments.base import get_sandbox_dir
        scratch_path = get_sandbox_dir(create=create) / "singularity"
