...
def _windows_bash_candidates(custom: "str | None") -> list[str]:
    ...
    # PATH's bash on Windows is routinely WSL's launcher (``System32\bash.exe``) or its
    # Microsoft Store stub (``WindowsApps\bash.exe``) — a default Git for Windows install
    # only adds ``Git\cmd``, which ships no bash. WSL bash runs commands inside the Linux
    # VM, where the ``C:\...``-style paths this environment builds do not resolve, so every
    # command exits 126/127. Refuse it HERE rather than relying on candidate order: when no
    # Git dir has bash it is the only candidate, and it passes the _bash_starts probe below
    # (the launcher starts fine — it just cannot see our paths), so the probe is actively
    # misleading.
    found = shutil.which("bash")
    if found and ("\\system32\\" in found.lower() or "\\windowsapps\\" in found.lower()):
        found = None
...
def _native_exec_path(path: str) -> str:
    ...
    :func:`_bash_safe_path` is the right translation for the MSYS coreutils this codebase
    shells out to (``cat``, ``sed``, ``find``, ``grep``): they are Git Bash binaries and
    understand ``/c/Users/x``. A *native* Windows executable invoked from bash does not —
    and because ``MSYS_NO_PATHCONV=1`` is set for these children, MSYS does not convert the
    argument back on its way into argv either. A ``/c/...`` root therefore reaches the
    binary verbatim and it fails with ``IO error ... (os error 3)``.

    ``rg`` is exactly such a binary in every mainstream Windows install (WinGet, scoop and
    choco all ship the MSVC build), which is why the search tools need this and the
    read/write tools do not. Cygwin- and MSYS2-compiled tools accept native drive paths too,
    so the native form is the correct choice for both kinds of binary.

    Accepts either form as input and is idempotent. Non-drive paths (relative paths, and
    genuine POSIX roots like ``/tmp``) are returned unchanged: there is no correct
    drive-letter answer for them, and a relative path already resolves against the shell's
    cwd. No-op off Windows.
    """
...
def _prepend_git_bash_dirs(existing_path: str) -> str:
    ...
    The platform check must live HERE, not only inside the cached ``_git_bash_bin_dirs``
    builder: that cache is populated once, and if it was filled on a real Windows host a
    later caller running with ``_IS_WINDOWS`` patched False (the POSIX-simulating env tests)
    would get the cached Windows dirs injected into a POSIX PATH."""
    if not _IS_WINDOWS:
...
class LocalEnvironment(BaseEnvironment):
    ...
    # MSYS bash treats a Windows reserved device name as an ordinary filename, so `> NUL`
    # writes a real, Win32-unreachable file instead of discarding. Only this backend opts in.
