...
def get_sandbox_dir(*, create: bool = True) -> Path:
    ...
    ``create=False`` resolves the path without materialising it, so a caller that only wants
    to CAPTURE the location — a deferred cleanup hook binding its target at registration —
    does not leave a sandbox tree behind on every process that merely asks where they go."""
...
def _register_inflight(
    env: "BaseEnvironment", proc: "ProcessHandle", tid: int | None = None
) -> int:
    """Record *proc* as in flight on thread *tid* (default: the calling thread).

    ``execute()`` passes the TOOL thread explicitly: since 0.21.1 the wait itself runs on a
    ``run_bounded_sync`` worker, but the scheduler's ``run_tool_thread_ids`` collects the
    threads the run's tools are CALLED on, so that is the ident the kill must be keyed by.
    """
    if tid is None:
...
class BaseEnvironment(ABC):
    ...
    # True only where commands run under MSYS bash on Windows, which treats a Windows
    # reserved device name (`NUL`, `CON`, `COM1`…) as an ordinary filename — so `> NUL`
    # writes a real, Win32-unreachable file instead of discarding. LocalEnvironment sets it
    # per-host; every sandboxed backend keeps it False because their shells are genuinely
    # POSIX, and there `NUL` really is an ordinary filename.
    ...
    def execute(
        ...
        *,
        ...
        # Repoint `> NUL` and friends at /dev/null: under the MSYS bash the local backend
        # shells out to, a Windows reserved device name is an ORDINARY FILENAME, so the
        # redirect creates a real, Win32-unreachable file instead of discarding. Only backends
        # that actually run through MSYS opt in — on a Linux container `NUL` is an ordinary
        # filename and rewriting it would destroy the caller's intent. Before the heredoc embed
        # below, so `> NUL` inside a heredoc BODY is left alone: there it is data, not a
        # redirect.
        if self._msys_windows_device_redirects:
            ...
            if _devices:
                logger.debug(
                    "Repointed %d Windows device redirect target(s) at /dev/null", _devices)
        ...
        def _spawn_and_wait() -> dict:
            ...
            # Visible to kill_inflight_processes() for exactly the wait window, keyed on the
            # tool thread (this closure runs on the deadline worker).
            _tid = _register_inflight(self, spawned, tid=parent_tid)
            try:
                ...
                _unregister_inflight(_tid, self, spawned)
