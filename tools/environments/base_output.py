...
from hermes_cli._subprocess_compat import windows_hide_flags, windows_pipe_readable_bytes
...
def _drain_stdout(proc: ProcessHandle, output: _BoundedOutputCollector, stop: "threading.Event | None" = None) -> None:
    ...
    on Windows, so PeekNamedPipe readiness is polled there instead.
    """
    stream = proc.stdout
    if stream is None:
        return
...
def _drain_fd_windows(proc, fd: int, output: _BoundedOutputCollector, decoder, stop=None) -> None:
    """Read only buffered bytes; an inherited writer must not prevent exit/yield."""
    idle_after_exit = 0
    while True:
        if stop is not None and stop.is_set():
            return
        available = windows_pipe_readable_bytes(fd)
        if available is None:
            return
        if available:
            chunk = os.read(fd, min(available, 4096))
            if not chunk:
                return
