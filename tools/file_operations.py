...
class ShellFileOperations(FileOperations):
    ...
    def _python_delete(self, path: str, recursive: bool) -> WriteResult:
        ...
        # Local backend fast-path: remove regular files (including read-only
        # ones) with direct Python I/O instead of a per-call Git Bash spawn
        # (0.3-1s on Windows) — one of the last spawns in the mutating set.
        # _delete_file_native returns None for anything it can't reproduce
        # byte-for-byte (directories, symlinks, POSIX-root, recursive trees),
        # so those fall through to the portable python -c path below.
        if self._lsp_local_only():
            native = self._delete_file_native(path)
            if native is not None:
                return native

        # Non-local backends (docker/ssh) or cases the native path declined:
    ...
    def patch_replace(self, path: str, old_string: str, new_string: str,
        ...
        # preserves the marker.  Applies to BOTH the native read and the
        # ``cat`` fallback so matching is consistent.
        ...
        # apples-to-apples.  Strip from ``verify_stdout`` (the merged var
        # set by either the native read or the ``cat`` fallback) so it works
        # on the fork's native-read fast-path, where ``verify_result`` is
        # never assigned.
        _verify_bomless, _ = _strip_bom(verify_stdout)
