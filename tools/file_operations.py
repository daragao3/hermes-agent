...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _cat(self, path: str) -> ExecuteResult:
        ...
        Reads locally without a shell when it can. This is the seam patch_replace and the
        post-write verify both go through, and neither reaches read_file_raw — so without it
        a single patch still costs two Git Bash spawns on Windows. None from the native read
        means the shell owns this path, and missing-file semantics stay exactly as they were."""
        if self._lsp_local_only():
            native = self._read_native_text(path)
            if native is not None:
                return ExecuteResult(stdout=native, exit_code=0)
    ...
    def _native_read_enabled(self) -> bool:
        ...
        # NOT excluded on Windows any more. That exclusion existed because the body below
        # joins env.cwd and stats the result directly, and on Windows the path can arrive in
        # MSYS `/c/Users/x` form — which os.path.isabs calls absolute and Windows cannot open.
        # _resolve_native_path now translates it, and returns None when it cannot, in which
        # case the read falls through to the shell exactly as before. Windows is where this
        # matters most: every shell round-trip there is a fresh Git Bash at 0.3-1s, and a
        # single read_file costs four of them.
    ...
    def _read_file_native(self, path: str, offset: int, limit: int) -> ReadResult:
        ...
        full = self._resolve_native_path(path)
        if full is None:
            # Not natively addressable (a POSIX-root path on Windows): the shell owns it.
            return self._read_file_sequential(path, offset, limit)
        try:
            st = os.stat(full)
    ...
    def read_file_raw(self, path: str) -> ReadResult:
        ...
        # Three shell round-trips otherwise (wc -c, head -c, cat), and raw reads feed the
        # patch/verify flows — a genuinely hot path.
        if self._lsp_local_only():
    ...
    def move_file(self, src: str, dst: str) -> WriteResult:
        ...
        for p in (src, dst):
            ...
            if denied:
                return WriteResult(error=denied)
        # os.replace for the simple case; None keeps mv's exact semantics for moving INTO a
        # directory, a read-only destination, a cross-device move, or a POSIX-root path.
        if self._lsp_local_only():
    ...
    def write_file(self, path: str, content: str, pre_content: Optional[str] = None) -> WriteResult:
        ...
        if denied:
            return WriteResult(error=denied)
        # Three to five shell round-trips otherwise, on the agent's hottest edit-loop path.
        # Placed AFTER the deny check so the native path can never bypass it.
        if self._lsp_local_only():
    ...
    # --- native fast paths: no shell, no Git Bash spawn ----------------------------------------
    #
    # Each returns None for anything that is not a plain local file, and the caller falls through
    # to the shell pipeline — so missing files, directories, read-only destinations, cross-device
    # moves and POSIX-root paths keep the shell's exact semantics.
