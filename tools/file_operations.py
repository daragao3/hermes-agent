...
_NATIVE_READ_MAX_BYTES = 50 * 1024 * 1024
...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        if self._lsp_local_only():
            native = self._atomic_write_native(path, content)
            if native is not None:
                return native
    ...
    def _atomic_write_native(self, path: str, content: str) -> Optional[ExecuteResult]:
        """Same-directory atomic replace for ordinary local files; links use the shell."""
        target = self._resolve_native_path(path)
        ...
        try:
            ...
            candidate = os.path.join(parent, f".hermes-tmp.{secrets.token_hex(16)}")
            # O_EXCL prevents following a pre-existing path. New files receive
            # normal umask permissions; existing files retain their mode.
            fd = os.open(candidate, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o666)
            temporary = candidate  # Cleanup owns only a file we successfully created.
    ...
    def _native_read_enabled(self) -> bool:
        """Whether ``read_file`` and ``search_files`` may bypass the shell: only ``LocalEnvironment``.
        Native reads translate MSYS drive paths; unresolved POSIX roots on Windows
        keep the shell path. ``HERMES_NATIVE_FILE_READ=0``
    ...
    def _read_file_raw_native(self, path: str) -> Optional[ReadResult]:
        ...
        try:
            ...
            if os.path.splitext(path)[1].lower() in BINARY_EXTENSIONS or self._is_likely_binary_bytes(sample):
                return ReadResult(
                    is_binary=True, file_size=file_size,
            ...
            return ReadResult(
                content=_strip_bom(_strip_terminal_fence_leaks(raw_bytes.decode("utf-8", errors="replace")))[0],
                file_size=file_size,
            )
        except Exception:
            # Any surprise (permission change, deletion race, exotic
    ...
    def _probe_write_target(self, path: str, pre_content: Optional[str], want_pre: bool,
        ...
        if self._lsp_local_only():
            native = self._resolve_native_path(path)
            if native is not None:
                try:
                    ...
                    if os.path.isfile(native):
                        with open(native, "rb") as stream:
                            ...
                            if want_pre and pre_content is None:
                                ...
                                pre_content = raw.decode("utf-8", errors="replace") or None
                        ending_source = pre_content or head.decode("utf-8", errors="replace")
                        return head.startswith(b"\xef\xbb\xbf"), pre_content, (
                            _detect_line_ending(ending_source) if ending_source else None)
                except OSError:
                    pass  # Preserve the shell's diagnostic path for inaccessible files.
        if want_pre and pre_content is None:
    ...
    def _verify_written_hash(self, path: str, content_bytes: bytes) -> tuple[Optional[bool], Optional[WriteResult]]:
        ...
        try:
            native = self._resolve_native_path(path) if self._lsp_local_only() else None
            if native is not None and os.path.isfile(native):
                with open(native, "rb") as stream:
                    digest = hashlib.file_digest(stream, "sha256").hexdigest()
                hash_result = ExecuteResult(stdout=digest, exit_code=0)
            else:
