...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None,
                            force_document=False, args=None, *, buttons=None):
    ...
    ``events.override_buttons.buttons_for``. It is forwarded only on the Telegram branch below
    (the only platform that currently renders it); every other platform ignores it. Defaults
    to ``None``, in which case no code path here differs from before it existed."""
    from gateway.config import Platform
    platform_name = platform.value if hasattr(platform, "value") else str(platform)
