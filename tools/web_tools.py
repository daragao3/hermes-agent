...
from tools.web_tools_extract import (
    _extract_safe_urls, _is_firecrawl_credit_result, _merge_in_order, _no_provider_error,
    _resolve_extract_provider, _resolve_run_fallback, _result_entry,
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            ...
            # Out of Firecrawl credits: one retry against one other provider. The choice is
            # memoised for the whole run, so a 10-call run resolves a fallback once.
            if _is_firecrawl_credit_result(response_data):
                _fallback = _resolve_run_fallback("search")
                if _fallback is not None:
                    response_data = _fallback.search(query, limit)
