...
from tools.web_tools_extract import (
    _is_firecrawl_credit_result, _resolve_run_fallback,
...
def _memoized_search(provider, query: str, limit: int) -> dict:
    ...
    def _paid_search() -> tuple[dict, bool]:
        ...
        # Account exhaustion has run-scoped routing; do not lose its typed
        # evidence to generic outage rescue or cache the alternate as primary.
        if _is_firecrawl_credit_result(resp):
            fallback = _resolve_run_fallback("search")
            return (fallback.search(query, limit) if fallback is not None else resp), True
