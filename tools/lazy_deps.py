...
def _run_installer(cmd: list[str], *, timeout: float, **kw) -> subprocess.CompletedProcess:
    """Spawn one installer command with a timeout that actually bounds it.

    run_text_capture, not subprocess.run(capture_output=True): uv forks a build backend and
    pip forks setuptools, so the real work is a GRANDCHILD that inherits the capture pipe and
    holds its write end open after the direct child is killed. subprocess.run then re-drains
    a pipe that can never reach EOF and blocks forever, so ``timeout`` bounds nothing on
    Windows. The helper captures into temp files instead and tree-kills. It sets
    CREATE_NO_WINDOW itself, which is the whole job of the flag helper this used to call.
    """
    from hermes_cli._subprocess_compat import run_text_capture

    return run_text_capture(cmd, timeout=timeout, stdin=subprocess.DEVNULL, **kw)
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    try:
        ...
        # is an MSIX app-execution alias, and [sys.executable, "-m", ...] is the exact shape
        # that strands suspended orphans — AppX starts the packaged child SUSPENDED and only
        # resumes it once the activation handoff completes, so a parent that dies mid-handoff
        # leaves a process that never runs an instruction. Tier 1 spawns the uv binary and is
        # unaffected.
        ...
        try:
            ...
            try:
                # check=True by hand: run_text_capture does not take it. The handler below
                # already expects CalledProcessError, so the control flow is unchanged.
                boot = _run_installer(
                    [real_executable(), "-m", "ensurepip", "--upgrade", "--default-pip"], timeout=120)
                if boot.returncode != 0:
                    raise subprocess.CalledProcessError(
                        boot.returncode, boot.args, boot.stdout, boot.stderr)
