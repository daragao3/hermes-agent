...
def _run_installer(cmd: list[str], **kw) -> subprocess.CompletedProcess:
    # File-backed capture bounds waits even when build grandchildren inherit handles.
    from hermes_cli._subprocess_compat import run_text_capture

    check = kw.pop("check", False)
    result = run_text_capture(cmd, **kw)
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    try:
        ...
        try:
            ...
            try:
                _run_installer([real_executable(), "-m", "ensurepip", "--upgrade", "--default-pip"], timeout=120, check=True)
